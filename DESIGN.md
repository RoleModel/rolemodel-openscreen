...
Every color resolves through an Optics custom property. Optics computes its ramps in CSS from three numbers with `light-dark()`, so setting one hue re-tints all 486 tokens; writing a hex here would break that and quietly drift from the system with every Optics release. The RoleModel sub-brand scales live in `brand/optics/rolemodel-scales.css`, generated from the Figma export because the open-source package does not publish them. All of them stay in that file — they are real products and the export is Optics' to define — while `SUB_BRANDS` in `lib/sync-brand.mjs` decides which four the toolkit offers as choices: academy, lcad, docks, railing.
