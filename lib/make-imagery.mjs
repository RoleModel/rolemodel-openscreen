...
const WANTED = [
	...
	// Academy-Slide is deliberately absent. Deleting the file was not enough and
	// never could be: this list is what gets fetched, so every run copied it
	// straight back and every `imagery:check` called its absence drift. The entry
	// is the only place a removal sticks.
...
async function main() {
	...
	for (const want of WANTED) {
		...
		if (!src) {
			/*
			 * No source repo is not the same as no image.
			 *
			 * These files are vendored and committed, and CI checks out this repo
			 * alone — no rolemodel-brand, no academy-hyperframes beside it. Recording
			 * the whole set as absent there dropped every name out of `keep`, so the
			 * committed copies read back as orphans and `imagery:check` failed on a
			 * tree that was in fact exactly right. Worse, the non-check run that the
			 * failure sends you to would have deleted all ten.
			 *
			 * So a wanted image already on disk from a previous run stands as it is.
			 * Only a name with no source *and* no local copy is recorded as null.
			 */
			const have = priorByName.get(want.name);
			const local = have?.file ? await readFile(join(OUT, have.file)).catch(() => null) : null;
			if (local) {
				vendored.push(want.name);
				entries.push(have);
				continue;
			}
			/*
	...
	if (vendored.length) say(`\n  no source repo, kept the vendored copy: ${vendored.join(", ")}`);
