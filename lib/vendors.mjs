 ...
 * expensive.
 */
export async function quoteAll({ sizeMm = 76.2, quantity = 100, product = "die-cut", country = "US", prodigiKey = "", prodigiSandbox = false, cache = null, fetchImpl = fetch } = {}) {
 ...
 */
export async function quoteRuns({ runs = RUNS, product = "die-cut", ...rest } = {}) {
	const cache = new Map();
	const each = await Promise.all(runs.map((quantity) => quoteAll({ ...rest, product, quantity, cache })));
	const names = new Map();
	...
	return { runs, product, vendors, best };
}

/** Both products, each across the runs. */
export async function quoteRunsBoth({ sizeMm = 76.2, sheetMm = 152.4, ...rest } = {}) {
	const [singles, sheets] = await Promise.all([quoteRuns({ ...rest, sizeMm, product: "die-cut" }), quoteRuns({ ...rest, sizeMm: sheetMm, product: "sheet" })]);
	return { singles, sheets };
}
