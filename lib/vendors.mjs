...
export async function quoteRuns({ runs = RUNS, product = "die-cut", page = null, ...rest } = {}) {
	 ...
	 *
	 * A shop whose smallest run is larger than the column cannot win it. Its
	 * unit price at that minimum was competing against everyone else's price
	 * for the run actually asked about, and it won — so the 50 column showed
	 * StickerGiant cheapest at $1.90 each when the only way to pay $1.90 was to
	 * buy a hundred.
	 */
	const best = runs.map((n, i) => {
		let win = null;
		for (const v of vendors) {
			const c = v.cells[i];
			if (!c || c.quantity > n) continue;
			if (!win || c.unit < win.unit) win = { id: v.id, unit: c.unit };
		}
		return win?.id ?? null;
	});
