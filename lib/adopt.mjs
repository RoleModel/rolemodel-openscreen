...
export async function writeFraming({ projectId, folder, root, framing = [], pip = null } = {}) {
	...
	/* `keep` is read from the composition every time, so recording it would be a
	   second copy that can go stale — the exact failure this whole file exists to
	   end. The composition is the source; clips.json is the recipe. */
	const saved = { ...spec };
	delete saved.keep;
	await writeFile(recipePath, `${JSON.stringify(saved, null, 2)}\n`, "utf8");
	return { clips: clips.length, seconds: built.seconds, adopted: recipe.adopted ?? [], pip: shot };
}
