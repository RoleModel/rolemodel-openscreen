...
export function foreignElements(html) {
	...
	if (!main) return [];
	/*
	 * Comments first, because the walker counts tags and a comment holds tags
	 * that are not there. A commented-out `<div>` left over from an edit raised
	 * the depth and never closed it, so everything after read as nested and the
	 * real top-level children went unseen — the same shape of bug as `\bid="`
	 * matching inside `data-hf-id`. Replaced rather than removed, so the offsets
	 * of everything after stay put.
	 */
	const body = main[1].replace(/<!--[\s\S]*?-->/g, (c) => " ".repeat(c.length));
	const kept = [];
 ...
 *
 * One exception to "nothing else", stated because the header above would
 * otherwise promise more than this delivers: a pip's `style` attribute is
 * rewritten whole rather than patched property by property, so this owns it. A
 * declaration added there by hand — a filter, an outline while debugging — does
 * not survive the next framing nudge. Rewriting whole is deliberate: patching
 * leaves a stale `--pip-zoom` behind when zoom returns to 1, and a value nobody
 * can see is worse than one that is gone. Put anything meant to last in the
 * stylesheet, not inline on a pip.
 */
...
export async function writeFraming({ projectId, folder, root, framing = [], pip = null } = {}) {
	...
	/*
	 * One clamp for every number that arrives from a panel.
	 *
	 * `Number(undefined) ?? 50` does not fall back: NaN is not nullish, so a
	 * framing row that omitted focus clamped NaN straight through to
	 * `--pip-focus:NaN%` in the composition and the recipe. Number.isFinite is
	...
	const clips = recipe.clips.map((clip, i) => {
		...
		const zoom = clamp(want.zoom, 1, 4, 1);
		return {
			...clip,
			focus: clamp(want.focus, 0, 100, 50),
			zoom,
			...
			...(zoom === 1 ? { focusY: 50 } : { focusY: clamp(want.focusY, 0, 100, 50) }),
		};
	});
	/*
	 ...
	 *
	 * applyFraming pairs framing to pips by position, so a composition holding
	 * more of them than the recipe knows about gets the first N framed and the
	 * rest silently left — which reads as "the framing tool only works on some
	 * of them" and is really the file having been duplicated underneath it. Say
	 * so instead: a count that does not match is a question, not a default.
	 */
	const live = readComposition(before).length;
	if (live !== clips.length) {
		throw new Error(
			`This composition has ${live} pips and the cut has ${clips.length}. Nothing was changed — the file has been edited elsewhere, so reload before framing.`,
		);
	}
