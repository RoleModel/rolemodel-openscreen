...
export const MODELS = [
	...
	/*
	 * OmniHuman: the same photo-and-voice job as Kling AI Avatar, done better.
	 *
	 * ByteDance's, and the audio is the control signal rather than a prompt —
	 * it drives mouth, expression and body together instead of lip position
	 * alone, which is the thing Kling's avatar does badly. It documents a hard
	 * 30-second ceiling on the audio, so a full narration has to be cut into
	 * takes; that limit is real and stated, unlike Kling's, which states none
	 * and simply disappoints.
	 */
	{
		id: "fal-ai/bytedance/omnihuman",
		label: "OmniHuman",
		takes: "image+audio",
		hint: "A photo and a voice track. The audio drives expression and movement, not just the mouth. Up to 30 seconds.",
		controls: [],
		limits: { maxAudioSeconds: 30 },
		input: ({ imageUrl, audioUrl }) => ({ image_url: imageUrl, audio_url: audioUrl }),
	},
	/*
	 * The lipsync pair: the real person, saying something else.
	 *
	 * A different and usually better answer to "recreate my video" than any
	 * avatar. Nothing is invented — the take is the take, the face is the face,
	 * and only the mouth is re-timed to a new voice. Where a photo-to-video
	 * model has to guess a person from one frame, these already have them.
	 *
	 * `sync_mode` is how a mismatch between clip length and voice length is
	 * resolved, and it is the decision that matters here: a 19-second take
	 * against a 90-second narration is cut off by default, which is rarely what
	 * anyone wants.
	 */
	{
		id: "fal-ai/sync-lipsync/v3",
		label: "Sync Lipsync 3",
		takes: "video+audio",
		hint: "Re-times a real clip's mouth to a new voice track. The person and the take are untouched.",
		controls: ["syncMode"],
		limits: { syncModes: ["cut_off", "loop", "bounce", "silence", "remap"], defaultSyncMode: "cut_off" },
		input: ({ videoUrl, audioUrl, syncMode }) => ({
			video_url: videoUrl,
			audio_url: audioUrl,
	...
	},
	{
		...
		takes: "video+audio",
		hint: "The older lipsync, tuned for close-ups. Re-times a real clip's mouth to a new voice track.",
		controls: ["syncMode"],
		limits: { syncModes: ["cut_off", "loop", "bounce", "silence", "remap"], defaultSyncMode: "cut_off" },
		input: ({ videoUrl, audioUrl, syncMode }) => ({
			video_url: videoUrl,
	...
	/*
	 * Pixverse, which takes the pair and nothing else.
	 *
	 * No sync_mode: the Sync models resolve a length mismatch by cut_off, loop,
	 * bounce, silence or remap, and Pixverse simply has no such parameter. It
	 * was declared with one — copied from the entry above it — which would have
	 * put a dial on the panel that changes nothing and sends a field the model
	 * does not know. Read from fal's own API page, like everything else here.
	 */
	{
		id: "fal-ai/pixverse/lipsync",
		label: "Pixverse Lipsync",
		takes: "video+audio",
		hint: "Re-times a real clip's mouth to a voice track. No control over what happens if the two are different lengths.",
		controls: [],
		limits: {},
	...
	/*
	 * Veo 3.1, which makes footage that was never shot.
	 *
	 * Two shapes of the same model. One takes a description and nothing else;
	 * the other animates a still the project already has, which is the one worth
	 * having here — a photograph from the shoot is a stronger starting point
	 * than a sentence, and it keeps a generated shot looking like the rest of
	 * the material.
	 *
	 * Eight seconds is the ceiling, not a default worth arguing with: this makes
	 * shots, not scenes, and a cut is where they become a video. Audio comes
	 * with it unless it is turned off — which it usually should be, because the
	 * project already has a voice track and two of them do not mix.
	 */
	{
		id: "fal-ai/veo3.1",
		label: "Veo 3.1",
		takes: "text",
		hint: "Describe a shot and it is generated. Up to eight seconds, and it lands in Footage like anything else.",
		controls: ["prompt", "aspect", "duration", "resolution", "generateAudio"],
		limits: {
			aspects: ["16:9", "9:16"],
			defaultAspect: "16:9",
			durations: ["4s", "6s", "8s"],
		...
		input: ({ prompt, aspect, duration, resolution, generateAudio }) => ({
			prompt,
	...
	},
	{
		...
		label: "Veo 3.1 from a still",
		takes: "image+text",
		hint: "A picture from this project, and what should happen in it. Up to eight seconds.",
		controls: ["prompt", "aspect", "duration", "resolution", "generateAudio"],
		limits: {
			/* `auto` as well here, and as the default: the still already has an
			   aspect, and forcing 16:9 on a portrait crops it for no reason. */
		...
		input: ({ imageUrl, prompt, aspect, duration, resolution, generateAudio }) => ({
			image_url: imageUrl,
			prompt,
			...(aspect ? { aspect_ratio: aspect } : {}),
			...(duration ? { duration } : {}),
	...
	/*
	 * Extending a generated shot, which is how anything gets past eight seconds.
	 *
	 * Veo's duration enum is exactly 4s, 6s and 8s — read from its schema, not
	 * from a page — so there is no ten-second call to make. A longer shot is a
	 * short one continued: generate, then extend, up to about thirty seconds in
	 * total.
	 *
	 * `duration` here is a free string with a default of 7s and no enumeration
	 * behind it, so no list is offered: a select of guesses is a 422 minutes into
	 * a paid call, which is the thing this file exists to avoid. The default
	 * stands unless the CLI is given something else on purpose.
	 *
	 * It extends Veo's own output. Handing it ordinary footage is not what it is
	 * for, and the hint says so rather than letting somebody find out slowly.
	 */
	{
		id: "fal-ai/veo3.1/extend-video",
		label: "Veo 3.1 — extend a shot",
		takes: "video+text",
		hint: "Continues a shot Veo generated, adding about seven seconds. Chain it to get past eight; roughly thirty is the ceiling.",
		controls: ["prompt", "aspect", "resolution", "generateAudio"],
		limits: {
			aspects: ["auto", "16:9", "9:16"],
			defaultAspect: "auto",
			resolutions: ["720p", "1080p", "4k"],
		...
		input: ({ videoUrl, prompt, aspect, duration, resolution, generateAudio }) => ({
			video_url: videoUrl,
			prompt,
			...(aspect ? { aspect_ratio: aspect } : {}),
			...(duration ? { duration } : {}),
 ...
 */
export async function avatarProblem({ image, video, audio, model = null }) {
	const spec = model ? modelById(model) : null;
	for (const [path, kinds, what] of [
		...(video === undefined ? [[image, IMAGE, "picture"]] : [[video, VIDEO, "clip"]]),
	...
	/* The one ceiling any of these state is on the voice, not the picture:
	   OmniHuman takes thirty seconds of audio and a narration is minutes. */
	const cap = spec?.limits?.maxAudioSeconds;
	if (cap) {
		const spoken = await durationOf(audio);
		if (!spoken) return `could not read how long ${basename(audio)} is, and ${spec.label} only takes ${cap}s of audio`;
		if (spoken > cap) return `that voice track is ${spoken.toFixed(1)}s; ${spec.label} takes at most ${cap}s — cut it shorter`;
	}
	return null;
}
...
export async function fal({ key }) {
	...
	return {
		 ...
		 */
		async edit({ model = DEFAULT_MODEL, videoUrl, imageUrl, audioUrl, prompt, keepAudio = true, imageUrls = [], resolution, syncMode, aspect, duration, generateAudio }, { onLog } = {}) {
			const spec = modelById(model);
			...
			if (takes === "image+audio" && !(imageUrl && audioUrl)) {
				throw new FalError(`${spec.label} needs a picture and a voice track, not a clip`);
			}
			if (takes === "video+audio" && !(videoUrl && audioUrl)) {
				throw new FalError(`${spec.label} needs a clip and a voice track`);
			}
			/* Generating: there is nothing to send but words, so the words are the
			   one thing that has to be there. */
			if ((takes === "text" || takes === "image+text" || takes === "video+text") && !prompt?.trim()) {
				throw new FalError("this model needs a description of the shot");
			}
			if (takes === "video+text" && !videoUrl) {
				throw new FalError(`${spec.label} needs the shot it is continuing`);
			}
			if (takes === "image+text" && !imageUrl) {
				throw new FalError(`${spec.label} needs a picture to start from`);
			}
			...
			try {
				const result = await client.subscribe(spec.id, {
					input: spec.input({
						...
						keepAudio,
						imageUrls,
						resolution: resolution || spec.limits.defaultResolution,
						syncMode: syncMode || spec.limits.defaultSyncMode,
						aspect: aspect || spec.limits.defaultAspect,
						duration: duration || spec.limits.defaultDuration,
						generateAudio,
					}),
					logs: Boolean(onLog),
