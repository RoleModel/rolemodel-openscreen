...
export async function fal({ key }) {
	...
	return {
		 ...
		 */
		async edit({ model = DEFAULT_MODEL, videoUrl, imageUrl, audioUrl, prompt, keepAudio = true, imageUrls = [], resolution, syncMode }, { onLog } = {}) {
			const spec = modelById(model);
			...
			try {
				const result = await client.subscribe(spec.id, {
					input: spec.input({ videoUrl, imageUrl, audioUrl, prompt: prompt?.trim(), keepAudio, imageUrls, resolution: resolution || spec.limits.defaultResolution, syncMode: syncMode || spec.limits.defaultSyncMode }),
					logs: Boolean(onLog),
