 ...
 */
async function fail(res, what, url = null) {
	let body = null;
	try {
		body = await res.json();
	} catch {
	...
	if (res.status === 404) {
		/* The schema hint belongs to a table that is missing, not to an auth call:
		   a spent or unknown sign-in code also answers 404, and telling somebody
		   to apply a SQL file sends them a long way from what went wrong. */
		const aboutATable = !String(url ?? "").includes("/auth/v1/");
		throw new SupabaseError(
			`${what}: ${aboutATable ? `no such table or endpoint (${detail}) — has sql/storyboards.sql been applied?` : detail}`,
			{ status: 404 },
		);
	}
...
async function req(url, options, what) {
	...
	if (!res.ok) await fail(res, what, url);
	return res;
}
...
/*
 * Signing in with a provider, which a local app has to do the long way round.
 *
 * There is no password to post: the browser goes to Supabase, Supabase sends it
 * to the provider, and the answer comes back as a redirect. So the exchange is
 * PKCE — a secret generated here, its hash sent out, and the returned code only
 * usable by whoever holds the original. The implicit flow would put the tokens
 * in a URL fragment instead, which never reaches a server at all.
 */
export function oauthUrl({ url, provider, redirectTo, codeChallenge }) {
...
/** Trade the returned code for a session, proving we started the exchange. */
export async function exchangeAuthCode({ url, key, code, verifier }) {
	const res = await req(
		`${String(url).replace(/\/+$/, "")}/auth/v1/token?grant_type=pkce`,
		{
			method: "POST",
			headers: { apikey: key, "content-type": "application/json" },
 ...
 * having to rebuild an archive in the browser.  This module remains deliberately
 * small: the table has one public shape, the authenticated caller owns every
 * request, and RLS is still the authority.
 */
const STUDIO_SKILLS_TABLE = "studio_skills";

export async function fetchStudioSkills({ url, key, token }) {
	const base = String(url).replace(/\/+$/, "");
	const res = await req(
		`${base}/rest/v1/${STUDIO_SKILLS_TABLE}?select=slug,name,description,entry_path,version,updated_at&order=name.asc`,
		{ headers: headers(key, token) },
		"read shared skills",
	);
	return res.json();
...
export async function fetchStudioSkill({ url, key, token, slug }) {
	const base = String(url).replace(/\/+$/, "");
	const res = await req(
		`${base}/rest/v1/${STUDIO_SKILLS_TABLE}?slug=eq.${encodeURIComponent(slug)}&select=*`,
		{ headers: headers(key, token) },
		"read shared skill",
	);
	const rows = await res.json();
...
export async function createStudioSkill({ url, key, token, userId, skill }) {
	const base = String(url).replace(/\/+$/, "");
	const res = await req(
		`${base}/rest/v1/${STUDIO_SKILLS_TABLE}`,
		{
			method: "POST",
			headers: { ...headers(key, token), Prefer: "return=representation" },
			body: JSON.stringify([{ ...skill, created_by: userId, updated_by: userId }]),
		},
		"create shared skill",
	);
	const rows = await res.json();
...
export async function updateStudioSkill({ url, key, token, userId, slug, skill }) {
	const base = String(url).replace(/\/+$/, "");
	const res = await req(
		`${base}/rest/v1/${STUDIO_SKILLS_TABLE}?slug=eq.${encodeURIComponent(slug)}`,
		{
			method: "PATCH",
			headers: { ...headers(key, token), Prefer: "return=representation" },
			body: JSON.stringify({ ...skill, updated_by: userId, updated_at: new Date().toISOString() }),
		},
		"update shared skill",
	);
	const rows = await res.json();
 ...
 *
 * `sql/studio-settings.sql` carries the security note that matters: a token in
 * this table is readable by every signed-in teammate, which is the point and
 * also the cost.
 */
const STUDIO_SETTINGS_TABLE = "studio_settings";

export async function fetchSetting({ url, key, token, name }) {
	const base = String(url).replace(/\/+$/, "");
	const res = await req(
		`${base}/rest/v1/${STUDIO_SETTINGS_TABLE}?key=eq.${encodeURIComponent(name)}&select=value`,
		{ headers: headers(key, token) },
		"read the shared setting",
	);
	const rows = await res.json();
...
export async function putSetting({ url, key, token, name, value, userId = null }) {
	const base = String(url).replace(/\/+$/, "");
	await req(
		`${base}/rest/v1/${STUDIO_SETTINGS_TABLE}?on_conflict=key`,
		{
			method: "POST",
			headers: { ...headers(key, token), Prefer: "resolution=merge-duplicates,return=minimal" },
			body: JSON.stringify([{ key: name, value, ...(userId ? { updated_by: userId } : {}), updated_at: new Date().toISOString() }]),
		},
		"write the shared setting",
	);
	return value;
}
