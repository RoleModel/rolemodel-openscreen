...
async function fail(res, what) {
	...
	if (res.status === 403) {
		throw new SupabaseError(`${what}: the row-level security policy refused this (${detail})`, {
			status: 403,
			hint: "the signed-in account is not a member of this board's team",
		});
	}
	if (res.status === 404) {
 ...
 */
export async function putBoard({ url, key, token, table, projectId, teamId, board }) {
	const base = String(url).replace(/\/+$/, "");
	await req(
		`${base}/rest/v1/${encodeURIComponent(table)}?on_conflict=project_id`,
		{
			...
			body: JSON.stringify([{ project_id: projectId, team_id: teamId, board, updated_at: new Date().toISOString() }]),
		},
		"write the board",
	);
	return board;
