	...
	for (const { track, y, h } of lanesOf(cut)) {
		...
		for (const clip of track.clips ?? []) {
			...
			if (track.kind === "graphic" || !clip.source) {
				...
				/*
				 * The clip's own sound, along the bottom of its picture.
				 *
				 * Linked audio belongs to the clip, so it is drawn inside it
				 * rather than duplicated onto a lane of its own — a separate row
				 * showing the same waveform is a second thing to keep in step and
				 * a second thing to drag out of sync by accident.
				 *
				 * Over a scrim, because a waveform on a bright filmstrip frame is
				 * unreadable and the strip underneath is still worth seeing.
				 */
				const own = peaks.get(clip.source);
				if (own?.peaks?.length) {
					const band = Math.min(22, Math.round(h * 0.32));
					ctx.fillStyle = "rgba(0,0,0,.42)";
					ctx.fillRect(x, y + h - band - 1, w, band);
					drawWave(ctx, { clip, peaks: own.peaks, rate: own.rate ?? 100, x, y: y + h - band - 1, w, h: band, view, colour: palette.accent });
				}
			}
		 ...
		 */
		/* Same rule the model uses: only footage dissolves. Two graphics on top of
		   each other are a stack, not a transition. */
		const ordered = track.kind === "video" ? [...(track.clips ?? [])].sort((a, b) => a.at - b.at) : [];
