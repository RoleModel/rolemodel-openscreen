...
export function heightFor(cut) {
	const rows = (cut.tracks ?? []).reduce((h, t) => h + (t.kind === "audio" ? AUDIO_H : VIDEO_H) + ROW_GAP, 0);
	return RULER_H + rows + ROW_GAP;
}
...
export function paintTimeline(canvas, { cut, palette, view, images, peaks, playhead = 0, selection = null }) {
	...
	for (const track of cut.tracks ?? []) {
		const h = track.kind === "audio" ? AUDIO_H : VIDEO_H;
		...
		for (const clip of track.clips ?? []) {
			...
			ctx.fillText(String(clip.id), x + 6, y + h - 9);
			ctx.restore();
...
export function framesInView(cut, view, widthPx) {
	...
	for (const track of cut.tracks ?? []) {
		...
		for (const clip of track.clips ?? []) {
			...
			const slot = Math.round((VIDEO_H * 16) / 9);
			for (let sx = Math.max(x, HEAD_W - slot); sx < Math.min(x + w, widthPx); sx += slot) {
				const t = xToTime(view, sx + slot / 2) - clip.at + clip.in;
