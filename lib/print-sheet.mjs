...
/* The radius a grown mask is worked at, in pixels — see dieLine. */
const TARGET_KERNEL_PX = 20;
...
export async function dieLine(svg, { size = 1024, offsetPx = 14, roundPx = null, page = null, drawn: given = null, fetchImpl = fetch } = {}) {
	...
	try {
		...
		/*
		 * The mask is scaled so the kernel never gets big.
		 *
		 * A morphology costs the area of its kernel times the area of the
		 * image, so the price runs away with the radius: a 5mm corner on a 41mm
		 * sticker is a Disk:62 over a megapixel, sixteen seconds, twice, per
		 * sticker. Working at whatever scale puts the largest radius at about
		 * twenty pixels keeps it near a second whatever is asked for — and
		 * costs nothing real, because a die line has no fine detail to lose.
		 */
		const biggest = Math.max(1, Math.round(offsetPx) + round);
		const scale = Math.min(0.5, TARGET_KERNEL_PX / biggest);
		...
		const grown = await capture("magick", [
			...
			"-bordercolor", "black",
			"-border", `${at(room)}`,
			"-morphology", "Dilate", `Disk:${at(Math.round(offsetPx) + round)}`,
			"-morphology", "Erode", `Disk:${at(round)}`,
			mask,
		]);
