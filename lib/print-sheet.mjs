 ...
 * A third of a millimetre by default, and settable in twentieths.
 *
 * Two is what a shop asks for on a big sticker, and on a 26mm one it is seven
 * per cent — wide enough that the band bridges the gap between a hammer and
 * the word beside it and the sticker becomes a blob. A bleed is a dilation and
 * a dilation joins anything closer together than twice its distance, so the
 * number has to suit the sticker rather than the trade convention. Quarter
 * millimetre steps could not express the third of one that suits these, so the
 * slider moves in twentieths.
 *
 * I set this to nothing once, having read a preview where the extra ink looked
 * like the cut being misplaced. It was not: it was the bleed, and taking it
 * away left the cut sitting on the very edge of the ink with nothing to be
 * wrong by. A die drifts. Where it drifts outward with no bleed it lands on
 * bare stock, and the sticker comes back with a white hairline down one side.
 *
 ...
 */
export function cutSheetSvg({ items, page = "letter", pageMm = null, stickerMm = 50.8, gapMm = 4, marginMm = 10, bleedMm = 3.175, stickerBleedMm = 0.35, radiusMm = 3, logo = null, title = "Stickers", skin = true, marks = true, legend = true, sideOut = {} }) {
	const p = pageMm ?? PAGES[page] ?? PAGES.letter;
	const W = p.wMm;
	const H = p.hMm;
	/* `true` is the full branded sheet; `'ground'` is the navy behind a lone sticker. */
	const brand = skin === true;
	const ground = skin === "ground" || brand;
	/* The foot carries the legend and the brand, so the grid stops above it. */
	const footMm = brand ? H * SHEET_SKIN.footFrac + 4 : ground ? 0 : logo ? 14 : 9;
	const usableW = W - marginMm * 2;
	const usableH = H - marginMm * 2 - footMm;
	/*
	 * Packed by shape, not into squares.
	 *
	 * A square cell for every sticker wastes whatever a sticker is not: a word
	 * three times as wide as it is tall used a third of its cell and the sheet
	 * came out two thirds empty with everything printed small. So each one is
	 * measured, laid left to right until the row is full, and the row is only
	 * as tall as its tallest member. The scale is then pushed up until the rows
	 * just fill the page.
	 */
	const shape = (item) => {
		const b = item.die?.inkBox;
		const n = item.die?.size ?? 1;
		if (b && b.w > 0 && b.h > 0) return { fw: b.w / n, fh: b.h / n, fx: b.x / n, fy: b.y / n };
		const box = item.svg ? svgBox(item.svg) : { x: 0, y: 0, w: 1, h: 1 };
		const long = Math.max(box.w, box.h) || 1;
		return { fw: box.w / long, fh: box.h / long, fx: 0, fy: 0 };
	};
	const shapes = items.map(shape);
	/* One attempt at a given square size: does everything fit, and where? */
	const shelve = (side) => {
		const rows = [];
		let row = [];
		let rowW = 0;
		let rowH = 0;
		let total = 0;
		for (const [i, f] of shapes.entries()) {
			const w = f.fw * side;
			const h = f.fh * side;
			if (row.length && rowW + gapMm + w > usableW) {
				rows.push({ row, rowW, rowH });
				total += rowH + gapMm;
				row = [];
				rowW = 0;
				rowH = 0;
			}
			rowW = row.length ? rowW + gapMm + w : w;
	...
	/* The biggest square that still fits, to half a millimetre. */
	let side = 10;
	for (let t = 400; t >= 10; t -= 0.5) {
		const { rows, total } = shelve(t);
		if (total <= usableH && rows.every((r) => r.rowW <= usableW)) {
			side = t;
			break;
		}
	}
	sideOut.value = side;
	const laid = shelve(side);
	const blockH = laid.total;
	let cursorY = marginMm + Math.max(0, (usableH - blockH) / 2);
	const place = new Array(items.length);
	for (const r of laid.rows) {
		let cursorX = marginMm + Math.max(0, (usableW - r.rowW) / 2);
		for (const cell of r.row) {
			/* Vertically centred in its row, so a short sticker beside a tall one
			   sits on the same line rather than at its top. */
			place[cell.i] = { x: cursorX, y: cursorY + (r.rowH - cell.h) / 2, w: cell.w, h: cell.h };
			cursorX += cell.w + gapMm;
		}
		cursorY += r.rowH + gapMm;
	}
	const used = items;
	...
	used.forEach((item, i) => {
		const f = shapes[i];
		const at = place[i];
		/* The square the artwork is drawn in, positioned so its ink lands where
		   the packing put it. */
		const stickerMm = side;
		const cx = at.x - f.fx * side;
		const cy = at.y - f.fy * side;
		...
		/* Where the cut lands, worked out once — the bleed's mask needs it too. */
		let cutPlace = "";
		let cutK = 1;
		let cutMid = null;
		if (item.die?.d) {
			const perPx = stickerMm / item.die.size;
			const room = item.die.room ?? 0;
			const inkMm = item.die.inkBox
				? { x: cx + (item.die.inkBox.x + room) * perPx, y: cy + (item.die.inkBox.y + room) * perPx, w: item.die.inkBox.w * perPx, h: item.die.inkBox.h * perPx }
				: null;
			const pb = item.die.pathBox;
			const fitted = inkMm && pb && pb.w > 0 && pb.h > 0;
			const k = fitted ? inkMm.w / pb.w : stickerMm / item.die.size;
			const ox = fitted ? inkMm.x - pb.x * k : cx;
			const oy = fitted ? inkMm.y - pb.y * k : cy;
			cutK = (k / 25.4) * 96;
			cutPlace = `translate(${(ox / 25.4) * 96} ${(oy / 25.4) * 96}) scale(${cutK})`;
			/* The sticker's own middle, not the cell's: the cut is fitted to the
			   ink and the ink is not always centred in its cell, so growing the
			   band about the cell pushed it off to one side. */
			if (inkMm) cutMid = { x: inkMm.x + inkMm.w / 2, y: inkMm.y + inkMm.h / 2 };
		}
		const bleedScale = item.die?.d && stickerMm > 0 ? 1 + (stickerBleedMm * 2) / stickerMm : 1;
		const midX = (((cutMid?.x ?? cx + stickerMm / 2)) / 25.4) * 96;
		const midY = (((cutMid?.y ?? cy + stickerMm / 2)) / 25.4) * 96;
		const bleedWrap = bleedScale === 1 ? null : `translate(${midX} ${midY}) scale(${bleedScale}) translate(${-midX} ${-midY})`;
		/*
		 * The bleed is a ring, not a second sticker.
		 *
		 * An enlarged copy behind the real one shows through wherever the real
		 * one is transparent — which, on artwork that is words on bars, is
		 * everywhere between the letters. It read as the sticker having been
		 * printed twice, slightly out of register, because that is what it was.
		 *
		 * So the copy is masked to the ring between the cut and the cut grown
		 * by the bleed: white where the enlarged silhouette is, black where the
		 * real one is. Nothing of it can appear inside the sticker.
		 */
		/*
		 * The bleed is one filled shape in the edge's own colour.
		 *
		 * Not an enlarged copy of the sticker: scaling moves the words outward
		 * too, and near the edge they showed in the band as a second, offset
		 * printing. A solid band of the keyline colour is what a prepress edge
		 * extension produces anyway, and it has nothing in it to ghost.
		 */
		const bleedBand =
			item.die?.d && item.die.bleedD && item.die.edgeColor
				? `<g transform="${cutPlace}"><g${item.die.transform ? ` transform="${item.die.transform}"` : ""}><path d="${item.die.bleedD}" fill="${item.die.edgeColor}"/></g></g>`
				: "";
		...
		if (item.svg) {
			...
			/*
			 * The bleed goes UNDER the sticker, not around a shrunken one.
			 *
			 * Scaling the drawing up and leaving the cut where it was does put
			 * ink outside the cut — and it also cuts a millimetre and a half
			 * inside the keyline somebody drew, so the finished sticker has a
			 * thinner white border than the artwork asked for. Which is what it
			 * looked like, because that is what it was.
			 *
			 * An enlarged copy behind the real one leaves the drawing exactly
			 * as drawn, the cut exactly on its edge, and colour beyond the cut
			 * for the die to be wrong into. That is the edge extension every
			 * prepress department makes by hand.
			 */
			art.push(bleedBand ? `${bleedBand}${one}` : one);
		} else if (item.bytes) {
		...
		if (item.die?.d) {
			/*
			 * The cut is laid onto the drawing by matching two measured boxes.
			 *
			 * Everything before this derived the placement: the square the ink
			 * was measured in, the square it was traced in, the border, the
			 * cell. Every step was defensible and the line still came out a
			 * fraction down and to the right, because potrace does not put its
			 * output exactly where its bitmap was and no amount of arithmetic
			 * here could know by how much.
			 *
			 * So both boxes are measured — where the ink sits in the drawing,
			 * where the path sits in its own output — and one is scaled and
			 * shifted onto the other. There is nothing left to accumulate.
			 */
			cuts.push(`<g transform="${cutPlace}"><g${item.die.transform ? ` transform="${item.die.transform}"` : ""}><path d="${item.die.d}" fill="none" stroke="${CUT_MAGENTA}" stroke-width="${CUT_UNITS / cutK}"/></g></g>`);
		} else {
	...
	const legendSvg = `<text x="${W}mm" y="${-bleedMm * 0.3}mm" text-anchor="end" font-family="Helvetica, Arial, sans-serif" font-size="2.2" fill="#000">${esc(title)} · ${used.length} on a ${p.label} sheet · cut lines are 100% magenta, do not print</text>`;
...
export function cutSheetPages(opts) {
	/*
	 * One page. The packer shrinks until everything fits, so there is no
	 * overflow to spill onto a second — and a sheet is priced by the sheet.
	 */
	const sideOut = {};
	const svg = cutSheetSvg({ ...opts, sideOut });
	if (opts.sideOut) opts.sideOut.value = sideOut.value;
	return [svg];
}
...
export async function cutSheetToCmykPdf({ items, out, work, profile = "", ...layout }) {
	...
	await mkdir(work, { recursive: true });
	const sideOut = {};
	const svgs = cutSheetPages({ ...layout, items, sideOut });
	const pdfs = [];
	...
	if (!joined.ok || !(await has(out))) throw new Error(`Ghostscript would not write the PDF: ${joined.err.trim().slice(0, 200)}`);
	await rm(work, { recursive: true, force: true });
	return { file: out, pages: pdfs.length, perSheet: items.length, sideMm: sideOut.value ?? null, profile: icc, profileName: icc ? basename(icc, extname(icc)) : "Ghostscript's own CMYK" };
}
...
export async function dieLine(svg, { size = 1024, offsetPx = 14, roundPx = null, bleedPx = 0, page = null, drawn: given = null, fetchImpl = fetch } = {}) {
	...
	try {
		...
		/*
		 * The colour the bleed should be.
		 *
		 * Bleed is an extension of the ink at the edge, and on these stickers
		 * that edge is a solid keyline. Sampling it gives a band that can be
		 * drawn as one filled shape — which cannot ghost, because there is
		 * nothing in it to ghost. Enlarging the artwork instead moved the words
		 * outward with it, and they showed in the band as a second, offset
		 * printing of the sticker.
		 *
		 * Taken from a thin ribbon just inside the silhouette: erode the shape,
		 * subtract it from itself, and average what is left.
		 */
		let edgeColor = null;
		try {
			const ribbon = join(dir, "edge.png");
			const band = join(dir, "band.png");
			await capture("magick", [png, "-alpha", "extract", "-threshold", "30%", "-morphology", "Distance", "Euclidean:1", "-threshold", `${Math.round(6 * 257)}`, ribbon]);
			await capture("magick", [png, "-alpha", "extract", "-threshold", "30%", ribbon, "-compose", "Difference", "-composite", band]);
			/*
			 * Averaged with the transparency kept, not turned off.
			 *
			 * Flattening first counts every transparent pixel as black, and the
			 * answer comes back a muddy sixth of the real colour. Resizing to a
			 * single pixel against no background makes the mean alpha-weighted,
			 * which is the average of what is actually painted.
			 */
			const mean = await capture("magick", [png, "-alpha", "off", band, "-compose", "CopyOpacity", "-composite", "-background", "none", "-resize", "1x1!", "-format", "%[pixel:p{0,0}]", "info:"]);
			const pct = String(mean.out).match(/srgba?\(([\d.]+)%,\s*([\d.]+)%,\s*([\d.]+)%/);
			const hex = String(mean.out).match(/#[0-9A-Fa-f]{6}/);
			if (pct) edgeColor = `#${pct.slice(1, 4).map((n) => Math.max(0, Math.min(255, Math.round((Number.parseFloat(n) / 100) * 255))).toString(16).padStart(2, "0")).join("")}`;
			else if (hex) edgeColor = hex[0];
		} catch {
			/* Without a colour the bleed is simply not drawn. */
		}
		/*
		...
		/*
		 * The bleed outline, traced rather than scaled.
		 *
		 * Scaling the cut up by a percentage is not an offset: on a sticker
		 * three times as wide as it is tall, the same percentage adds three
		 * times as much ink at the sides as at the top, and the band comes out
		 * fat on two edges and thin on the others. A second distance transform
		 * at the bleed distance gives a band the same width all the way round,
		 * which is what a bleed is.
		 */
		let bleedD = null;
		if (bleedPx > 0) {
			try {
				const wide = join(dir, "wide.png");
				const widePgm = join(dir, "wide.pgm");
				const wideSvg = join(dir, "wide.svg");
				await capture("magick", [
					png,
					"-alpha", "extract",
					"-threshold", "30%",
					...(scale < 1 ? ["-resize", `${(scale * 100).toFixed(4)}%`] : []),
					...
					"+repage",
					"-negate",
					"-morphology", "Distance", "Euclidean:1",
					"-threshold", `${q(Math.round(offsetPx) + round + bleedPx)}`,
					"-negate",
					wide,
				]);
				await capture("magick", [wide, "-negate", "-depth", "8", widePgm]);
				const t2 = await capture("potrace", [widePgm, "-s", "-a", "1.0", "-O", "0.2", "-t", "8", "-o", wideSvg]);
				if (t2.ok) {
					const w2 = await readFile(wideSvg, "utf8");
					const d2 = [...w2.matchAll(/<path[^>]*\bd="([^"]+)"/g)].map((m) => m[1]);
					/* Its own transform is the same shape as the cut's, and it is
					   placed by the same fit, so only the path differs. */
					if (d2.length) bleedD = d2.join(" ");
				}
			} catch {
				/* Without it there is simply no bleed. */
			}
		}
		return { d: joined, bleedD, transform, size: grownSize, square, art: drawn.square, shapeBox, pathBox, inkBox: drawn.inkPx, artSize: drawn.size ?? size, room: at(room), edgeColor };
	} catch {
		return null;
	} finally {
		await rm(dir, { recursive: true, force: true });
 ...
 */
export async function withDieLines(items, { offsetPx = 14, roundPx = null, bleedPx = 0, size = 1024, lanes = 4 } = {}) {
	const out = new Array(items.length);
	/* A raster cutout is wrapped as an <image> so potrace can trace its
	   silhouette; only something with neither shape is left untraced. */
	if (!items.some((i) => i.svg || i.bytes)) return items.slice();
	const { chromium } = await import("playwright");
		...
		const worker = async () => {
			...
			try {
				while (next < items.length) {
					const i = next++;
					const item = items[i];
					const svg = item.svg ?? (item.bytes ? rasterToSvg(item.bytes, item.type) : null);
					out[i] = svg ? { ...item, die: await dieLine(svg, { offsetPx, roundPx, bleedPx, size, page }) } : item;
				}
			} finally {
				await page.close();
			}
...
/** A raster cutout as a standalone SVG, so the die tracer and the page draw it in one frame. */
export function rasterToSvg(bytes, type = "image/png") {
	const data = `data:${type};base64,${Buffer.from(bytes).toString("base64")}`;
	return `<svg xmlns="http://www.w3.org/2000/svg" width="1024" height="1024" viewBox="0 0 1024 1024"><image href="${data}" x="0" y="0" width="1024" height="1024" preserveAspectRatio="xMidYMid meet"/></svg>`;
}

/**
 * The page one sticker needs, sized to its own shape plus a margin.
 *
 * Fed back to cutSheetSvg as `pageMm`, it makes the packer land the sticker's
 * long side at `stickerMm` with `marginMm` of ground around it — no wasted 4x6.
 */
export function fitSheetPage(item, { stickerMm = 76.2, marginMm = 6 } = {}) {
	const b = item.die?.inkBox;
	const n = item.die?.size ?? 1;
	let fw;
	let fh;
	if (b && b.w > 0 && b.h > 0) {
		...
		const box = item.svg ? svgBox(item.svg) : { w: 1, h: 1 };
		const long0 = Math.max(box.w, box.h) || 1;
		fw = box.w / long0;
		fh = box.h / long0;
	}
	const long = Math.max(fw, fh) || 1;
	const wMm = (fw / long) * stickerMm + marginMm * 2;
	const hMm = (fh / long) * stickerMm + marginMm * 2;
	return { label: `${Math.round(wMm)}\u00d7${Math.round(hMm)}mm`, wMm, hMm };
}

/**
 * Die-cut singles as one CMYK PDF: each sticker alone on a fitted page, on the
 * navy ground, with its traced cut line and no branding.
 *
 * The same rsvg + Ghostscript road the sheet takes, one sticker per page. A
 * raster is wrapped as an <image> so it both traces and draws in one frame.
 */
export async function dieCutSinglesToCmykPdf({ items, out, work, sizeMm = 76.2, marginMm = 6, stickerBleedMm = 0.35, offsetPx = 0, roundPx = 0, bleedPx = 0, die = true, profile = "" }) {
	const stop = await printProblem();
	if (stop) throw new Error(stop);
	if (!items.length) throw new Error("there are no stickers to print");
	await rm(work, { recursive: true, force: true });
	await mkdir(work, { recursive: true });
	const prepared = items.map((it) => (it.svg || !it.bytes ? it : { ...it, svg: rasterToSvg(it.bytes, it.type) }));
	const traced = die ? await withDieLines(prepared, { offsetPx, roundPx, bleedPx }) : prepared;
	const pdfs = [];
	for (const [i, item] of traced.entries()) {
		const pageMm = fitSheetPage(item, { stickerMm, marginMm });
		const svg = cutSheetSvg({ items: [item], pageMm, gapMm: 0, marginMm, bleedMm: 0, stickerBleedMm, logo: null, skin: "ground", marks: false, legend: false });
		const src = join(work, `p${String(i).padStart(3, "0")}.svg`);
		const pdf = join(work, `p${String(i).padStart(3, "0")}.pdf`);
		await writeFile(src, svg, "utf8");
		const made = await capture("rsvg-convert", ["-f", "pdf", "-o", pdf, src]);
	...
	if (!joined.ok || !(await has(out))) throw new Error(`Ghostscript would not write the PDF: ${joined.err.trim().slice(0, 200)}`);
	await rm(work, { recursive: true, force: true });
	return { file: out, pages: pdfs.length, sizeMm, profile: icc, profileName: icc ? basename(icc, extname(icc)) : "Ghostscript's own CMYK" };
}
