...
export function cutSheetSvg({ items, page = "letter", stickerMm = 50.8, gapMm = 4, marginMm = 10, bleedMm = 3, radiusMm = 3, logo = null, title = "Stickers", skin = true }) {
	...
	used.forEach((item, i) => {
		...
		if (item.die?.d) {
			...
			cuts.push(`<g transform="translate(${(cx / 25.4) * 96} ${(cy / 25.4) * 96}) scale(${(k / 25.4) * 96})"><g${item.die.transform ? ` transform="${item.die.transform}"` : ""}><path d="${item.die.d}" fill="none" stroke="${CUT_MAGENTA}" stroke-width="${CUT_UNITS / ((k / 25.4) * 96)}"/></g></g>`);
		} else {
	...
	const marks = [
		...
		.map(([x1, y1, x2, y2]) => `<line x1="${x1}mm" y1="${y1}mm" x2="${x2}mm" y2="${y2}mm" stroke="#fff" stroke-width="${CUT_UNITS}"/>`)
		.join("");
	...
	const markMm = footMm * 0.5;
	const skinFore = skin
		? `<text x="${marginMm}mm" y="${footTop + footMm * 0.52}mm" font-family="Helvetica, Arial, sans-serif" font-weight="700" font-size="${nameSize}mm" fill="${sk.name}">RoleModel Software</text>` +
			`<text x="${marginMm}mm" y="${footTop + footMm * 0.82}mm" font-family="Helvetica, Arial, sans-serif" font-size="${nameSize * 0.62}mm" fill="${sk.kind}">Sticker Sheet</text>` +
			`<rect x="${W - marginMm - markMm}mm" y="${footTop + (footMm - stripMm - markMm) / 2}mm" width="${markMm}mm" height="${markMm}mm" rx="${markMm * 0.12}mm" fill="${sk.mark}"/>` +
			`<text x="${W - marginMm - markMm / 2}mm" y="${footTop + (footMm - stripMm) / 2 + markMm * 0.26}mm" text-anchor="middle" font-family="Helvetica, Arial, sans-serif" font-weight="700" font-size="${markMm * 0.72}mm" fill="${sk.name}">R</text>`
		: logo
