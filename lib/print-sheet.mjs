 ...
 */
export function cutSheetSvg({ items, page = "letter", stickerMm = 50.8, gapMm = 4, marginMm = 10, bleedMm = 3.175, stickerBleedMm = 0.35, radiusMm = 3, logo = null, title = "Stickers", skin = true, sideOut = {} }) {
		...
		const worker = async () => {
			...
			try {
				while (next < items.length) {
					const i = next++;
					const item = items[i];
					out[i] = item.svg ? { ...item, die: await dieLine(item.svg, { offsetPx, roundPx, bleedPx, size, page }) } : item;
				}
			} finally {
				await page.close();
			}
