 ...
 * Two millimetres by default, which is what a sticker shop asks for.
 *
 ...
 */
export function cutSheetSvg({ items, page = "letter", stickerMm = 50.8, gapMm = 4, marginMm = 10, bleedMm = 3.175, stickerBleedMm = 2, radiusMm = 3, logo = null, title = "Stickers", skin = true }) {
	...
	used.forEach((item, i) => {
		 ...
		 */
		const bleedBand =
			item.die?.d && bleedWrap && item.die.edgeColor
				? `<g transform="${bleedWrap}"><g transform="${cutPlace}"><g${item.die.transform ? ` transform="${item.die.transform}"` : ""}><path d="${item.die.d}" fill="${item.die.edgeColor}"/></g></g></g>`
				: "";
...
export async function dieLine(svg, { size = 1024, offsetPx = 14, roundPx = null, page = null, drawn: given = null, fetchImpl = fetch } = {}) {
	...
	try {
		...
		return { d: joined, transform, size: grownSize, square, art: drawn.square, shapeBox, pathBox, inkBox: drawn.inkPx, artSize: drawn.size ?? size, room: at(room), edgeColor };
	} catch {
		return null;
	} finally {
		await rm(dir, { recursive: true, force: true });
