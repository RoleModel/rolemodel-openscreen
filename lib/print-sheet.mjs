...
export function cutSheetSvg({ items, page = "letter", stickerMm = 50.8, gapMm = 4, marginMm = 10, bleedMm = 3.175, stickerBleedMm = 1, radiusMm = 3, logo = null, title = "Stickers", skin = true }) {
	...
	used.forEach((item, i) => {
		 ...
		 */
		const grow = item.die?.d ? stickerBleedMm * 2 : 0;
		const aw = item.die?.d ? stickerMm + grow : w;
		const ah = item.die?.d ? stickerMm + grow : h;
		const ax = item.die?.d ? cx - grow / 2 : cx + (stickerMm - w) / 2;
		const ay = item.die?.d ? cy - grow / 2 : cy + (stickerMm - h) / 2;
