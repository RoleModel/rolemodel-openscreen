...
export async function dieLine(svg, { size = 1024, offsetPx = 14, roundPx = null, page = null, drawn: given = null, fetchImpl = fetch } = {}) {
	...
	try {
		...
		const grown = await capture("magick", [
			...
			"-negate",
			/*
			 * Shrink: on the grown shape, a distance transform measures how far
			 * each of its pixels is from the outside, and keeping only those
			 * further than the radius is the erode. No inversion afterwards —
			 * one there turned the answer inside out and drew a rectangle round
			 * the whole cell, which is what a complement looks like.
			 */
			"-morphology", "Distance", "Euclidean:1",
			"-threshold", `${q(round)}`,
			mask,
