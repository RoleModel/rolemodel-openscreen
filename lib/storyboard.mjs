...
import { NODE_GAP_X, NODE_HEIGHT, NODE_WIDTH, clampPoint, idFor as graphId, runningOrder } from "./board-graph.mjs";
...
/**
 * The graph's layout for this board, derived if it has none.
 *
 * Node ids ARE slot ids. That is the whole reason this is additive rather than a
 * migration: every take, rating and pick already points at a slot, and giving
 * the canvas its own node identity would have meant re-linking all of them —
 * for a layout change. The graph adds POSITION and ORDER; the slot remains what
 * a shot is.
 *
 * A board saved before the canvas existed gets a row, wired in its stored order,
 * so opening it shows the plan it already had rather than a heap.
 */
export function graphFor(board) {
	const slots = [...(board?.slots ?? [])].sort((a, b) => a.order - b.order);
	const stored = board?.graph;
	const byId = new Map((stored?.nodes ?? []).map((n) => [n.id, n]));
	const nodes = slots.map((s, i) => {
		const at = byId.get(s.id);
		const placed = at ? clampPoint(at.x, at.y) : clampPoint(400 + i * (NODE_WIDTH + NODE_GAP_X), 400);
		return { id: s.id, kind: at?.kind ?? "shot", name: s.name, intent: s.intent, seconds: s.seconds, ...placed };
	});
	// Nodes the graph has that no slot backs — a note, a title — keep their own
	// record, since nothing else in the board describes them. A shot is different:
	// it is only the graph position for a storyboard slot. When a new script or
	// interview replaces the slot list, keeping its old shot node would put a
	// clickable card back on the Canvas that no longer has a shot behind it. The
	// Scene editor then receives that retired id and cannot save the relationship.
	for (const n of stored?.nodes ?? []) {
		if (nodes.some((x) => x.id === n.id) || n.kind === "shot" || !n.kind) continue;
		nodes.push({ ...n, ...clampPoint(n.x, n.y) });
	}
	const wires = (stored?.wires ?? []).filter(
		(w) => nodes.some((n) => n.id === w.from) && nodes.some((n) => n.id === w.to),
	);
	// No wires yet means a board that predates the canvas: string it together in
	// the order it already had, so the plan is what you see on opening it.
	if (!wires.length && nodes.length > 1 && !stored?.wires) {
		for (let i = 1; i < slots.length; i++) {
			wires.push({ id: graphId("wire", `${slots[i - 1].id}->${slots[i].id}`), from: slots[i - 1].id, to: slots[i].id });
		}
	}
	return { nodes, wires };
}
...
/**
 * Slots in the order the video plays.
 *
 * The WIRES decide it once a board has any — that is what makes dragging a wire
 * an edit rather than a decoration. `order` remains the fallback for a board
 * nobody has wired, and for the arithmetic in the tests that predate the canvas.
 */
export function orderedSlots(board) {
	const slots = [...(board?.slots ?? [])];
	const graph = board?.graph;
	if (graph?.wires?.length) {
		const bySlot = new Map(slots.map((s) => [s.id, s]));
		const run = runningOrder(graphFor(board));
		const ordered = run.order.map((n) => bySlot.get(n.id)).filter(Boolean);
		// Anything the walk did not reach still belongs to the video — see
		// runningOrder on why a shot is never silently dropped.
		for (const s of slots) if (!ordered.includes(s)) ordered.push(s);
		return ordered;
	}
...
export function toCutlist(board) {
	...
	for (const slot of orderedSlots(board)) {
		...
		if (!take) continue;
		out.push({
			// The Canvas scene is part of the edit decision, not presentation
			// metadata to rediscover later. Keeping it beside its winning take lets
			// the HyperFrames handoff carry the title, shader, and lower thirds that
			// were composed for this exact slot.
			slotId: slot.id,
