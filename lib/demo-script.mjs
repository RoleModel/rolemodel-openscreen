...
export const DIRECTIVES = {
	...
	// Marks the following script section as belonging to a person. It is planning
	// metadata for Claude, never something Voice should read aloud.
	speaker: { help: '/speaker "Dallas"', hint: "the person responsible for the following section" },
	...
	sub: { help: '/sub "Selected from three recordings"', hint: "the line under the title" },
	// A halftone reveal of a brand image behind the opening card, and — because
	// it carries the cut's boundaries — a background that shifts ahead of every
	// edit. The one motion decision DESIGN.md makes; see "Motion".
	reveal: { help: "/reveal academy-logo-clay.png", hint: "a brand image, pixel-revealed behind the title" },
...
/**
 * Named ownership for a contiguous portion of a script.
 *
 * `/speaker "Alex"` applies until the next speaker line. The original markdown
 * stays intact for Claude — this is deliberately not a second script model — but
 * a render brief can make the handoff explicit instead of guessing from clip
 * filenames or the order people happened to upload them.
 */
/**
 * Does this line look like somebody's name, standing alone?
 *
 * Two to four words, each starting with a capital, no sentence punctuation and
 * no markdown furniture. Deliberately strict: mistaking a line of narration for
 * a name removes it from what that person says, which is worse than missing a
 * lower third. `/speaker` remains the explicit way to say so when a name is
 * unusual enough that this will not see it.
 */
export function isNameLine(line) {
	...
	// Every word capitalised, allowing O'Brien, Smith-Jones and initials.
	return words.every((w) => /^[A-Z][A-Za-z'’-]*\.?$/.test(w));
}

/**
 * A job title standing under a name — the lower third's second line.
 *
 * Looser than `isNameLine` because a role is not capitalised the same way
 * ("Chief Craftsman", "software engineer", "VP of Delivery"), and only ever
 * consulted on the line directly beneath a name, which is what keeps it from
 * eating narration. Still refuses anything that ends like a sentence.
 */
...
/**
 * Split "Blaine Irvin, Chief Craftsman" into its two parts.
 *
 * A comma or a dash on the name line is the unambiguous way to give a role, and
 * the one worth supporting first: it cannot be confused with the sentence that
 * follows. Returns `{ speaker, role }` with `role` null when there is none.
 */
export function splitSpeaker(line, { strict = true } = {}) {
	const text = String(line ?? "").trim();
	const at = text.search(/\s*(?:,|—|–|\s-\s)\s*/);
	if (at > 0) {
		const speaker = text.slice(0, at).trim();
		const role = text.replace(text.slice(0, at), "").replace(/^\s*(?:,|—|–|\s-\s)\s*/, "").trim();
		/*
		 * `strict` is for a BARE line, where the left side has to look like a name
		 * or any sentence containing a comma would be read as a speaker. After an
		 * explicit `/speaker`, the person has already said this is a name — a
		 * one-word "Mina, Designer" is theirs to write and must not be refused.
		 */
		if (role && (!strict || isNameLine(speaker))) return { speaker, role };
	}
	return { speaker: text, role: null };
}

export function speakerSections(md) {
	const sections = [];
	let speaker = null;
	let role = null;
	let lines = [];
	let inFence = false;
	/* `justNamed` is true only on the line straight after a name, which is the
	   single place a bare role line is allowed to be read as one. */
	let justNamed = false;
	const flush = () => {
		const text = lines.join("\n").trim();
		if (speaker && text) sections.push({ speaker, ...(role ? { role } : {}), text });
		lines = [];
	};
	...
	for (const raw of all) {
		const line = raw.trim();
		const wasJustNamed = justNamed;
		// Blank lines between a name and its role are allowed; anything else ends it.
		justNamed = wasJustNamed && !line;
		if (line.slice(0, 3) === "```") {
			inFence = !inFence;
			if (speaker) lines.push(raw);
			continue;
		}
		if (!inFence) {
			const match = /^\/speaker\s+(.+)$/i.exec(line);
			if (match) {
				flush();
				const said = splitSpeaker(match[1].trim().replace(/^"(.*)"$/, "$1"), { strict: false });
				speaker = said.speaker;
				role = said.role;
				justNamed = !role;
				continue;
			}
			// Other directives configure the render; they are not part of a person's words.
			if (/^\/[a-z][a-z-]*(\s|$)/i.test(line)) continue;
			/*
			 * A bare name on its own line is a speaker too.
			 *
			 * This is how people write a script — the name, then what they say —
			 * and requiring `/speaker "Blaine Irvin"` instead meant a script that
			 * named five people produced zero speaker sections. Nothing errored:
			 * the beats simply carried no speaker, the picks inherited none, and
			 * the exporter's `clip.speaker && ...` was false for every clip, so a
			 * finished video came out with no lower thirds and nothing to say why.
			 *
			 * Kept deliberately narrow, because a false positive silently eats a
			 * line of narration: two to four capitalised words, no trailing
			 * punctuation, and actual words beneath it. "Blaine Irvin" matches;
			 * "Introduction" (one word) and "Time wasn't the only problem." (ends
			 * in a full stop) do not.
			 */
			/*
			 * The line under a name is that person's title — checked FIRST.
			 *
			 * "Chief Craftsman" is two capitalised words and so passes isNameLine
			 * perfectly well; testing for a name before a role made every job title
			 * replace the person who had just been named. Directly under a name,
			 * a role is the likelier reading, and it is the only place this fires:
			 * `wasJustNamed` is false everywhere else, so a short line elsewhere in
			 * the script stays narration.
			 */
			if (wasJustNamed && speaker && !role && line && isRoleLine(line)) {
				role = line;
				continue;
			}
			const named = splitSpeaker(line);
			if (named.role || isNameLine(line)) {
				flush();
				speaker = named.speaker;
				role = named.role;
				/* Only look for a role on the next line when this one did not
				   already carry one after a comma or a dash. */
				justNamed = !role;
				continue;
			}
		}
