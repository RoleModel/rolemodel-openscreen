 ...
 * about whether somebody wrote the code. `syncFor` refuses an adapter that
 * answers no and falls back to local, so a half-configured project degrades to a
 * working board rather than to a broken panel.
 *
...
import { fetchBoard, liveSession, probe, putBoard, signIn as sbSignIn, signUp as sbSignUp } from "./supabase.mjs";
...
export const SUPABASE_SYNC = {
	...
	detail: "One shared board. Ratings and picks from everybody, merged on every sync.",
	/*
 ...
 *
 * An unknown or not-ready id falls back to local rather than throwing, for the
 * same reason `agentFor` does: a settings file naming a backend this build cannot
 * reach should degrade to a working board, not a broken panel. The one difference
 * is that not-ready falls back too — an agent that is merely untested still runs,
 * whereas an unimplemented sync adapter would throw on the first rating.
 */
export async function syncFor(id) {
	const want = SYNCS[String(id ?? "")];
	if (!want) return SYNCS[DEFAULT_SYNC];
	// `ready` is a value on the local adapter and a question on the hosted one:
	// whether this machine is configured to reach a project is not knowable
	// without reading the settings.
	const ok = typeof want.ready === "function" ? await want.ready() : want.ready;
	return ok ? want : SYNCS[DEFAULT_SYNC];
}
...
export async function syncBoard(dir, board, adapter = LOCAL_SYNC) {
	...
	await adapter.push(merged).catch(() => {});
