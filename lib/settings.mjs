 ...
 *
 * Defaulted rather than required, and the default is a URL that works TODAY: the
 * fork is public, and GitHub renders the synced markdown readably without any
 * site being deployed. A "Docs" button that opens nothing until somebody
 * configures infrastructure is a button that teaches people the app is broken.
 *
 * Set it to a Docusaurus deployment when there is one — see docs/AI-AGENTS.md
 * for how the site is built and where it goes.
 */
export const DEFAULT_DOCS_URL = "https://github.com/RoleModel/openscreen/tree/main/website/docs/rolemodel";
