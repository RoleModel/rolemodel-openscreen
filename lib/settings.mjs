...
export async function stickerSettings() {
	const stored = await read();
	/* Prodigi is the one vendor with a real quotes API; the key is a machine
	   setting like every other key here, never a project file. */
	return { dataApi: stored.stickers?.dataApi ?? null, prodigiKey: stored.stickers?.prodigiKey ?? "", prodigiSandbox: Boolean(stored.stickers?.prodigiSandbox) };
}
