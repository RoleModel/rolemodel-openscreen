 ...
 *
 * Credentials were once environment variables only, which works from a
 ...
 *
 * Stored in the config file: environment
 ...
 */
/** Shorter than this is not a token anyone was issued. */
const MIN_TOKEN = 24;
...
/** Store them for later runs. Write-only from the UI, like every credential here. */
...
export async function setSidebarRail(on) {
	const stored = await read();
	await mkdir(dirname(CONFIG_FILE), { recursive: true });
	// 0600, same reason as setLastView below: credentials live in this
...
export async function setLastView(view) {
	const stored = await read();
	await mkdir(dirname(CONFIG_FILE), { recursive: true });
	// 0600 and tabs, matching the other writers: this file also holds the
	// credentials, and `writeFile` applies a mode only when it CREATES the file.
 ...
 *
 * Kept in the same 0600 file as the other credentials because it holds a GitHub
...
/** Where the published sticker pages read and write comments: a Neon Data API URL. */
export async function stickerSettings() {
	const stored = await read();
	/*
	 * Prodigi is the one vendor with a real quotes API. Its key is a machine
	 * setting like every other key here, never a project file — and the
	 * environment wins, the same way FAL_KEY does, so a key in .env or in a
	 * shell is the one used without anything being typed into the Studio.
	 */
	const envKey = process.env.PRODIGI_API_KEY?.trim();
	return {
		dataApi: stored.stickers?.dataApi ?? null,
		prodigiKey: envKey || stored.stickers?.prodigiKey?.trim() || "",
		prodigiSource: envKey ? "environment" : stored.stickers?.prodigiKey ? "stored" : null,
		prodigiSandbox: Boolean(stored.stickers?.prodigiSandbox),
	};
}

export async function setStickerSettings(patch) {
	const stored = await read();
	await mkdir(dirname(CONFIG_FILE), { recursive: true });
	await writeFile(CONFIG_FILE, `${JSON.stringify({ ...stored, stickers: { ...(stored.stickers ?? {}), ...patch } }, null, "\t")}\n`, { mode: 0o600 });
}
...
/*
 * The toolkit's own .env, for the keys somebody keeps in a file.
 *
 * Only what is not already set: an environment variable given on the command
 * line or exported in a shell is a deliberate act for this run, and a file on
 * disk should not quietly outrank it. Missing or unreadable is the ordinary
 * case, not a fault.
 */
export async function loadEnvFile(root) {
	const text = await readFile(join(root, ".env"), "utf8").catch(() => null);
	if (!text) return [];
	const set = [];
	for (const line of text.split(/\r?\n/)) {
		...
		process.env[name] = m[2].trim().replace(/^(['"])(.*)\1$/, "$2");
		set.push(name);
	}
	return set;
}
