...
export function slack({ token, fetchImpl = fetch } = {}) {
	...
	async function findChannel(name) {
		...
		/*
		 * A DM has no name to match, so the id is the only way to name one.
		 *
		 * Slack does not show a conversation id anywhere obvious, but it is the
		 * tail of the conversation's link — Copy link, and take the C/G/D… off
		 * the end. Settings takes an id as readily as a name and skips this
		 * lookup entirely when it sees one.
		 */
		throw new SlackError(
			`no conversation called #${wanted} that this app can see. Check the spelling and that the app has been invited to it`
				+ " — and if it is a DM or a group conversation rather than a channel, it has no name to look up:"
				+ " use Copy link on it in Slack and paste the id off the end (it starts with C, G or D).",
		);
	}
	...
	async function scopes() {
		...
		const granted = (res.headers.get("x-oauth-scopes") ?? "")
			...
			.map((s) => s.trim())
			.filter(Boolean);
		const missing = NEEDED.filter((n) => !granted.includes(n.scope));
		const canRead = READ_SCOPES.filter((r) => granted.includes(r.scope));
		return {
			team: json.team ?? null,
			user: json.user ?? null,
			granted,
			...
			/*
			 * No read scope at all is the case worth naming, because it is the one
			 * that fails as `channel_not_found` later instead of as a permission.
			 */
			problem: missing.length
				? `this token cannot ${missing.map((m) => m.why).join(" or ")} — it is missing ${missing.map((m) => m.scope).join(" and ")}`
				: canRead.length === 0
					? "this token has no read scope, so Slack will report every conversation as `channel_not_found`"
						+ ` however correct the id is — add ${READ_SCOPES.map((r) => `\`${r.scope}\` for ${r.why}`).join(", ")}`
						+ ", whichever matches where you are posting, then reinstall the app"
					: null,
			reaches: canRead.map((r) => r.why),
		};
	}
	...
	return { call, postVideo, findChannel, whoami, scopes };
}
