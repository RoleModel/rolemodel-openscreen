 ...
 */
export function videoPage({ title, videoFile, altFile = null, posterFile = null, made = new Date(), comments = null, project = "", slug = "" }) {
	const typeOf = (f) => ({ ".mp4": "video/mp4", ".webm": "video/webm", ".mov": "video/quicktime", ".m4v": "video/mp4" })[String(f).slice(String(f).lastIndexOf(".")).toLowerCase()] ?? "video/mp4";
	/* The MP4 first: every browser plays H.264; the original follows for the ones that prefer it. */
	const sources = [videoFile, altFile].filter(Boolean).map((f) => `<source src="${esc(f)}" type="${typeOf(f)}" />`).join("");
	return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8" />
...
<div class="wrap">
  <main>
    <video id="v" controls playsinline preload="metadata"${posterFile ? ` poster="${esc(posterFile)}"` : ""}>${sources}</video>
...
export async function publishShare({ projectDir, projectId, file, title, remote, publicBase, publicBucket, dataApi, remotePath = (r, p) => `${r}:${p}`, onStep = () => {} }) {
	...
	/*
	 * The page's video is an H.264 MP4, whatever came in. A VP9 WebM plays in
	 * Chrome and not in Safari, and a share page that only works in some
	 * browsers is a page that does not work. The original rides along as a
	 * second source when it was not already that MP4.
	 */
	let videoFile = `video${ext}`;
	let altFile = null;
	await copyFile(file, join(site, videoFile));
	const probe = await capture("ffprobe", ["-v", "quiet", "-select_streams", "v:0", "-show_entries", "stream=codec_name", "-of", "csv=p=0", file]).catch(() => ({ ok: false, out: "" }));
	const codec = String(probe.out ?? "").trim();
	if (!(ext === ".mp4" && codec === "h264")) {
		onStep("making an MP4 every browser plays");
		const made = await capture("ffmpeg", ["-y", "-i", file, "-c:v", "libx264", "-pix_fmt", "yuv420p", "-preset", "veryfast", "-crf", "20", "-movflags", "+faststart", "-c:a", "aac", "-b:a", "160k", join(site, "video.mp4")]).catch(() => ({ ok: false }));
		if (made.ok && (await stat(join(site, "video.mp4")).catch(() => null))) {
			altFile = videoFile === "video.mp4" ? null : videoFile;
			videoFile = "video.mp4";
		}
	}
	...
	const made = new Date();
	await writeFile(join(site, "index.html"), videoPage({ title: name, videoFile, altFile, posterFile, made, comments: dataApi ? { dataApi } : null, project: projectId, slug }), "utf8");
	...
	const record = { slug, title: name, file: basename(file), rel: null, remote, bucket: publicBucket, url, video: videoFile, alt: altFile, poster: posterFile, at: made.toISOString(), comments: Boolean(dataApi) };
