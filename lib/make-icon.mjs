...
async function macIconSource() {
	const icns = join(srcDir, "macos.icns");
	if (existsSync(icns)) return { kind: "icns", path: icns, from: "brand/icon/source/macos.icns" };
...
if (macIsIcns) await copyFile(macSource.path, icns);
else execFileSync("iconutil", ["-c", "icns", iconset, "-o", icns]);
