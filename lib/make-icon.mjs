 ...
 * size (icon_32x32 and icon_16x16@2x are both 32px, so one file serves both), then
 * from the compile, and only then by REDUCING a larger slot. Nothing to fill means
 * the export is already complete and is passed straight through to be copied byte
 * for byte.
 *
 * The reduction is what makes ten out of ten possible. Copying an exact pixel
 * match was the only move available, and Icon Composer's exporter omits three
 * slots for which nothing of that size exists anywhere — 64px, and 512px twice —
 * while carrying 128px and 1024px, which are exactly 2x and 2x of them. So the
 * .icns shipped with seven of ten slots filled, and macOS fell back to scaling
 * whatever it could find at the sizes it wanted: the same reduction, done later,
 * by something with no say in which source it started from.
 */
async function completeIcns(exported, compiled, staging, target) {
	...
	for (const { px, name } of ICONSET) {
		...
		/*
		 * Nothing at this size — reduce the smallest larger one.
		 *
		 * Smallest larger, not largest available: 512 from 1024 is one halving and
		 * 512 from a hypothetical 2048 would be two, and every extra step is
		 * resampling error on artwork somebody approved. Sorting ascending and
		 * taking the first also lands the exact 2x cases (64 from 128, 512 from
		 * 1024), which resample cleanly.
		 */
		const larger = [...new Set([...from.byPx.keys(), ...(also ? also.byPx.keys() : [])])]
			.filter((p) => p > px)
			.sort((a, b) => a - b)[0];
		const source = larger === undefined ? null : (from.byPx.get(larger) ?? also?.byPx.get(larger));
		if (!source) {
			missing.push(`${name} (${px}px)`);
			continue;
		}
		const out = join(merged, name);
		// sips rather than a dependency: it is on every Mac, and iconutil below
		// already makes this a macOS-only path.
		execFileSync("sips", ["-z", String(px), String(px), source, "--out", out], { stdio: "pipe" });
		filled.push({ name, px, reducedFrom: larger });
	}
...
const { chromium } = await import("playwright").catch(() => {
	throw new Error("playwright is not installed — pnpm install");
});
...
if (macIsIcns) {
	...
	for (const { name, px, own, reducedFrom } of built.filled) {
		console.log(`  filled    ${name.padEnd(21)} ${String(px).padStart(4)}px  from ${reducedFrom ? `${reducedFrom}px, reduced` : own ? macSource.from : doc.from}`);
	}
