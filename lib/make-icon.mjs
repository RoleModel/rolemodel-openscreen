...
/*
 * Three inputs, because these are not one asset.
 *
 *   brand/icon/mark.svg    REQUIRED. The mark the interface uses — the Studio's
 *                          sidebar and favicon, and the editor's top bar. It is a
 *                          vector because it is drawn at 22px in one place and 180
 *                          in another.
 *   brand/icon/app.png     OPTIONAL. The application icon, for every platform's
 *                          icon set. A finished app icon is not a scaled-up mark:
 *                          it carries its own margins and, on macOS, the rounded
 *                          rectangle itself.
 *   brand/icon/macos.png   OPTIONAL. macOS only, overriding app.png for the .icns.
 *                          Apple's icon grid and corner curvature are specific
 *                          enough that a Mac icon is usually its own drawing.
 *
 * Rasters win where they exist and are used exactly as given. Icon Composer and
 * anything like it export a finished PNG, and re-deriving that from a vector would
 * be second-guessing whoever exported it — including the margins, which is the part
 * that goes wrong when an icon is "fixed" automatically.
 */
const iconDir = join(ROOT, "brand", "icon");

const raster = async (...names) => {
	for (const name of names) {
		const bytes = await readFile(join(iconDir, name)).catch(() => null);
...
/**
 * What to draw an icon set from, for one platform.
 *
 * Falls back through macos.png -> app.png -> the mark, so supplying nothing still
 * produces a complete set and supplying only the Mac icon changes only the Mac.
 */
async function appSource(platform, mark) {
	if (platform === "mac") {
		const mac = await raster("macos.png", "macos@1024.png");
		if (mac) return mac;
	}
	return (await raster("app.png", "app@1024.png")) ?? mark;
}
...
const macSource = await appSource("mac", mark);
const pngSource = await appSource("png", mark);
console.log("");
console.log(`  mark      ${mark.from}`);
console.log(`  mac icon  ${macSource.from}${macSource === mark ? "  (no macos.png or app.png — using the mark)" : ""}`);
console.log(`  png set   ${pngSource.from}`);
console.log("");
 ...
 */
const shared = macSource === pngSource;
const macSizes = ICONSET.map((i) => i.px);
process.stdout.write("  mac ");
const macPx = await renderAll(macSource, shared ? [...macSizes, ...PNGS] : macSizes);
const pngPx = shared ? macPx : (process.stdout.write("\n  png "), await renderAll(pngSource, PNGS));
if (shared) process.stdout.write("   (one source for both sets)");
console.log("");
await browser.close();

for (const { px, name } of ICONSET) await writeFile(join(iconset, name), macPx.get(px));
