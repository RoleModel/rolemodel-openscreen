...
export function newManifest({ name, client = null, brand = "rolemodel", remote, bucket, prefix = "", driver = "rclone" }) {
	return {
		...
		name,
		// Projects belong to a client. "Ridgeline" and "Hershey" are two clients, not
...
export function capture(cmd, args) {
	return new Promise((resolvePromise) => {
		// Same two rules as lib/jobs.mjs: stdin is /dev/null so a child that reads
		// it cannot hang a detection probe, and Electron's internal switches are
		// not inherited — `openscreen info --json` is the reason this matters, and
		// it is the very call that decides whether the tool shows as present.
...
/*
 * A directory holding this is not the user's media and is not indexed.
 *
 * Renders stage their own copies of the brand — marks, fonts, a wallpaper, the
 * clay imagery — so the renderer can work with no network. Those copies sat in
 * `Renders/<name>/assets/`, which the catalog walked like anything else, so every
 * render quietly added its staged pictures to the project's own still count. One
 * render put a wallpaper in; the imagery staging would have put eleven more in
 * per render.
 *
 * A marker rather than a path rule: `assets/` is an ordinary name somebody may
 * legitimately use for their own material, and a directory that says outright
 * that it is generated cannot be mistaken for one.
 */
export const IGNORE_MARKER = ".rmignore";
...
export async function* walk(dir, base = dir) {
	...
	if (entries.some((e) => e.isFile() && e.name === IGNORE_MARKER)) return;
 ...
 *
 * `previous` makes this cheap enough to run every time the library is opened,
 * which is the point: nobody should have to remember to press Re-index after
 * making a narration. Walking the tree costs a readdir; the expensive part is
 * ffprobe, so an entry whose path, size and mtime are unchanged is reused as-is
 * and never probed again. A cold index of a big library is still slow — that one
 * is unavoidable — but the steady state is a directory walk.
 *
 * `force` re-probes everything, for when a file changed underneath its own mtime.
 */
export async function buildCatalog(root, { onFile, previous, force = false } = {}) {
	const cached = new Map();
	if (!force) for (const f of previous?.files ?? []) cached.set(f.rel, f);
	...
	for await (const { full, rel } of walk(root)) {
		...
		if (hit && hit.bytes === s.size && hit.mtime === s.mtime.toISOString()) {
			bytes += s.size;
			files.push(hit);
			reused++;
			onFile?.(hit);
	...
	return { schema: 1, indexedAt: new Date().toISOString(), files, bytes, reused };
}

/**
 * Search the catalog. Plain substring matching over name, path, tags, and
 * codec — enough to answer "where's the Ridgeline footage" today.
 *
