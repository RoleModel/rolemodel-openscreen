/*
 * The shared tables, as Drizzle sees them.
 *
...
export const styleImages = pgTable("style_images", {
	...
	file: text("file"),
	/** The project it was made in, and its path under that project's media. */
	projectId: text("project_id"),
	projectRel: text("project_rel"),
	createdBy: text("created_by"),
	createdAt: timestamp("created_at", { withTimezone: true, mode: "string" }).notNull().defaultNow(),
});
...
/** The Creator's saved looks. The `look` string is the design; the rest is a label. */
...
/*
 * Notes on shared videos (sql/video-comments.sql) and on stickers
 * (sql/sticker-comments.sql). The published pages write these through Neon's
 * Data API as the anonymous role; the Studio reads and tidies them here as
 * studio_app. `atMs` is the moment in the video a note is about, or null.
 */
...
export const stickerComments = pgTable("sticker_comments", {
	id: uuid("id").primaryKey().defaultRandom(),
	project: text("project").notNull(),
