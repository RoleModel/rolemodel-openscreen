...
/*
 * Standalone audio: music, a voice track, anything not carried by a clip.
 *
 * A video clip's own sound travels with the clip and is drawn inside it. This is
 * the other kind — a bed that runs under everything and has to be trimmable on
 * its own — so it is read separately and lands on its own lane.
 */
export function audioFromHtml(html) {
	const out = [];
	for (const [tag] of String(html).matchAll(/<audio\b[^>]*>/gi)) {
		const attr = (name) => new RegExp(`\\b${name}="([^"]*)"`).exec(tag)?.[1];
		const src = attr("src");
		const seconds = Number(attr("data-duration") ?? 0);
		/* A silent clock track is timing machinery, not audio somebody edits. */
		if (!src || !(seconds > 0) || /data-assembly-clock/.test(tag)) continue;
		out.push({
			id: attr("id") ?? `audio-${out.length + 1}`,
			file: src.replace(/^source\//, ""),
			at: Number(attr("data-start") ?? 0),
			in: Number(attr("data-media-start") ?? 0),
			seconds,
		});
	}
	return out;
...
export async function seedCut({ dir, mediaRoot, onStep = () => {} }) {
	...
	/* Audio last, so it is the bottom lane — which is where every editor puts it
	   and where the eye expects to find it. */
	const audio = { id: "a1", kind: "audio", clips: [] };
	for (const item of audioFromHtml(html)) {
		const file = join(mediaRoot, item.file);
		if (!(await stat(file).catch(() => null))) {
			skipped.push(item.file);
			continue;
	...
	if (audio.clips.length) cut.tracks.push(audio);
