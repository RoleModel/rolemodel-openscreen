...
export function overlapAt(cut, trackId) {
	...
	/*
	 * Only footage dissolves.
	 *
	 * Two overlapping graphics are two things on screen at once — a closing card
	 * under a title is the intended picture, not a transition — and reporting it
	 * as a dissolve told the editor to draw a transition wedge across a perfectly
	 * ordinary stack. An overlap means a dissolve when there are two pictures
	 * competing for the same frame, which is what a video lane is.
	 */
...
export function cutProblems(cut) {
	...
	for (const { track, clip } of allClips(cut ?? {})) {
		...
		seen.add(clip.id);
		/*
		 * A clip without a source is a graphic, not a mistake.
		 *
		 * A motion piece is made of beats — a title, a card, a mounted
		 * composition — and each one is a clip in every sense a timeline cares
		 * about: it starts somewhere and it lasts a while. What it does not have
		 * is footage, so it has no in point into anything and nothing to check a
		 * length against. Requiring `source` made the whole class unrepresentable
		 * and left the editor unable to open the one project that is all graphics.
		 */
		const source = clip.source ? cut.sources?.[clip.source] : null;
		if (clip.source && !source) problems.push(`${where} points at source ${clip.source}, which this cut does not list`);
		if (!clip.source && !clip.name) problems.push(`${where} is a graphic with no name, so nothing can label it`);
		/*
		 ...
		 */
		if (source?.seconds && clip.out > source.seconds + 1 / (cut.fps || 60)) {
