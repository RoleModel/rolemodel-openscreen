...
export const BINARIES = new Set([
	...
	"playwright-recast",
	"rclone",
	/* Homebrew, for one thing only: the Studio's own update button. The args are
	   fixed by lib/update.mjs and never come from the page. */
	"brew",
	"ffmpeg",
	"ffprobe",
	"claude",
...
let NODE_EXECUTABLE = null;
export function setTrustedRoot(dir) {
	TRUSTED_ROOT = resolve(dir);
}

/** The Node executable that started Studio itself, not a PATH lookup. */
export function setNodeExecutable(file) {
	NODE_EXECUTABLE = file ? resolve(file) : null;
}

function permitted(bin) {
	if (BINARIES.has(bin)) return true;
	if (NODE_EXECUTABLE && resolve(bin) === NODE_EXECUTABLE) return true;
...
/** A finite number from a CLI event, or zero when it was not reported. */
function metric(value) {
	const number = Number(value);
	return Number.isFinite(number) ? number : 0;
}

/* Some older CLI streams preserved an ANSI bold suffix as literal text. */
function modelName(value) {
	return String(value).replace(/\[\d+(?:;\d+)*m\]/g, "");
}

/**
 * Claude's stream is deliberately kept as job output, not a second analytics
 * store. Pull the one terminal result event back out of that durable output.
 *
 * This means the Usage page can account for jobs made before the page existed,
 * while future journals also keep a compact snapshot (see `record` below).
 */
function streamedUsage(job) {
	for (const line of [...(job.lines ?? [])].reverse()) {
		if (line.stream !== "out" || !String(line.text).startsWith("{")) continue;
		try {
			const event = JSON.parse(line.text);
			if (event?.type !== "result") continue;
			const raw = event.usage ?? {};
			const models = Object.entries(event.modelUsage ?? {}).map(([model, detail]) => ({
				model: modelName(model),
				costUsd: metric(detail?.costUSD),
				inputTokens: metric(detail?.inputTokens),
				cacheCreationTokens: metric(detail?.cacheCreationInputTokens),
				cacheReadTokens: metric(detail?.cacheReadInputTokens),
				outputTokens: metric(detail?.outputTokens),
			}));
			const inputTokens = metric(raw.input_tokens);
			const cacheCreationTokens = metric(raw.cache_creation_input_tokens);
			const cacheReadTokens = metric(raw.cache_read_input_tokens);
			const outputTokens = metric(raw.output_tokens);
			return {
				provider: "Claude Code",
				model: models.map((item) => item.model).join(", ") || null,
				costUsd: typeof event.total_cost_usd === "number" ? event.total_cost_usd : models.reduce((sum, item) => sum + item.costUsd, 0),
				inputTokens,
				cacheCreationTokens,
				cacheReadTokens,
				outputTokens,
				totalTokens: inputTokens + cacheCreationTokens + cacheReadTokens + outputTokens,
				turns: metric(event.num_turns),
				durationMs: metric(event.duration_ms),
				models,
			};
		} catch {
			// A non-Claude JSON line is still useful Console output, just not usage.
		}
	}
	return null;
}
...
function usageForJob(job) {
	return job.usage ?? streamedUsage(job);
}
...
/**
 * A durable, honest accounting of every job still in Studio's journal.
 *
 * Local jobs have real elapsed time but no model bill; agent jobs whose stream
 * did not include a result event are kept in the unknown bucket rather than
...
export function usageReport() {
	const runs = [...jobs.values()]
		...
		.map((job) => {
			...
			const agent = Boolean(usage) || /(?:^|\s)(claude|pi)(?:\s|$)/.test(String(job.command));
			return {
				id: job.id,
				label: job.label,
	...
	for (const run of runs) {
		summary.elapsedMs += run.elapsedMs ?? 0;
		if (!run.agent) {
			summary.localJobs += 1;
			continue;
		}
		summary.agentJobs += 1;
		if (!run.usage) {
			summary.unknownAgentJobs += 1;
			continue;
		}
		summary.reportedJobs += 1;
		for (const key of ["costUsd", "inputTokens", "cacheCreationTokens", "cacheReadTokens", "outputTokens", "totalTokens"]) summary[key] += metric(run.usage[key]);
		for (const model of run.usage.models ?? []) {
			const current = models.get(model.model) ?? { model: model.model, jobs: 0, costUsd: 0, totalTokens: 0 };
			current.jobs += 1;
			current.costUsd += model.costUsd;
			current.totalTokens += model.inputTokens + model.cacheCreationTokens + model.cacheReadTokens + model.outputTokens;
			models.set(model.model, current);
		}
	}
	return { summary, models: [...models.values()].sort((a, b) => b.costUsd - a.costUsd), runs };
}
...
const record = (j) => ({
	...
	lines: j.lines,
	// The raw stream remains the source of truth. This is a compact copy for a
	// finished job, so a later stream format change cannot erase its accounting.
	usage: usageForJob(j),
});
...
export function run({ bin, args = [], label, cwd, shell = false, env, project = null, keep = false, onDone }) {
	...
	const job = {
		...
		/*
		 * The parts, kept beside the joined string.
		 *
		 * `command` is for reading; it cannot be run again, because splitting it
		 * on spaces breaks the moment an argument contains one — a prompt, a
		 * project called "CCC Days", a file with a space in its name. Every one
		 * of those is ordinary here, so a rerun uses what was actually passed.
		 */
		bin,
		args,
		shell,
		project,
		/*
		 * Survives a restart the Studio does to itself.
		 *
		 * A scene render is the better part of an hour, and the Studio reloads when
		 * its own code changes — which used to take ffmpeg with it and leave a file
		 * with every frame in it and no index. A spared job is not killed; it is
		 * reparented and finishes, which is why it writes its own trace rather than
		 * waiting to be told to. It is still stopped on the way out of the machine,
	...
	child.on("error", (e) => {
		// An absolute executable cannot be "missing from PATH".  ENOENT can also
		// mean the requested working folder disappeared, so report that honestly
		// instead of sending someone to reinstall Node when Node is right there.
		const missing = e.code === "ENOENT"
			? (bin.includes("/") || bin.includes("\\")
				? `${bin}: could not start (the executable or its working folder is missing)`
				: `${bin}: not found on PATH`)
			: String(e.message ?? e);
		push(job, "err", missing);
	});
	child.on("close", (code, signal) => {
		flush(job);
		/*
		 * A bare "signal SIGTERM" tells nobody anything. Every SIGTERM here was
		 * sent from this file, so the reason is always known, and the job list
		 * is the one place someone reads to find out what went wrong.
		 */
		job.code = code ?? (signal ? (job.killedBy ? `stopped: ${job.killedBy}` : `signal ${signal}`) : null);
...
/*
 * Run a finished job again, from the argv it actually had.
 *
 * The one that matters is `interrupted`: a job whose server was restarted under
 * it did not fail, it was cut off, and the work is simply not done. Before this
 * the only way back was to remember which panel started it and repeat the
 * clicks — and for a render that had been going twenty minutes, to remember
 * exactly which options were set.
 *
 * A new job, not a resumed one. Nothing here can continue a process that is
 * gone; it starts the same command again and gets its own id and its own log,
 * so the record of what was interrupted survives beside the new attempt.
 */
/*
 * Argv again from a command that was only ever joined, for jobs older than the
 * recording of it.
 *
 * Splitting on spaces alone is wrong here, and expensively so: one real job in
 * the journal is a fal restyle whose prompt is a forty-three word sentence, and
 * a plain split hands `--prompt` the word "Preserve" and the rest to nobody —
 * a paid call that runs, succeeds, and returns the wrong thing.
 *
 * Every job this UI starts is flag-shaped: a binary, then `--flag value` pairs
 * whose values never begin with a dash. So a value is everything up to the next
 * flag, and rejoining on that rule reconstructs the original exactly. The two
 * leading tokens are the interpreter and its script, neither of which is a flag.
 */
function fromCommand(command) {
	const tokens = String(command ?? "").split(" ").filter(Boolean);
	const out = [];
	for (const token of tokens) {
		const continuing = out.length > 1 && !token.startsWith("--") && !out.at(-1).startsWith("--") && out.length > 2;
		if (continuing) out[out.length - 1] += ` ${token}`;
		else out.push(token);
	}
	return out;
}

export function replay(id, { onDone } = {}) {
	const old = jobs.get(id);
	if (!old) throw new Error("that job is not in the list any more");
	if (old.child) throw new Error("that job is still running");
	const [bin, ...args] = old.bin ? [old.bin, ...(old.args ?? [])] : fromCommand(old.command);
	if (!bin) throw new Error("that job kept no command at all, so there is nothing to start");
	return run({
		bin,
		args,
		label: old.label,
		cwd: old.cwd,
		shell: Boolean(old.shell),
...
export function stop(id) {
	...
	j.killedBy = "you pressed Stop";
	push(j, "err", "\u2014 stopped: you pressed Stop");
 ...
 *
 * `spare` leaves the long jobs alone — a restart the Studio does to itself is
 * not a reason to throw away fifty minutes of rendering. They are reparented
 * and finish on their own, which is what `keep` is for. Shutting the machine
 * down passes nothing, because then there is no one left to finish for.
 */
export function stopAll(why = "Studio is shutting down", { spare = false } = {}) {
	const spared = [];
	for (const j of jobs.values()) {
		if (!j.child) continue;
		if (spare && j.keep) {
			spared.push(j);
			// Said in its own log, because the stream is about to end and the silence
			// would otherwise read as the job dying here.
			push(j, "out", `\u2014 the Studio restarted; this is still running (${j.id})`);
			continue;
		}
		j.killedBy = why;
		push(j, "err", `\u2014 stopped: ${why}`);
		j.child.kill("SIGTERM");
	}
	return spared;
}
