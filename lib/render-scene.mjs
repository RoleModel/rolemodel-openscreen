...
import { markupAudioLayers, sceneHtml } from "./compose.mjs";
...
export async function renderScene({
	...
	try {
		...
		browser = await chromiumLib.launch();
	} catch (err) {
	...
	/* Lane browsers, so the finally can close them however this ends. */
	const extraBrowsers = [];
	...
	try {
		...
		/*
		 * How many frames are drawn at once.
		 *
		 * A render is a loop of seek, settle, screenshot — all of it serial, all of
		 * it in one page, and it used about one core of the sixteen this machine
		 * has while a 96-second scene took the better part of an hour.
		 *
		 * It parallelises because a frame is a pure function of its time. Nothing
		 * plays: `RM.seek(ms)` sets a custom property and every animation is a
		 * function of it, so frame 1,200 does not depend on frame 1,199 having been
		 * drawn. That is not a hope — two renders of this scene came out
		 * byte-identical, which is the property this relies on, measured.
		 *
		 * So the frames are cut into contiguous ranges, a page each, and the
		 * segments concatenated. Measured on the c12 walkthrough at 1080p/2x:
		 * one page is 1.09 frames a second, four is 3.33 — 3x, with each page only
		 * 30% slower than it was alone.
		 *
		 * A quarter of the cores rather than all of them: each page is a browser
		 * renderer holding a 3840x2160 surface, the encoder wants a core too, and
		 * this is somebody's laptop — they are probably using it for something
		 * else while this runs.
		 */
		const auto = Math.max(1, Math.min(4, Math.floor((cpus().length || 4) / 4)));
		const lanes = Math.max(1, Math.min(8, Math.round(Number(workers) || auto)));

		/*
		 * A lane is its own browser, not another tab in this one.
		 *
		 * Four tabs in one browser bought 13%; four browsers bought 3x. Chromium
		 * puts same-site pages in one renderer process, so the tabs were taking
		 * turns on a single thread and paying the coordination on top — which is
		 * the worst of both. A process each is what actually reaches the other
		 * cores. `chromium.launch()` is about a second, once per lane, against
		 * minutes of frames.
		 */
		const newPage = async (own) => {
			const br = own ? await chromiumLib.launch() : browser;
			if (own) extraBrowsers.push(br);
			const pg = await br.newPage({ viewport: { width, height }, deviceScaleFactor: shot });
			await pg.addInitScript(() => {
				window.__hyperframes = true;
			});
			await pg.goto(`http://127.0.0.1:${port}${SCENE_PATH}`, { waitUntil: "networkidle" });
			await pg.evaluate(() => window.RM.ready());
			return pg;
		};

		const first = await newPage(false);
		const total = Math.max(200, Math.round(durationMs ?? (await first.evaluate(() => window.RM.duration()))));
		...
		const encoderArgs = () => {
			...
			a.push("-vf", `scale=${width}:${height}:flags=lanczos`);
			a.push(
				...(fmt.id === "webm"
		 ...
		 *
		 * A render of an 85-second scene takes a long time, and the ways it does
		 * not finish are ordinary: the window is closed, the app is quit, the
		 * Studio restarts because its own code changed. ffmpeg dies with it,
		 * having written every frame and no `moov` atom — and the half-file was
		...
		/** Encode frames [from, to) from `page` into `file`. */
		const renderRange = async (pg, from, to, file, onFrame) => {
			const enc = spawn("ffmpeg", [...encoderArgs(), file], { stdio: ["pipe", "ignore", "pipe"] });
			let said = "";
			...
			const closed = new Promise((res, rej) => {
				enc.on("error", rej);
				enc.on("close", (code) => (code === 0 ? res() : rej(new Error(`ffmpeg gave up (${code})${said ? ` — ${said.trim().split("\n").pop()}` : ""}`))));
			});
			try {
				for (let n = from; n < to; n++) {
					/* The frame's time is its GLOBAL index, so a range renders exactly
					   the frames it would have as part of one long pass. */
					const t = Math.round((n / fps) * 1000);
					await pg.evaluate((ms) => window.RM.seek(ms), t);
				...
				try {
					enc.stdin.destroyed || enc.stdin.end();
				} catch {
					/* already closed */
				}
			}
		...
		/* Progress is the count across every lane, not one lane's position, so it
		   still runs 0..frames once however many are drawing. */
		let drawn = 0;
		let lastMs = 0;
		const tick = (ms) => {
			drawn++;
			lastMs = Math.max(lastMs, ms);
			onProgress?.({ frame: drawn, frames, ms: lastMs, total });
		};

		if (lanes === 1) {
			await renderRange(first, 0, frames, partial, tick);
		} else {
			/* Contiguous ranges, biggest first is not worth it: the cost per frame
			   varies along the scene, so equal ranges finish unevenly and the last
			   lane sets the wall clock. Equal is close enough and keeps the frame
			   arithmetic obvious. */
			const size = Math.ceil(frames / lanes);
			const ranges = [];
			for (let i = 0; i < lanes && i * size < frames; i++) ranges.push([i * size, Math.min(frames, (i + 1) * size)]);
			const segs = ranges.map((_, i) => `${out}.part${String(i).padStart(2, "0")}${fmt.ext}`);
			for (const f of segs) await unlink(f).catch(() => {});
			const pages = [first, ...(await Promise.all(ranges.slice(1).map(() => newPage(true))))];
			try {
				await Promise.all(ranges.map(([from, to], i) => renderRange(pages[i], from, to, segs[i], tick)));
				/*
				 * Joined by copying, not re-encoding. Every segment came out of the
				 * same encoder with the same settings and size, so the streams are
				 * compatible and the join is lossless — re-encoding here would spend
				 * the render's own quality a second time for nothing.
				 */
				const list = `${out}.parts.txt`;
				await writeFile(list, `${segs.map((f) => `file '${f.replace(/'/g, "'\\''")}'`).join("\n")}\n`, "utf8");
				const join = spawn("ffmpeg", ["-y", "-f", "concat", "-safe", "0", "-i", list, "-c", "copy", "-movflags", "+faststart", partial], { stdio: ["ignore", "ignore", "pipe"] });
				let joinSaid = "";
				join.stderr.on("data", (d) => {
					joinSaid = `${joinSaid}${d}`.slice(-4000);
				});
				await new Promise((res, rej) => {
					join.on("error", rej);
					join.on("close", (code) => (code === 0 ? res() : rej(new Error(`joining the render failed (${code})${joinSaid ? ` — ${joinSaid.trim().split("\n").pop()}` : ""}`))));
				});
		...
		/*
		 * The sound the scene asked for.
		 *
		 * A render is a stack of stills, so nothing can play during it — rm-audio
		 * says as much in its own comment, and draws nothing. The track is laid on
		 * at the end instead, which is the only point at which there is a video of
		 * a known length to lay it against.
		 *
		 * It was not laid on at all until now. You could add a sound layer in the
		 * Showcase panel, hear it while scrubbing, and get a silent file, with
		 * nothing anywhere saying the track had been dropped.
		 *
		 * `adelay` puts second 0 of the file at the layer's `at`, `volume` is the
		 * gain the editor was set to, and `apad` + `-shortest` makes the audio
		 * exactly as long as the picture: without the pad a track shorter than the
		 * scene would cut the video off at the end of the sound.
		 */
		const sound = markupAudioLayers(body)
			.map((layer) => ({ ...layer, file: resolveFile?.(layer.src.startsWith("/media/") ? layer.src : `/media/_/${layer.src}`) ?? null }))
			.filter((layer) => layer.file);
		const named = markupAudioLayers(body).length;
		if (named && !sound.length) {
			await unlink(partial).catch(() => {});
			throw new Error(`this scene names a sound layer but the file could not be found — rendering it silently would look finished`);
		}
		if (sound.length) {
			const withSound = `${out}.sound${fmt.ext}`;
			const inputs = sound.flatMap((layer) => ["-i", layer.file]);
			const graph = sound
				.map((layer, i) => `[${i + 1}:a]volume=${layer.gain},adelay=${layer.atMs}|${layer.atMs}[a${i}]`)
				.join(";");
			/* One track or several, mixed to one stream. `normalize=0` so two quiet
			   layers do not get quieter for being two. */
			const mix = sound.length === 1 ? `${graph};[a0]apad[aout]` : `${graph};${sound.map((_, i) => `[a${i}]`).join("")}amix=inputs=${sound.length}:normalize=0[amix];[amix]apad[aout]`;
			const codec = fmt.id === "webm" ? ["-c:a", "libopus", "-b:a", "160k"] : ["-c:a", "aac", "-b:a", "192k"];
			const mux = spawn(
				"ffmpeg",
				["-y", "-i", partial, ...inputs, "-filter_complex", mix, "-map", "0:v", "-map", "[aout]", "-c:v", "copy", ...codec, "-shortest", "-movflags", "+faststart", withSound],
				{ stdio: ["ignore", "ignore", "pipe"] },
			);
			let muxSaid = "";
			mux.stderr.on("data", (d) => {
				muxSaid = `${muxSaid}${d}`.slice(-4000);
			});
			await new Promise((res, rej) => {
				mux.on("error", rej);
				mux.on("close", (code) => (code === 0 ? res() : rej(new Error(`laying the sound on failed (${code})${muxSaid ? ` — ${muxSaid.trim().split("\n").pop()}` : ""}`))));
			});
			await unlink(partial).catch(() => {});
			await rename(withSound, partial);
		}
		...
		leftover = null;
		return { file: out, format: fmt.id, fps, durationMs: total, frames, width, height, transparent, shotScale: shot, workers: lanes, sound: sound.length };
	} finally {
		/* Closing the browsers is what stops every lane: each is a page in one of
		   them, and a page that goes away takes its screenshot loop with it. Each
		   lane ends its own encoder in its own finally. */
		await browser.close();
		await Promise.all(extraBrowsers.map((b) => b.close().catch(() => {})));
		srv.close();
