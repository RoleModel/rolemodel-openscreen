 ...
 */
function vScenes(m) {
  m.append(el('p', 'lede', 'A scene is markup inside the stage — components, your own CSS, or both. It is previewed with the same wrapper that renders it, and seeked rather than played, so the frame you scrub to is the frame that gets encoded.'))
  ...
  about.placeholder = 'What is this scene? One or two sentences — Claude writes the markup from the contract in SCENE.md.'
  mk('Ask Claude for one', about)

  const body = el('textarea')
  body.spellcheck = false
  body.placeholder = '<rm-title at="0" for="2600" eyebrow="Product tour" title="Estimating, in one pass"></rm-title>'
  mk('Scene body', body, 'Goes inside <rm-scene>. Do not write html, head, body or rm-scene — the wrapper supplies them.')
  ...
  const out = el('pre', 'full')
  out.style.display = 'none'
  f.append(frame, scrubRow, bar, out)
  m.append(f)
  ...
  /*
   * Stash the body, then point the frame at it.
   *
   * Neither srcdoc nor a blob: URL works here: both give the document an opaque
   * origin with no base, so rm-video.js, the brand faces and the wallpaper all
   * fail to resolve — the scene renders unstyled, in the fallback face, with no
   * component upgraded. It has to be served from this origin, and an iframe
   * cannot POST, hence the two steps.
   */
  const preview = async () => {
    const r = await (
      await fetch('/api/scene/preview', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ body: body.value, wallpaper: wp.value, name: name.value }),
      })
    ).json()
  ...
  const seek = (ms) => {
    ...
    // The scene owns RM; reach into the frame rather than keeping a second clock.
    try {
      frame.contentWindow?.RM?.seek(ms)
    } catch {
      /* not loaded yet, or cross-origin — the next paint will catch up */
    }
  }
  scrub.oninput = () => seek(Number(scrub.value))
  frame.onload = () => {
    seek(Number(scrub.value))
    // Ask the scene how long it is, so the scrubber matches the render length.
    try {
      const d = frame.contentWindow?.RM?.duration?.()
      if (d) scrub.max = Math.max(2000, Math.round(d))
    } catch {
  ...
  save.onclick = async () => {
    const r = await (
      await fetch('/api/scene', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ projectId: proj.value, name: name.value, body: body.value }),
      })
    ).json()
  ...
  draft.onclick = async () => {
    ...
    out.innerHTML = ''
    if (r.error) return say(r.error, 'bad')
    out.append(Object.assign(el('div', 'path'), { textContent: 'writes  ' + r.dest }))
    out.append(runRow(r.step, 'Write the scene'))
    out.append(Object.assign(el('div', 'path'), { textContent: 'When it finishes, pick it above to load and preview it.' }))
  }
 ...
 */
function vCompose(m) {
  m.append(el('p', 'lede', 'Titles, lower thirds and stats over a brand wallpaper, cut together with your footage. Each scene is rendered to a clip and the whole thing opens in the editor.'))
  ...
  const paint = () => {
    ...
    segments.forEach((seg, i) => {
      ...
      up.title = 'Move earlier'
      up.disabled = i === 0
      up.onclick = () => { segments.splice(i - 1, 0, segments.splice(i, 1)[0]); paint() }
      const down = el('button', 'btn ghost', '↓')
      down.title = 'Move later'
      down.disabled = i === segments.length - 1
      down.onclick = () => { segments.splice(i + 1, 0, segments.splice(i, 1)[0]); paint() }
      const kill = el('button', 'btn ghost del', 'Remove')
      kill.onclick = () => { segments.splice(i, 1); paint() }
      ...
      if (seg.kind === 'footage') {
        ...
        for (const c of files) {
          pickF.append(Object.assign(el('option', null, c.rel), { value: c.path }))
        }
        if (!pickF.options.length) {
          pickF.append(el('option', null, 'no footage indexed in this project'))
          pickF.disabled = true
        }
        pickF.value = seg.path || pickF.value
        pickF.onchange = () => { seg.path = pickF.value }
        seg.path = seg.path || pickF.value
        body.append(pickF)
      } else {
        ...
        seg.elements.forEach((e, j) => {
          ...
          at.oninput = () => { e.at = Number(at.value) || 0 }
          ...
          dur.oninput = () => { e.for = Number(dur.value) || 0 }
          line.append(at, dur)
          for (const fieldName of spec?.fields ?? []) {
            ...
            if (choices) {
              ...
              sel.onchange = () => { e.attrs[fieldName] = sel.value }
            ...
            inp.oninput = () => { e.attrs[fieldName] = inp.value }
            line.append(inp)
          }
          const rmEl = el('button', 'btn ghost del', '×')
          rmEl.title = 'Remove this element'
          rmEl.onclick = () => { seg.elements.splice(j, 1); paint() }
          line.append(rmEl)
          els.append(line)
        })
        ...
        wp.onchange = () => { seg.wallpaper = wp.value }
  ...
  const addScene = el('button', 'btn ghost', 'Add a scene')
  addScene.onclick = () => {
    segments.push({ kind: 'scene', name: `scene-${segments.length + 1}`, wallpaper: cat.wallpapers[0]?.name ?? '', elements: [] })
    paint()
  }
  const addFootage = el('button', 'btn ghost', 'Add footage')
  addFootage.onclick = () => { segments.push({ kind: 'footage', path: '' }); paint() }
  ...
  bar.append(addScene, addFootage, go)
