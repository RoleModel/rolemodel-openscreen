...
function vMake(m) {
  ...
  for (const [v, l] of [['narration', 'Narration — the spoken track'], ['music', 'Music bed — under everything']]) {
  ...
  const fillClips = () => {
    ...
    const files = p?.catalog?.files ?? []
    for (const [sel, kind, none] of [[webcam, 'video', 'No webcam'], [audio, 'audio', 'No audio']]) {
  ...
  audio.onchange = () => { audioRole.disabled = !audio.value }
  ...
  go.onclick = async () => {
    ...
    const r = await (await fetch('/api/make', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ projectId: proj.value, brand: brand.value, title: title.value, seconds: secs.value, source: src.value, wallpaper: bg.value, browser: on(cBrowser), browserUrl: chromeUrl.value.trim(), captions: on(cCaps), motion: mo.value, voice: vo.value, titleCard: titleCard.value.trim(), eyebrow: eyebrow.value.trim(), webcam: webcam.value, audio: audio.value, audioRole: audioRole.value }) })).json()
    out.innerHTML = ''
    runHere.innerHTML = ''
    runSlot.innerHTML = ''
    if (r.error) {
...
function vScriptEditor(m, doc) {
  ...
  const recount = () => {
    ...
    stat.textContent = spoken.length
...
function vBrand(m) {
  ...
  for (const b of S.logos || []) {
    for (const [variant, v] of Object.entries(b.variants || {})) {
      ...
      t2.style.cssText = 'display:grid;place-items:center;background:' +
        (DARK_GROUND.test(variant) ? (pal.dark || 'var(--op-color-neutral-plus-eight)') : (pal.light || 'var(--op-color-neutral-plus-max)'))
      t2.append(img)
