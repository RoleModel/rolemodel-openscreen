...
function vProject(m, id) {
  ...
  void Promise.all([
    ...
    .then(([multi, aligned]) => {
      ...
      const result = picks.length ? multi.state : alignment
      if (!result) return
      const count = picks.length || alignment.segments.length
      const isAlignment = !picks.length
  ...
  const assembly = el('button', 'btn ghost btn--pill', 'Assembly')
  assembly.type = 'button'
  assembly.onclick = () => go('paperedit')
  row.append(assembly)
  row.append(re)
