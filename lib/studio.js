...
function vStoryboard(m) {
  ...
  const applyZoom = () => {
    ...
    // The scroller has to grow with the content or zooming out leaves the board
    // floating in a box the size it used to be.
  ...
  scroller.addEventListener('pointerdown', (e) => {
    const onCard = e.target.closest('.board__take, .board__slot-head, button, input, textarea, select')
  ...
  function paint() {
    ...
    for (const slot of slots) {
      surface.append(slotColumn(slot))
    }
    applyZoom()
  }

  /** One shot: what it must show, and every take offered for it. */
  function slotColumn(slot) {
    const col = el('div', 'board__slot')
    const takes = (board.takes ?? []).filter((t) => t.slotId === slot.id)
    const chosen = chosenFor(slot)

    const head = el('div', 'board__slot-head')
    head.append(
      Object.assign(el('div', 'board__slot-order'), { textContent: String(slot.order + 1) }),
      Object.assign(el('div', 'board__slot-name'), { textContent: slot.name }),
    )
    if (slot.intent) head.append(Object.assign(el('div', 'board__slot-intent'), { textContent: slot.intent }))

    /*
     * The state of the shot, in one line, in the words somebody would use.
     *
     * Not a progress bar. "3 takes, none chosen" is actionable and "60%" is not,
     * and the whole reason to look at a board is to find what still needs doing.
     */
    ...
    if (!takes.length) state.append(Object.assign(el('span', 'board__flag board__flag--empty'), { textContent: 'nothing shot yet' }))
      ...
      const auto = !board.picks?.[slot.id]
      state.append(
        Object.assign(el('span', 'board__flag board__flag--set'), {
          // "Leading" rather than "chosen" when nobody picked: the ratings put it
          // in front, and saying "chosen" would claim a decision nobody made.
          textContent: auto ? 'leading on ratings' : 'chosen',
        }),
      )
    }
    if (slot.seconds) state.append(Object.assign(el('span', 'board__slot-target'), { textContent: `target ${slot.seconds}s` }))
    head.append(state)
    col.append(head)

    const stack = el('div', 'board__stack')
    for (const t of takes.sort((a, b) => String(b.addedAt ?? '').localeCompare(String(a.addedAt ?? '')))) {
      stack.append(takeCard(slot, t, chosen?.id === t.id))
    }
  ...
  function paintRail() {
    ...
    const saySync = () => {
       ...
       */
      tone(syncHint, a.ready ? 'ok' : 'warn')
      syncHint.textContent = a.ready ? a.detail : a.problem ? `${a.detail}\n\nNot yet: ${a.problem}` : a.detail
    }
    ...
    const refreshSharing = async () => {
      ...
      sbWho.textContent = sh.signedInAs
        ...
        : configured
          ...
          : sh.problem ?? 'Sharing is not set up for this build yet.'
