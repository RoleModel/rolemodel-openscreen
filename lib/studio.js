...
function vStickers(m) {
  ...
  const buildRail = () => {
    ...
    else pb.append(control('p', { className: 'hint', textContent: 'No shared storage yet. Set one up under Storage, with a public bucket and base URL.' }))
    intoRail(panel)
  }
  ...
  ui.quote.onclick = async () => {
     ...
     */
    for (const [group, label, per] of [[r.singles, 'Die-cut singles', 'sticker'], [r.sheets, 'Sticker sheets', 'sheet']]) {
    ...
    const bits = [r.singles?.vendors?.length && cheapestOf(r.singles, 'sticker'), r.sheets?.vendors?.length && cheapestOf(r.sheets, 'sheet')].filter(Boolean)
