...
function vConsole(m) {
  ...
  const paintStatus = () => {
    ...
    if (cur.interrupted) {
      ...
      status.textContent = `${counts} · ${cur.label} was interrupted — the Studio stopped while it was running, started ${cur.startedAt.slice(...ISO_TIME)}. The output below is how far it got.${olderThanReruns(cur)}`
      return
    }
    const ran = cur.endedAt ? ` in ${secondsBetween(cur.startedAt, cur.endedAt)}s` : ''
    status.className = cur.code === 0 ? 'hint' : 'hint bad'
    status.textContent = cur.code === 0 ? `${counts} · ${cur.label} finished${ran}. This is the full output, kept until the server stops.` : `${counts} · ${cur.label} exited ${cur.code}${ran}. The output below is why.${olderThanReruns(cur)}`
  }

  /*
   * Why there is no Run again on some old jobs.
   *
   * Jobs recorded before reruns existed kept only the command as one string,
   * and that cannot be run again: the joining lost the boundaries, so a prompt
   * with spaces in it is indistinguishable from several arguments. Saying so is
   * better than a button that is quietly absent on exactly the job somebody
   * came here to restart.
   */
  const olderThanReruns = (job) =>
    job.bin ? '' : ' Its command was recorded before reruns existed, so it cannot be started again from here.'
  ...
  const paintHead = () => {
     ...
     */
    ui.rerun.hidden = !cur || cur.running || !cur.bin || (cur.code === 0 && !cur.interrupted)
    if (!cur) return
