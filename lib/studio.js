...
/* ══════════════════════════════════════════════════════════════════════════
 * Showcase: footage or a still, presented — on a look, in a frame, in 3D.
 *
 * The state is the <rm-showcase> tag's attributes and nothing else. The rail's
 * dials come from the component's own schema, the backdrop is one of the
 * Creator's looks, and the footage is the project's. Save PNG asks the server
 * to draw it, because CSS 3D over a WebGL look is nothing a canvas can read.
 * ══════════════════════════════════════════════════════════════════════════ */
function vShowcase(m) {
  const ui = mountPanel('showcase', m)
  const { show: stage, scrub, clock, markup, play: playBtn } = ui
  const LOOP = 8000
  let rt = null
  let state = null
  let looks = []
  ...
  const tag = () => `<rm-showcase at="0" for="${LOOP}" ${rt.encodeShowcase(state)}></rm-showcase>`
  ...
  const apply = () => {
    ...
    queued = requestAnimationFrame(() => {
      queued = 0
      for (const f of rt.SHOWCASE_SCHEMA) stage.setAttribute(f.key, String(state[f.key]))
      if (state.media) stage.setAttribute('media', state.media)
      else stage.removeAttribute('media')
      if (state.look) stage.setAttribute('look', state.look)
      else stage.removeAttribute('look')
      markup.textContent = tag()
      for (const c of ui.media.children) c.classList.toggle('showcase__card--on', c.dataset.media === state.media)
    })
  }
  ...
  const rangeRow = (box, f) => {
    ...
    const i = Object.assign(row.input, { min: f.min, max: f.max, step: f.step, value: state[f.key] })
    const show = () => (row.readout.textContent = String(Number(i.value)))
    show()
    i.oninput = () => {
      state[f.key] = Number(i.value)
      show()
      apply()
    }
    box.append(...nodes)
  ...
  const selectRow = (box, f) => {
    ...
    row.input.value = state[f.key]
    row.input.onchange = () => {
      state[f.key] = row.input.value
      apply()
    }
    box.append(...nodes)
  }
  const buildRail = () => {
    ...
    for (const [group, label] of rt.SHOWCASE_GROUPS) {
      ...
      if (group === 'media') {
        ...
        row.name.textContent = state.media ? state.media.split('/').pop() : 'none'
        ...
        row.file.onchange = async () => {
          ...
          if (r.error) return say(r.error, 'bad')
          state.media = `/added/${r.file}`
          row.name.textContent = r.file
          apply()
        }
        box.append(...nodes)
        /* The backdrop: one of the Creator's looks, or none. */
        ...
        lookRow.el.input.value = looks.some((l) => l.look === state.look) ? state.look : ''
        lookRow.el.input.onchange = () => {
          state.look = lookRow.el.input.value
          apply()
        }
    ...
    const outlets = g()
    outlets.append(...sceneOutlets({ tag, startMs: () => Number(scrub.value), name: 'showcase', durationMs: LOOP }))
    intoRail(panel)
  }

  /* ── templates: a device, a backdrop and a camera in one press ── */
  let templateOn = null
  const paintTemplates = () => {
    ...
    for (const t of rt.SHOWCASE_TEMPLATES) {
      ...
      root.onclick = () => {
        ...
        state.look = looks.find((l) => l.name === look)?.look ?? state.look
        buildRail()
        paintTemplates()
        apply()
      }
  ...
  const paintMedia = async () => {
    ...
    for (const f of files) {
      ...
      root.classList.toggle('showcase__card--on', media === state.media)
  ...
  ui.png.onclick = async () => {
    ...
    const name = `showcase ${state.media ? state.media.split('/').pop().replace(/\.[^.]+$/, '') : 'empty'}`
  ...
  void (async () => {
    ...
    state = { media: '', look: rt.LOOK_PRESETS[0]?.look ?? '', ...rt.showcaseDefaults() }
    ...
    if (!state.media && ui.media.firstElementChild) {
      state.media = ui.media.firstElementChild.dataset.media
      apply()
    }
  })()
}
