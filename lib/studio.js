...
function vRestyle(m) {
  ...
  const paintChoice = () => {
    for (const card of list.querySelectorAll('.card')) card.classList.remove('on')
  ...
  const loadClips = async () => {
    ...
    for (const clip of result.clips ?? []) {
      ...
      const pick = el('button', 'btn ghost btn--small', 'Use this clip')
      pick.type = 'button'
      pick.disabled = Boolean(clip.problem) && !clip.trimmable
      pick.onclick = () => {
        state.file = clip.rel
        state.inSec = null
        state.outSec = null
      ...
      row.append(pick, trim, replace, picker)
      body.append(row)

      card.append(art, body)
      if (clip.rel === state.file) card.classList.add('on')
      list.append(card)
    }
    paintChoice()
  ...
  const paintPanel = () => {
    ...
    if (spec?.controls.includes('keepAudio')) {
      const box = g()
      const check = Object.assign(el('input'), { type: 'checkbox', checked: state.keepAudio })
      check.onchange = () => { state.keepAudio = check.checked }
      box.append(el('label', 'form-label', 'Keep audio'), check)
    }
    ...
    if (spec?.controls.includes('images')) {
      ...
      if (!projectImages.length) {
        panel.append(Object.assign(el('p', 'form-hint hint'), { textContent: 'This project has no pictures yet — add some to its media folder.' }))
      }
      for (const rel of projectImages) {
        const row = g()
        const box2 = Object.assign(el('input'), { type: 'checkbox', checked: state.images.includes(rel) })
        box2.onchange = () => {
          if (box2.checked && !state.images.includes(rel)) state.images.push(rel)
          else state.images = state.images.filter((x) => x !== rel)
        ...
        row.append(el('label', 'form-label', rel.split('/').pop()), box2)
      }
    }
  ...
  runBtn.onclick = async () => {
    ...
    // The Console owns it from here: a model call runs for minutes.
    await start(built.step)
    toast('Restyling — the Console has it. The result lands in Footage.', 'ok')
  }
