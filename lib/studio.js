...
function vStyle(m) {
  ...
  ui.addPerson.onclick = async () => {
    ...
    for (const [i, file] of files.entries()) {
      ui.addPerson.textContent = `Adding ${i + 1}/${files.length}…`
      try {
    ...
    ui.addPerson.textContent = 'Add people'
  ...
  ui.enhance.prepend(icon('sparkles'))
  ...
  const generate = async () => {
    ...
    ui.go.disabled = ui.footGo.disabled = true
    ...
    ui.go.disabled = ui.footGo.disabled = false
    if (r.error) return say(r.error, 'bad')
    const failures = []
