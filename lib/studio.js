...
function vStickers(m) {
  ...
  const setWork = (next) => {
    ...
    ui.affinity.disabled = !work || work.kind !== 'vector'
  ...
  ui.affinity.onclick = async () => {
    if (!work?.rel || work.kind !== 'vector') return say('Vectorize first — Affinity edits the SVG.', 'warn')
    const r = await post('/api/stickers/affinity', { projectId: project(), rel: work.rel })
    if (r.error) return say(r.error, 'bad')
    say(`Opened in ${r.app}. Save there and the sticker updates here.`)
