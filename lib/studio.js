...
function vWorkflow(m) {
  ...
  m.append(el('p', 'lede', 'Create a video for ' + (project?.name ?? 'this project')))
...
function vStoryboard(m) {
  ...
  function paintRail() {
    ...
    /*
     * Sign in with Slack, which is the account people already have.
     *
     * A full-page navigation rather than a popup: the browser has to leave and
     * come back with a code, and a popup is the thing a browser is most likely
     * to block. Studio is a local page with nothing unsaved on it, so leaving
     * costs nothing.
     */
    const slackIn = el('button', 'btn btn--small ghost', 'Sign in with Slack')
    slackIn.type = 'button'
    ...
    btnRow.append(signIn, signUp, slackIn)
    fieldRow(sf, btnRow)
    ...
    const out = el('button', 'btn ghost btn--small', '−')
    const inn = el('button', 'btn ghost btn--small', '+')
    const fit = el('button', 'btn ghost btn--small', 'Fit')
...
function vRestyle(m) {
  ...
  m.append(clipHead, list, faceHead, faces, voiceHead, voices, promptHead, promptIn, clipNote)
