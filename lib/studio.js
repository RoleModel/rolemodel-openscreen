...
function vMoodboard(m) {
  ...
  const buildRail = () => {
    ...
    if (!hasKey) lo.append(control('hint', { textContent: 'There is no fal key yet. Add one under Restyle and come back.' }))
      ...
      go_.onclick = async () => {
        go_.disabled = true
        status('Zipping the wall and handing it to fal…')
        const r = await post('/api/boards/train', { projectId: project(), name: board.name, model: make.model })
        go_.disabled = false
        ...
        status(`Training on ${r.pictures} pictures${r.drawn ? `, ${r.drawn} drawn from vectors` : ''}. Ten to thirty minutes.`, 'ok')
        buildRail()
        watchTraining()
      }
      lo.append(go_)
