...
function vCreator(m) {
  ...
  const flag = (key) => ({ key, is: () => Boolean(stateLook[key]), on: () => (stateLook[key] = 1), off: () => (stateLook[key] = 0) })
  const amount = (keys, first) => ({
    is: () => keys.some((k) => Number(stateLook[k]) > 0),
    on: () => (stateLook[keys[0]] = first ?? def(keys[0])),
    off: () => keys.forEach((k) => (stateLook[k] = 0)),
  })
  const mode = (key, first) => ({ is: () => stateLook[key] !== 'none', on: () => (stateLook[key] = first), off: () => (stateLook[key] = 'none') })
  ...
  const GROUP_GATE = {
    ...
    picture: { is: () => pictureOpen || Boolean(stateLook.img), on: () => (pictureOpen = true), off: () => ((pictureOpen = false), (stateLook.img = '')) },
  }
  ...
  const rangeRow = (box, f) => {
    const { nodes, el: row } = mountRow('designer-range')
    row.label.textContent = f.label
    const i = Object.assign(row.input, { min: f.min, max: f.max, step: f.step, value: stateLook[f.key] })
    const show = () => (row.readout.textContent = String(Number(i.value)))
    show()
    i.oninput = () => {
      stateLook[f.key] = Number(i.value)
      show()
      apply()
    }
    box.append(...nodes)
  ...
  const selectRow = (box, f) => {
    ...
    row.label.textContent = f.label
    for (const o of f.options) row.input.append(new Option(o, o))
    row.input.value = stateLook[f.key]
    row.input.onchange = () => {
      stateLook[f.key] = row.input.value
      apply()
    }
    box.append(...nodes)
    return row.input
  ...
  const toggleRow = (box, f) => {
    ...
    row.input.checked = Boolean(stateLook[f.key])
    row.input.onchange = () => {
      stateLook[f.key] = row.input.checked ? 1 : 0
      apply()
    }
    box.append(...nodes)
    return row.input
  ...
  const colorRow = (box, f) => {
    ...
    row.label.textContent = f.label
    row.readout.textContent = stateLook[f.key]
    row.label.after(
      colorMenu({
        families: brandFamilies(),
        value: stateLook[f.key],
        format: 'hex',
        onPick: (hex) => {
          stateLook[f.key] = hex
  ...
  const paintStops = () => {
    ...
    stateLook.stops.forEach((stop, idx) => {
      ...
      el.remove.disabled = stateLook.stops.length <= 2
      el.remove.onclick = () => {
        stateLook.stops.splice(idx, 1)
        paintStops()
        apply()
      }
      stopsBox.append(root)
    })
    if (stateLook.stops.length < rt.LOOK_MAX_STOPS) {
      const add = control('button', { className: 'btn ghost btn--small', textContent: 'Add a stop' })
      add.onclick = () => {
        const last = stateLook.stops[stateLook.stops.length - 1]
        stateLook.stops.push({ c: last.c, p: Math.min(1, last.p + 0.1) })
        paintStops()
        apply()
      }
      stopsBox.append(add)
  ...
  const buildDials = () => {
    ...
    for (const [group, label] of rt.LOOK_GROUPS) {
      ...
      if (group === 'picture') {
        ...
        const paintPicture = () => {
          row.name.textContent = stateLook.img ? stateLook.img.split('/').pop() : 'none'
          row.clear.hidden = !stateLook.img
        }
        paintPicture()
        row.pick.onclick = () => row.file.click()
        row.clear.onclick = () => {
