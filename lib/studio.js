...
function vTimeline(m) {
  ...
  const { lede, empty, stage, video, gap, play, clock, under, stat, save, canvas } = ui
  ...
  /* Which clip is under the playhead. Two can be — that is a dissolve — and the
     later one is what a render shows on top, so the later one wins here too. */
  const clipUnder = (t) => {
    let found = null
  ...
  const frame = () => {
    ...
    gap.hidden = Boolean(clip)
    under.textContent = clip ? clip.id : ''
    if (!clip) {
      showing = null
      return
    }
    ...
    if (Math.abs(video.currentTime - into) > 0.02) video.currentTime = into
  }
  ...
  ;(async () => {
    ...
    stage.hidden = false
    empty.hidden = true
    lede.textContent = `${cut.tracks.flatMap((t) => t.clips).length} clips · ${cut.width}x${cut.height} @ ${cut.fps}fps`
    ...
    canvas.addEventListener('wheel', (e) => {
      e.preventDefault()
      const before = mod.canvas.xToTime(view, e.offsetX)
      view.pxPerSecond = Math.min(400, Math.max(4, view.pxPerSecond * (e.deltaY < 0 ? 1.12 : 1 / 1.12)))
      view.scrollSeconds = Math.max(0, before - (e.offsetX - mod.canvas.HEAD_W) / view.pxPerSecond)
      draw()
      fill()
    }, { passive: false })
    fill()
  ...
  play.onclick = () => {
    ...
    if (!playing) return void video.pause()
    video.play().catch(() => {})
    const step = () => {
      if (!playing) return
      const clip = clipUnder(state.playhead)
      if (clip) state.playhead = clip.at + (video.currentTime - clip.in)
      else state.playhead += 1 / (cut?.fps || 60)
      draw()
