...
function vTimeline(m) {
  ...
  const chooseFrom = (compositions) => {
    ...
    if (!compositions.length) {
       ...
       * in another panel. The server can lay the takes end to end in a fresh
       * cut; the button that says so belongs where the dead end was.
       */
      empty.append(control('hint', { textContent: 'No composition here yet — but a cut does not need one to start.' }))
      const start = control('button', { className: 'btn', textContent: 'Start a cut from this project’s footage' })
      start.onclick = async () => {
        start.disabled = true
        ...
        if (made.error) {
          ...
          start.textContent = 'Start a cut from this project’s footage'
