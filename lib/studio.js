...
function vRecord(m) {
  ...
  const recheck = () => {
    ...
    checking = setTimeout(async () => {
      ...
      if (!d.urls.length) {
        tone(scriptHint, 'warn')
        scriptHint.textContent = `${d.actions} action${d.actions === 1 ? '' : 's'}, but the script never goes to a page — a scripted capture drives its own browser and it starts blank. Add "Go to a page" as the first step.`
        return
      }
      tone(scriptHint, 'ok')
  ...
  go.onclick = async () => {
    ...
    steps.append(
      plan([
        r.script
          ? [
              'Drive a browser through the script, and record that browser.',
              'Not the window picked above — a scripted capture drives a browser it opens itself, so that is what gets recorded. The capture ends when the script does. ' +
