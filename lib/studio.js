...
function vShowcase(m) {
  ...
  const layerTag = (l) => {
    const { at, dur, ...rest } = l
    return `<rm-showcase at="${Math.round(at * 1000)}" for="${Math.round(dur * 1000)}" ${rt.encodeShowcase(rest)}></rm-showcase>`
  }
  ...
  const apply = () => {
    ...
    queued = requestAnimationFrame(() => {
      ...
      scene.layers.forEach((l, i) => {
        ...
        for (const f of rt.SHOWCASE_SCHEMA) el.setAttribute(f.key, String(l[f.key]))
  ...
  const buildRail = () => {
    ...
    for (const [group, label] of rt.SHOWCASE_GROUPS) {
      ...
      if (group === 'media') {
        ...
        box.append(...nodes)
        /* The backdrop is the scene's, not the layer's: one look under every layer. */
  ...
  void (async () => {
    ...
    scene = { look: rt.LOOK_PRESETS[0]?.look ?? '', layers: [] }
