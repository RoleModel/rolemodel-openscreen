...
function vMake(m) {
   ...
   */
  const pick = mk('Script', el('select'), 'A saved script from this project, or the shared shelf. Choosing one fills the box below, which you can still edit.')
  ...
  fillScripts()
  proj.addEventListener('change', fillScripts)
  pick.onchange = () => {
    const sc = S.scripts[Number(pick.value)]
    if (sc) src.value = sc.body
  }
  field(f, 'Script or URL', src, 'Paste a script, or a URL to build one from. Type / for a setting — brand, voice, motion, title. A saved script above fills this in.')
  /*
  ...
  const eyebrow = mk('Eyebrow', Object.assign(el('input'), { placeholder: 'FEENEY · WALKTHROUGH' }), 'Small mono label above the title. The client or the series, not a second headline.')
  ...
  proj.addEventListener('change', fillClips)
  fillClips()
...
function vScriptEditor(m, doc) {
  ...
  for (const p of S.projects) proj.append(Object.assign(el('option', null, (p.client ? p.client + ' · ' : '') + p.name), { value: p.id }))
  proj.value = doc.project || ''
  mk('Project', proj, "Moving it saves a copy into that project's scripts/ folder.")

  const name = Object.assign(el('input'), { value: doc.name || '' })
  mk('Name', name, 'The filename, without .md. Voice matches a narration run to a script by this.')

  const body = el('textarea')
  body.className = 'form-control'
  body.spellcheck = false
  body.value = doc.body || ''
  // Tall, because this is the point of the screen.
  body.style.blockSize = '26rem'
  body.style.minBlockSize = '26rem'
   ...
   * the menu cannot offer a wallpaper the library does not have or a setting the
   * parser would reject. The project comes from the select above, because which
   * clips and tracks exist depends on it.
   */
  slashField(body, () => proj.value)

  /*
   * What Voice will actually say, counted the way it counts it.
   *
   * The same parser the synthesiser uses, so the number here and the number of lines
   * spoken cannot disagree — a preview that argues with the synthesiser is worse than
   * no preview.
   */
  const recount = () => {
    // estimateSeconds takes the parsed lines, not the markdown — handing it the raw
    // string threw and took the whole screen with it, which is why the editor came up
    ...
    stat.textContent = spoken.length ? `${spoken.length} spoken line${spoken.length === 1 ? '' : 's'}${secs ? ` · about ${Math.round(secs)}s` : ''}` : 'Nothing to speak yet.'
  }
  body.oninput = recount
  recount()

  /*
   * One way back, and it is the breadcrumb.
   *
   * This row also carried a ghost "← All scripts" button, which went exactly where
   * the "Scripts" crumb above already goes. Two controls for one destination is not
   * redundancy you can ignore: it puts a second navigation model in a panel whose
   * header is already the navigation, and it competes with Save for the eye at the
   * bottom of the form. The breadcrumb is the trail; the button row is for acting on
   * what is in front of you.
   */
  const row = el('div', 'full')
  row.style.cssText = 'display:flex;gap:var(--op-space-x-small);flex-wrap:wrap;align-items:center'
  const save = el('button', 'btn')
  save.append(icon('floppy-disk'), Object.assign(el('span'), { textContent: 'Save' }))
  const said = el('div', 'hint')
  row.append(save, said)
  f.append(row)

  save.onclick = async () => {
    if (!name.value.trim()) {
      tone(said, 'bad')
      said.textContent = 'Give it a name first.'
      return
    }
    save.disabled = true
    tone(said)
    ...
    const r = await fetch('/api/script', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ name: name.value, body: body.value, projectId: proj.value || null }),
    })
      .then((x) => x.json())
      .catch(() => ({ error: 'could not reach the Studio' }))
    save.disabled = false
    ...
    // Keep editing rather than bouncing to the list: saving is not finishing. The
    // state is updated so the list behind is right whenever you do go back.
    editingScript = { name: name.value, body: body.value, project: proj.value || '' }
    await load()
    tone(said, 'ok')
    said.textContent = 'Saved' + (r?.dest ? ' to ' + r.dest : '')
  }
...
function vScripts(m) {
  // Editing one script is its own screen. Nothing else on this page matters while you
  // are writing, and leaving the drafter above it was the complaint.
  if (editingScript) return vScriptEditor(m, editingScript)
  ...
  if (S.scripts.length) {
    ...
    for (const s of S.scripts) {
      ...
      /*
       * Redo, for a script that was drafted rather than typed.
       *
       * The brief was assembled and thrown away, so "same idea, one change" meant
       * retyping it from memory. This puts the original inputs back in the form
       * above — editable, because a redo that could not change anything would just
       * be a re-run.
       *
       * Saving under the same name overwrites; the field is prefilled with it and
       * left editable so keeping both versions is a matter of typing a new one.
       */
      if (s.brief) {
        const redo = el('button', 'btn ghost', 'Redo this brief')
        redo.title = `Drafted ${new Date(s.brief.drafted).toLocaleString()} · ${s.brief.seconds}s`
        redo.onclick = (e) => {
          e.stopPropagation() // the card opens the script; this is not that
          dproj.value = s.brief.projectId ?? s.project ?? ''
          dname.value = s.brief.name
          dsecs.value = s.brief.seconds
          dabout.value = s.brief.about
          // Repaint the highlight layer: setting .value fires no input event, so
          // the chips behind the text would still show the previous brief.
          dabout.dispatchEvent(new Event('input', { bubbles: true }))
      ...
      c.style.cursor = 'pointer'
      c.onclick = () => {
        editingScript = { name: s.name, body: s.body, project: s.project || '' }
        render()
      }
      g.append(c)
    }
...
/*
 * Fields whose value is one of the brand pictures.
 *
 * Same idea as ENUM_FIELDS: a field with a known set of valid values should offer
 * them rather than take dictation. These are shown as the pictures themselves,
 * because a filename is not what anybody is choosing between.
 */
const IMAGE_FIELDS = new Set(['src', 'image'])
 ...
 */
function vScenes(m) {
  m.append(el('p', 'lede', 'A scene is a stage with parts on it. Add a part, fill it in, watch it. Everything is seeked rather than played, so what you scrub to is what gets encoded.'))
  ...
  const proj = mk('Project', el('select'))
  for (const p of S.projects) proj.append(Object.assign(el('option', null, (p.client ? p.client + ' · ' : '') + p.name), { value: p.id }))
  const pick = mk('Scene', el('select'), 'Saved in the project under scenes/.')
  const name = mk('Name', Object.assign(el('input'), { placeholder: 'opening-card' }))
  const wp = mk('Wallpaper', el('select'))
  /*
   * The sub-brand, which today means the typeface.
   *
   * Set on the stage, not per part: custom properties cross into a component's
   * shadow root, so one choice reaches every part in the scene — and a face picked
   * per element is not a thing anybody wants.
   */
  const brand = mk('Brand', el('select'), 'Academy scenes set in Space Grotesk. RoleModel keeps DM Sans.')
  for (const [value, label] of [['', 'RoleModel — DM Sans'], ['academy', 'Craftsmanship Academy — Space Grotesk']]) {
    brand.append(Object.assign(el('option', null, label), { value }))
  }

  /*
   * Colour comes from the brand, never from a free picker.
   *
   * A hex field invites a colour the brand does not have, and one wrong swatch in
   * a title card is more obviously off-brand than any amount of bad layout. These
   * are the palette and the four sub-brand hues from brand/tokens.json — the same
   * values every other RoleModel surface spends.
   *
   * It is applied per element rather than to the stage: each component declares
   * `--brand` on its own `:host`, and an inherited value loses to that. Set on the
   * element from outside, it wins.
   */
  const swatches = el('div')
  swatches.style.cssText = 'display:flex;flex-wrap:wrap;gap:var(--op-space-2x-small);align-items:center'
  const accentWrap = mk('Accent', swatches, 'The brand colour the parts pick up. Leave it unset to use the preset default.')

  /*
   * The brief, above the button that acts on it.
   *
   * It was declared last, so `mk` appended it after everything — the field you
   * have to fill sat 120px BELOW "Draft it with Claude". Pressing the button with
   * nothing in it is then the obvious thing to do, and the 400 it returns reads as
   * a broken button rather than an empty field.
   */
  const about = el('textarea')
  about.placeholder = 'What is this scene? One or two sentences — Claude writes it from the contract in SCENE.md.'
  mk('Ask Claude for one', about)

  const cards = el('div', 'full')
  cards.style.cssText = 'display:flex;flex-direction:column;gap:var(--op-space-small)'
  const palette = el('div', 'full')
  /*
   * Flex, not the grid `.full` gives it.
   *
   * `.form .full` is `display:grid` with `place-content:start`, so each button
   * became a stretched cell in a two-column grid — a toolbar rendered as a menu.
   * The same fix the step rows needed, for the same reason: a column of controls
   * wants one reading of "put these in a row".
   */
  palette.style.cssText = 'display:flex;flex-wrap:wrap;gap:var(--op-space-x-small);align-items:center'
  palette.append(Object.assign(el('span', 'path'), { textContent: 'Add a part' }))

  const frame = el('iframe', 'full')
  frame.style.cssText = 'width:100%;aspect-ratio:16/9;border:var(--op-border-width) solid var(--line);border-radius:var(--op-radius-large);background:var(--bg)'
  const scrubRow = el('div', 'full row')
  const scrub = Object.assign(el('input', 'form-control'), { type: 'range', min: 0, max: 8000, step: 16, value: 0 })
  const tv = el('span', 'path', '0 ms')
  scrubRow.append(scrub, tv)

  /* The generated markup, behind a summary. Visible because an editor that cannot
     show its output is one nobody can debug; collapsed because it is no longer the
     thing you work in. */
  const raw = el('details', 'full')
  const rawSummary = el('summary', null, 'Markup')
  const rawText = el('textarea', 'form-control')
  rawText.spellcheck = false
  rawText.rows = 6
  raw.append(rawSummary, rawText)

  const bar = el('div', 'full row')
  const out = el('pre', 'full')
  out.style.display = 'none'
  /*
   * The scene first, then the parts.
   *
   * With the preview below the cards, three parts pushed it off the bottom — and
   * the scene is the thing being edited. Above them it stays put however many
   * parts there are, and the list grows downward into the scroll.
   */
  f.append(frame, scrubRow, palette, cards, raw, bar, out)
  m.append(f)
  ...
  const toMarkup = () =>
    elements
      .map((e) => {
        ...
        const style = accent ? ` style="--brand:${accent}"` : ''
  ...
  function paintCards() {
    ...
    elements.forEach((e, i) => {
      ...
      const kill = el('button', 'btn btn--small ghost del', '×')
      ...
      const labelled = (text, control) => {
        ...
        control.classList.add('form-control', 'form-control--small')
        return g
      }
      ...
      for (const fld of spec?.fields ?? []) {
         ...
         */
        if (IMAGE_FIELDS.has(fld) && cat.imagery?.length) {
          const group = el('div', 'form-group')
  ...
  const buildPalette = (components) => {
    for (const c of components ?? []) {
      ...
      b.onclick = () => {
        const attrs = {}
        for (const fld of c.fields) if (SCENE_SAMPLE[fld] != null) attrs[fld] = SCENE_SAMPLE[fld]
   ...
   */
  const swatchFor = (value, title, pressed) => {
    const b = el('button', 'btn ghost swatch')
    b.title = title
    b.style.cssText = '--_op-btn-height-large:1.9rem;inline-size:1.9rem;max-block-size:1.9rem;padding:0;border-radius:var(--op-radius-small);'
    b.append(
      Object.assign(el('span'), {
        style: 'display:block;inline-size:1.9rem;block-size:1.9rem;border-radius:var(--op-radius-small);' + `border:1px solid var(--line);background:${value || 'transparent'}`,
      }),
    )
  ...
  /** What a CSS value actually paints, so two spellings of one colour collapse. */
  const resolved = (cssValue) => {
    const probe = el('span')
  ...
  const buildSwatches = () => {
    const { scales = [], steps = [] } = cat.colors ?? {}
    swatches.innerHTML = ''
    const scaleRow = el('div')
    scaleRow.style.cssText = 'display:flex;flex-wrap:wrap;gap:var(--op-space-2x-small);align-items:center'
    const stepRow = el('div')
    stepRow.style.cssText = 'display:flex;flex-wrap:wrap;gap:var(--op-space-2x-small);align-items:center;margin-top:var(--op-space-x-small)'
    swatches.append(scaleRow, stepRow)

    /*
     * An empty catalogue is reported, not rendered as one square.
     *
     * `cat.colors` is absent if the fetch failed or the server predates it, and
     * the loop below then adds nothing — leaving the preset-default swatch alone
     * and looking exactly like a colour picker with one colour in it. Saying so
     * costs a line and turns "the swatches are gone" into a cause.
     */
    if (!scales.length) {
      swatches.append(
        Object.assign(el('span', 'hint'), {
          textContent: 'No colour scales came back — the Studio server is older than this page. Quit and reopen the app; a page reload alone will not pick it up.',
        }),
      )
    }

    const none = swatchFor('', 'Preset default', !accent)
    none.onclick = () => {
      accent = ''
      chosenScale = ''
      buildSwatches()
    ...
    scaleRow.append(none)

    /*
     * Dedupe only what actually resolved.
     *
     * Collapsing by painted colour is right when the tokens resolve and
     * catastrophic when they do not: an unresolvable `var()` falls back to the
     * inherited colour, every scale paints the same, and the whole palette
     * dedupes down to a single swatch. That is not a colour picker with a bug in
     * it, it is a colour picker that has silently disappeared — and the cause
     * (a missing token, a stylesheet that did not load) is nowhere near the
     * symptom.
     *
     * So a value that paints what an intentionally undefined token paints is
     * treated as unresolved: kept, never deduped, and counted so the panel can
     * say the palette is not loading rather than showing one square.
     */
    // Built, not written: a literal here reads as a token the UI spends and fails
    // the check that every --op- name resolves. This one is meant not to.
    const unresolved = resolved(`var(--op-color-${'__none'}-base)`)
    let unresolvedCount = 0
    const seen = new Map()
    for (const scale of scales) {
      const value = `var(--op-color-${scale}-base)`
      const paints = resolved(value)
      if (paints === unresolved) {
        unresolvedCount += 1
        scaleRow.append(swatchFor(value, `${scale} (not resolving)`, chosenScale === scale))
        continue
      }
      if (seen.has(paints)) continue
      seen.set(paints, scale)
      const b = swatchFor(value, scale, chosenScale === scale)
      b.onclick = () => {
        chosenScale = scale
        accent = value
        buildSwatches()
    ...
    if (unresolvedCount) {
      scaleRow.append(
        Object.assign(el('span', 'hint'), {
          textContent: `${unresolvedCount} of ${scales.length} scales are not resolving — Optics is not loaded or a token is missing.`,
          style: 'align-self:center',
        }),
      )
    }

    if (!chosenScale) return
    stepRow.append(Object.assign(el('span', 'path'), { textContent: `${chosenScale} —`, style: 'align-self:center' }))
    for (const step of steps) {
      const value = `var(--op-color-${chosenScale}-${step})`
      const b = swatchFor(value, `${chosenScale} ${step}`, accent === value)
      // The base is the seed; outline it so the ladder has a landmark.
      if (step === 'base') b.style.outline = '1px solid var(--accent)'
      b.onclick = () => {
        accent = value
        buildSwatches()
        sync({ repaint: false })
  ...
  proj.onchange = loadScenes
  wp.onchange = () => sync({ repaint: false })

  const save = el('button', 'btn ghost', 'Save scene')
  save.onclick = async () => {
    /*
  ...
  const draft = el('button', 'btn', 'Draft it with Claude')
  draft.onclick = async () => {
...
function vCompose(m) {
  ...
  const paint = () => {
    ...
    segments.forEach((seg, i) => {
      ...
      for (const [value, label] of [['', 'RoleModel — DM Sans'], ['academy', 'Academy — Space Grotesk']]) {
 ...
 */
function vCut(m) {
  m.append(el('p', 'lede', 'Take a recording off the shelf, drag its handles to keep only the part you want, and stack the pieces. Nothing is re-encoded — the cut points at your footage.'))

  const f = el('div', 'form')
  const proj = field(f, 'Project', el('select'))
  for (const p of S.projects) proj.append(Object.assign(el('option', null, (p.client ? p.client + ' · ' : '') + p.name), { value: p.id }))
  ...
  const fillShelf = () => {
    ...
    for (const file of files) {
      ...
      const chip = el('button', 'btn ghost', file.name + (silent ? ' · silent' : ''))
  ...
  const build = el('button', 'btn', 'Build and open')
  build.onclick = async () => {
    out.style.display = 'block'
...
function vBrand(m) {
  ...
  for (const b of S.logos || []) {
    for (const [variant, v] of Object.entries(b.variants || {})) {
      ...
      const img = Object.assign(el('img'), { src: `/brand/logos/${v.file}`, alt: `${b.label} — ${variant}` })
      img.style.cssText = 'max-inline-size:78%;max-block-size:62%;object-fit:contain'
  ...
  if (!marks.children.length) {
    ...
    if ((S.imagery || []).some((i) => i.file)) {
      ...
      for (const item of S.imagery) {
        ...
        const tw = el('div', 'thumbwrap')
        const t2 = el('div', 'thumb')
        const img = Object.assign(el('img'), { src: `/brand/imagery/${item.file}`, alt: item.name, loading: 'lazy' })
        img.style.cssText = 'max-inline-size:82%;max-block-size:82%;object-fit:contain'
...
function vStorage(m) {
  ...
  go.onclick = async () => {
    go.disabled = true
    const body = { name: name.value, endpoint: ep.value, accessKeyId: ak.value, secretAccessKey: sk.value }
    const r = await (
  ...
  for (const r of S.remotes) {
    ...
    /* Same click-again arming as deleteButton, but this deletes a config entry
       rather than a file, so there is no .trash to drag anything back out of. */
    const del = el('button', 'btn ghost del', 'Delete')
    let armed = false
    del.onclick = async () => {
      if (!armed) {
        armed = true
        del.className = 'btn ghost del armed'
        del.textContent = 'Delete "' + r + '"? Click again'
        setTimeout(() => {
          if (!armed) return
          armed = false
      ...
      del.disabled = false
      if (!d.ok) {
        status.textContent = 'could not delete — ' + (d.err || '').split('\n')[0].slice(0, 90)
        return
      }
