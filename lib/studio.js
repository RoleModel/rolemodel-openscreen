...
function vTimeline(m) {
  ...
  const scenesAt = (t) => {
    ...
    try {
      ...
      /*
       * The wallpaper is CSS, not an element, so switching hosts never touched
       * it: the composition paints its brand wall onto every composition root
       * as a background image, and each mounted graphic root repeats it. Behind
       * the piece's own pip footage that is the ground; above the proxy it is a
       * curtain over the whole picture. Stripped only while footage is actually
       * under the playhead — a graphics-only cut has nothing underneath, and
       * there the wallpaper IS the picture.
       */
      doc.documentElement.classList.toggle('tl-footage', Boolean(under_(t, 'video')))
  ...
  scene.onload = async () => {
    ...
    style.textContent = 'html,body{background:transparent !important}[data-assembly-media],[data-assembly-clock]{display:none !important}html.tl-footage [data-composition-id]{background:none !important}'
