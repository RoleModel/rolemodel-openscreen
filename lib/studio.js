...
function vCreator(m) {
  ...
  const buildDials = () => {
    ...
    for (const [group, label] of rt.LOOK_GROUPS) {
      ...
      if (group === 'picture') {
        ...
        row.label.textContent = 'Picture URL or name'
        row.input.value = stateLook.img || ''
        row.input.placeholder = 'a brand picture name, or a URL'
        row.input.onchange = () => {
          stateLook.img = row.input.value.trim()
          apply()
        }
        box.append(...nodes)
      }
  ...
  copyButton(ui.copyFramer, 'Copy Framer component', () => rt.lookFramerSource())
