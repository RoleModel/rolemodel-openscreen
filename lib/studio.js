...
function openPlayer(project, f) {
  const dialog = el('dialog', 'player')
  const frame = el('div', 'player__frame')
  const video = el('video', 'player__video')
  video.src = `/media/${encodeURIComponent(project.id)}/${encodeURI(f.rel)}`
  video.controls = true
  video.autoplay = true
  ...
  dialog.addEventListener('close', () => {
    ...
    // buffering, and these are 30MB files.
    video.pause()
...
function vHyperframes(m) {
  ...
  if (hyperframesWorkspace?.url) {
    ...
    const watchBox = Object.assign(el('input'), { type: 'checkbox', checked: hyperframesWatchSource })
    watchRow.append(watchBox, el('span', '', 'Reload when the composition changes on disk'))
...
function vRestyle(m) {
  ...
  const state = { model: null, file: null, prompt: '', keepAudio: true, resolution: null, images: [], inSec: null, outSec: null, image: null, audio: null }
  let models = []
  ...
  const avatarMode = () => models.find((x) => x.id === state.model)?.takes === 'image+audio'
  ...
  const paintAvatar = () => {
    ...
    for (const rel of projectImages) {
      ...
      tile.append(art)
      if (state.image === rel) tile.classList.add('on')
      tile.onclick = () => {
    ...
    for (const voice of projectVoices) {
      ...
      play.controls = true
      play.preload = 'none'
      play.src = `/media/${encodeURIComponent(currentProject())}/${encodeURI(voice.rel)}`
      body.append(play)
      card.append(body)
      if (state.audio === voice.rel) card.classList.add('on')
      card.onclick = (event) => {
        // Not when the click was meant for the player's own controls.
    ...
    if (state.image && state.audio) {
      ...
      says(chosen, `${state.image.split('/').pop()} speaking ${state.audio.split('/').pop()}${spoken ? ` · ${spoken.toFixed(1)}s` : ''}`, 'ok')
  ...
  const paintMode = () => {
    ...
    for (const node of [clipHead, list]) node.hidden = avatar
    for (const node of [faceHead, faces, voiceHead, voices]) node.hidden = !avatar
    lede.textContent = avatar
    ...
    clipNote.hidden = true
    says(dropHint, avatar ? 'Paste a picture, or drop one here, to use it as the face.' : 'Paste a picture, or drop one here, to use it as a reference.', '')
    ...
    chosen.hidden = true
    if (avatar) paintAvatar()
    else paintChoice()
  }

  m.append(clipHead, list, faceHead, faces, voiceHead, voices, chosen, el('div', 'sec', 'The instruction'), promptIn, clipNote)
   ...
   *
   * assets/mark-animated.svg is an SVGator export: the motion comes from a
   * script inside the file, not from CSS or SMIL. That decides how it is
   * embedded — an <img> never runs it and innerHTML does not execute injected
   * script tags, so both would show a still frame. An <object> loads it as a
   * document and the script runs.
   *
   * pointer-events: none in the stylesheet, because it is a document of its own
   * and would otherwise swallow clicks meant for the card underneath.
   */
  const spinnerFor = (card) => {
    const veil = el('div', 'working')
    const mark = Object.assign(el('object', 'working__mark'), {
      type: 'image/svg+xml',
      data: '/assets/mark-animated.svg',
    })
  ...
  runBtn.onclick = async () => {
    const avatar = avatarMode()
    if (avatar && !(state.image && state.audio)) return toast('Pick a picture and a voice track first.', 'bad')
    if (!avatar && !state.file) return toast('Pick a clip first.', 'bad')
    const built = await fetch('/api/fal/edit', {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ projectId: currentProject(), model: state.model, file: state.file, prompt: state.prompt, keepAudio: state.keepAudio, resolution: state.resolution, images: state.images, inSec: state.inSec, outSec: state.outSec, image: state.image, audio: state.audio }),
    }).then(responseJson).catch((error) => ({ error: error.message }))
    ...
    const held = avatar ? state.image : state.file
    const card = (avatar ? faces : list).querySelector(`.card[data-rel="${CSS.escape(held)}"]`)
