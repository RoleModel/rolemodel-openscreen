...
function vWorkflow(m) {
  ...
  const { panel } = mountPanel('workflow', m)
  ...
  const draw = ({ interview, board, workflow, assembly, alignment }) => {
    ...
    step.el.detail.textContent = ` · ${active.detail}${workflow?.currentStage ? ` · progress saved at ${workflow.currentStage}` : ''}`
    const { progress, proceed } = step.el
    proceed.textContent = active.action
...
function sceneOutlets({ tag, startMs, name, durationMs = 4000, withSave = true }) {
  ...
  const { target, where, insert: insertBtn, copy: copyBtn, name: nameIn, save: saveBtn } = outlet
