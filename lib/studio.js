...
function vStickers(m) {
  ...
  const settings = { sticker: 'telegram', cutout: '', vectorize: '', endpoint: '', prompt: '', sheetName: 'stickers', columns: 4, size: 300, gap: 40, remote: '', printSizeMm: 76.2, printBleedMm: 3, quoteQty: 250, quoteProduct: 'die-cut' }
  let affinityWatch = 0
  let makeBtn = null
  ...
  const buildRail = () => {
    ...
    rangeRow(pr, 'Longest side (mm)', 'printSizeMm', 10, 300, 1)
  ...
  ui.build.onclick = async () => {
    ...
    const r = await post('/api/stickers/sheet', { projectId: project(), name: settings.sheetName, items: [...ticked], columns: settings.columns, size: settings.size, gap: settings.gap })
    if (r.error) {
      busy(false)
      return say(r.error, 'bad')
    }
  ...
  ui.printPdf.onclick = async () => {
    busy(true, 'Drawing each sticker as a page, then converting to CMYK…')
    const r = await post('/api/stickers/print', { projectId: project(), name: settings.sheetName, sizeMm: settings.printSizeMm, bleedMm: settings.printBleedMm })
    busy(false)
    if (r.error) return say(r.error, 'bad')
    const skipped = r.skipped ? `, ${r.skipped} missing sticker${r.skipped === 1 ? '' : 's'} skipped` : ''
    status(`${r.pages} page${r.pages === 1 ? '' : 's'} in ${r.rel}, CMYK through ${r.profileName}${skipped}.`, 'ok')
  ...
  ui.publish.onclick = async () => {
    ...
    if (ticked.size) {
      busy(true, 'Laying out the sheet…')
      const built = await post('/api/stickers/sheet', { projectId: project(), name, items: [...ticked], columns: settings.columns, size: settings.size, gap: settings.gap })
      if (built.error) {
        busy(false)
