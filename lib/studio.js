 ...
 */
function sceneOutlets({ tag, startMs, name, durationMs = 4000 }) {
  ...
  return [target, insertBtn, copyBtn, nameIn, saveBtn]
}
...
function vHaze(m) {
  ...
  const tag = () => {
    ...
    if (f.motion !== HAZE_DEFAULTS.motion) attrs.push(`motion="${f.motion}"`)
  ...
  for (const [value, label] of [['flow', 'Flow — advances with the scene clock'], ['still', 'Still — one frozen frame']]) {
    motion.append(Object.assign(el('option', null, label), { value, selected: value === f.motion }))
  }
  motion.onchange = () => {
