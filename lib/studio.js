...
async function start(step, { openConsole = true } = {}) {
...
function vRecord(m) {
  ...
  session.value = 'fresh'
  const attach = { get checked() { return session.value === 'attach' } }
...
function vInterview(m) {
  m.append(el('p', 'lede', 'Answer a few practical questions about the video. Claude turns the conversation into a shot list you can load straight into Storyboard.'))
  ...
  const request = async (path, body = {}) => {
    const response = await fetch(`/api/interview${path}`, {
      method: path ? 'POST' : 'GET',
      headers: { 'content-type': 'application/json' },
      body: path ? JSON.stringify({ projectId: currentProject(), ...body }) : undefined,
    }).catch((err) => ({ error: err.message }))
    if (response.error) return response
    const raw = await response.text()
    try { return JSON.parse(raw) } catch { return { error: response.status === 404 ? 'Studio needs a restart to load Interview.' : raw || `Studio returned ${response.status}.` } }
  }
  ...
  const loadReply = async () => {
    ...
    answer.value = ''
    render()
    if (state.plan) say('The shot list is ready. Load it into Storyboard when it looks right.', 'ok')
  }

  const watchReply = (job) => {
    const check = async () => {
      const data = await fetch('/api/jobs').then((response) => response.json()).catch(() => null)
  ...
  const loadStoryboard = async () => {
    ...
    const response = await fetch('/api/board/slots', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ projectId: currentProject(), brief: state.plan }),
    }).catch((err) => ({ error: err.message }))
    if (response.error) return say(response.error, 'bad')
    const raw = await response.text()
    let r
    try { r = JSON.parse(raw) } catch { return say(raw || `Studio returned ${response.status}.`, 'bad') }
    if (r.error) return say(r.error, 'bad')
    go('storyboard')
  }
  ...
  const render = () => {
    ...
    if (state.plan) {
      ...
      const board = el('button', 'btn btn--primary', 'Load into Storyboard')
      board.onclick = loadStoryboard
      const restart = el('button', 'btn ghost', 'Start over')
      restart.onclick = startFresh
      actions.append(board, restart)
      return
    }
 ...
 * cut.  It is intentionally a separate panel from Storyboard: Storyboard is
 * about comparing many takes for a planned shot; this is about reading one
 * person talking and deciding which parts of that one recording belong.
 */
function vPaperEdit(m) {
  m.append(el('p', 'lede', 'Turn a recording into a first assembly without touching a timeline. Add its transcript, give Claude the beats you need, then review the actual lines it chose.'))
  ...
  const askClaude = el('button', 'btn btn--primary', 'Ask Claude for a first assembly')
  const loadClaude = el('button', 'btn ghost', 'Load Claude’s selection')
  field(form, 'Notes for Claude', notes, 'Claude can choose only timed transcript passages. It cannot rewrite, paraphrase or invent a timecode.')
  ...
  workspace.append(form, review)
  m.append(workspace)

  let state = null
  const planFromLines = () => String(beats.value).split('\n').map((line) => line.trim()).filter(Boolean).map((line) => {
  ...
  const request = async (path, body = {}) => {
    ...
    if (response.error) return response
    const raw = await response.text()
    try { return JSON.parse(raw) } catch { return { error: response.status === 404 ? 'Studio needs a restart to load the Paper edit actions.' : raw || `Studio returned ${response.status}.` } }
  }
  const say = (message, level = '') => says(status, message, level)
  ...
  const selectionFromReview = () => {
    ...
    for (const [cueIndex, shot] of reviewAssignments) {
      const cue = state.transcript.cues[cueIndex]
      byShot.get(shot)?.push({ from: cue.from, to: cue.to, why: 'Selected in review' })
    }
    return { shots: [...byShot.entries()].map(([shot, ranges]) => ({ shot, ranges })) }
  }
  ...
  const paintReview = () => {
    ...
    head.append(
      el('div', 'paper-edit__title', '3. Choose the lines to keep'),
      el('p', 'hint', canEdit ? (allIncluded ? 'Everything is in the first cut. Click a line or drag across several to take them out.' : 'Choose a beat, then click a line or drag across several. Click a selected line again to take it back out.') : 'Add at least one beat above. The transcript will become editable as soon as it has a name.'),
    )
    review.append(head)
    if (!canEdit) {
    ...
    review.append(lines)
    if (!canEdit) return
    const save = el('button', 'btn btn--primary', 'Make the editable cut')
    save.onclick = async () => {
      window.clearTimeout(reviewSaveTimer)
      ...
      say(`${cut.clips} clips are ready in the visual editor. Rendering stays manual.`, 'ok')
    }
    review.append(save)
  }
  ...
  const loadState = async () => {
    ...
    const raw = await response.text()
    let r
    try { r = JSON.parse(raw) } catch { return say(response.status === 404 ? 'Studio needs a restart to load the Paper edit actions.' : raw || `Studio returned ${response.status}.`, 'bad') }
    if (r.error) return say(r.error, 'bad')
    state = r.state
    if (!state) {
  ...
  const finishFirstAssembly = async () => {
    ...
    paintReview()
    const cut = await request('cut', { name: 'paper-edit' })
    if (cut.error) return cut
    const opened = await openDocument({ projectId: currentProject(), path: cut.document })
    if (opened?.error) return { error: `${cut.clips} clips are ready for review, but the editor did not open: ${opened.error}` }
    return { ...cut, coverage: selected.coverage, opened }
  }

  /*
   * Claude writes a constrained selection, not a video. Once that file exists,
   * Studio turns it into the editable OpenScreen document and opens it for
   * review. Rendering deliberately remains a decision in the editor.
   */
  const watchFirstAssembly = (job, rel) => {
    const check = async () => {
      ...
      if (clip.value !== rel) return
      const cut = await finishFirstAssembly()
      if (cut.error) return say(`Claude finished, but Studio could not prepare the first cut: ${cut.error}`, 'bad')
      say(`Claude's first cut is ready in the editor. Rendering stays manual.`, 'ok')
    }
    void check()
  }

  clip.onchange = loadState
  beats.oninput = () => {
  ...
  askClaude.onclick = async () => {
    ...
    if (r.error) return say(r.error, 'bad')
    say('Claude is making the first cut in the background. Studio will open the editable result for review when it finishes.', 'ok')
    const job = await start(r.step, { openConsole: false })
  ...
  loadClaude.onclick = async () => {
    const cut = await finishFirstAssembly()
    if (cut.error) return say(cut.error, 'bad')
    const missing = cut.coverage?.empty?.length ? ` Still missing: ${cut.coverage.empty.join(', ')}.` : ''
    say(`Loaded Claude’s first assembly: ${cut.coverage?.ranges ?? 0} passages. It is open in the editor for review.${missing}`, cut.coverage?.empty?.length ? 'warn' : 'ok')
  }

  if (videos().length === 1) {
    clip.value = videos()[0].rel
    void loadState()
  }
}
...
function vStoryboard(m) {
  m.append(
    el(
      'p',
      'lede',
      'The shots the brief asks for, and every take you have for each. Rate them, pick the one that wins, and the picks are the cut — there is no assembling step after this.',
    ),
  )
  ...
  const endPan = async () => {
    ...
    if (dragging) {
      ...
      if (moved) void api('node/move', { nodeId: id, x, y }).then((r) => { if (r.graph) graph = r.graph })
    }
    panning = null
  ...
  function select(id) {
    selected = id
    for (const c of nodeLayer.children) c.classList.toggle('board__node--selected', c.dataset.node === id)
    paintRail()
  }
  ...
  const load = async () => {
    ...
    const r = await fetch(`/api/board?project=${encodeURIComponent(proj.value)}`).then((x) => x.json())
    if (r.error) {
  ...
  function paint() {
    ...
    if (!slots.length) {
      ...
      empty.append(
        el('div', 'board__empty-head', 'No shots yet'),
        el(
          'p',
          'board__empty-body',
          'A shot is a hole in the video — "open on the problem", "the price appears", "the logo". The brief lists them, and each one becomes a column here that you fill with takes.',
        ),
      )
  ...
  function takeCard(slot, take, isChosen) {
    ...
    meta.append(
      Object.assign(el('div', 'board__take-name'), { textContent: take.rel.split('/').pop() }),
      Object.assign(el('div', 'board__take-span'), { textContent: `${clock(take.inSec ?? 0)} → ${clock(take.outSec ?? 0)}` }),
    )
    card.append(meta)
  ...
  function paintRail() {
    ...
    if (node) {
      ...
      del.onclick = async () => {
        ...
        const ok = confirm(
          takes
            ? `Delete "${node.name}" and its ${takes} take${takes === 1 ? '' : 's'}? The shot before it will lead to the shot after. Every rating stays in history.jsonl.`
            : `Delete "${node.name}"? The shot before it will lead to the shot after.`,
        )
        if (!ok) return
    ...
    if (board?.brief?.shots?.length && !briefBox.value) {
      briefBox.value = board.brief.shots
        .map((s) => [s.name, s.intent, s.seconds ? `${s.seconds}s` : null].filter(Boolean).join(' — '))
        .join('\n')
    }
    ...
    applyBrief.onclick = async () => {
      ...
      if (renamed && (board.takes ?? []).length) {
        const ok = confirm(
          'Some shot names changed. Takes stay attached to the old shot rather than moving to a renamed one, so they will need re-adding. Continue?',
        )
        if (!ok) return
      }
    ...
    const signOut = el('button', 'btn btn--small', 'Sign out')
    signOut.onclick = async () => {
      await fetch('/api/board/signout', { method: 'POST' })
      await refreshSharing()
    }
    /*
    ...
    btnRow.append(signIn, signUp, signOut)
    fieldRow(sf, btnRow)
    sf.append(sbWho)
    setup.append(sf)
    ...
    const refreshSharing = async () => {
      const [sh, bd] = await Promise.all([
        fetch('/api/board/sharing').then((x) => x.json()),
        fetch(`/api/board?project=${encodeURIComponent(proj.value)}`).then((x) => x.json()),
      ])
      ...
      sbWho.textContent = sh.signedInAs
        ? `Signed in as ${sh.signedInAs}. Ratings you give are signed with this account.`
        : configured
          ? 'Sign in and this board is shared with everyone on the team.'
          : ''
