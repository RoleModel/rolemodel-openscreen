...
function vShowcase(m) {
  ...
  const paintLayers = () => {
    ...
    scene.layers.forEach((l, i) => {
      const { root, el } = mountRow('showcase-layer')
      root.classList.toggle('showcase__layer--on', i === sel)
      el.art.style.setProperty('--pic', thumbOf(l) ? `url('${thumbOf(l)}')` : 'none')
      const when = () => {
        el.name.textContent = `${i + 1}. ${nameOf(l)}${l.device !== 'none' ? ` · ${l.device}` : ''}`
        el.when.textContent = `${l.at.toFixed(1)}s → ${(l.at + l.dur).toFixed(1)}s · in ${l.enter}, out ${l.exit}`
      }
      when()
      whenLabels.set(l, when)
      el.pick.onclick = () => {
        sel = i
        buildRail()
        paintLayers()
      ...
      el.up.disabled = i === 0
      el.down.disabled = i === scene.layers.length - 1
      const swap = (j) => {
      ...
      el.up.onclick = () => swap(i - 1)
      el.down.onclick = () => swap(i + 1)
      el.remove.disabled = scene.layers.length === 1
      el.remove.onclick = () => {
      ...
      ui.layerList.append(root)
    })
  }
  /*
   * A new layer joins the selected one, a beat later, and ends with it: a
   * build-up of pictures is the common case, and a layer that started only
   * when the last one ended was a layer nobody could find. Start and On
   * screen are dials, so a sequence is two moves away.
   */
  const newLayer = (from) => {
