...
function vStyle(m) {
  ...
  let state = { hasKey: false, problem: null, template: '', defaultTemplate: '', models: [], images: [], people: [] }
  const ASPECT_OF = { '1:1': '1 / 1', '4:3': '4 / 3', '16:9': '16 / 9', '3:4': '3 / 4', '9:16': '9 / 16' }
  const modelInfo = (id) => state.models.find((x) => x.id === id) || (/\/edit$/.test(id) ? { short: 'Refined', tone: 'orange' } : /fill$/.test(id) ? { short: 'Inpainted', tone: 'orange' } : { short: String(id || '').split('/').pop(), tone: 'neutral' })
  const badge = (el, id) => {
  ...
  const generate = async () => {
    ...
    const r = await post('/api/style/generate', { subject, models, aspect, count, people, template: ui.template.value })
  ...
  const refine = (rec) => {
    ...
    el.go.onclick = async () => {
      ...
      const r = await post('/api/style/refine', { imageUrl: rec.url, instruction, mask: painted ? exportMask() : null, subject: rec.subject, aspect: rec.aspect || '1:1' })
      if (r.error) {
        tone(el.status, 'bad')
