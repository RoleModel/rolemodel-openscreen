...
/* A copy that gives up after a moment, for copies nobody clicked for. */
const copyQuick = (text) => Promise.race([copyText(text), new Promise((r) => setTimeout(() => r('the clipboard did not answer'), 1500))])
 ...
 */
/*
 * The component runtime, stamped with this client's build.
 *
 * The URL carries the stamp the page was served with, so a new Studio never
 * runs an old rm-video.js the browser kept: the address is new, so the file
 * is fetched. Both places that load the runtime go through here, so there is
 * one module instance and one RM clock.
 */
function runtimeModule() {
  const stamp = document.querySelector('meta[name="rm-studio-client"]')?.content ?? ''
  return import(`/components/rm-video.js?v=${encodeURIComponent(stamp)}`)
}

/*
 * Ask for a name.
 *
 * window.prompt(), which this replaced, does not exist in the desktop app:
 * Electron returns null without showing anything, so "Save look" did nothing
 * and said nothing. A dialog works everywhere and looks like the Studio.
 */
function askName(title, initial = '', { lede = '', confirm = 'Save' } = {}) {
  return new Promise((done) => {
    const { root: dialog, el: modal } = mountRow('asset-dialog')
    modal.title.textContent = title
    modal.lede.textContent = lede
    modal.lede.hidden = !lede
    modal.label.textContent = 'Name'
    const input = control('input', { className: 'form-control', type: 'text', value: initial })
    modal.slot.append(input)
...
/*
 * Share a video as a page, from anywhere in the Studio: the first storage
 * remote with a public base takes it. Answers the record, or { error }.
 */
async function sharePage({ projectId, rel, title }) {
  const list = await fetch(`/api/shares?project=${encodeURIComponent(projectId)}`)
    .then(responseJson)
    .catch((e) => ({ error: e.message }))
  if (list.error) return list
  const remote = list.remotes?.[0]
  if (!remote) return { error: 'no storage remote has a public bucket and base URL — set one under Storage' }
  const r = await fetch('/api/shares/publish', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ projectId, rel, title, remote }) })
    .then(responseJson)
    .catch((e) => ({ error: e.message }))
  return r.error ? r : r.share
}
 ...
 */
const GLOBAL_VIEWS = new Set(['skills', 'brand', 'style', 'creator', 'showcase', 'moodboard', 'stickers', 'components', 'field', 'haze', 'wallpapers', 'storage', 'usage', 'console', 'library', 'new'])
...
const VIEW_ICON = {
  ...
  brand: 'paint-board',
  style: 'ai-image',
  creator: 'magic-wand-02',
  showcase: 'perspective',
  moodboard: 'dashboard-square-02',
  stickers: 'sticker',
  wallpapers: 'image-01',
...
/*
 * The prompt library, under any prompt box.
 *
 * WHY IT SITS UNDER THE BOX AND NOT IN A DRAWER
 *
 * The moment you want a saved prompt is the moment you are looking at the empty
 * box, and a library one click away somewhere else is a library nobody opens.
 * So it is a row: a list to pick from, and one button to keep what you typed.
 *
 * `where` names the box, so the list puts prompts written here first — see
 * lib/prompts.mjs. Everything the project has saved is still in the list.
 */
function promptLibrary(box, { get, set, where = '', onPick = null }) {
  const { nodes, el: row } = mountRow('prompt-library')
  let saved = []
  let picked = ''
  const draw = () => {
    row.input.textContent = ''
    row.input.append(new Option(saved.length ? 'Saved prompts…' : 'Nothing saved yet', ''))
    for (const p of saved) row.input.append(new Option(p.label || p.text.slice(0, 48), p.id))
    row.input.value = picked
    row.input.disabled = !saved.length
    row.drop.hidden = !picked
  }
  const load = async () => {
    if (!currentProject()) return
    const r = await responseJson(await fetch(`/api/prompts?project=${encodeURIComponent(currentProject())}`)).catch(() => null)
    /* Its own first, the rest after: the prompt you want is usually one you
       wrote in this box, but the one from next door is two lines down. */
    const all = r?.prompts ?? []
    saved = where ? [...all.filter((p) => p.where === where), ...all.filter((p) => p.where !== where)] : all
    draw()
  }
  row.input.onchange = () => {
    picked = row.input.value
    const p = saved.find((x) => x.id === picked)
    if (!p) return draw()
    set(p.text)
    onPick?.(p.text)
    draw()
  }
  row.keep.onclick = async () => {
    const text = String(get() ?? '').trim()
    if (!text) return toast('Write something first, then Keep it.', 'bad')
    /* askName, not window.prompt: the desktop app is Electron, where prompt()
       returns null without showing anything — the button would look broken and
       say nothing about why. See askName. */
    const name = await askName('Keep this prompt', text.length > 48 ? `${text.slice(0, 47)}…` : text, {
      lede: 'It is kept with this project, and offered under every prompt box.',
      confirm: 'Keep',
    })
    if (!name) return
    const r = await responseJson(await fetch('/api/prompts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ projectId: currentProject(), text, label: name, where }) })).catch((e) => ({ error: e.message }))
    if (r.error) return toast(r.error, 'bad')
    picked = r.saved.id
    await load()
    toast(`Kept “${r.saved.label}”.`, 'ok')
  }
  row.drop.onclick = async () => {
    const p = saved.find((x) => x.id === picked)
    if (!p) return
    if (!confirm(`Forget “${p.label}”? The prompt is removed from this project.`)) return
    await fetch('/api/prompts', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ projectId: currentProject(), id: p.id }) })
    picked = ''
    await load()
  }
  box.append(...nodes)
...
/*
 * The update button, at the floor of the sidebar.
 *
 * WHY A BUTTON AND NOT AN AUTO-UPDATE
 *
 * Upgrading swaps the code out from under a running server and every job it
 * started. That is a fine thing to do and a terrible thing to have happen to
 * you mid-render, so the choosing stays with the person: the Studio finds out,
 * and offers.
 *
 * Absent entirely when there is nothing to apply, and inert-but-explaining on a
 * checkout, where the answer is git rather than brew.
 */
async function paintUpdate() {
  const b = $('#update')
  if (!b || b.dataset.wired) return
  b.dataset.wired = '1'
  const found = await responseJson(await fetch('/api/update')).catch(() => null)
  if (!found || !found.behind) return
  b.hidden = false
  b.append(icon('download-04'), ` Update to ${found.latest}`)
  b.title = found.how.can ? `You are on ${found.current}. This runs ${found.how.say}.` : `You are on ${found.current}. ${found.how.why}`
  b.onclick = async () => {
    if (!found.how.can) return toast(`${found.how.why} Run ${found.how.say} yourself.`, 'bad')
    /* Named plainly, because it changes what is installed on the machine. */
    if (!confirm(`Update the toolkit from ${found.current} to ${found.latest}?\n\nThis runs ${found.how.say} and takes a few minutes. Finish anything that is rendering first.`)) return
    b.disabled = true
    const r = await responseJson(await fetch('/api/update', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' }))
    if (r.error) {
      b.disabled = false
      return toast(r.error, 'bad')
    }
    toast(`Updating to ${r.latest}. Watch it in the Console, then restart the Studio.`, 'ok')
    go('console')
  }
}
...
async function paintSignOut() {
  ...
  if (!b.dataset.wired) {
    ...
    b.onclick = async () => {
      window.posthog?.capture('signed_out', { project_id: currentProject() ?? null })
...
function paintNoticeDot() {
  ...
  for (const trigger of document.querySelectorAll('.projmenu--stage .projmenu__trigger')) {
    ...
    trigger.title = n ? `${n} new note${n === 1 ? '' : 's'}` : ''
  }
  const count = $('#notesCount')
  if (count) {
    count.textContent = n ? String(n) : ''
    const bell = $('#notesBell')
    if (bell) bell.title = n ? `${n} new note${n === 1 ? '' : 's'}` : 'Notes'
  }
}
 ...
 * The notes centre.
 *
 * A badge says how many; it does not say what anybody wrote, and a note left on
 * a sticker sheet reached the page and nothing else at all. This is the one
 * place to read both, newest first, with a press to open the page it is on.
 */
function openNotes() {
  const host = $('#notesBell')?.parentElement
  if (!host) return
  const already = host.querySelector('.notes__panel')
  if (already) return void already.remove()
  const { root, el } = mountRow('notes-panel')
  host.append(root)
  const away = (e) => {
    if (root.contains(e.target) || e.target.closest?.('#notesBell')) return
    root.remove()
    document.removeEventListener('pointerdown', away)
  }
  setTimeout(() => document.addEventListener('pointerdown', away), 0)
  el.close.onclick = () => root.remove()
  el.markSeen.onclick = async () => {
    await markReviewNoticesSeen()
    root.remove()
  }
  void fetch('/api/notifications/all')
    .then(responseJson)
    .catch(() => ({ notes: [] }))
    .then((d) => {
      const notes = d.notes ?? []
      ...
      for (const n of notes) {
        const { root: item, el: row } = mountRow('notes-item')
        row.who.textContent = n.author || 'Someone'
        row.on.textContent = n.kind === 'sticker' ? `${n.on} · ${n.title}` : n.title
        row.when.textContent = new Date(n.at).toLocaleString()
        row.body.textContent = n.body
        /* Straight to the page, at the moment the note is about when there is one. */
        row.go.onclick = () => open(n.atMs != null ? `${n.url}#t=${Math.round(n.atMs / 1000)}` : n.url)
        el.list.append(item)
      }
    })
}

/* Wired once, when the chrome is first drawn. */
function wireNotesBell() {
  const bell = $('#notesBell')
 ...
 * `false`: whether a page was shared is recorded beside the project, not in the
 * hub, and the strip should not claim a step is unstarted when it cannot tell.
 */
...
function render() {
  ...
  /* The page's one first-level heading, named for where you are. It lives
     outside main, which render() empties, and inside main's landmark by
     being labelled as main's own heading. */
  const heading = $('#viewHeading')
  if (heading) heading.textContent = `${page} — RoleModel Studio`
  ...
  VIEWS[view](m)
  /* After the view has built itself, and again once anything it fetched lands. */
  const named = () => {
    nameLooseControls(document.body)
    if (heading) heading.textContent = `${page} — RoleModel Studio`
  }
  named()
  setTimeout(named, 400)
  setTimeout(named, 1500)
}
...
function go(v, { preserveProject = false } = {}) {
  ...
  if (!document.startViewTransition) {
    view = v
    window.posthog?.capture('view_changed', { view: v, project_id: currentProject() ?? null })
...
function jobDetailLink(status, id) {
  const details = control('button', { className: 'btn btn--hint', textContent: 'View details' })
  details.onclick = () => {
    jobId = id
    go('console')
  }
...
function vProject(m, id) {
  ...
  const actions = [
    ...
    { icon: 'exchange-01', text: 'Convert files…', run: async () => void (await convertMany(p)) },
...
/**
 * Convert one asset into another format, as a Console job.
 *
 * One question — which format, and how big — then ffmpeg runs where every other
 * long job runs, and the card says how it is going. The result lands beside the
 * source under the same name with the new extension; the catalog picks it up
 * when the job ends.
 */
function convertAsset(project, f) {
  return new Promise((done) => {
    const { root: dialog, el: modal } = mountRow('asset-dialog')
    modal.title.textContent = `Convert ${f.name}`
    modal.lede.textContent = 'A new file is written next to this one. Nothing is replaced.'
    const format = control('select', { className: 'form-control' })
    format.setAttribute('aria-label', 'Format')
    const size = control('select', { className: 'form-control' })
    size.setAttribute('aria-label', 'Size')
    modal.label.textContent = 'Format'
    modal.slot.append(format, size)
    const status = modal.status
    statusSink(status)
    const cancel = modal.cancel
    cancel.onclick = () => dialog.close()
    const go_ = modal.confirm
    go_.textContent = 'Convert'
    go_.disabled = true
    fetch(`/api/media/convert?kind=${encodeURIComponent(f.kind)}`)
      .then(responseJson)
      .then((r) => {
        // An older server has no such route and says so; an empty list with no
        // sentence beside it reads as a broken dialog rather than a stale server.
        if (r.error) {
          tone(status, 'bad')
          status.hidden = false
          status.textContent = r.error
        ...
        for (const x of r.formats ?? []) format.append(new Option(x.label, x.id))
        for (const x of r.sizes ?? []) size.append(new Option(x.label, x.id))
        // Size is a video question; audio-only and GIF targets do their own sizing.
        const sizeApplies = () => {
          const chosen = (r.formats ?? []).find((x) => x.id === format.value)
          size.hidden = !chosen || chosen.audioOnly || chosen.gif
        }
        format.onchange = sizeApplies
        sizeApplies()
        go_.disabled = !(r.formats ?? []).length
      })
      .catch(() => {
        tone(status, 'bad')
        status.hidden = false
        status.textContent = 'the Studio did not answer — is it still running on this port?'
      })
    go_.onclick = async () => {
      go_.disabled = true
      cancel.disabled = true
      ...
      const result = await fetch('/api/media/convert', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ projectId: project.id, rel: f.rel, format: format.value, size: size.hidden ? 'source' : size.value }),
      })
        .then(responseJson)
        .catch((error) => ({ error: error.message }))
      if (result.error) {
...
/*
 * Convert many files at once, one after another.
 *
 * The same dialog as a single convert, with a checklist: every video (or every
 * sound file) in the project, ticked. The jobs run in turn rather than all at
 * once — eight ffmpegs at a time is a machine nobody can use — and the dialog
 * counts them off. Nothing is replaced; each file gets a new copy beside it.
 */
function convertMany(project) {
  return new Promise((done) => {
    const files = project.catalog?.files ?? []
    const of = (kind) => files.filter((f) => f.kind === kind)
    const kinds = [
      ['video', `Videos (${of('video').length})`],
      ['audio', `Sound (${of('audio').length})`],
    ].filter(([k]) => of(k).length)
    if (!kinds.length) {
      toast('This project has no videos or sound files to convert.', 'warn')
      return done(null)
    }
    const { root: dialog, el: modal } = mountRow('asset-dialog')
    modal.title.textContent = 'Convert files'
    modal.lede.textContent = 'Each file gets a new copy next to it, one after another. Nothing is replaced.'
    const kind = control('select', { className: 'form-control' })
    kind.setAttribute('aria-label', 'Kind')
    for (const [k, label] of kinds) kind.append(new Option(label, k))
    const format = control('select', { className: 'form-control' })
    format.setAttribute('aria-label', 'Format')
    const size = control('select', { className: 'form-control' })
    size.setAttribute('aria-label', 'Size')
    const list = mountRow('convert-list').root
    modal.label.textContent = 'Convert'
    modal.slot.append(kind, format, size, list)
    const status = modal.status
    statusSink(status)
    const cancel = modal.cancel
    cancel.onclick = () => dialog.close()
    const go_ = modal.confirm
    go_.textContent = 'Convert all'
    go_.disabled = true
    let offered = { formats: [], sizes: [] }
    const sizeApplies = () => {
      const chosen = offered.formats.find((x) => x.id === format.value)
      size.hidden = !chosen || chosen.audioOnly || chosen.gif
    }
    const paintList = () => {
      list.replaceChildren()
      for (const f of of(kind.value)) {
        const { root, el } = mountRow('convert-item')
        el.name.textContent = f.rel
        el.pick.value = f.rel
        list.append(root)
      }
    }
    const load = async () => {
      ...
      const r = await fetch(`/api/media/convert?kind=${encodeURIComponent(kind.value)}`)
        .then(responseJson)
      ...
      offered = { formats: r.formats ?? [], sizes: r.sizes ?? [] }
      for (const x of offered.formats) format.append(new Option(x.label, x.id))
      for (const x of offered.sizes) size.append(new Option(x.label, x.id))
      sizeApplies()
      paintList()
      go_.disabled = !offered.formats.length
    }
    format.onchange = sizeApplies
    kind.onchange = load
    void load()
    /* One job, waited for. watchJobInPlace reports in the header; this only
       needs to know when it is over. */
    const finished = (job) => new Promise((r) => watchJobInPlace(job, null, r))
    go_.onclick = async () => {
      const picked = [...list.querySelectorAll('input:checked')].map((x) => x.value)
      if (!picked.length) return
      go_.disabled = true
      cancel.disabled = true
      kind.disabled = format.disabled = size.disabled = true
      status.hidden = false
      tone(status)
      let made = 0
      let failed = 0
      for (const [i, rel] of picked.entries()) {
        status.textContent = `${i + 1} of ${picked.length}: ${rel.split('/').pop()}…`
        const result = await fetch('/api/media/convert', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ projectId: project.id, rel, format: format.value, size: size.hidden ? 'source' : size.value }),
        })
          .then(responseJson)
          .catch((error) => ({ error: error.message }))
        if (result.error) {
        ...
        const job = result.alreadyRunning ? result.job : await start(result.step)
        if (!job) {
          failed += 1
          continue
        }
        const end = await finished(job)
        if (end?.code === 0) made += 1
        else failed += 1
      }
      tone(status, failed ? 'warn' : 'ok')
      status.textContent = failed ? `${made} converted, ${failed} failed — see the Console.` : `${made} converted.`
      toast(status.textContent, failed ? 'warn' : 'ok')
      cancel.disabled = false
      cancel.lastChild.textContent = 'Done'
      dialog.addEventListener('close', () => done({ made, failed }), { once: true })
    }
...
function fileCard(project, f) {
  ...
  const transcribe = async () => {
    ...
    watchJobInPlace(job, note, (finished) => {
      if (finished?.code === 0) {
        tone(note, 'ok')
        note.textContent = `Transcript ready. Captions saved beside the clip as ${f.name.replace(/\.[^.]+$/, '')}.vtt. Open Assembly to review it.`
      }
    })
    return null
  }
  ...
  c.append(
    actionMenu([
      ...
      /*
       * A new file in another container, from the card that holds the source.
       *
       * Slack wants an MP4, a web page a WebM, a slide a GIF, a transcriber a
       * WAV; the recipe for each is in lib/convert.mjs and the encode runs as a
       * Console job, reported here on the card the way transcription is.
       */
      ...(f.kind === 'video' || f.kind === 'audio' ? [{
        ...
        busy: 'Converting…',
        run: async () => {
          const started = await convertAsset(project, f)
          if (!started) return null
          tone(note, 'ok')
          showJobStatus(note, started.job)
          watchJobInPlace(started.job, note, (finished) => {
            if (finished?.code === 0) {
              tone(note, 'ok')
              note.textContent = `Wrote ${started.out}.`
              void load()
            }
          })
          return null
      ...
      {
        icon: 'share-08',
        text: 'Share as a page',
        busy: 'Sharing…',
        run: async () => {
          const r = await sharePage({ projectId: project.id, rel: f.rel, title: f.name.replace(/\.[^.]+$/, '') })
          if (r.error) return r.error
          /* Stay put: the link is copied, and Review lists the page for later. */
          const copyErr = await copyQuick(r.url)
          say(copyErr ? 'Shared. The link is on the Review page.' : 'Shared. Link copied.')
          return null
        },
      },
      /*
       * Posted, not linked.
       *
       * A share page is a link; this uploads the file itself, so the video is
       * delivered by a tool that is already in everyone's dock — and it keeps
       * working for a team that never opens the Review page.
       *
      ...
      /*
       * A link to the asset's copy in shared storage, from the card itself.
       *
       * The file on this machine has no URL anyone else can open; its sent
       * copy does. The server finds which remote holds it — the manifest's
       * `remote` field is not an rclone remote name, and sending it produced
       * "[object Object] is not a path" — and it checks the object is actually
       * there before minting anything, so a project never sent gets a
       * sentence, not a dead URL. Permanence follows the remote's
       * public-bucket setting, same as the storage browser.
       */
      /*
       * The page's address, once this file has been shared as one. Looked up on
       * the press rather than carried on the card: the shelf is drawn for every
       * file and few of them have a page.
       */
      {
        icon: 'link-square-01',
        text: 'Copy page link',
        busy: 'Finding the page…',
        run: async () => {
          const list = await fetch(`/api/shares?project=${encodeURIComponent(project.id)}`)
            .then(responseJson)
            .catch(() => ({ error: 'the Studio did not answer' }))
          if (list.error) return list.error
          const page = (list.shares ?? []).find((sh) => sh.rel === f.rel)
          if (!page) return 'Not shared as a page yet — press Share as a page first.'
          const failed = await copyText(page.url)
          if (failed) return failed
          toast('Page link copied.', 'ok')
        },
      },
      ...(S?.remotes?.length ? [{
        icon: 'link-01',
        ...
        busy: 'Making a link…',
        run: async () => {
          const r = await fetch(`/api/project/storage/link?project=${encodeURIComponent(project.id)}&rel=${encodeURIComponent(f.rel)}`)
            .then(responseJson)
            .catch(() => ({ error: 'the Studio did not answer' }))
          if (r.error) return r.error
          const failed = await copyText(r.url)
          if (failed) return failed
          toast(r.permanent ? 'Permanent link copied.' : r.expiry ? `Link copied — it expires in ${r.expiry}.` : 'Link copied.', 'ok')
        },
      }] : []),
      /*
      ...
      /*
       * This one video, straight onto the timeline.
       *
       * The Timeline's own starter cuts what you tick; this is the shorter
       * road when the video you want is the card in front of you. Sending the
       * same take twice reopens the cut it already has — the folder is named
       * after the file, so it cannot multiply.
       */
      ...(f.kind === 'video' ? [{
        icon: 'timeline',
        text: 'Send to Timeline',
        busy: 'Preparing the cut…',
        run: async () => {
          const r = await fetch('/api/edit/start', {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ project: project.id, rels: [f.rel] }),
          }).then(responseJson).catch((e) => ({ error: e.message }))
          if (r.error) return r.error
          timelineFolder = r.folder
          go('timeline')
        },
      }] : []),
      {
...
function vReview(m) {
  ...
  const post = (path, body, method = 'POST') =>
    fetch(path, { method, headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) })
      .then(responseJson)
      .catch((e) => ({ error: e.message }))
  const mediaUrl = (rel) => `/media/${encodeURIComponent(project())}/${encodeURI(rel)}`
  const thumbUrl = (rel) => `/thumb/${encodeURIComponent(project())}/${encodeURI(rel)}`
  const mmss = (ms) => {
    const sec = Math.max(0, Math.round(ms / 1000))
    return `${Math.floor(sec / 60)}:${String(sec % 60).padStart(2, '0')}`
  }
  let data = { shares: [], remotes: [], comments: false }

  /*
   * The renders: every video in the project, the Renders folder first. Each
   * card takes a title and a Share button; the page's name is the title, or the
   * file's. A render already out is marked, and sharing it again republishes.
   */
  const paintRenders = () => {
    ui.renders.replaceChildren()
    const proj = S.projects.find((p) => p.id === project())
    const videos = (proj?.catalog?.files ?? []).filter((f) => f.kind === 'video')
    videos.sort((x, y) => Number(/^Renders\//.test(y.rel)) - Number(/^Renders\//.test(x.rel)) || String(y.mtime ?? '').localeCompare(String(x.mtime ?? '')))
    ui.renderCount.textContent = videos.length ? `· ${videos.length}` : ''
    ui.rendersEmpty.hidden = videos.length > 0
    for (const f of videos) {
      const { root, el } = mountRow('review-render')
      el.art.style.backgroundImage = `url('${thumbUrl(f.rel)}')`
      el.name.textContent = f.rel
      const out = data.shares.find((sh) => sh.rel === f.rel)
      el.facts.textContent = out ? `shared ${out.at.slice(0, 10)} · ${out.notes} note${out.notes === 1 ? '' : 's'}` : /^Renders\//.test(f.rel) ? 'a render' : 'project media'
      el.title.value = out?.title ?? f.name.replace(/\.[^.]+$/, '')
      el.share.lastChild.textContent = out ? 'Share again' : 'Share'
      el.share.disabled = !data.remotes.length
      el.share.onclick = () => share(f, el.title.value.trim(), el.share, el.out)
      /* The link, once there is one: a button, not a line of URL. */
      el.copy.hidden = !out
      if (out) copyButton(el.copy, 'Copy link', out.url)
      ui.renders.append(root)
    }
  }
  const share = async (f, title, button, out) => {
    const remote = data.remotes[0]
    if (!remote) return say('Set a public bucket on a storage remote first, under Storage.', 'warn')
    button.disabled = true
    tone(out)
    out.textContent = 'Copying the video, drawing a poster, uploading… a render takes as long as it takes.'
    const r = await post('/api/shares/publish', { projectId: project(), rel: f.rel, title, remote })
    button.disabled = false
    if (r.error) {
      tone(out, 'bad')
    ...
    out.textContent = ''
    /* The cards first, then the clipboard: a browser tab that is not focused can
       hold a clipboard write open for ever, and the page must not wait on it. */
    await reload()
    const copyErr = await copyQuick(r.share.url)
    say(copyErr ? 'Shared. Copy link is on the card.' : 'Shared. Link copied.')
  }

  /*
   * The pages out there. Poster, title, when, how many notes; open, copy the
   * link, read the notes here, publish again, or take it down. Taking down is
   * behind the armed menu: a live link is not a thing to kill on one press.
   */
  const paintShares = () => {
    ui.shares.replaceChildren()
    ui.shareCount.textContent = data.shares.length ? `· ${data.shares.length}` : ''
    ui.sharesEmpty.hidden = data.shares.length > 0
    for (const sh of data.shares) {
      const { root, el } = mountRow('review-share')
      if (sh.poster) el.art.style.backgroundImage = `url('${sh.url.replace(/index\.html$/, sh.poster)}')`
      else if (sh.rel) el.art.style.backgroundImage = `url('${thumbUrl(sh.rel)}')`
      el.name.textContent = sh.title
      el.facts.textContent = [`shared ${sh.at.slice(0, 10)}`, sh.comments ? `${sh.notes} note${sh.notes === 1 ? '' : 's'}${sh.latest ? `, last ${new Date(sh.latest).toLocaleDateString()}` : ''}` : 'notes off'].join(' · ')
      el.link.href = sh.url
      copyButton(el.copy, 'Copy link', sh.url)
      el.notes.hidden = !sh.comments
      el.notes.onclick = async () => {
        if (!el.thread.hidden) return void (el.thread.hidden = true)
        el.notes.disabled = true
        const r = await fetch(`/api/shares/notes?project=${encodeURIComponent(project())}&video=${encodeURIComponent(sh.slug)}`)
          .then(responseJson)
          .catch((e) => ({ error: e.message }))
        el.notes.disabled = false
        if (r.error) return say(r.error, 'bad')
        el.thread.replaceChildren()
        if (!r.notes.length) el.thread.append(control('hint', { textContent: 'No notes yet.' }))
        for (const n of r.notes) {
          const note = mountRow('review-note')
          note.el.at.textContent = n.at_ms != null ? mmss(n.at_ms) : ''
          /* A press on the time opens the page at that moment. */
          if (n.at_ms != null) note.el.at.onclick = () => open(`${sh.url}#t=${Math.round(n.at_ms / 1000)}`)
          note.el.who.textContent = n.author || 'Someone'
          note.el.when.textContent = new Date(n.created_at).toLocaleString()
          note.el.body.textContent = n.body
          /* Tidying is the Studio's alone: the page's role can only add. */
          note.el.tidy.hidden = !r.canTidy
          note.el.tidy.onclick = async () => {
            note.el.tidy.disabled = true
            const d = await post('/api/shares/note', { id: n.id }, 'DELETE')
            if (d.error) {
              note.el.tidy.disabled = false
              return say(d.error, 'bad')
            }
            note.root.remove()
            void reload()
          }
          el.thread.append(note.root)
        }
        el.thread.hidden = false
      }
      el.again.hidden = !sh.rel
      el.again.onclick = () => share({ rel: sh.rel }, sh.title, el.again, el.facts)
      el.acts.append(
        actionMenu([{
          icon: 'delete-02',
          text: 'Take the page down',
          busy: 'Taking down…',
          danger: true,
          run: async () => {
            const r = await post('/api/shares', { projectId: project(), slug: sh.slug }, 'DELETE')
            if (r.error) return r.error
            await reload()
            return null
          },
  ...
  const reload = async () => {
    if (!project()) {
      tone(ui.status, 'warn')
      ui.status.textContent = 'Pick a project first.'
      return
    }
    const r = await fetch(`/api/shares?project=${encodeURIComponent(project())}`)
      .then(responseJson)
      .catch((e) => ({ error: e.message }))
    if (r.error) return say(r.error, 'bad')
    data = r
    ui.noStorage.hidden = data.remotes.length > 0
    ui.noNotes.hidden = data.comments
    tone(ui.status)
    ui.status.textContent = ''
    paintRenders()
    paintShares()
...
function vNew(m) {
  ...
  go.onclick = async () => {
    ...
    window.posthog?.capture('project_created', { brand: brand.value, storage: store.value })
...
function vRecord(m) {
  ...
  go.onclick = async () => {
    ...
    window.posthog?.capture('recording_started', {
      project_id: proj.value || null,
      scripted: Boolean(script.value.trim()),
      headless: headless.checked,
    })
    steps.append(
      plan([
...
function vVoice(m) {
  ...
  go.onclick = async () => {
    ...
    window.posthog?.capture('voice_generated', {
      project_id: proj.value || null,
...
function vScripts(m) {
  ...
  const saveScript = async () => {
    ...
    if (fresh && !fresh.error) S = fresh
    paintSaved()
    window.posthog?.capture('script_saved', { project_id: project.value || null, line_count: body.value.split('\n').length })
    tone(status, 'ok')
    status.hidden = false
 ...
 */
/*
 * Remembered per value. Every swatch in a colour menu paints its token through
 * a probe element and a computed style — 369 of them, so one menu cost a
 * second and a page with seven menus froze for most of ten. A token's painted
 * colour only changes with the theme, and that is when the memory is dropped.
 */
const paintedColors = new Map()
matchMedia('(prefers-color-scheme: dark)').addEventListener?.('change', () => paintedColors.clear())
function paintedColor(cssValue) {
  const known = paintedColors.get(cssValue)
 ...
 */
/*
 * "This is the one you are on", on a card or a row.
 *
 * aria-selected was here and means nothing on a div or a plain button — it
 * belongs to a listbox or a tab set, and axe counted it as an attribute the
 * element may not carry. aria-current says the same thing and is allowed
 * anywhere; absent is the honest way to say no.
 */
/*
 * A last look for controls nobody named.
 *
 * Most rows tie their label to their control in mountRow, but a select or a
 * field built in a line of code has only what is around it — a placeholder, a
 * tooltip, or the words immediately before it, which is what a sighted person
 * reads as its name anyway. Borrowing that is better than a control announced
 * as nothing at all; it does not excuse a row that should carry a real label.
 */
function nameLooseControls(root) {
  for (const c of root.querySelectorAll('select, input, textarea')) {
    if (c.id && root.querySelector(`label[for="${CSS.escape(c.id)}"]`)) continue
    if (c.closest('label') || c.getAttribute('aria-label') || c.getAttribute('aria-labelledby')) continue
    if (c.type === 'hidden' || c.type === 'checkbox' || c.type === 'radio') continue
    const before = c.previousElementSibling
    const near = /^(LABEL|SPAN|B|STRONG)$/.test(before?.tagName ?? '') ? before.textContent.trim() : ''
    const name = c.title?.trim() || c.placeholder?.trim() || near
    if (name) c.setAttribute('aria-label', name.slice(0, 80))
  }
}
...
function mountRow(name) {
  ...
  for (const node of frag.querySelectorAll('[data-el]')) el_[node.dataset.el] = node
  /*
   * A label beside a control is not a label to a screen reader.
   *
   * These rows are a <label> and an <input> as siblings, which looks right and
   * says nothing: axe counted 120 controls with no accessible name across
   * fourteen views. Rows are cloned by the hundred, so tying the two together
   * here fixes every one of them at once rather than in every template.
   */
  const label = el_.label
  const input = el_.input ?? el_.select ?? el_.field
  if (label?.tagName === 'LABEL' && input && !label.htmlFor && !input.id && /^(INPUT|SELECT|TEXTAREA)$/.test(input.tagName)) {
    const id = `rw${(mountRow.n = (mountRow.n ?? 0) + 1)}`
    input.id = id
    label.htmlFor = id
  }
  ...
  const drawPanel = () => {
    ...
    for (const family of families) {
      ...
      for (const step of live) {
        ...
        markCurrent(b, hit?.family === family && hit?.step === step)
...
function vScenes(m) {
  ...
  /*
   * The scene as a video file.
   *
   * A scene is a design with a clock, and until now the only way to get moving
   * pictures out of one was to record the screen. It is rendered frame by
   * frame instead — seeked, shot, encoded — so what comes out is the same on
   * any machine and as long as the scene says it is.
   *
   * WebM when it has to keep its transparency, because VP9 carries an alpha
   * channel and H.264 does not; a transparent scene asked for as MP4 would
   * arrive on black without saying why.
   */
  ui.render.onclick = async () => {
    if (!name.value.trim()) {
      say('Give the scene a name first — it becomes the filename.', 'bad')
      name.focus()
      return
    }
    /* No wallpaper means the scene is meant to sit on something else, so it is
       rendered WebM and keeps its transparency; a scene with a ground is an MP4,
       which every editor and every phone will play. */
    const wantsAlpha = !wp.value
    ui.render.disabled = true
    ui.renderSave.hidden = true
    say(`Rendering ${name.value.trim()}…`)
    const r = await responseJson(
      await fetch('/api/scenes/render', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          projectId: proj.value,
          ...
          wallpaper: wp.value,
          brand: brand.value || null,
          footage: previewFootage ? { src: footageSource(previewFootage.rel), inSec: previewFootage.inSec, outSec: previewFootage.outSec } : null,
          format: wantsAlpha ? 'webm' : 'mp4',
    ...
    say(`${r.rel} · ${secs}s · ${r.height}p${r.transparent ? ' · transparent' : ''}`, 'ok')
    ui.renderSave.hidden = false
    ui.renderSave.href = `/media/${encodeURIComponent(proj.value)}/${encodeURI(r.rel)}?v=${Date.now()}`
    ui.renderSave.download = r.rel.split('/').pop()
    ui.renderSaveText.textContent = `Save the ${r.format.toUpperCase()}`
  }
  ...
  const draw = ({ interview, board, workflow, assembly, alignment }) => {
    ...
    const steps = [
      ...
      },
      {
        ...
        /* No local evidence here: the share records live beside the project,
           and the hub does not read them. Left not done rather than guessed — by
           this point it is the last step anyway. */
        done: false,
...
function vMultiAssembly(m) {
  ...
  const openAlignedTimeline = control('button', { className: 'btn ghost', textContent: 'Open in HyperFrames', hidden: true })
  const reviewAlignedVideo = control('button', { className: 'btn ghost', textContent: 'Review rendered video', hidden: true })
  const renderAligned = control('button', { className: 'btn btn--primary', textContent: 'Render audio review video', hidden: true })
  ...
  const build = async () => {
    ...
    window.posthog?.capture('video_assembled', {
      project_id: currentProject() ?? null,
      clip_count: made.selections ?? state?.picks?.length ?? 0,
    })
    await refresh()
  ...
  renderAndShare.onclick = async () => {
    ...
    await runWithStatus(planned.step, renderAndShare, status, async (finished) => {
      if (finished?.code !== 0) return
      const shared = await sharePage({ projectId: currentProject(), rel: planned.renderedRel, title: planned.title })
      if (shared.error) return say(`Rendered and saved to this project, but could not share it: ${shared.error}`, 'warn')
      await copyQuick(shared.url)
      renderAndShare.textContent = 'Open review'
      renderAndShare.onclick = () => go('review')
      await refresh()
      say('Review cut rendered, saved to this project, and shared as a page.', 'ok')
    })
  }
  void refresh().then(() => {
...
function vStoryboard(m) {
  ...
  function paintRail() {
    ...
    if (node) {
      ...
      const del = control('button', { className: 'btn', textContent: 'Delete this shot' })
      del.onclick = async () => {
      ...
      const editScene = control('button', { className: 'btn ghost', textContent: 'Open scene editor' })
      editScene.onclick = () => {
      ...
      const reviewFootage = control('button', { className: 'btn ghost', textContent: 'Review footage' })
     ...
     *
     * Those now come with the release (lib/deployment.mjs). What is left here is
     * the only part that is actually about the person at the keyboard.
     */
     ...
     *
     * GitHub, because a tool for a software team can assume one, and a second
     * credential to invent and remember is the thing the first person on a
     * project always trips on. The device flow: press the button, get a code,
     * type it at github.com in another tab, and this panel notices. No redirect
     * back to a port that changes every launch.
     */
    const ghIn = mountRow('board-github-sign-in').root
    const device = mountRow('board-device-code')
    device.root.hidden = true
    let devicePoll = null
    const stopDevice = () => {
    ...
    ghIn.onclick = async () => {
      ghIn.disabled = true
      const r = await fetch('/api/board/github/start', { method: 'POST' })
        .then(responseJson)
        .catch(() => ({ error: 'the Studio did not answer — is it still running on this port?' }))
      ghIn.disabled = false
      if (r.error) return sbTell(r.error, 'bad')
      device.el.code.textContent = r.userCode
      device.el.where.href = r.verificationUri
      device.el.where.textContent = r.verificationUri.replace(/^https?:\/\//, '')
      device.el.wait.textContent = 'Waiting for GitHub…'
      device.root.hidden = false
      // Opened for them, since the code is the only thing they have to carry.
      window.open(r.verificationUri, '_blank', 'noopener')
      const every = Math.max(2000, r.intervalMs || 5000)
      const tick = async () => {
        const p = await fetch('/api/board/github/poll', { method: 'POST' })
          .then(responseJson)
          .catch(() => ({ status: 'pending' }))
        if (p.error) {
          stopDevice()
          return sbTell(p.error, 'bad')
        }
        if (p.status === 'ok') {
          stopDevice()
          sbTell(`Signed in as ${p.signedInAs}.`, 'ok')
          window.posthog?.capture('github_signed_in', { project_id: currentProject() ?? null })
          return refreshSharing()
        }
        if (p.status === 'denied') {
          stopDevice()
          return sbTell('GitHub said you cancelled the sign-in.', 'warn')
        }
        if (p.status === 'expired' || p.status === 'none') {
          stopDevice()
          return sbTell('That code expired — press Sign in with GitHub again.', 'warn')
        }
        devicePoll = setTimeout(tick, every)
      }
      devicePoll = setTimeout(tick, every)
    }

    const btnRow = mountRow('board-setup-acts').root
    btnRow.append(ghIn, device.root)
    fieldRow(sf, btnRow)
    ...
    const refreshSharing = async () => {
      ...
      const configured = Boolean(sh.configured)
      /*
      ...
      ghIn.hidden = !askingForCredentials
      if (!askingForCredentials) stopDevice()
     ...
     */
    const syncNow = control('button', { className: 'btn', textContent: 'Sync now' })
...
function vSkills(m) {
  ...
  const paint = (result) => {
    ...
    if (!skills.length) {
      ...
      for (const skill of skills) {
        ...
        card.source.textContent = skill.source === 'shared' ? 'Shared with the team' : 'Bundled with this Studio install'
  ...
  const loadSkills = async () => {
    ...
    status.textContent = result.shared?.available
      ? `${result.shared.count || 0} shared skill${result.shared.count === 1 ? '' : 's'} synced from the team library.`
  ...
  const upload = async (file, replace = false) => {
    ...
    window.posthog?.capture('skill_run', { action: 'upload', skill_count: result.installed?.length ?? 0 })
    await loadSkills()
...
/* ── Brand ───────────────────────────────────────────────── */
/**
 * Style: the brand collage generator.
 *
 * The style app, brought into the Studio — see lib/style-gen.mjs for where it
 * came from. Same job, the Studio's own clothes: Optics controls, cards, a
 * kebab per tile, and the fal key that Restyle already saved. Pictures live in
 * the team's database and the library's Style folder, and "Save to project"
 * puts one into a project's Stills.
 */
function vStyle(m) {
  const ui = mountPanel('style', m)
  /*
   * Said twice when it matters. The hint sits under the form; the buttons sit
   * in the footer, a screen away on a long library. A refusal that only appears
   * up there reads as a button that does nothing.
   */
  const say = (text, kind) => {
    ui.said.hidden = !text
    tone(ui.said, kind)
    ui.said.textContent = text || ''
    if (text && (kind === 'warn' || kind === 'bad')) toast(text, kind)
  }

  let state = { hasKey: false, problem: null, template: '', defaultTemplate: '', models: [], images: [], people: [], palette: [], cutoutModel: '', projectNames: {} }
  const ASPECT_OF = { '1:1': '1 / 1', '4:3': '4 / 3', '16:9': '16 / 9', '3:4': '3 / 4', '9:16': '9 / 16' }
  const modelInfo = (id) =>
    state.models.find((x) => x.id === id) ||
    (id === state.cutoutModel
      ? { short: 'Cutout', tone: 'neutral' }
      : id === 'composite'
        ? { short: 'On colour', tone: 'neutral' }
        : /\/edit$/.test(id)
          ? { short: 'Refined', tone: 'orange' }
          : /fill$/.test(id)
            ? { short: 'Inpainted', tone: 'orange' }
            : { short: String(id || '').split('/').pop(), tone: 'neutral' })
  const isCutout = (rec) => rec.model === state.cutoutModel
  const badge = (el, id) => {
    const info = modelInfo(id)
    el.textContent = info.short
    el.className = `kind stylegen__badge stylegen__badge--${info.tone}`
  }
  const post = (path, body, method = 'POST') =>
    fetch(path, { method, headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) })
      .then(responseJson)
  ...
  /* ── the tiles ── */
  const lightbox = (url) => {
    const { root: box, el } = mountRow('style-lightbox')
    el.img.src = url
    // The backdrop and the button close it; a click on the picture does not.
    box.onclick = (e) => e.target === box && box.close()
    el.close.onclick = () => box.close()
  ...
  const tile = (rec) => {
    const { root, el } = mountRow('style-tile')
    const src = rec.file ? `/style/${encodeURIComponent(rec.file)}` : rec.url
    // The picture is a variable, so the cutout style can stack a checker under it.
    el.open.style.setProperty('--pic', `url('${src}')`)
    el.open.style.aspectRatio = ASPECT_OF[rec.aspect] || '1 / 1'
    el.open.onclick = () => lightbox(src)
    if (isCutout(rec)) el.open.classList.add('stylegen__thumb--cutout')
    badge(el.badge, rec.model)
    const home = rec.project_id ? state.projectNames[rec.project_id] || rec.project_id : null
    el.where.textContent = home ? `in ${home}${rec.project_rel ? ` · ${rec.project_rel}` : ''}` : ''
    el.where.hidden = !home
    el.subject.value = rec.subject || ''
    el.subject.onkeydown = (e) => {
      if (e.key === 'Enter') el.subject.blur()
    }
    el.subject.onblur = async () => {
      const next = el.subject.value.trim()
      if (!next || next === rec.subject) {
        el.subject.value = rec.subject
        return
      }
      const r = await post('/api/style/images', { id: rec.id, subject: next }, 'PATCH')
      if (r.error) {
        el.subject.value = rec.subject
        return say(r.error, 'bad')
      }
    ...
    el.when.textContent = rec.created_at ? ago(rec.created_at) : ''
    if (rec.copyProblem) {
      tone(el.note, 'warn')
      el.note.textContent = `kept in the database only — ${rec.copyProblem}`
    }
    const projects = () => S.projects.map((p) => ({ id: p.id, name: p.name }))
    const saveTo = async (projectId) => {
      const r = await post('/api/style/save', { id: rec.id, projectId })
      if (r.error) return r.error
      const proj = S.projects.find((p) => p.id === projectId)
      tone(el.note, 'ok')
      el.note.textContent = `Saved to ${proj?.name || 'the project'} as ${r.rel}.`
      if (!rec.project_id) {
        rec.project_id = projectId
        rec.project_rel = r.rel
        el.where.textContent = `in ${proj?.name || projectId} · ${r.rel}`
    ...
    const addResult = (image) => {
      ui.results.prepend(tile(image))
      state.images.unshift(image)
      paintLibrary()
    }
    root.append(
      actionMenu([
        { icon: 'edit-02', text: 'Refine…', busy: 'Opening…', run: async () => void refine(rec) },
        ...(isCutout(rec)
          ? [{ icon: 'paint-brush-01', text: 'On brand colours…', busy: 'Opening…', run: async () => void colours(rec) }]
          : [
              {
                icon: 'magic-wand-01',
                ...
                busy: 'Cutting out…',
                run: async () => {
                  const r = await post('/api/style/remove-bg', { id: rec.id, projectId: currentProject() || rec.project_id || null })
                  if (r.error) return r.error
                  addResult(r.image)
                  return null
                },
        ...
        ...(currentProject()
          ? [{ icon: 'folder-add', text: `Save to ${S.projects.find((p) => p.id === currentProject())?.name || 'this project'}`, busy: 'Saving…', run: () => saveTo(currentProject()) }]
          : []),
        ...projects()
          .filter((p) => p.id !== currentProject())
          .slice(0, 6)
          .map((p) => ({ icon: 'folder-add', text: `Save to ${p.name}`, busy: 'Saving…', run: () => saveTo(p.id) })),
        /*
         * Getting the picture out. Download saves the library file through the
         * browser; Copy link copies the public address when there is one, else
         * this Studio's; Show in Finder selects the file on this disk.
         */
        {
          icon: 'download-04',
          text: 'Download',
          busy: 'Saving…',
          run: async () => {
            const a = Object.assign(document.createElement('a'), { href: src, download: rec.file || `${rec.subject || 'style'}.png` })
            a.click()
            return null
          },
        },
        { icon: 'link-01', text: 'Copy link', busy: 'Copying…', run: () => copyText(/^https?:\/\//.test(rec.url || '') ? rec.url : new URL(src, location.href).href) },
        ...(rec.file && !window.rmStudio?.hosted ? [{ icon: 'folder-01', text: 'Show in Finder', busy: 'Opening…', run: async () => (await post('/api/style/reveal', { id: rec.id })).error ?? null }] : []),
        { icon: 'arrow-up-right-01', text: 'Open original', busy: 'Opening…', run: async () => void window.open(rec.url, '_blank', 'noopener') },
        {
          icon: 'delete-02',
          text: 'Delete',
          busy: 'Deleting…',
          run: async () => {
            const r = await post('/api/style/images', { id: rec.id }, 'DELETE')
            if (r.error) return r.error
            state.images = state.images.filter((x) => x.id !== rec.id)
            root.remove()
            paintLibrary()
            return null
          },
  ...
  const paintLibrary = () => {
    ui.library.replaceChildren(...state.images.map(tile))
    ui.libCount.textContent = state.images.length ? `${state.images.length} image${state.images.length === 1 ? '' : 's'}` : ''
    ui.libEmpty.hidden = state.images.length > 0
  }

  /* ── models and people to tick ── */
  const check = ({ text, sub, value, checked, face }) => {
    const { root, el } = mountRow('style-check')
    el.box.value = value
    el.box.checked = Boolean(checked)
    ...
    return { root, box: el.box }
  }
  const chosen = (holder) => [...holder.querySelectorAll('input:checked')].map((x) => x.value)
  const paintModels = () => {
    ui.models.replaceChildren()
    const before = chosen(ui.models)
    for (const mdl of state.models) {
      const { root } = check({ text: mdl.label, sub: mdl.maker, value: mdl.id, checked: before.length ? before.includes(mdl.id) : mdl.id === 'fal-ai/nano-banana-pro' })
      root.dataset.people = mdl.people ? '1' : ''
      ui.models.append(root)
    }
  }
  // FLUX 2 and GPT Image 2 cannot take reference photos; they step aside when people are ticked.
  const gateModels = () => {
    const hasPeople = chosen(ui.peoplePick).length > 0
    for (const label of ui.models.querySelectorAll('.stylegen__check')) {
      const box = label.querySelector('input')
      const off = hasPeople && !label.dataset.people
      box.disabled = off
      if (off) box.checked = false
      label.classList.toggle('stylegen__check--off', off)
      label.title = off ? 'This model cannot use reference photos of people' : ''
    }
  }
  const paintPeoplePick = () => {
    const before = chosen(ui.peoplePick)
    ui.peoplePick.replaceChildren()
    if (!state.people.length) {
      ui.peoplePick.append(control('span', { className: 'hint', textContent: 'No people yet — add teammates below.' }))
      return
    }
    for (const person of state.people) {
      const { root, box } = check({ text: person.name, value: person.id, checked: before.includes(person.id), face: person.photo })
      box.onchange = gateModels
      ui.peoplePick.append(root)
    }
    gateModels()
  }
  ...
  const personTile = (person) => {
    const { root, el } = mountRow('style-person')
    el.face.style.backgroundImage = `url('${person.photo}')`
    el.name.value = person.name
    el.name.onkeydown = (e) => {
      if (e.key === 'Enter') el.name.blur()
    }
    el.name.onblur = async () => {
      const next = el.name.value.trim()
      if (!next || next === person.name) {
        el.name.value = person.name
        return
      }
      const r = await post('/api/style/people', { id: person.id, name: next }, 'PATCH')
      if (r.error) {
        el.name.value = person.name
        return say(r.error, 'bad')
      }
      ...
      actionMenu([
        {
          ...
          busy: 'Removing…',
          run: async () => {
            const r = await post('/api/style/people', { id: person.id }, 'DELETE')
            if (r.error) return r.error
            state.people = state.people.filter((x) => x.id !== person.id)
            root.remove()
            paintPeoplePick()
            return null
          },
  ...
  const paintPeople = () => ui.people.replaceChildren(...state.people.map(personTile))

  // Resized here so a phone photo does not become two megabytes in a database row.
  const shrink = async (file, max = 768) => {
    // createImageBitmap reads the file straight into pixels: no blob URL, no
    // data URL of the original, only the small JPEG that leaves here.
    const bmp = await createImageBitmap(file).catch(() => null)
    if (!bmp) throw new Error(`could not read ${file.name}`)
  ...
  ui.addPerson.onclick = async () => {
    const files = [...ui.personPhoto.files]
    if (!files.length) return say('Choose one or more photos first.', 'warn')
    const override = ui.personName.value.trim()
    ui.addPerson.disabled = true
    const failed = []
    for (const [i, file] of files.entries()) {
      ui.addPerson.lastChild.textContent = `Adding ${i + 1}/${files.length}…`
      try {
        const photo = await shrink(file)
        const r = await post('/api/style/people', { name: files.length === 1 && override ? override : nameFrom(file.name), photo })
        if (r.error) throw new Error(r.error)
        state.people.push(r.person)
      } catch (e) {
        failed.push(`${file.name}: ${e.message}`)
      }
    }
    ui.addPerson.disabled = false
    ui.addPerson.lastChild.textContent = 'Add people'
    ui.personPhoto.value = ''
    ui.personName.value = ''
    state.people.sort((a, b) => a.name.localeCompare(b.name))
    paintPeople()
    paintPeoplePick()
    say(failed.length ? failed.join(' · ') : '', failed.length ? 'bad' : '')
  }

  /* The subject is this view's prompt, and a house subject gets retyped more
     than anything else here. */
  promptLibrary(ui.subjectLib, {
    where: 'style',
    get: () => ui.subject.value,
    set: (v) => {
      ui.subject.value = v
    },
  })
  ...
  ui.enhance.onclick = async () => {
    const subject = ui.subject.value.trim()
    if (!subject) return say('Type a rough idea first — Enhance fleshes it out.', 'warn')
    ui.enhance.disabled = ui.go.disabled = true
    say('Enhancing…')
    const r = await post('/api/style/enhance', { subject })
    ui.enhance.disabled = ui.go.disabled = false
    if (r.error) return say(r.error, 'bad')
    ui.subject.value = r.improved
    say('')
  }

  const generate = async () => {
    const subject = ui.subject.value.trim()
    if (!subject) return say('Describe what the image should show first.', 'warn')
    const models = chosen(ui.models)
    if (!models.length) return say('Pick at least one model. With people ticked, only the Nano Banana models can be used.', 'warn')
    const aspect = ui.aspect.value
    const count = Number(ui.count.value)
    const people = chosen(ui.peoplePick)
    ui.go.disabled = true
    say('Generating… this takes a little while.')
    const pending = []
    for (const model of [...models].reverse()) {
      for (let i = 0; i < count; i++) {
        const { root, el } = mountRow('style-placeholder')
        el.badge && badge(el.badge, model)
        root.querySelector('.stylegen__thumb').style.aspectRatio = ASPECT_OF[aspect] || '1 / 1'
        ui.results.prepend(root)
        pending.push(root)
      }
    }
    // The placeholders land at the top of the results; bring them into view,
    // because the button that made them is at the bottom of the page.
    ui.results.scrollIntoView({ block: 'nearest', behavior: 'smooth' })
    const r = await post('/api/style/generate', { subject, models, aspect, count, people, template: ui.template.value, projectId: currentProject() || null })
    for (const node of pending) node.remove()
    ui.go.disabled = false
    if (r.error) return say(r.error, 'bad')
    const failures = []
    for (const result of [...r.results].reverse()) {
      if (result.error) failures.push(`${modelInfo(result.model).short}: ${result.error}`)
      for (const rec of [...result.images].reverse()) {
        ui.results.prepend(tile(rec))
        state.images.unshift(rec)
      }
    }
    paintLibrary()
    say(failures.length ? `Some models failed — ${failures.join(' · ')}` : '', failures.length ? 'warn' : '')
  }
  ui.go.onclick = generate
  ui.subject.onkeydown = (e) => {
    if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') generate()
  }

  /* ── the template ── */
  ui.template.onchange = async () => {
    const r = await post('/api/style/template', { template: ui.template.value })
    if (r.error) return say(r.error, 'bad')
    ui.template.value = r.template
  }
  ui.resetTemplate.onclick = async () => {
    ui.template.value = ''
    const r = await post('/api/style/template', { template: '' })
    if (!r.error) ui.template.value = r.template
  }

  /* ── refine: paint a mask, or describe an edit ── */
  const refine = (rec) => {
    const { root: dialog, el } = mountRow('style-refine')
    el.title.textContent = `Refine ${rec.subject}`
    const src = rec.file ? `/style/${encodeURIComponent(rec.file)}` : rec.url
    ...
    const paintTo = (pt) => {
      ...
      ctx.strokeStyle = 'rgba(232,155,48,0.65)'
      ctx.lineWidth = Number(el.brush.value) * scale
      ctx.lineCap = ctx.lineJoin = 'round'
      ctx.beginPath()
      ctx.moveTo((last || pt).x, (last || pt).y)
      ctx.lineTo(pt.x, pt.y)
      ctx.stroke()
      last = pt
      if (!painted) {
        painted = true
        el.maskState.textContent = 'Mask painted — inpainting just that area'
      }
    }
    let down = false
    ...
    canvas.onpointermove = (e) => {
      if (down) paintTo(pointOf(e))
    }
    canvas.onpointerup = () => {
      down = false
      last = null
    }
    el.clearMask.onclick = () => {
      ctx.clearRect(0, 0, canvas.width, canvas.height)
      painted = false
      el.maskState.textContent = 'No mask — whole-image edit'
    }
    // Black keeps, white repaints: the shape FLUX Fill expects.
    const exportMask = () => {
      const m = document.createElement('canvas')
      m.width = canvas.width
      m.height = canvas.height
      const mc = m.getContext('2d')
      const from = ctx.getImageData(0, 0, canvas.width, canvas.height)
      // Pixel by pixel, no colour names: every pixel opaque, black where the
      // brush did not go and white where it did.
      const out = mc.createImageData(m.width, m.height)
      for (let i = 0; i < from.data.length; i += 4) {
        const on = from.data[i + 3] > 40 ? 255 : 0
    ...
    el.go.onclick = async () => {
      ...
      el.go.disabled = el.cancel.disabled = true
      el.status.hidden = false
      tone(el.status)
      el.status.textContent = painted ? 'Inpainting…' : 'Refining…'
      const r = await post('/api/style/refine', { imageUrl: rec.url, instruction, mask: painted ? exportMask() : null, subject: rec.subject, aspect: rec.aspect || '1:1', projectId: currentProject() || rec.project_id || null })
      if (r.error) {
        tone(el.status, 'bad')
      ...
      dialog.close()
      ui.results.prepend(tile(r.image))
  ...
  /* ── a cutout on brand colours ── */
  // Luminance auto-levels, as the old tool did it: stretch the 2nd–98th percentile.
  const autoLevels = (data) => {
    const lums = []
    for (let i = 0; i < data.length; i += 4) if (data[i + 3] >= 128) lums.push(0.299 * data[i] + 0.587 * data[i + 1] + 0.114 * data[i + 2])
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      for (const c of [0, 1, 2]) data[i + c] = Math.max(0, Math.min(255, (data[i + c] - lo) * k))
    }
  }
  const colours = (rec) => {
    const { root: dialog, el } = mountRow('style-colours')
    el.title.textContent = `${rec.subject} on brand colours`
    for (const c of state.palette) {
      const { root, el: row } = mountRow('style-check')
      row.box.value = c.hex
      row.box.checked = true
      row.text.textContent = c.name
      row.sub.textContent = c.hex
      const dot = control('span', { className: 'stylegen__swatch' })
      dot.style.background = c.hex
      row.box.after(dot)
      el.swatches.append(root)
    }
    el.cancel.onclick = () => dialog.close()
    el.go.onclick = async () => {
      const picked = [...el.swatches.querySelectorAll('input:checked')].map((x) => ({ hex: x.value, name: x.closest('label').querySelector('span').textContent }))
      if (!picked.length) return
      el.go.disabled = el.cancel.disabled = true
      el.status.hidden = false
      tone(el.status)
      el.status.textContent = 'Loading the cutout…'
      try {
        const src = rec.file ? `/style/${encodeURIComponent(rec.file)}` : rec.url
        const picture = await fetch(src)
        if (!picture.ok) throw new Error('the cutout could not be loaded')
        const bmp = await createImageBitmap(await picture.blob())
        ...
        bmp.close()
        if (el.normalize.checked) {
          const px = bctx.getImageData(0, 0, base.width, base.height)
          autoLevels(px.data)
          bctx.putImageData(px, 0, 0)
        }
        let made = 0
        for (const c of picked) {
          el.status.textContent = `Making ${c.name}…`
          ...
          const r = await post('/api/style/upload', {
            dataUrl: out.toDataURL('image/png'),
            subject: `${rec.subject.replace(/ \(cutout\)$/, '')} on ${c.name.toLowerCase()}`,
            model: 'composite',
            aspect: rec.aspect || null,
            prompt: rec.prompt || null,
            projectId: currentProject() || rec.project_id || null,
          })
          if (r.error) throw new Error(r.error)
          addResultGlobal(r.image)
          made++
        }
        dialog.close()
        say(`Made ${made} picture${made === 1 ? '' : 's'} on colour.`, 'ok')
      } catch (e) {
        tone(el.status, 'bad')
  ...
  /* ── load ── */
  ;(async () => {
    const r = await fetch('/api/style/state').then(responseJson).catch((e) => ({ error: e.message }))
    if (r.error) return say(r.error, 'bad')
    state = r
    ...
    if (state.problem) {
      ...
      tone(ui.problem, 'warn')
      ui.problem.textContent = `The library is off: ${state.problem}`
    }
    if (!state.hasKey) say('Add a fal key under Restyle first — the same key makes these pictures.', 'warn')
  })()
}
...
/* ══════════════════════════════════════════════════════════════════════════
 * Stickers: a picture made into a sticker, cut out, traced, on a sheet, online.
 *
 * Each step is one server call to fal, and each result lands in the project's
 * Stickers folder as its own file, so nothing is lost between steps and any
 * file can be the start of the next one. The rail holds the models and the
 * prompt; the footer holds the steps. A sheet is the ticked stickers laid out
 * as one SVG, and Publish copies it, with a page, to shared storage.
 * ══════════════════════════════════════════════════════════════════════════ */
/* ══════════════════════════════════════════════════════════════════════════
 * Mood board — the wall you put references on before anything is designed
 *
 * Everything else in the Studio makes something. This holds what somebody
 * found: pictures pasted, dropped or picked, arranged by hand, each with a
 * name and a sentence about why it is there. Positions are fractions of the
 * wall rather than pixels, so a board arranged on a laptop is the same board
 * on a large screen and the published page is the arrangement, not a re-flow.
 *
 * Published like a sticker sheet, and commented on the same way: a press on a
 * picture opens its note and its thread, because the thing you are looking at
 * is where the conversation about it belongs.
 * ══════════════════════════════════════════════════════════════════════════ */
function vMoodboard(m) {
  const ui = mountPanel('moodboard', m)
  const project = () => currentProject()
  const post = (path, body, method = 'POST') =>
    fetch(path, { method, headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) })
      .then(responseJson)
  ...
  /*
   * This instance, or a ghost of one.
   *
   * reload() is asynchronous, and a second visit to this page builds a second
   * view while the first one's fetch is still in the air. That fetch then
   * appended its own rail beside the new one — and the stale rail is the one
   * with no Picture section, so a picture could be selected with nowhere to
   * remove it from. A view that has been navigated away from draws nothing.
   */
  /*
   * What can be seen, which is not what is saved.
   *
   ...
   * published page is what somebody arranged rather than where they were last
   * standing.
   */
  ...
  let gone = false
  window.addEventListener('rm:before-navigate', () => (gone = true), { once: true })
  ...
  let notes = []
  const nodeEls = []
  const make = { model: '', prompt: '', scale: 1, count: 1 }

  const dirty = { t: 0 }
  /* Saved on a beat, not on every pixel of a drag: a board is a hundred small
     moves and one file, and writing it on each of them is a hundred writes. */
  const save = () => {
    clearTimeout(dirty.t)
    dirty.t = setTimeout(async () => {
      if (!board) return
      const r = await post('/api/boards', { projectId: project(), board }, 'PUT')
      if (r.error) return say(r.error, 'bad')
      board.published = r.board.published ?? board.published
    }, 400)
  }

  const applyView = () => {
    ui.scene.style.transform = `translate(${view.x}px, ${view.y}px) scale(${view.z})`
    ui.zoomAt.textContent = `${Math.round(view.z * 100)}%`
  }
  /* Zoom about a point on screen, so whatever is under the pointer stays there. */
  const zoomAbout = (z, cx, cy) => {
    const box = ui.wall.getBoundingClientRect()
    const next = Math.min(MAX_Z, Math.max(MIN_Z, z))
  ...
  const fit = () => {
    ...
    /* The box everything sits in, in scene fractions, with a little air. */
    const x0 = Math.min(...ns.map((n) => n.x))
    const y0 = Math.min(...ns.map((n) => n.y))
    const x1 = Math.max(...ns.map((n) => n.x + n.w))
    const y1 = Math.max(...ns.map((n) => n.y + n.w * 0.9))
    const box = ui.wall.getBoundingClientRect()
    const z = Math.min(MAX_Z, Math.max(MIN_Z, 0.92 / Math.max(x1 - x0, (y1 - y0) * 1.02)))
    view.z = z
    view.x = (box.width - (x1 - x0) * box.width * z) / 2 - x0 * box.width * z
    view.y = (box.height - (y1 - y0) * box.height * z) / 2 - y0 * box.height * z
    applyView()
  }
  ui.zoomIn.onclick = () => zoomAbout(view.z * 1.25, ...centreOfWall())
  ui.zoomOut.onclick = () => zoomAbout(view.z / 1.25, ...centreOfWall())
  ui.zoomFit.onclick = () => fit()
  function centreOfWall() {
    const b = ui.wall.getBoundingClientRect()
    return [b.left + b.width / 2, b.top + b.height / 2]
  }
  /*
   * The wheel. A trackpad pinch arrives as a wheel with ctrl held, which is the
   * only way a page can tell the two apart; a plain wheel scrolls the board
   * rather than the page, because the board is the thing under the hand.
   */
  ui.wall.addEventListener(
    'wheel',
    (e) => {
      e.preventDefault()
      if (e.ctrlKey || e.metaKey) return zoomAbout(view.z * (1 - e.deltaY * 0.01), e.clientX, e.clientY)
      view.x -= e.deltaX
      view.y -= e.deltaY
      applyView()
    },
  ...
  /* Keys, for when the hands are already on them. */
  ui.wall.addEventListener('keydown', (e) => {
    if (e.target.closest?.('input, textarea, [contenteditable]')) return
    if (e.key === '=' || e.key === '+') return void zoomAbout(view.z * 1.25, ...centreOfWall())
    if (e.key === '-' || e.key === '_') return void zoomAbout(view.z / 1.25, ...centreOfWall())
  ...
  const paintWall = () => {
    if (gone) return
    ui.scene.querySelectorAll('.mood__node').forEach((n) => n.remove())
    nodeEls.length = 0
    ui.empty.hidden = Boolean(board?.nodes?.length)
    if (!board) return
    board.nodes.forEach((n, i) => {
      const { root, el } = mountRow('mood-node')
      root.style.left = `${n.x * 100}%`
      root.style.top = `${n.y * 100}%`
      root.style.width = `${n.w * 100}%`
      root.classList.toggle('mood__node--on', i === sel)
      el.art.src = mediaUrl(n.rel)
      el.title.textContent = n.title || ''
      el.note.textContent = n.note || ''
      const count = notes.filter((x) => x.item === n.rel.split('/').pop()).length
      el.count.textContent = count ? String(count) : ''
      /*
       * One drag, on the window. Pointer capture is the obvious tool and the
       * wrong one here for the reason the Showcase timeline gives: the node
       * under the hand is rebuilt when a drag ends, so the next press on a
       * fresh element captures nothing and only the first drag ever works.
       */
      const grab = (e, mode) => {
        e.preventDefault()
        e.stopPropagation()
        pick(i)
        const box = ui.wall.getBoundingClientRect()
        /* A pointer moves in screen pixels; a place is a fraction of the scene,
           and the scene is scaled. Without the divide, a zoomed-in drag runs
           away from the hand. */
        const start = { x: e.clientX, y: e.clientY, nx: n.x, ny: n.y, nw: n.w, w: box.width * view.z, h: box.height * view.z }
        const move = (ev) => {
          const dx = (ev.clientX - start.x) / start.w
          const dy = (ev.clientY - start.y) / start.h
          if (mode === 'move') {
            n.x = Math.min(0.98, Math.max(-0.2, start.nx + dx))
            n.y = Math.min(0.98, Math.max(-0.2, start.ny + dy))
            root.style.left = `${n.x * 100}%`
            root.style.top = `${n.y * 100}%`
          } else {
            n.w = Math.min(1, Math.max(0.04, start.nw + dx))
            root.style.width = `${n.w * 100}%`
          }
          save()
        }
      ...
      root.onpointerdown = (e) => grab(e, 'move')
      el.size.onpointerdown = (e) => grab(e, 'size')
      ui.scene.append(root)
      nodeEls.push(root)
    })
  }
  /* The picked picture, off the wall. The file stays in the project: a board is
     an arrangement of pictures, not the only place they live. */
  const removePicked = () => {
    if (!board || sel < 0) return
    board.nodes.splice(sel, 1)
    sel = -1
    paintWall()
    buildRail()
    save()
    status('Taken off the board. The picture is still in the project.', 'ok')
  }
  const pick = (i) => {
    sel = i
    for (const [k, el] of nodeEls.entries()) el.classList.toggle('mood__node--on', k === i)
    buildRail()
  }
  /*
   * A press on the wall itself pans it. Letting go without having moved is a
   * press rather than a drag, and a press on nothing is "nothing is picked".
   */
  ui.wall.onpointerdown = (e) => {
    if (e.target.closest('.mood__node') || e.target.closest('.mood__zoom')) return
    const start = { x: e.clientX, y: e.clientY, vx: view.x, vy: view.y }
    let moved = false
    ui.wall.classList.add('mood__wall--panning')
    const move = (ev) => {
    ...
    const end = () => {
      ...
      window.removeEventListener('pointerup', end)
      window.removeEventListener('pointercancel', end)
      ui.wall.classList.remove('mood__wall--panning')
      if (!moved) pick(-1)
    }
    window.addEventListener('pointermove', move)
    window.addEventListener('pointerup', end)
  ...
  /* Backspace and Delete, the keys anybody reaches for, while the wall has focus
  ...
  /* ── pictures in: pasted, dropped, or chosen ── */
  /*
   * A first picture makes the board it lands on.
   *
   * Pasting into a project with no board did nothing and said nothing, because
   * there was nothing to put it on — and "start a board first" is a rule nobody
   * should have to know before dropping a picture on a wall.
   */
  const ensureBoard = async () => {
    if (board) return true
    const r = await post('/api/boards', { projectId: project(), name: 'board' })
    if (r.error && !/already a board/.test(r.error)) {
      status(r.error, 'bad')
      return false
    }
    await reload(r.board?.name ?? 'board')
  ...
  const addFiles = async (list) => {
    ...
    status(`Adding ${files.length} picture${files.length === 1 ? '' : 's'}…`)
    let placed = 0
    for (const file of files) {
      const r = await fetch(`/api/boards/upload?project=${encodeURIComponent(project())}&name=${encodeURIComponent(file.name || 'pasted.png')}`, {
        method: 'POST',
        headers: { 'content-type': 'application/octet-stream' },
        body: file,
        duplex: 'half',
        ...
        .catch((e) => ({ error: e.message }))
      if (r.error) continue
      /* Somewhere sensible and not on top of the last one: a diagonal from the
         corner, wrapping, so a paste of twenty lands as twenty things you can
         see rather than one pile. */
      const k = board.nodes.length
      board.nodes.push({ rel: r.rel, title: (file.name || '').replace(/\.[^.]+$/, ''), note: '', x: 0.04 + ((k * 0.13) % 0.7), y: 0.06 + ((k * 0.09) % 0.6), w: 0.2 })
      placed += 1
    }
    status(placed ? `${placed} added.` : 'Nothing could be added.', placed ? 'ok' : 'bad')
    sel = board.nodes.length - 1
    paintWall()
    buildRail()
    save()
  }
  /*
   * Paste, anywhere on this page. Only while the board is showing, and never
   * over a field somebody is typing in — a name pasted into the title box is
   * a paste, not a picture.
   */
  const onPaste = (e) => {
    if (!ui.wall.isConnected || ui.wall.offsetParent === null) return
    if (e.target.closest?.('input, textarea, [contenteditable]')) return
    /*
     * Both ways a clipboard offers a picture. `files` is the usual one; a paste
     * out of some applications arrives only as an item you have to ask for, and
     * reading one of the two is how a paste silently does nothing.
     */
    ...
    void addFiles(files.map((f, i) => new File([f], `pasted-${stamp}${files.length > 1 ? `-${i + 1}` : ''}.${(f.type.split('/')[1] || 'png').replace('jpeg', 'jpg')}`, { type: f.type })))
  }
  document.addEventListener('paste', onPaste)
  window.addEventListener('rm:before-navigate', () => document.removeEventListener('paste', onPaste), { once: true })

  for (const ev of ['dragenter', 'dragover']) {
    ui.wall.addEventListener(ev, (e) => {
      e.preventDefault()
      ui.wall.classList.add('mood__wall--over')
    })
  }
  for (const ev of ['dragleave', 'drop']) ui.wall.addEventListener(ev, () => ui.wall.classList.remove('mood__wall--over'))
  ui.wall.addEventListener('drop', (e) => {
    e.preventDefault()
    void addFiles([...(e.dataTransfer?.files ?? [])])
  })
  ui.add.onclick = () => ui.files.click()
  ui.files.onchange = () => {
    const list = [...(ui.files.files ?? [])]
    ui.files.value = ''
    void addFiles(list)
  }

  /* Rows across the wall, biggest gaps closed. For when a board has been
     shuffled into a mess and the arrangement is not the point. */
  ui.tidy.onclick = () => {
    if (!board?.nodes.length) return
    const per = Math.ceil(Math.sqrt(board.nodes.length * 1.6))
    const w = Math.min(0.28, 0.92 / per)
    board.nodes.forEach((n, i) => {
      n.w = w
      n.x = 0.04 + (i % per) * ((0.92 - w) / Math.max(1, per - 1) + w * 0.02)
      n.y = 0.05 + Math.floor(i / per) * (w * 0.82 + 0.06)
    })
    paintWall()
    save()
  }

  /* ── the rail: the board, then whatever is picked ── */
  const panel = mountRow('designer-panel').root
  const sec = (t) => {
    const h = mountRow('designer-sec').root
    h.textContent = t
    panel.append(h)
  }
  const g = () => {
  ...
  const textRow = (box, label, get, set, placeholder = '') => {
    const { nodes, el } = mountRow('designer-text')
  ...
  const areaRow = (box, label, get, set, placeholder = '') => {
    const { nodes, el } = mountRow('designer-textarea')
    el.label.textContent = label
    el.input.value = get() ?? ''
    el.input.placeholder = placeholder
    el.input.oninput = () => {
  ...
  const newBoardButton = () => {
    const fresh = control('button', { className: 'btn ghost', textContent: 'New board', type: 'button' })
    fresh.onclick = async () => {
      const name = await askName('A name for the board', '', { lede: 'Short, and its own — it becomes part of the address when you publish.', confirm: 'Make it' })
      if (!name) return
      const r = await post('/api/boards', { projectId: project(), name })
      if (r.error) return say(r.error, 'bad')
      await reload(r.board.name)
    }
    return fresh
  }
  ...
  const buildRail = () => {
    ...
    panel.replaceChildren()
    /*
     * With no board the rail was empty, so the one thing a person needed — a
     * way to start one — was the one thing that was not there.
     */
    if (!board) {
      sec('Boards')
      const only = g()
      only.append(control('hint', { textContent: 'No board yet. Start one, or paste a picture and one will be made for you.' }))
      only.append(newBoardButton())
      intoRail(panel)
      return
    }
    sec('Board')
    const b = g()
    textRow(b, 'Title', () => board.title, (v) => (board.title = v), board.name)
    areaRow(b, 'What it is for', () => board.note, (v) => (board.note = v), 'A line for whoever opens the link')
    const n = board.nodes[sel]
    if (n) {
      sec(`Picture ${sel + 1} of ${board.nodes.length}`)
      const pn = g()
      textRow(pn, 'Name', () => n.title, (v) => {
        n.title = v
        nodeEls[sel]?.querySelector('[data-el="title"]')?.replaceChildren(v)
      })
      areaRow(pn, 'Note', () => n.note, (v) => {
        n.note = v
        const el = nodeEls[sel]?.querySelector('[data-el="note"]')
        if (el) el.textContent = v
      })
      const acts = g()
      /* Plainly, not only inside a menu: taking a picture off a wall is the
         commonest thing anybody does to one. */
      const drop = control('button', { className: 'btn ghost del', type: 'button' })
      drop.append(icon('delete-02'), ' Remove')
      drop.onclick = () => removePicked()
      acts.append(drop)
      const front = control('button', { className: 'btn ghost ', textContent: 'Bring to front', type: 'button' })
      front.onclick = () => {
        board.nodes.push(board.nodes.splice(sel, 1)[0])
        sel = board.nodes.length - 1
        paintWall()
        buildRail()
        save()
      ...
      acts.append(front)
      /* What people said about this one, in the Studio rather than only on the
         page: a note nobody in the room can see is a note nobody acts on. */
      const mine = notes.filter((x) => x.item === n.rel.split('/').pop())
      if (hasComments) {
        sec(mine.length ? `Notes · ${mine.length}` : 'Notes')
        const th = g()
        const list = control('full', { className: 'mood__thread' })
        if (!mine.length) list.append(control('hint', { textContent: 'Nothing said about this one yet.' }))
        for (const c of mine) {
          const { root, el } = mountRow('mood-note')
          el.who.textContent = c.author || 'Someone'
          el.on.textContent = ''
          el.when.textContent = new Date(c.created_at).toLocaleString()
    ...
    /*
     * The look this wall teaches.
     *
     * A board is a dozen pictures somebody chose because they share something.
     * A LoRA is the smallest way to hand that something to a model, and the
     * point of collecting them in the first place.
     */
    sec(board.lora ? 'Its look · trained' : 'Train a look')
    const lo = g()
    if (!hasKey) lo.append(control('hint', { textContent: 'There is no fal key yet. Add one under Restyle and come back.' }))
    else if (board.training) {
      lo.append(control('hint', { textContent: `Training on ${board.nodes.length} pictures. This takes ten to thirty minutes; you can close the Studio and it carries on.` }))
      const stop = control('button', { className: 'btn ghost ', textContent: 'Check now', type: 'button' })
      stop.onclick = () => void checkTraining(true)
      lo.append(stop)
    } else {
      const pick = mountRow('designer-select')
      pick.el.label.textContent = 'Trainer'
      for (const t of trainers) pick.el.input.append(new Option(t.label, t.id))
      make.model ||= trainers[0]?.id ?? ''
      pick.el.input.value = make.model
      pick.el.input.onchange = () => (make.model = pick.el.input.value)
      lo.append(...pick.nodes)
      /* A board is pasted screenshots; trained as they are, the weights learn
         the crop rather than the drawing. On unless somebody says otherwise. */
      const tidy = mountRow('small-switch')
      tidy.el.label.textContent = 'Tidy the pictures first'
      tidy.el.input.id = 'mood-tidy'
      tidy.el.label.htmlFor = 'mood-tidy'
      tidy.el.input.checked = make.tidy !== false
      tidy.el.input.onchange = () => (make.tidy = tidy.el.input.checked)
      lo.append(...tidy.nodes)
      lo.append(control('hint', { textContent: 'Each picture is enlarged and set alone on the same white square. Adds about ten seconds each, and is remembered.' }))
      const go_ = control('button', { className: 'btn ghost ', type: 'button' })
      go_.append(icon('magic-wand-01'), board.lora ? ' Train it again' : ' Train a look')
      go_.disabled = (board.nodes?.length ?? 0) < 5
      go_.title = go_.disabled ? 'A look needs at least five pictures' : 'Teach a model what this wall has in common'
      go_.onclick = async () => {
        go_.disabled = true
        status(make.tidy === false ? 'Zipping the wall and handing it to fal…' : 'Tidying each picture, then handing the wall to fal…')
        const r = await post('/api/boards/train', { projectId: project(), name: board.name, model: make.model, tidy: make.tidy !== false })
        go_.disabled = false
        if (r.error) return status(r.error, 'bad')
        board.training = r.training
        /* Vectors are drawn on the way in; saying so explains the pause. */
        status(`Training on ${r.pictures} pictures${r.tidied ? ', tidied first' : ''}${r.drawn ? `, ${r.drawn} drawn from vectors` : ''}. Ten to thirty minutes.`, 'ok')
        buildRail()
        watchTraining()
      }
      lo.append(go_)
    }
    if (board.trainingError) lo.append(control('hint', { className: 'hint bad', textContent: board.trainingError }))
    if (board.lora) {
      sec('Make one in this look')
      const mk = g()
      mk.append(control('hint', { textContent: `Trained ${new Date(board.lora.at).toLocaleDateString()} · the word it answers to is ${board.lora.trigger}.` }))
      const wantRow = areaRow(mk, 'What to make', () => make.prompt, (v) => (make.prompt = v), 'a hero image of a deck at dusk')
      promptLibrary(mk, {
        where: 'board',
        get: () => make.prompt,
        set: (v) => {
          make.prompt = v
          wantRow.input.value = v
        },
      })
      const strength = mountRow('designer-range')
      strength.el.label.textContent = 'How strongly'
      Object.assign(strength.el.input, { min: 0, max: 2, step: 0.05, value: make.scale })
      strength.el.readout.textContent = String(make.scale)
      strength.el.input.oninput = () => {
        make.scale = Number(strength.el.input.value)
        strength.el.readout.textContent = String(make.scale)
      }
      mk.append(...strength.nodes)
      const draw = control('button', { className: 'btn ', type: 'button' })
      draw.append(icon('magic-wand-01'), ' Make one')
      draw.onclick = async () => {
        draw.disabled = true
        status('Drawing…')
        const r = await post('/api/boards/generate', { projectId: project(), name: board.name, prompt: make.prompt, scale: make.scale })
        draw.disabled = false
        if (r.error) return status(r.error, 'bad')
        status('Made, and put on the wall.', 'ok')
        await reload(board.name)
      }
      mk.append(draw)
    }
    sec('Boards')
    const bs = g()
    bs.append(newBoardButton())
    if (board) {
      bs.append(
        actionMenu([{
          icon: 'delete-02',
          ...
          danger: true,
          run: async () => {
            const r = await post('/api/boards', { projectId: project(), name: board.name }, 'DELETE')
            if (r.error) return r.error
            await reload(null)
            return null
          },
  ...
  /* ── the boards in this project ── */
  const paintBoards = () => {
    ui.boards.replaceChildren()
    ui.boardCount.textContent = boards.length ? `· ${boards.length}` : ''
    ui.boardsEmpty.hidden = boards.length > 0
    for (const b of boards) {
      const { root, el } = mountRow('mood-board')
      el.name.textContent = b.title || b.name
      el.facts.textContent = [`${b.nodes?.length ?? 0} item${(b.nodes?.length ?? 0) === 1 ? '' : 's'}`, b.published ? 'published' : null, b.notes ? `${b.notes} note${b.notes === 1 ? '' : 's'}` : null].filter(Boolean).join(' · ')
      root.classList.toggle('mood__card--on', board?.name === b.name)
      root.onclick = () => void reload(b.name)
      ui.boards.append(root)
    }
  }

  ui.publish.onclick = async () => {
    if (!board) return say('Start a board first.', 'warn')
    const remote = remotes[0]
    if (!remote) return say('Set a public bucket on a storage remote first, under Storage.', 'warn')
    ui.publish.disabled = true
    status('Copying the pictures and putting the page up…')
    const r = await post('/api/boards/publish', { projectId: project(), name: board.name, remote })
    ...
    status('')
    board.published = r.board.published
    paintFooter()
    await copyQuick(r.url)
    say('Board published. Link copied.')
    await reload(board.name)
  }
  const paintFooter = () => {
    ui.copyLink.hidden = !board?.published?.url
    if (board?.published?.url) copyButton(ui.copyLink, 'Copy link', board.published.url)
    ui.publish.lastChild.textContent = board?.published ? 'Publish again' : 'Publish board'
  }

  /*
   * How the training is going, asked of the server rather than remembered here:
   * the ticket lives on the board, so a reloaded page — or a different one —
   * picks up a run somebody else started.
   */
  const checkTraining = async (loud) => {
    if (!board?.training) return
    const r = await fetch(`/api/boards/train?project=${encodeURIComponent(project())}&board=${encodeURIComponent(board.name)}`)
      .then(responseJson)
      .catch(() => null)
    if (!r || r.error) return
    if (r.state === 'failed') {
      board.training = null
      board.trainingError = r.error
      status(r.error, 'bad')
      buildRail()
      return
    }
    if (r.lora && !r.training) {
      board.training = null
      board.lora = r.lora
      clearInterval(watching)
      watching = 0
      status('The look is trained. Make one in it below.', 'ok')
      say('The board\'s look is trained.')
      buildRail()
      return
    }
    if (loud) status(`Still ${r.state ?? 'going'}${r.position != null ? `, ${r.position} ahead in the queue` : ''}.`)
  }
  const watchTraining = () => {
    clearInterval(watching)
    /* Half a minute: a run is half an hour, and a tighter poll only makes noise. */
    watching = setInterval(() => void checkTraining(false), 30000)
    window.addEventListener('rm:before-navigate', () => clearInterval(watching), { once: true })
  }

  const reload = async (want) => {
    if (!project()) {
      ui.noProject.hidden = false
      return
    }
    ui.noProject.hidden = true
    const d = await fetch(`/api/boards?project=${encodeURIComponent(project())}`)
      .then(responseJson)
      .catch((e) => ({ error: e.message }))
    if (d.error) return say(d.error, 'bad')
    boards = d.boards ?? []
    remotes = d.remotes ?? []
    hasComments = Boolean(d.comments)
    trainers = d.trainers ?? []
    hasKey = Boolean(d.hasKey)
    const name = want ?? board?.name ?? boards[0]?.name ?? null
    board = boards.find((b) => b.name === name) ?? null
    if (board) {
      board.nodes ??= []
      const r = await fetch(`/api/boards/notes?project=${encodeURIComponent(project())}&board=${encodeURIComponent(board.name)}`)
        .then(responseJson)
        .catch(() => ({ notes: [] }))
      notes = r.notes ?? []
    } else notes = []
    ...
    paintFooter()
    buildRail()
    if (board?.training) watchTraining()
  }
  void reload(null)
}

function vStickers(m) {
  const ui = mountPanel('stickers', m)
  const say = (text, kind) => toast(text, kind)
  const post = (path, body) =>
    fetch(path, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) })
      .then(responseJson)
      .catch((e) => ({ error: e.message }))
  const project = () => currentProject()
  const mediaUrl = (rel, bust = '') => `/media/${encodeURIComponent(project())}/${encodeURI(rel)}${bust ? `?v=${bust}` : ''}`

  let data = { files: [], sheets: [], models: { sticker: [], cutout: [], vectorize: [] }, hasKey: false }
  let work = null // { rel, url, name, kind } — the picture the steps act on
  const ticked = new Set()
  /* 76.2mm is three inches, which is what a sticker shop's default die is. */
  /* Filled from the vendor tables the first time the rail is drawn. */
  let sheetSizeList = []
  const settings = { sticker: 'telegram', cutout: '', vectorize: '', endpoint: '', prompt: '', sheetName: 'stickers', columns: 4, size: 300, gap: 40, remote: '', printSizeMm: 76.2, printBleedMm: 0.35, printLayout: 'sheet', printPage: '4x6', dieOffsetMm: 0, dieRoundMm: 0, quoteQty: 250, quoteProduct: 'die-cut', quotePublish: true }
  let affinityWatch = 0
  let makeBtn = null
  ...
  const busy = (on, label) => {
    for (const b of [makeBtn, ui.cut, ui.trace, ui.affinity, ui.build, ui.publish, ui.reveal]) if (b) b.disabled = on
  ...
  const setWork = (next) => {
    work = next
    ui.previewEmpty.hidden = Boolean(work)
    ui.previewImg.hidden = !work
    ui.clearWork.hidden = !work
    if (work) {
      ui.previewImg.src = work.url
      ui.workName.textContent = work.name
      ui.workNote.textContent = work.kind === 'vector' ? 'vector · ready for Affinity and the sheet' : work.kind === 'library' ? 'from the Style library' : work.kind === 'still' ? 'a project still' : 'raster'
    } else {
      ui.workName.textContent = ''
      ui.workNote.textContent = ''
    }
    /* Any file of the project's opens in Affinity; a library picture has to be
       brought in first (run a step on it, or upload it). SVG round-trips best. */
    ui.affinity.disabled = ticked.size >= 2 ? false : !work?.rel
    ui.affinity.title = !work ? 'Pick a sticker first' : !work.rel ? 'Run a step on this picture first — Affinity opens files in the project' : 'Open in Affinity; export an SVG or PNG into this project’s Stickers folder to bring the edit back'
    for (const c of ui.stickers.children) c.classList.toggle('stickers__card--on', c.dataset.rel === work?.rel)
    for (const c of ui.sources.children) c.classList.toggle('stickers__card--on', c.dataset.key === work?.key)
    if (makeBtn) buildRail()
  }

  /* Put the picture down. The steps that need one say so again, and the rail's
     reference line goes back to asking for one. */
  ui.clearWork.onclick = () => {
    setWork(null)
    buildRail()
  }
  ui.reveal.onclick = async () => {
    const r = await post('/api/stickers/reveal', { projectId: project() })
    if (r.error) return say(r.error, 'bad')
    say(`Opened ${r.dir} in Finder.`)
  }

  /* ── the project's stickers ── */
  /*
   * Take a card out of the grid with the others sliding into its place. The
   * card shrinks away; the rest are measured before and after, shifted back to
   * where they were, and let go, so each glides into the gap.
   */
  const leaveGrid = (card) => {
    const others = [...ui.stickers.children].filter((c) => c !== card)
    const before = new Map(others.map((c) => [c, c.getBoundingClientRect()]))
    card.classList.add('stickers__card--going')
    return new Promise((done) => {
      setTimeout(() => {
        card.remove()
        ...
        requestAnimationFrame(() => {
          for (const c of others) {
            c.style.transition = 'transform 260ms cubic-bezier(0.2, 0, 0, 1)'
            c.style.transform = ''
  ...
  const paintStickers = () => {
    ui.stickers.replaceChildren()
    ui.stickerCount.textContent = data.files.length ? `· ${data.files.length}` : ''
    ui.stickersEmpty.hidden = data.files.length > 0
    for (const f of data.files) {
      const { root, el } = mountRow('stickers-card')
      root.dataset.rel = f.rel
      root.classList.toggle('stickers__card--on', work?.rel === f.rel)
      el.pick.style.setProperty('--pic', `url('${mediaUrl(f.rel, f.mtime)}')`)
      el.name.textContent = f.name
      el.kind.textContent = f.kind === 'vector' ? 'SVG' : 'PNG'
      el.tick.checked = ticked.has(f.rel)
      el.tick.onchange = () => {
        if (el.tick.checked) ticked.add(f.rel)
        else ticked.delete(f.rel)
        paintTicked()
      }
      el.pick.onclick = () => setWork({ rel: f.rel, url: mediaUrl(f.rel, f.mtime), name: f.name, kind: f.kind, key: f.rel })
      /* No dialog: the button asks, then does it if pressed again within a moment. */
      let sure = 0
      const paintRemoveIcon = (name) => el.remove.replaceChildren(icon(name))
      paintRemoveIcon('delete-02')
      el.remove.onclick = async () => {
        if (!sure) {
          sure = setTimeout(() => {
            sure = 0
            el.remove.classList.remove('stickers__remove--sure')
            paintRemoveIcon('delete-02')
          }, 3000)
          el.remove.classList.add('stickers__remove--sure')
          paintRemoveIcon('tick-02')
          return
        }
        clearTimeout(sure)
        const r = await fetch('/api/stickers/file', { method: 'DELETE', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ projectId: project(), rel: f.rel }) })
          .then(responseJson)
          .catch((e) => ({ error: e.message }))
        if (r.error) return say(r.error, 'bad')
        /*
         * The card goes, nothing else moves. Redrawing the grid emptied the
         * page for a frame and the scroll position went with it; the list
         * behind it is trimmed to match, and the sheets refresh on their own.
         */
        ticked.delete(f.rel)
        data.files = data.files.filter((x) => x.rel !== f.rel)
        await leaveGrid(root)
        ui.stickerCount.textContent = data.files.length ? `· ${data.files.length}` : ''
        ui.stickersEmpty.hidden = data.files.length > 0
        paintTicked()
        if (work?.rel === f.rel) setWork(null)
        say(`Removed ${f.name}.`)
        void reload()
      }
      ui.stickers.append(root)
    }
    paintTicked()
  }
  /*
   * Tick all, or none. It flips to Untick all once everything is ticked, rather
   * than sitting there having apparently done nothing on a second press.
   */
  ui.tickAll.onclick = () => {
    const allOn = data.files.length > 0 && data.files.every((f) => ticked.has(f.rel))
    for (const f of data.files) {
      if (allOn) ticked.delete(f.rel)
      else ticked.add(f.rel)
    }
    for (const c of ui.stickers.children) {
      const box = c.querySelector('[data-el="tick"]') ?? c.querySelector('input[type="checkbox"]')
      if (box) box.checked = ticked.has(c.dataset.rel)
    }
    paintTicked()
  }
  /* What the buttons say follows the ticks. */
  const paintTicked = () => {
    const allOn = data.files.length > 0 && data.files.every((f) => ticked.has(f.rel))
    ui.tickAll.textContent = allOn ? 'Untick all' : `Tick all${data.files.length ? ` ${data.files.length}` : ''}`
    /*
     * The buttons say how many they will act on.
     *
     * Opening a sheet ticks everything on it, which is right — Publish means
     * "this sheet again" — but it made Print PDF print thirty-one when
     * somebody thought they had picked one, with nothing on screen to say so.
     * A count on the button is the whole fix.
     */
    /*
     * And which of the two it will make.
     *
     * The branded sheet only appears in the sheet layout, so somebody who
     * pressed Print PDF on the default got one sticker a page on white and
     * reported the template as missing. The button now names what it makes.
     */
    previewSoon()
    if (ui.printPdf?.lastChild) {
      const shape = settings.printLayout === 'sheet' ? 'sheet' : 'one a page'
      ui.printPdf.lastChild.textContent = ` Print PDF · ${shape}${ticked.size ? ` · ${ticked.size}` : ''}`
    }
    ui.tickAll.disabled = !data.files.length
    ui.build.disabled = ticked.size === 0
    ui.affinity.lastChild.textContent = ticked.size >= 2 ? `Open ${ticked.size} in Affinity` : 'Open in Affinity'
    ui.affinity.disabled = ticked.size >= 2 ? false : !work?.rel
    ui.publish.lastChild.textContent = ticked.size ? `Publish ${ticked.size} as a sheet` : 'Publish sheet'
  }
  const paintSheets = () => {
    ui.sheets.replaceChildren()
    ui.sheetCount.textContent = data.sheets.length ? `· ${data.sheets.length}` : ''
    ui.sheetsEmpty.hidden = data.sheets.length > 0
    for (const sh of data.sheets) {
      const { root, el } = mountRow('stickers-sheet')
      el.pick.style.setProperty('--pic', `url('${mediaUrl(sh.rel, sh.madeAt)}')`)
      el.name.textContent = sh.name
      el.note.textContent = `${sh.items.length} sticker${sh.items.length === 1 ? '' : 's'} · ${sh.columns} across${sh.published ? ' · published' : ''}`
      el.link.hidden = !sh.published?.url
      if (sh.published?.url) el.link.href = sh.published.url
      el.copy.hidden = !sh.published?.url
      if (sh.published?.url) copyButton(el.copy, 'Copy link', sh.published.url)
      el.svg.href = mediaUrl(sh.rel, sh.madeAt)
      el.svg.download = `${sh.name}.svg`
      el.zip.href = `/api/stickers/zip?project=${encodeURIComponent(project())}&name=${encodeURIComponent(sh.name)}`
      el.pick.onclick = () => {
        settings.sheetName = sh.name
        ticked.clear()
        for (const rel of sh.items) ticked.add(rel)
        buildRail()
        paintStickers()
        window.open(mediaUrl(sh.rel, sh.madeAt), '_blank', 'noopener')
      }
      /* The whole sheet, every sticker in one file, opened in Affinity. Save
         there and Build sheet is not needed again: the sheet file is the sheet. */
      el.affinity.onclick = async () => {
        const r = await post('/api/stickers/affinity', { projectId: project(), rel: sh.rel })
        if (r.error) return say(r.error, 'bad')
        say(`Opened the sheet in ${r.app}. Export it as SVG over ${sh.rel.split('/').pop()} to change what publishes.`)
      }
      ui.sheets.append(root)
    }
  }

  /* ── pictures to start from: project stills, and the Style library ── */
  const paintSources = async () => {
    ui.sources.replaceChildren()
    const id = project()
    if (!id) return
    const [media, style] = await Promise.all([
      fetch(`/api/project/media?project=${encodeURIComponent(id)}`)
        .then(responseJson)
        .catch(() => ({})),
      fetch('/api/style/state')
        .then(responseJson)
        .catch(() => ({})),
    ])
    const stills = (media.catalog?.files ?? []).filter((f) => f.kind === 'still' && !f.rel.startsWith('Stickers/'))
    const library = (style.images ?? []).filter((x) => /^https?:\/\//.test(x.url ?? ''))
    ui.sourceCount.textContent = `· ${stills.length + library.length}`
    for (const f of stills) {
      const { root, el } = mountRow('stickers-source')
      root.dataset.key = `still:${f.rel}`
      el.art.style.setProperty('--pic', `url('/thumb/${encodeURIComponent(id)}/${encodeURI(f.rel)}')`)
      el.name.textContent = f.rel
      el.kind.textContent = 'STILL'
      root.onclick = () => setWork({ rel: f.rel, url: mediaUrl(f.rel), name: f.rel.split('/').pop(), kind: 'still', key: `still:${f.rel}` })
      ui.sources.append(root)
    }
    for (const x of library) {
      const { root, el } = mountRow('stickers-source')
      root.dataset.key = `lib:${x.id}`
      const src = x.file ? `/style/${encodeURIComponent(x.file)}` : x.url
      el.art.style.setProperty('--pic', `url('${src}')`)
      el.name.textContent = x.subject || 'library picture'
      el.kind.textContent = 'LIBRARY'
      root.onclick = () => setWork({ rel: null, imageUrl: x.url, url: src, name: x.subject || 'library picture', kind: 'library', key: `lib:${x.id}` })
      ui.sources.append(root)
    }
  }
  ...
  const selectRow = (box, label, options, key, onChange) => {
    const { nodes, el: row } = mountRow('designer-select')
    row.label.textContent = label
    for (const o of options) row.input.append(new Option(o.label, o.id))
    if (!options.some((o) => o.id === settings[key])) settings[key] = options[0]?.id ?? ''
    row.input.value = settings[key]
    row.input.onchange = () => {
      settings[key] = row.input.value
      onChange?.()
    }
    box.append(...nodes)
    return row
  }
  const textRow = (box, label, key, placeholder = '') => {
    const { nodes, el: row } = mountRow('designer-text')
    row.label.textContent = label
    row.input.value = settings[key]
    row.input.placeholder = placeholder
    row.input.oninput = () => (settings[key] = row.input.value)
    box.append(...nodes)
    return row
  }
  /* A prompt is a sentence or three, not a line: a single-line box hides most
     of what somebody wrote the moment it is longer than the rail is wide. */
  const areaRow = (box, label, key, placeholder = '') => {
    const { nodes, el: row } = mountRow('designer-textarea')
    row.label.textContent = label
    row.input.value = settings[key]
    row.input.placeholder = placeholder
    row.input.oninput = () => (settings[key] = row.input.value)
  ...
  const rangeRow = (box, label, key, min, max, step, onChange) => {
    const { nodes, el: row } = mountRow('designer-range')
    row.label.textContent = label
    const i = Object.assign(row.input, { min, max, step, value: settings[key] })
    row.readout.textContent = String(settings[key])
    i.oninput = () => {
  ...
  const buildRail = () => {
    ...
    sec('Make')
    const mk = g()
    selectRow(mk, 'Sticker style', data.models.sticker, 'sticker')
    const promptRow = areaRow(mk, 'Prompt', 'prompt', 'what the sticker shows')
    promptLibrary(mk, {
      where: 'sticker',
      get: () => settings.prompt,
      set: (v) => {
        settings.prompt = v
        promptRow.input.value = v
      },
    })
    /*
     * The reference: the picked picture, when there is one. Paste a picture
     * anywhere on this page and it lands here. The "From reference" styles
     * need it; the others use it when it is there.
     */
    const refHint = control('hint')
    refHint.textContent = work ? `Reference: ${work.name}. Styles marked "From reference" keep this subject.` : 'No reference. Paste a picture (⌘V), drop one below, or pick one, and "From reference" styles will work from it.'
    mk.append(refHint)
    /* The button sits with its dials: style, prompt, then Make. */
    makeBtn = control('button', { className: 'btn btn--primary', type: 'button' })
    makeBtn.append(icon('magic-wand-01'), work ? ' Make sticker from the picture' : ' Make sticker')
    /* Rebuilt mid-run when the result lands: stays off until the run is over. */
    makeBtn.disabled = ui.cut.disabled
    makeBtn.onclick = () => {
      const m_ = data.models.sticker.find((x) => x.id === settings.sticker)
      if (m_?.ref && !work) return say('That style works from a picture. Paste or pick one first.', 'warn')
      run('generate', makeBtn, work ? 'Making a sticker from the picture…' : 'Making a sticker from the prompt…')
    }
    mk.append(makeBtn)
    sec('Cut out')
    selectRow(g(), 'Model', data.models.cutout, 'cutout')
    sec('Vectorize')
    const vz = g()
    const showCustom = () => {
      const on = settings.vectorize === 'custom'
      customRow.label.hidden = !on
      customRow.input.hidden = !on
    }
    selectRow(vz, 'Model', data.models.vectorize, 'vectorize', showCustom)
    const customRow = textRow(vz, 'Custom endpoint', 'endpoint', 'fal-ai/…')
    showCustom()
    sec('Sheet')
    const sh = g()
    textRow(sh, 'Sheet name', 'sheetName', 'stickers')
    rangeRow(sh, 'Across', 'columns', 1, 12, 1)
    rangeRow(sh, 'Sticker size', 'size', 100, 800, 10)
    rangeRow(sh, 'Gap', 'gap', 0, 200, 5)
    sec('Print')
    const pr = g()
    pr.append(control('hint', { textContent: 'Print PDF, in the footer, writes one sticker per page in CMYK — what a print shop is sent.' }))
    /* Millimetres: every shop that cuts stickers quotes in them, and the long
       side is the one a die-cutter is set from. */
    /* Repaint on change: a sheet needs paper and a cell size, singles need
       neither, and showing both is showing a control that does nothing. */
    selectRow(pr, 'Layout', [{ id: 'sheet', label: 'Branded sheet with cut lines' }, { id: 'singles', label: 'One sticker a page (die-cut)' }], 'printLayout', () => {
      buildRail()
      paintTicked()
      previewSoon()
    })
    if (settings.printLayout === 'sheet') {
      /* The sizes shops sell, not the sizes paper comes in: A4 is paper, and
         no shop here prints a sticker sheet on it. */
      const paper = sheetSizeList.length ? sheetSizeList.map((z) => ({ id: z.id, label: `${z.label}${z.vendors.length ? ` — ${z.vendors.join(', ')}` : ''}` })) : [{ id: '4x6', label: '4" × 6"' }]
      selectRow(pr, 'Sheet size', paper, 'printPage', previewSoon)
      /*
       * There is no sticker size to choose.
       *
       * A sheet is priced by the sheet, so they have to fit on one — which
       * decides the size on its own. A control that can only be set to the one
       * value that works is a control that wastes somebody's attention.
       */
      pr.append(control('hint', { textContent: sheetFitAt ? `Sized to ${sheetFitAt}mm so they all fit on one sheet.` : 'Sized so they all fit on one sheet.' }))
      /* Zero sits the cut on the artwork's own keyline, which is what these
         stickers are drawn with. */
      rangeRow(pr, 'Cut outside (mm)', 'dieOffsetMm', 0, 5, 0.25, previewSoon)
      rangeRow(pr, 'Cut corner (mm)', 'dieRoundMm', 0, 8, 0.25, previewSoon)
    } else rangeRow(pr, 'Longest side (mm)', 'printSizeMm', 10, 300, 1)
    /* How far the ink runs past the cut on every sticker, so a die that drifts
       outward still lands on colour. Not the sheet's own trim bleed, which is
       fixed at the template's eighth of an inch. */
    rangeRow(pr, 'Sticker bleed (mm)', 'printBleedMm', 0, 3, 0.05, previewSoon)
    rangeRow(pr, 'How many', 'quoteQty', 50, 5000, 50)
    /* Whoever decides to order is usually not whoever ran the comparison. */
    const pub = mountRow('small-switch')
    pub.el.label.textContent = 'Publish the prices too'
    pub.el.input.id = 'sticker-quote-publish'
    pub.el.label.htmlFor = 'sticker-quote-publish'
    pub.el.input.checked = settings.quotePublish !== false
    pub.el.input.onchange = () => (settings.quotePublish = pub.el.input.checked)
    pr.append(...pub.nodes)
    sec('Publish')
    const pb = g()
    const remotes = (S?.remotes ?? []).map((r) => ({ id: r, label: r }))
    if (remotes.length) selectRow(pb, 'Storage', remotes, 'remote')
    else pb.append(control('hint', { textContent: 'No shared storage yet. Set one up under Storage, with a public bucket and base URL.' }))
    intoRail(panel)
  }

  /* ── the steps ── */
  const loadSheetSizes = async () => {
    const r = await responseJson(await fetch('/api/stickers/sheet-sizes')).catch(() => null)
    if (r?.sizes?.length) {
      sheetSizeList = r.sizes
      if (settings.printLayout === 'sheet') buildRail()
    }
  }
  loadSheetSizes()
  ...
  const reload = async () => {
    const id = project()
    if (!id) return
    const r = await fetch(`/api/stickers?project=${encodeURIComponent(id)}`)
      .then(responseJson)
      .catch((e) => ({ error: e.message }))
    if (r.error) return say(r.error, 'bad')
    /*
     * Repaint only what changed. The folder watch calls this every time the
     * folder's hash moves, including for a delete this page just made and
     * already drew; redrawing the grid then threw the scroll away.
     */
    const sig = (d) => JSON.stringify({ f: d.files.map((f) => [f.rel, f.mtime]), s: d.sheets.map((x) => [x.name, x.madeAt, x.published?.at]) })
    const was = sig(data)
    data = r
    ui.keyNote.hidden = data.hasKey
    const now = sig(data)
    if (was === now) return
    if (JSON.parse(was).f.length !== data.files.length || JSON.stringify(JSON.parse(was).f) !== JSON.stringify(JSON.parse(now).f)) paintStickers()
    paintSheets()
  }
  const run = async (step, button, label) => {
    if (!project()) return say('Pick a project first.', 'warn')
    if (step !== 'generate' && !work) return say('Pick a picture first.', 'warn')
    busy(true, label)
    const body = { projectId: project(), step, name: work?.name?.replace(/\.[^.]+$/, '') || settings.prompt.slice(0, 40) || 'sticker' }
    if (work?.rel) body.rel = work.rel
    else if (work?.imageUrl) body.imageUrl = work.imageUrl
    if (step === 'generate') Object.assign(body, { model: settings.sticker, prompt: settings.prompt })
    if (step === 'cutout') body.model = settings.cutout
    if (step === 'vectorize') Object.assign(body, { model: settings.vectorize, endpoint: settings.endpoint, prompt: settings.prompt })
    const r = await post('/api/stickers/run', body)
    busy(false)
    if (r.error) {
      status(r.error, 'bad')
      return say(r.error, 'bad')
    }
    status(`Done: ${r.rel.split('/').pop()}`, 'ok')
    await reload()
    const f = data.files.find((x) => x.rel === r.rel)
    if (f) setWork({ rel: f.rel, url: mediaUrl(f.rel, f.mtime), name: f.name, kind: f.kind, key: f.rel })
  }
  ui.cut.onclick = () => run('cutout', ui.cut, 'Cutting the background out…')
  ui.trace.onclick = () => run('vectorize', ui.trace, 'Tracing to vector…')

  /*
   * Many pictures at once, dropped or chosen. Uploaded one after another so
   * the count reads truthfully; the last one becomes the picture being worked
   * on, the rest wait in the grid, ticked for a sheet.
   */
  const uploadMany = async (list) => {
    ...
    let failed = 0
    for (const [i, file] of files.entries()) {
      ui.dropStatus.hidden = false
      ui.dropStatus.textContent = `Uploading ${i + 1} of ${files.length}: ${file.name}`
      const r = await fetch(`/api/stickers/upload?project=${encodeURIComponent(project())}&name=${encodeURIComponent(file.name)}`, { method: 'POST', headers: { 'content-type': 'application/octet-stream' }, body: file, duplex: 'half' })
        .then(responseJson)
        .catch((e) => ({ error: e.message }))
      if (r.error) failed += 1
      else {
    ...
    busy(false)
    ui.dropStatus.textContent = failed ? `${files.length - failed} added, ${failed} failed.` : `${files.length} added.`
    tone(ui.dropStatus, failed ? 'warn' : 'ok')
    await reload()
    const f = data.files.find((x) => x.rel === last)
    if (f) setWork({ rel: f.rel, url: mediaUrl(f.rel, f.mtime), name: f.name, kind: f.kind, key: f.rel })
  }
  /*
   * Paste a picture. From a screenshot, a browser, Figma — whatever is on
   * the clipboard lands in the project's Stickers folder and becomes the
   * reference. Only while this page is showing, and never over a text field.
   */
  const onPaste = (e) => {
    if (!ui.drop.isConnected || ui.drop.offsetParent === null) return
    if (e.target.closest?.('input, textarea, [contenteditable]')) return
    const files = [...(e.clipboardData?.files ?? [])].filter((f) => /^image\//.test(f.type))
    if (!files.length) return
    e.preventDefault()
    const stamp = new Date().toISOString().slice(0, 19).replace(/[T:]/g, '-')
    void uploadMany(files.map((f, i) => new File([f], `pasted-${stamp}${files.length > 1 ? `-${i + 1}` : ''}.${(f.type.split('/')[1] || 'png').replace('jpeg', 'jpg')}`, { type: f.type })))
  }
  document.addEventListener('paste', onPaste)
  ui.drop.onclick = () => ui.files.click()
  ui.drop.onkeydown = (e) => {
  ...
  ui.files.onchange = () => {
    /* Copied before the input is cleared: the FileList is live, and clearing
       the input empties it under the upload. */
    const list = [...ui.files.files]
    ui.files.value = ''
  ...
  for (const ev of ['dragenter', 'dragover']) {
    ui.drop.addEventListener(ev, (e) => {
      e.preventDefault()
      ui.drop.classList.add('stickers__drop--over')
    })
  }
  for (const ev of ['dragleave', 'drop']) ui.drop.addEventListener(ev, () => ui.drop.classList.remove('stickers__drop--over'))
  ...
  /*
   * Affinity opens the project's own SVG, and saves back into it. The page
   * polls the file's hash and, when it moves, reloads the picture: the edit
   * is already where it belongs.
   */
  /*
   * Affinity 3 saves .af files and will not write back to an SVG, so the
   * round trip is File → Export into the project's Stickers folder. The page
   * watches that whole folder while it is open: anything exported there — the
   * same name or a new one — appears as a sticker within a couple of seconds.
   */
  const EXPORT_HINT = 'In Affinity use File → Export → SVG (or PNG) into this project’s Stickers folder. Save writes an .af file, which the Studio cannot read.'
  ui.affinity.onclick = async () => {
    /* Two or more ticked: one new file with all of them, no sheet made. */
    if (ticked.size >= 2) {
      busy(true, `Combining ${ticked.size} stickers…`)
      const r = await post('/api/stickers/combine', { projectId: project(), items: [...ticked], columns: settings.columns, size: settings.size })
      busy(false)
      if (r.error) return say(r.error, 'bad')
      status(`Opened ${ticked.size} stickers in ${r.app} as ${r.rel.split('/').pop()}. ${EXPORT_HINT}`, 'ok')
      return say(`Opened ${ticked.size} stickers in ${r.app}. Export the result into Stickers to bring it back.`)
    }
    if (!work?.rel) return say('Run a step on this picture first — Affinity opens files in the project.', 'warn')
    const r = await post('/api/stickers/affinity', { projectId: project(), rel: work.rel })
    if (r.error) return say(r.error, 'bad')
    status(`Opened ${work.name} in ${r.app}. ${EXPORT_HINT}`, 'ok')
    say(`Opened in ${r.app}. Export into Stickers to bring the edit back.`)
  }
  /* The folder watch: on while the page is, off when it goes. */
  let folderHash = null
  const watchFolder = async () => {
    if (!project()) return
    const h = await fetch(`/api/stickers/hash?project=${encodeURIComponent(project())}`)
      .then(responseJson)
      .catch(() => null)
    if (!h?.hash) return
    if (folderHash && h.hash !== folderHash) {
      const before = new Set(data.files.map((f) => f.rel))
      await reload()
      const fresh = data.files.filter((f) => !before.has(f.rel))
      if (fresh.length) {
        const f = fresh[fresh.length - 1]
        setWork({ rel: f.rel, url: mediaUrl(f.rel, f.mtime), name: f.name, kind: f.kind, key: f.rel })
        say(`Picked up ${fresh.map((x) => x.name).join(', ')}.`)
      } else if (work?.rel) {
        const f = data.files.find((x) => x.rel === work.rel)
        if (f) setWork({ ...work, url: mediaUrl(f.rel, f.mtime) })
        say('Picked up your Affinity export.')
      }
    }
    folderHash = h.hash
  }
  affinityWatch = setInterval(watchFolder, 2000)
  window.addEventListener('rm:before-navigate', () => clearInterval(affinityWatch), { once: true })

  /*
   * Build lays the sheet out again. A sheet that is already on the web goes up
   * again in the same press: a rebuilt file that the page did not show read as
   * the sheet not updating.
   */
  ui.build.onclick = async () => {
    if (!ticked.size) return say('Tick the stickers for the sheet first.', 'warn')
    busy(true, 'Laying out the sheet…')
    const r = await post('/api/stickers/sheet', { projectId: project(), name: settings.sheetName, items: [...ticked], columns: settings.columns, size: settings.size, gap: settings.gap, quote: settings.quotePublish, quoteQty: settings.quoteQty, quoteSizeMm: settings.printSizeMm, quoteProduct: settings.quoteProduct })
    if (r.error) {
      busy(false)
      return say(r.error, 'bad')
    }
    if (r.sheet.published?.remote) {
      status('Sheet rebuilt — publishing it again…')
      const pub = await post('/api/stickers/publish', { projectId: project(), name: r.sheet.name, remote: r.sheet.published.remote })
      busy(false)
      if (pub.error) return say(`Rebuilt, but not published: ${pub.error}`, 'bad')
      status(`Sheet rebuilt and republished: ${pub.url}`, 'ok')
      say('Sheet rebuilt and republished.')
    } else {
      busy(false)
      status(`Sheet saved as ${r.sheet.rel}. Publish sheet puts it on the web.`, 'ok')
    }
    await reload()
  }
  /*
   * What it would cost, from every shop this knows.
   *
   * Cheapest first, and every row says where its number came from and when:
   * asked live, read off a shop's own page, or written down by hand. A price
   * from March shown in September as though it were a quote is worse than no
   * price, so the provenance sits beside the money rather than in a footnote.
   */
  /* The best price anywhere in the grid, for the status line. */
  const cheapestOf = (group, per) => {
    let best = null
    for (const v of group.vendors) for (const [i, c] of v.cells.entries()) if (c && (!best || c.unit < best.unit)) best = { name: v.name, unit: c.unit, run: group.runs[i] }
    return best ? `${best.name} $${best.unit.toFixed(2)} a ${per} at ${best.run}` : null
  }

  ui.quote.onclick = async () => {
    busy(true, 'Asking every shop…')
    const r = await post('/api/stickers/quotes', { sizeMm: settings.printSizeMm, quantity: settings.quoteQty, })
    busy(false)
    if (r.error) return say(r.error, 'bad')
    ui.quotesHead.hidden = false
    ui.quotes.hidden = false
    ui.quotesAsked.textContent = ''
    ui.quotes.textContent = ''
    /*
     * Two grids, never one: a sheet is priced by the page and a single one at
     * a time, and ranking those together is how you order the wrong thing.
     * Within each, a column per run — the cheapest at 50 is regularly not the
     * cheapest at 500, so the mark moves along the row.
     */
    /* One table per sheet size: a sheet's price is mostly its size. */
    const groups = [[r.singles, 'Die-cut singles', 'sticker'], ...(r.sheets ?? []).map((z) => [z.group, `Sticker sheets · ${z.label}`, 'sheet'])]
    for (const [group, label, per] of groups) {
      if (!group?.vendors?.length) continue
      ui.quotes.append(control('div', { className: 'client', textContent: label }))
      const head = mountRow('quote-head')
      for (const n of group.runs) {
        const c = mountRow('quote-cell')
        c.el.unit.textContent = String(n)
        c.el.total.textContent = per + 's'
        head.el.cells.append(...c.nodes)
      }
      ui.quotes.append(...head.nodes)
      for (const v of group.vendors) {
        const { nodes, el: row } = mountRow('quote-row')
        row.name.textContent = v.name
        let first = null
        v.cells.forEach((c, i) => {
          const cell = mountRow('quote-cell')
          if (c) {
            first ??= c
            cell.el.unit.textContent = `$${c.unit.toFixed(2)}`
            /* On its own line: "$0.84$84" is two numbers that read as one. And
               when the shop's smallest run is bigger than the column, the run
               is named — "$190" under a 50 invites arithmetic that fails. */
            const over = c.quantity > group.runs[i]
            cell.el.total.textContent = over ? `$${c.total} for ${c.quantity}` : `$${c.total}`
            if (over) cell.nodes[0].classList.add('stickers__cell--over')
            if (group.best[i] === v.id) cell.nodes[0].classList.add('stickers__cell--best')
          } else {
            cell.el.unit.textContent = '—'
            cell.el.total.textContent = ''
          }
          row.cells.append(...cell.nodes)
        })
        const how = first?.live ? 'read from their page' : v.kind === 'api' ? 'quoted live' : 'from the table'
        row.source.textContent = v.error ? v.error : first ? `${how} · ${first.checkedAt}` : ''
        ui.quotes.append(...nodes)
      }
    }
    const bestSheet = (r.sheets ?? []).filter((z) => z.group?.vendors?.length).map((z) => cheapestOf(z.group, `${z.label} sheet`)).filter(Boolean)
    const bits = [r.singles?.vendors?.length && cheapestOf(r.singles, 'sticker'), ...bestSheet].filter(Boolean)
    if (bits.length) status(`Cheapest — ${bits.join(' · ')}.`, 'ok')
  }

  /*
   * The sheet as a print shop wants it.
   *
   * Not the grid: a die-cutter wants each sticker alone on its own page, so a
   * shop never has to take a layout apart and scale a sticker by a hair doing
   * it. Built from the sheet's record rather than from what is ticked, so what
   * is printed is what was published.
   */
  /*
   * The sheet, redrawn as the settings move.
   *
   * Asked for on a short delay rather than on every tick of a slider: a drag
   * is fifty events and the answer only matters once the hand stops. One
   * request at a time, and the last one asked for wins — a slower earlier
   * answer arriving after a faster later one would show the wrong sheet.
   */
  /* What the fit worked out to, so the rail can say it. */
  let sheetFitAt = 0
  let previewAt = 0
  let previewing = false
  const paintPreview = async () => {
    if (settings.printLayout !== 'sheet' || !project()) {
      ui.printPreviewHead.hidden = true
      ui.printPreview.hidden = true
      return
    }
    const mine = ++previewAt
    if (previewing) return
    previewing = true
    ui.printPreview.classList.add('stickers__preview-sheet--busy')
    try {
      while (mine === previewAt || previewAt !== mine) {
        const want = previewAt
        const r = await post('/api/stickers/print-preview', {
          projectId: project(),
          name: settings.sheetName,
          items: [...ticked],
          page: settings.printPage,
          bleedMm: settings.printBleedMm,
          dieOffsetMm: settings.dieOffsetMm,
          dieRoundMm: settings.dieRoundMm,
        })
        if (want !== previewAt) continue
        if (r.error) {
          ui.printPreviewHead.hidden = true
          ui.printPreview.hidden = true
          return
        }
        ui.printPreviewHead.hidden = false
        ui.printPreview.hidden = false
        ui.printPreviewAt.textContent = `· page ${r.at + 1} of ${r.pages}${r.stickerMm ? ` · ${r.stickerMm}mm each` : ''}${settings.printBleedMm ? ` · ${settings.printBleedMm}mm bleed` : ' · no bleed'}${r.skipped ? ` · ${r.skipped} missing` : ''}`
        if (r.stickerMm && r.stickerMm !== sheetFitAt) {
          sheetFitAt = r.stickerMm
          buildRail()
        }
        /* The SVG is put in as markup rather than as an <img src="data:…">, so
           it scales as vector and a wrong colour is visible rather than
           resampled away. */
        ui.printPreview.innerHTML = r.svg.replace(/<\?xml[^>]*\?>/, '')
        return
      }
    } finally {
      previewing = false
      ui.printPreview.classList.remove('stickers__preview-sheet--busy')
    }
  }
  let previewTimer = 0
  const previewSoon = () => {
    clearTimeout(previewTimer)
    previewTimer = setTimeout(paintPreview, 350)
  }

  ui.printPdf.onclick = async () => {
    busy(true, settings.printLayout === 'sheet' ? 'Laying the sheet with cut lines, then converting to CMYK…' : 'Drawing each sticker as a page, then converting to CMYK…')
    /* What is ticked wins, so Print PDF means what it says without Build
       sheet having been pressed first. */
    const r = await post('/api/stickers/print', { projectId: project(), name: settings.sheetName, items: [...ticked], sizeMm: settings.printSizeMm, bleedMm: settings.printBleedMm, layout: settings.printLayout, page: 'all', dieOffsetMm: settings.dieOffsetMm, dieRoundMm: settings.dieRoundMm })
    busy(false)
    if (r.error) return say(r.error, 'bad')
    const skipped = r.skipped ? `, ${r.skipped} missing sticker${r.skipped === 1 ? '' : 's'} skipped` : ''
    const how = r.layout === 'sheet' ? (r.sheets ?? []).map((x) => `${x.label} at ${x.stickerMm}mm`).join(' · ') : 'one sticker a page'
    /*
     * A link to the file, right here.
     *
     * Print PDF only ever printed a path into the status line, so the nearest
     * thing to a download was the copy on the published page — which is
     * whatever was there at the last Publish. Somebody printed three stickers
     * on the 6 x 8 and downloaded twelve on a letter sheet, four times.
     */
    ui.printMade.hidden = false
    /* The href is stamped so a second print is not served from the first one's
       place in the browser's cache. */
    const href = mediaUrl(r.rel, Date.now())
    ui.printSave.href = href
    ui.printSave.download = r.rel.split('/').pop()
    ui.printSaveText.textContent = r.layout === 'sheet' && r.sheets?.length ? `Save the ${r.sheets[0].label} sheet` : `Save the pages · ${r.pages} page${r.pages === 1 ? '' : 's'}`
    ui.printOpen.href = href
    /* The other sizes get their own link rather than being made and forgotten. */
    for (const extra of (r.sheets ?? []).slice(1)) {
      const a = control('link', { className: 'btn ghost ', href: mediaUrl(extra.rel, Date.now()), textContent: `Save the ${extra.label} sheet` })
      a.download = extra.rel.split('/').pop()
      ui.printMade.append(' ', a)
    }
    status(r.layout === 'sheet' ? `${(r.sheets ?? []).length} sheet${(r.sheets ?? []).length === 1 ? '' : 's'} — ${how}, CMYK through ${r.profileName}${skipped}.` : `${r.pages} page${r.pages === 1 ? '' : 's'} in ${r.rel} — ${how}, CMYK through ${r.profileName}${skipped}.`, 'ok')
    say(`Print PDF saved: ${r.pages} pages.`)
    await reload()
  }

  /*
   * Publish sends the ticked stickers, rebuilding the sheet under this name
   * first — so adding to a live page is: open its card, tick more, Publish.
   * With nothing ticked, the sheet as last built goes up again.
   */
  ui.publish.onclick = async () => {
    if (!settings.remote) return say('Set up shared storage with a public bucket first, under Storage.', 'warn')
    const name = settings.sheetName
    if (ticked.size) {
      busy(true, 'Laying out the sheet…')
      const built = await post('/api/stickers/sheet', { projectId: project(), name, items: [...ticked], columns: settings.columns, size: settings.size, gap: settings.gap, quote: settings.quotePublish, quoteQty: settings.quoteQty, quoteSizeMm: settings.printSizeMm, quoteProduct: settings.quoteProduct })
      if (built.error) {
        busy(false)
        return say(built.error, 'bad')
      }
    } else if (!data.sheets.some((x) => x.name === safeSlug(name))) return say('Tick stickers first, or open a sheet card to publish it again.', 'warn')
    busy(true, 'Publishing…')
    const r = await post('/api/stickers/publish', { projectId: project(), name: safeSlug(name), remote: settings.remote })
    busy(false)
    if (r.error) return say(r.error, 'bad')
    status(`Published: ${r.url}`, 'ok')
    say('Published. The page address is in the sheet card and the status line.')
    await reload()
  }
  /* The server's safe name, mirrored, so the page can find the sheet it just built. */
  const safeSlug = (name) => (String(name).replace(/[/\\:*?"<>|\x00-\x1f]/g, '').replace(/^\.+/, '').trim() || 'sheet').replace(/\s+/g, '-').slice(0, 60)
...
/* ══════════════════════════════════════════════════════════════════════════
 * Showcase: footage and stills, presented — on a look, in frames, in 3D, in time.
 *
 * A scene is one look and a list of layers. Each layer is an <rm-showcase>:
 * what it shows, the device and camera it is shown with, when it is on screen
 * and how it comes and goes. The rail works on the selected layer; the footage
 * cards and the templates do too. Save PNG asks the server to draw the frame
 * under the playhead, because CSS 3D over a WebGL look is nothing a canvas can
 * read; the tag is the whole scene, so a composition plays it as designed.
 * ══════════════════════════════════════════════════════════════════════════ */
function vShowcase(m) {
  const ui = mountPanel('showcase', m)
  const { look: lookEl, stage: stageEl, scrub, clock, markup, play: playBtn } = ui
  const layerEls = () => [...stageEl.querySelectorAll('rm-showcase')]
  let rt = null
  let scene = null
  let sceneLook = null
  let editOpen = false
  let sel = 0
  let looks = []
  const exportSettings = { width: '1920', shape: '16/9' }
  const say = (text, kind) => toast(text, kind)
  const post = (path, body, method = 'POST') =>
    fetch(path, { method, headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) })
      .then(responseJson)
  ...
  const totalMs = () => Math.max(1000, ...scene.layers.map((l) => Math.round((l.at + l.dur) * 1000)))

  /*
   * ── keyframes ──
   *
   * A layer with keys is animated between them; a layer without is still, and
   * its dials are the values. Reading a keyed value asks the runtime for the
   * value under the playhead; writing one goes to the key under the playhead,
   * or makes a key there first. That is the whole model: move the playhead,
   * change a dial, and the change is a key.
   */
  const KEY_SNAP = 0.05
  const nowSec = () => Number(scrub.value) / 1000
  const layerT = (l) => Math.max(0, Math.min(l.dur, Math.round((nowSec() - l.at) / KEY_SNAP) * KEY_SNAP))
  const keyAt = (l, t = layerT(l)) => (l.keys ?? []).find((k) => Math.abs(k.t - t) < KEY_SNAP / 2) ?? null
  const valueOf = (l, p) => (l.keys?.length && rt.KEY_PROPS.includes(p) ? rt.keysAt(l.keys, layerT(l), l)[p] : l[p])
  const addKey = (l, t = layerT(l)) => {
    const have = keyAt(l, t)
    if (have) return have
    /* Nor may a new one: the same tenth of a second, for the same reason. */
    const near = (l.keys ?? []).find((k) => Math.abs(k.t - t) < 0.1)
    if (near) return near
    const pose = l.keys?.length ? rt.keysAt(l.keys, t, l) : Object.fromEntries(rt.KEY_PROPS.map((p) => [p, l[p]]))
    const k = { t: Math.round(t / KEY_SNAP) * KEY_SNAP, ease: 'out', ...pose }
    l.keys = [...(l.keys ?? []), k].sort((a, b) => a.t - b.t)
    return k
  }
  const dropKey = (l, k) => {
    l.keys = (l.keys ?? []).filter((x) => x !== k)
    /* One key left is a still pose: it becomes the dials, so nothing jumps. */
    if (l.keys.length === 1) {
      for (const p of rt.KEY_PROPS) if (l.keys[0][p] !== undefined) l[p] = l.keys[0][p]
      l.keys = []
    }
  }
  const setProp = (l, p, v) => {
    if (rt.KEY_PROPS.includes(p) && l.keys?.length) addKey(l)[p] = v
    else l[p] = v
  }
  const round = (v, step) => Math.round(v / step) * step
  const layerTag = (l) => {
    const { at, dur, ...rest } = l
    return `<rm-showcase at="${Math.round(at * 1000)}" for="${Math.round(dur * 1000)}" mat="0" mfor="${totalMs()}" ${rt.encodeShowcase({ ...rest, move: scene.move })}></rm-showcase>`
  }
  const tag = () => [scene.look ? `<rm-look at="0" for="${totalMs()}" look="${esc(scene.look)}"></rm-look>` : '', ...scene.layers.map(layerTag)].filter(Boolean).join('\n')
  const nameOf = (l) => (l.media ? decodeURIComponent(l.media.split('/').pop()) : 'Empty layer')
  const thumbOf = (l) => {
    if (!l.media) return ''
    const m_ = l.media.match(/^\/media\/([^/]+)\/(.+)$/)
    if (m_) return `/thumb/${m_[1]}/${m_[2]}`
    return /\.(mp4|mov|webm|m4v)$/i.test(l.media) ? '' : l.media
  }

  /* One write per frame: a slider fires far faster than the stage can draw. */
  let queued = 0
  const whenLabels = new Map()
  const apply = () => {
    if (queued) return
    queued = requestAnimationFrame(() => {
      queued = 0
      const total = totalMs()
      if (scene.look) lookEl.setAttribute('look', scene.look)
      else lookEl.removeAttribute('look')
      lookEl.hidden = !scene.look
      lookEl.setAttribute('for', String(total))
      const els = layerEls()
      while (els.length > scene.layers.length) els.pop().remove()
      scene.layers.forEach((l, i) => {
        let el = els[i]
        if (!el) {
          el = document.createElement('rm-showcase')
          stageEl.append(el)
          els.push(el)
        }
        el.setAttribute('at', String(Math.round(l.at * 1000)))
        el.setAttribute('for', String(Math.round(l.dur * 1000)))
        for (const f of rt.SHOWCASE_SCHEMA) el.setAttribute(f.key, String(f.group === 'scene' ? scene[f.key] : l[f.key]))
        el.setAttribute('mat', '0')
        el.setAttribute('mfor', String(total))
        if (l.keys?.length) el.setAttribute('keys', rt.encodeKeys(l.keys))
        else el.removeAttribute('keys')
        if (l.media) el.setAttribute('media', l.media)
        else el.removeAttribute('media')
        el.removeAttribute('look')
        whenLabels.get(l)?.call(null)
      })
      scrub.max = String(total)
      if (Number(scrub.value) > total) scrub.value = String(total)
      markup.textContent = tag()
      for (const c of ui.media.children) c.classList.toggle('showcase__card--on', c.dataset.media === L().media)
      /* After the components have laid out: the box follows the card. */
      requestAnimationFrame(() => {
        paintGizmo()
        paintPlayhead()
      })
  ...
  /*
   * Put the playhead inside a layer's window. At time zero the first layer
   * has not entered yet, so a fresh page looked empty until Play was pressed;
   * selecting a layer that is not on screen at the current time looked the
   * same. A second in, the enter is done and the layer is plainly there.
   */
  const seekTo = (ms) => {
    ms = Math.max(0, Math.min(Number(scrub.max), ms))
    scrub.value = String(ms)
    clock.textContent = `${ms} ms`
    rt.RM.seek(ms)
    paintPlayhead()
  }
  const seekInto = (l) => {
    const ms = Math.round((l.at + Math.min(1, l.dur / 2)) * 1000)
    scrub.value = String(ms)
    clock.textContent = `${ms} ms`
    rt.RM.seek(ms)
    paintPlayhead()
  ...
  /*
   * One drag, on the window. Pointer capture was the obvious tool and the wrong
   * one: the bar under the hand is rebuilt when a drag ends, and the next press
   * on a fresh node failed to capture, so only the first drag ever worked.
   */
  ...
  /* ── the timeline: a row per layer, a bar for its time on screen ── */
  const spanSec = () => Math.max(6, Math.ceil(totalMs() / 1000) + 2)
  const snap = (v) => Math.round(v * 10) / 10
  const paintPlayhead = () => {
    if (!scene) return
    const lane = ui.rows.querySelector('.showcase__lane') ?? ui.ruler
    const left = lane.getBoundingClientRect().left - ui.tl.getBoundingClientRect().left
    const width = lane.getBoundingClientRect().width
    ui.playhead.style.left = `${left + (Number(scrub.value) / 1000 / spanSec()) * width}px`
    syncDials()
    for (const paint of keyMarks.values()) paint()
    /* Keys move the card with the clock, so the box follows the playhead too. */
    requestAnimationFrame(paintGizmo)
  }
  /* The keyed dials follow the playhead, so a slider always shows the value on screen. */
  const dials = new Map()
  const keyMarks = new Map()
  const syncDials = () => {
    const l = scene?.layers[sel]
    if (!l) return
    for (const [p, d] of dials) {
      const v = valueOf(l, p)
      if (Number(d.input.value) !== v) {
        d.input.value = String(v)
        d.readout.textContent = String(Math.round(v * 100) / 100)
      }
      d.mark.classList.toggle('showcase__keybtn--on', !!(l.keys?.length && keyAt(l)))
      d.mark.classList.toggle('showcase__keybtn--keyed', !!l.keys?.length)
    }
  }
  const paintRuler = () => {
    ...
    for (let t = 0; t <= span; t += step) {
      const tick = control('span', { className: 'showcase__tick', textContent: `${t}s` })
      tick.style.left = `${(t / span) * 100}%`
      ui.ruler.append(tick)
    }
    ui.tl.style.setProperty('--tick', `${(step / span) * 100}%`)
  }
  ui.ruler.onpointerdown = (e) => {
    const r = ui.ruler.getBoundingClientRect()
    const seekAt = (x) => {
      const ms = Math.round(Math.max(0, Math.min(1, (x - r.left) / r.width)) * spanSec() * 1000)
      scrub.value = String(Math.min(ms, Number(scrub.max)))
      clock.textContent = `${scrub.value} ms`
      rt.RM.seek(Number(scrub.value))
      paintPlayhead()
    }
    seekAt(e.clientX)
    drag(e, (ev) => seekAt(ev.clientX))
  }
  const select = (i) => {
    sel = i
    buildRail()
  ...
  const paintLayers = () => {
    ...
    whenLabels.clear()
    keyMarks.clear()
    ui.layerCount.textContent = `· ${scene.layers.length}`
    paintRuler()
    const span = spanSec()
    scene.layers.forEach((l, i) => {
      const { root, el } = mountRow('showcase-track')
      root.classList.toggle('showcase__track--on', i === sel)
      const paint = () => {
        el.name.textContent = `${i + 1}. ${nameOf(l)}${l.device !== 'none' ? ` · ${l.device}` : ''}`
        el.bar.style.left = `${(l.at / span) * 100}%`
        el.bar.style.width = `${(l.dur / span) * 100}%`
        el.when.textContent = `${l.at.toFixed(1)}s – ${(l.at + l.dur).toFixed(1)}s · ${l.enter} in, ${l.exit} out${l.keys?.length ? ` · ${l.keys.length} keys` : ''}`
        paintKeys()
      }
      /*
       * The keys, as diamonds along the bar's lower edge. Drag one to move it
       * in time; click one to put the playhead on it. The one under the
       * playhead is lit: that is the key the dials are writing to.
       */
      const paintKeys = () => {
        el.keys.replaceChildren()
        for (const k of l.keys ?? []) {
          const d = control('span', { className: 'showcase__key' })
          d.style.left = `${((l.at + k.t) / span) * 100}%`
          d.classList.toggle('showcase__key--on', i === sel && keyAt(l) === k)
          d.onpointerdown = (e) => {
            e.stopPropagation()
            const lane = el.lane.getBoundingClientRect()
            const start = { x: e.clientX, t: k.t }
            let moved = false
            if (i !== sel) select(i)
            drag(
              e,
              (ev) => {
                const dt = ((ev.clientX - start.x) / lane.width) * span
                if (Math.abs(ev.clientX - start.x) > 2) moved = true
                /*
                 * A key may not land on its neighbour.
                 *
                 * Two keys at the same moment have no time between them, so the
                 * card jumps from one pose to the other in a single frame — the
                 * motion reads as broken rather than as fast. A tenth of a
                 * second is the closest two keys are allowed to be.
                 */
                const others = l.keys.filter((x) => x !== k).map((x) => x.t).sort((a, b) => a - b)
                const under = others.filter((t) => t < start.t).pop()
                const over = others.find((t) => t > start.t)
                const low = under === undefined ? 0 : under + 0.1
                const high = over === undefined ? l.dur : over - 0.1
                k.t = Math.max(low, Math.min(Math.max(low, high), round(start.t + dt, KEY_SNAP)))
                l.keys.sort((a, b) => a.t - b.t)
                seekTo(Math.round((l.at + k.t) * 1000))
                paint()
                apply()
              },
              () => {
                if (!moved) seekTo(Math.round((l.at + k.t) * 1000))
                buildRail()
                paintLayers()
                apply()
              },
      ...
      paint()
      whenLabels.set(l, paint)
      keyMarks.set(l, paintKeys)
      el.pick.onclick = (e) => {
        if (e.target === el.remove) {
          if (scene.layers.length === 1) return
          scene.layers.splice(i, 1)
          sel = Math.min(sel, scene.layers.length - 1)
          select(sel)
          return
        }
        select(i)
      ...
      /*
       * The bar: drag its body to move the layer in time, an end to trim it.
       * Time is read off the lane's width, so the feel is the same at any size;
       * a tenth of a second is the grid.
       */
      el.bar.onpointerdown = (e) => {
        const lane = el.lane.getBoundingClientRect()
        const span = spanSec()
        const mode = e.target.dataset.trim === 'l' ? 'l' : e.target.dataset.trim === 'r' ? 'r' : 'move'
        const start = { x: e.clientX, at: l.at, dur: l.dur }
        if (i !== sel) {
          sel = i
          buildRail()
          for (const row of ui.rows.children) row.classList.toggle('showcase__track--on', row === root)
        }
        drag(
          e,
          (ev) => {
            const dt = ((ev.clientX - start.x) / lane.width) * span
            if (mode === 'move') l.at = snap(Math.max(0, start.at + dt))
            else if (mode === 'l') {
              const at = snap(Math.max(0, Math.min(start.at + start.dur - 0.3, start.at + dt)))
              l.dur = snap(start.at + start.dur - at)
              l.at = at
            } else l.dur = snap(Math.max(0.3, start.dur + dt))
            paint()
            apply()
          },
          () => {
  ...
  /*
   * ── the gizmo: the selected layer, outlined on the stage ──
   *
   * Drawn from where the layer's card actually is, after its 3D transform, so
   * the box follows a tilt. Drag the box to move the layer, a corner to size it;
   * the layer's own X, Y and Size dials change underneath, in the card's own
   * units, so the rail and the stage always agree.
   */
  const stageBox = ui.gizmo.parentElement
  const cardRectOf = (el) => el.shadowRoot?.querySelector('.card')?.getBoundingClientRect() ?? null
  const paintGizmo = () => {
    const el = layerEls()[sel]
    const r = el ? cardRectOf(el) : null
    if (!r || !r.width) return void (ui.gizmo.hidden = true)
    const host = stageBox.getBoundingClientRect()
    ui.gizmo.hidden = false
    Object.assign(ui.gizmo.style, { left: `${r.left - host.left}px`, top: `${r.top - host.top}px`, width: `${r.width}px`, height: `${r.height}px` })
  }
  const layerAt = (x, y) => {
    const els = layerEls()
    for (let i = els.length - 1; i >= 0; i--) {
      const r = cardRectOf(els[i])
      if (r && x >= r.left && x <= r.right && y >= r.top && y <= r.bottom) return i
    }
    return -1
  }
  ui.gizmo.onpointerdown = (e) => {
    e.preventDefault()
    e.stopPropagation()
    const corner = e.target.dataset.corner
    /* A press inside the box but on a layer above the selected one is a press
       on that layer: select it, and the drag moves it. Corners stay with the box. */
    if (!corner) {
      const hit = layerAt(e.clientX, e.clientY)
      if (hit !== -1 && hit !== sel) select(hit)
    }
    const l = L()
    const el = layerEls()[sel]
    const r0 = cardRectOf(el)
    if (!r0) return
    /*
     * The card's own size, not the box it casts.
     *
     * X and Y are percentages of the card, and getBoundingClientRect returns the
     * projected outline — a tilted card throws a box far wider than it is, so a
     * drag moved it a fraction of the distance the hand went and it slid away
     * under the pointer.
     */
    const own = { w: el.shadowRoot?.querySelector('.card')?.offsetWidth || r0.width, h: el.shadowRoot?.querySelector('.card')?.offsetHeight || r0.height }
    const start = { x: e.clientX, y: e.clientY, lx: valueOf(l, 'x'), ly: valueOf(l, 'y'), w: l.w, zoom: valueOf(l, 'zoom'), rect: r0, own }
    drag(e, (ev) => {
      const dx = ev.clientX - start.x
      const dy = ev.clientY - start.y
      if (!corner) {
        /* X and Y are percentages of the card's own size, as translate() reads them. */
        setProp(l, 'x', Math.round((start.lx + (dx / start.own.w) * 100) * 2) / 2)
        setProp(l, 'y', Math.round((start.ly + (dy / start.own.h) * 100) * 2) / 2)
      } else {
        /* A corner scales the card about its centre; the far side of the box
           is what the hand is measuring against. */
        const sx = corner.includes('e') ? 1 : -1
        const grow = (start.own.w + sx * dx * 2) / start.own.w
        const f = Math.max(0.1, grow)
        if (l.device === 'none' && start.w < 100) l.w = Math.max(10, Math.min(100, Math.round(start.w * f)))
        else if (l.device !== 'none') l.w = Math.max(10, Math.min(100, Math.round(start.w * f)))
        else setProp(l, 'zoom', Math.max(0.4, Math.min(2.5, Math.round(start.zoom * f * 100) / 100)))
      }
      apply()
    }, () => buildRail())
  }
  /* A click on the stage selects whatever layer is under it, topmost first. */
  stageBox.onpointerdown = (e) => {
    if (e.target === ui.gizmo || ui.gizmo.contains(e.target)) return
    const hit = layerAt(e.clientX, e.clientY)
    if (hit !== -1 && hit !== sel) select(hit)
  }
  new ResizeObserver(() => {
  ...
  const newLayer = (from) => {
    const at = from ? from.at + 0.4 : 0
    const end = from ? Math.max(from.at + from.dur, at + 1) : 4
    return { ...rt.showcaseDefaults(), ...(from ? { ...from } : {}), keys: (from?.keys ?? []).map((k) => ({ ...k })), media: '', at, dur: Math.round((end - at) * 10) / 10 }
  }
  ui.addLayer.onclick = () => {
    /* Where it goes: the same spot, sized down a step and shifted, so two
       pictures never land exactly on top of one another. */
    const fresh = newLayer(L())
    fresh.w = Math.max(30, Math.round((L().w ?? 100) * 0.8))
    fresh.x = (L().x ?? 0) + 8
    fresh.y = (L().y ?? 0) + 6
    scene.layers.push(fresh)
    sel = scene.layers.length - 1
    /* The next footage card that is not yet on a layer, so Add layer is one
       press and the layer is not blank. */
    const used = new Set(scene.layers.map((l) => l.media))
    const next = [...ui.media.children].map((c) => c.dataset.media).find((x) => !used.has(x))
    if (next) L().media = next
    buildRail()
    paintLayers()
    apply()
    seekInto(L())
  ...
  /* A picture or clip from this machine onto the selected layer. Kept with
     the added brand assets and served from there, so a render finds it too. */
  const uploadOnto = async (file, button) => {
    if (!file) return
    button.disabled = true
    const r = await fetch(`/api/brand/asset?name=${encodeURIComponent(file.name)}`, { method: 'POST', headers: { 'content-type': 'application/octet-stream' }, body: file, duplex: 'half' })
      .then(responseJson)
      .catch((e) => ({ error: e.message }))
    button.disabled = false
    if (r.error) return say(r.error, 'bad')
    L().media = `/added/${r.file}`
    buildRail()
    paintLayers()
    apply()
  }
  ui.addImage.onclick = () => ui.imageFile.click()
  ui.imageFile.onchange = () => {
    const file = ui.imageFile.files?.[0]
    ui.imageFile.value = ''
    void uploadOnto(file, ui.addImage)
  }

  /* ── the rail: the selected layer's dials ── */
  const panel = mountRow('designer-panel').root
  const sec = (t) => {
    const heading = mountRow('designer-sec').root
    heading.textContent = t
  ...
  const rangeRow = (box, f, target = L) => {
    const { nodes, el: row } = mountRow('designer-range')
    row.label.textContent = f.label
    const keyed = target === L && rt.KEY_PROPS.includes(f.key)
    const i = Object.assign(row.input, { min: f.min, max: f.max, step: f.step, value: keyed ? valueOf(L(), f.key) : target()[f.key] })
    const show = () => (row.readout.textContent = String(Math.round(Number(i.value) * 100) / 100))
    show()
    i.oninput = () => {
      if (keyed) setProp(L(), f.key, Number(i.value))
      else target()[f.key] = Number(i.value)
      show()
      if (keyed) {
        keyMarks.get(L())?.call(null)
        syncDials()
      }
      apply()
    }
    if (keyed) {
      /* The mark: lit when a key sits under the playhead. Press it to key this
         moment, or to take that key away. */
      const mark = control('button', { type: 'button', className: 'showcase__keybtn', title: 'Keyframe at the playhead' })
      mark.onclick = (e) => {
        e.preventDefault()
        const l = L()
        const k = l.keys?.length ? keyAt(l) : null
        if (k) dropKey(l, k)
        else addKey(l)
        buildRail()
        paintLayers()
      ...
      row.label.prepend(mark)
      dials.set(f.key, { input: i, readout: row.readout, mark })
    }
    box.append(...nodes)
  }
  const selectRow = (box, f, target = L) => {
    const { nodes, el: row } = mountRow('designer-select')
    row.label.textContent = f.label
    for (const o of f.options) row.input.append(new Option(o, o))
    row.input.value = target()[f.key]
    row.input.onchange = () => {
      target()[f.key] = row.input.value
      apply()
    }
    box.append(...nodes)
  }
  const buildRail = () => {
    panel.replaceChildren()
    dials.clear()
    for (const [group, label] of rt.SHOWCASE_GROUPS) {
      sec(group === 'motion' ? `Layer ${sel + 1}: ${label}` : label)
      const box = g()
      if (group === 'scene') {
        /* The scene's own: the backdrop, and one camera move over everything. */
        const lookRow = mountRow('designer-select')
        lookRow.el.label.textContent = 'Backdrop'
        lookRow.el.input.append(new Option('None', ''))
        for (const l of looks) lookRow.el.input.append(new Option(l.name, l.look))
        /* A reopened scene's look may be nobody's preset; it is still the look. */
        if (scene.look && !looks.some((l) => l.look === scene.look)) lookRow.el.input.append(new Option("This scene's look", scene.look))
        lookRow.el.input.value = scene.look || ''
        lookRow.el.input.onchange = () => {
          scene.look = lookRow.el.input.value
          sceneLook = rt.decodeLook(scene.look)
          if (editOpen) paintLookDials()
          apply()
        }
        box.append(...lookRow.nodes)
        for (const f of rt.SHOWCASE_SCHEMA.filter((f) => f.group === 'scene')) selectRow(box, f, () => scene)
        /*
         * The Creator's dials, here, behind a switch. A look chosen from the
         * list is a starting point; editing it makes it this scene's own, and
         * the tag carries the result, so nothing has to be saved first.
         */
        const { root: heading, el: head } = mountRow('creator-sec')
        head.title.textContent = 'Edit the backdrop'
        head.input.id = 'showcase-edit-look'
        Object.assign(head.track, { textContent: 'Edit the backdrop', htmlFor: head.input.id })
        panel.append(heading)
        /* A panel, not a group: the dials bring their own headings and groups,
           and inside a group's grid they would fall into its two columns. */
        const lookBox = mountRow('designer-panel').root
        panel.append(lookBox)
        const paintLookDials = () => {
          lookBox.replaceChildren()
          lookDials({
            rt,
            look: () => sceneLook,
            onChange: () => {
              scene.look = rt.encodeLook(sceneLook)
              apply()
            },
            host: lookBox,
            say,
        ...
        head.input.checked = editOpen
        lookBox.hidden = !editOpen
        if (editOpen) paintLookDials()
        head.input.onchange = () => {
          editOpen = head.input.checked
          lookBox.hidden = !editOpen
          if (editOpen) {
            if (!scene.look) {
              scene.look = rt.LOOK_PRESETS[0].look
              sceneLook = rt.decodeLook(scene.look)
              apply()
            }
            paintLookDials()
      ...
      if (group === 'motion') {
        rangeRow(box, { key: 'at', label: 'Start (s)', min: 0, max: 60, step: 0.1 })
        rangeRow(box, { key: 'dur', label: 'On screen (s)', min: 0.5, max: 30, step: 0.1 })
        /*
         * Keyframes for this layer. Add keys where the playhead is; the placement
         * dials below then write to the key under the playhead, and make one
         * where there is none. Ease is the curve out of the lit key.
         */
        const { nodes, el: kr } = mountRow('showcase-keys')
        const l = L()
        const k = l.keys?.length ? keyAt(l) : null
        const n = l.keys?.length ?? 0
        kr.info.textContent = !n ? 'No keys: the dials are the pose.' : k ? `Key ${l.keys.indexOf(k) + 1} of ${n}, at ${k.t.toFixed(2)}s` : `${n} keys · between keys`
        kr.add.lastChild.textContent = k ? 'Keyed here' : n ? 'Add key here' : 'Start keyframing'
        kr.add.disabled = !!k
        kr.add.onclick = () => {
          if (!n) {
            /* The first key holds the pose as it is; a second one, a beat on,
               gives the playhead somewhere to go. */
            addKey(l, layerT(l))
            const t2 = Math.min(l.dur, layerT(l) + 1)
            if (t2 > layerT(l)) addKey(l, t2)
          } else addKey(l)
          buildRail()
          paintLayers()
          apply()
        ...
        kr.clear.hidden = !n
        kr.clear.onclick = () => {
          const pose = rt.keysAt(l.keys, layerT(l), l)
          for (const p of rt.KEY_PROPS) l[p] = pose[p]
          l.keys = []
          buildRail()
          paintLayers()
          apply()
        ...
        kr.easeRow.hidden = !k
        for (const e of Object.keys(rt.KEY_EASES)) kr.ease.append(new Option(e, e))
        if (k) kr.ease.value = k.ease
        kr.ease.onchange = () => {
          k.ease = kr.ease.value
          apply()
        }
        box.append(...nodes)
      }
      if (group === 'media') {
        /* The picture, or a short clip, from this machine; it is kept with the
           added brand assets and served from there, so a render finds it too. */
        const { nodes, el: row } = mountRow('showcase-picture')
        row.name.textContent = L().media ? nameOf(L()) : 'none'
        row.pick.onclick = () => row.file.click()
        row.file.onchange = () => {
          const file = row.file.files?.[0]
          row.file.value = ''
          void uploadOnto(file, row.pick)
        }
        box.append(...nodes)
      }
      for (const f of rt.SHOWCASE_SCHEMA.filter((f) => f.group === group)) {
        if (f.type === 'range') rangeRow(box, f)
        else selectRow(box, f)
      }
    }
    sec('Export')
    const ex = g()
    const sizeRow = mountRow('designer-select')
    sizeRow.el.label.textContent = 'PNG width'
    for (const w of [1280, 1920, 2560, 3840]) sizeRow.el.input.append(new Option(`${w} px`, String(w)))
    sizeRow.el.input.value = exportSettings.width
    sizeRow.el.input.onchange = () => (exportSettings.width = sizeRow.el.input.value)
    ex.append(...sizeRow.nodes)
    const shapeRow = mountRow('designer-select')
    shapeRow.el.label.textContent = 'PNG shape'
    for (const [v, l] of [['16/9', '16:9'], ['1/1', '1:1'], ['4/3', '4:3'], ['9/16', '9:16'], ['4/5', '4:5']]) shapeRow.el.input.append(new Option(l, v))
    shapeRow.el.input.value = exportSettings.shape
    shapeRow.el.input.onchange = () => (exportSettings.shape = shapeRow.el.input.value)
    ex.append(...shapeRow.nodes)
    sec('Into a composition')
    const outlets = g()
    outlets.append(...sceneOutlets({ tag, startMs: () => Number(scrub.value), name: 'showcase', durationMs: totalMs() }))
    /* The outlets' own name field, so a reopened scene saves over itself. */
    outletName = outlets.querySelector('input[data-el="name"]')
    if (outletName && sceneName) outletName.value = sceneName
    /* A save lands in the list above, once the server has written it. */
    outlets.querySelector('button[data-el="save"]')?.addEventListener('click', () => {
      sceneName = outletName?.value?.trim() || sceneName
      setTimeout(paintScenes, 1200)
    })
    intoRail(panel)
  }

  /* ── saved scenes: reopened here, layer by layer ── */
  let sceneName = null
  let outletName = null
  const readScene = (body) => {
    const doc = new DOMParser().parseFromString(body, 'text/html')
    const lookEl_ = doc.querySelector('rm-look')
    const shows = [...doc.querySelectorAll('rm-showcase')]
    if (!shows.length) return null
    const num = (el, k, d) => {
      const n = Number(el.getAttribute(k))
      return Number.isFinite(n) ? n : d
    }
    const layers = shows.map((el) => {
      const l = { ...rt.showcaseDefaults(), media: el.getAttribute('media') || '', at: num(el, 'at', 0) / 1000, dur: num(el, 'for', 4000) / 1000, keys: rt.parseKeys(el.getAttribute('keys')) }
      for (const f of rt.SHOWCASE_SCHEMA) {
        if (!el.hasAttribute(f.key)) continue
        const raw = el.getAttribute(f.key)
        l[f.key] = f.type === 'range' ? num(el, f.key, f.def) : f.options.includes(raw) ? raw : f.def
      }
      return l
    })
    return { look: lookEl_?.getAttribute('look') || '', move: layers[0].move ?? 'none', layers }
  }
  const paintScenes = async () => {
    ui.scenes.replaceChildren()
    ui.sceneCount.textContent = ''
    const id = currentProject()
    if (!id) return void (ui.scenesEmpty.hidden = false)
    const r = await fetch(`/api/scenes?project=${encodeURIComponent(id)}`)
      .then(responseJson)
      .catch(() => ({ scenes: [] }))
    const mine = (r.scenes ?? []).filter((x) => /<rm-showcase[\s>]/.test(x.body ?? ''))
    ui.scenesEmpty.hidden = mine.length > 0
    ui.sceneCount.textContent = mine.length ? `· ${mine.length}` : ''
    for (const x of mine) {
      const { root, el } = mountRow('showcase-template')
      const parsed = readScene(x.body)
      el.name.textContent = x.name
      el.note.textContent = parsed ? `${parsed.layers.length} layer${parsed.layers.length === 1 ? '' : 's'} · ${parsed.move && parsed.move !== 'none' ? parsed.move : 'no camera move'}` : 'could not read'
      el.device.textContent = 'SCENE'
      root.classList.toggle('showcase__template--on', sceneName === x.name)
      root.onclick = () => {
        if (!parsed) return say('That scene could not be read.', 'bad')
        scene = parsed
        sceneLook = rt.decodeLook(scene.look)
        sel = 0
        sceneName = x.name
        buildRail()
        if (outletName) outletName.value = x.name
        paintLayers()
        paintScenes()
        apply()
        seekInto(L())
        say(`Opened ${x.name}. Save scene with the same name writes over it.`)
      }
      ui.scenes.append(root)
    }
  }

  /* ── templates: a device, a backdrop and a camera in one press, on the selected layer ── */
  let templateOn = null
  const paintTemplates = () => {
    ui.templates.replaceChildren()
    for (const t of rt.SHOWCASE_TEMPLATES) {
      const { root, el } = mountRow('showcase-template')
      el.name.textContent = t.name
      el.note.textContent = t.note
      el.device.textContent = t.device === 'none' ? 'CARD' : t.device.toUpperCase()
      root.classList.toggle('showcase__template--on', templateOn === t.name)
      root.onclick = () => {
        templateOn = t.name
        const { name, note, look, ...values } = t
        for (const [k, v] of Object.entries(values)) setProp(L(), k, v)
        scene.look = looks.find((l) => l.name === look)?.look ?? scene.look
        sceneLook = rt.decodeLook(scene.look)
        buildRail()
        paintTemplates()
        paintLayers()
      ...
      ui.templates.append(root)
    }
  }

  /* ── the project's footage: a click puts it on the selected layer ── */
  const paintMedia = async () => {
    ...
    const d = await fetch(`/api/project/media?project=${encodeURIComponent(id)}`)
      .then(responseJson)
      .catch((e) => ({ error: e.message }))
    const files = (d.catalog?.files ?? []).filter((f) => f.kind === 'video' || f.kind === 'still')
    ui.mediaEmpty.hidden = files.length > 0
    ui.mediaCount.textContent = files.length ? `· ${files.length}` : ''
    for (const f of files) {
      const { root, el } = mountRow('showcase-card')
      const media = `/media/${encodeURIComponent(id)}/${encodeURI(f.rel)}`
      root.dataset.media = media
      root.classList.toggle('showcase__card--on', media === L().media)
      el.art.style.setProperty('--pic', `url('/thumb/${encodeURIComponent(id)}/${encodeURI(f.rel)}')`)
      el.name.textContent = f.rel
      el.kind.textContent = f.kind === 'video' ? 'CLIP' : 'STILL'
      root.onclick = () => {
        L().media = media
        paintLayers()
        apply()
      }
      ui.media.append(root)
    }
  }
  ...
  const tick = (now) => {
    if (!playing) return
    const ms = (now - base) % totalMs()
    scrub.value = String(Math.round(ms))
    clock.textContent = `${Math.round(ms)} ms`
    rt.RM.seek(ms)
    paintPlayhead()
    if (!(Math.round(ms) % 6)) paintGizmo()
    raf = requestAnimationFrame(tick)
  }
  playBtn.onclick = () => {
    playing = !playing
    playBtn.lastChild.textContent = playing ? 'Pause' : 'Play'
    if (playing) {
      base = performance.now() - Number(scrub.value)
      raf = requestAnimationFrame(tick)
  ...
  /* ── outlets ── */
  copyButton(ui.copyTag, 'Copy tag', () => tag())
  ui.png.onclick = async () => {
    ui.png.disabled = true
    ui.png.lastChild.textContent = 'Drawing…'
    const name = `showcase ${nameOf(L()).replace(/\.[^.]+$/, '')}`
    const r = await post('/api/showcase/png', { tag: tag(), projectId: currentProject(), name, width: exportSettings.width, shape: exportSettings.shape, atMs: Number(scrub.value) })
    ui.png.disabled = false
    ui.png.lastChild.textContent = 'Save PNG'
    if (r.error) return say(r.error, 'bad')
    say(`Saved ${r.where}.`)
    if (r.rel) void paintMedia()
  }

  /* ── boot ── */
  void (async () => {
    rt = await runtimeModule()
    scene = { look: rt.LOOK_PRESETS[0]?.look ?? '', move: 'none', layers: [] }
    sceneLook = rt.decodeLook(scene.look)
    scene.layers.push(newLayer(null))
    const saved = await fetch('/api/looks')
      .then(responseJson)
      .catch(() => ({ looks: [] }))
    looks = [...rt.LOOK_PRESETS, ...(saved.looks ?? []).map((l) => ({ name: l.name, look: l.look }))]
    /* A look handed over from the Creator: the backdrop, and in the list. */
    let handed = null
    try {
      handed = sessionStorage.getItem('rm-showcase-look')
      sessionStorage.removeItem('rm-showcase-look')
    } catch {
      /* no storage */
    }
    if (handed) {
      if (!looks.some((l) => l.look === handed)) looks.unshift({ name: 'From the Creator', look: handed })
      scene.look = handed
      sceneLook = rt.decodeLook(handed)
    }
    buildRail()
    paintTemplates()
    paintLayers()
    ...
    await paintMedia()
    await paintScenes()
    /* The first clip, so the stage is never a prompt when there is footage. */
    if (!L().media && ui.media.firstElementChild) {
      L().media = ui.media.firstElementChild.dataset.media
      paintLayers()
      apply()
    }
  })()
...
/*
 * The look's dials, for any page that has a look to edit.
 *
 * Built for the Creator and lifted out when the Showcase needed to edit its
 * backdrop in place: stops, then every group of the schema behind its switch,
 * the picture as an upload. `look()` returns the decoded look to edit;
 * `onChange()` is called after every change; everything lands in `host`.
 */
function lookDials({ rt, look, onChange, host, say = toast }) {
  const sec = (t) => {
    const heading = mountRow('designer-sec').root
    heading.textContent = t
    host.append(heading)
  ...
  const rangeRow = (box, f) => {
    const { nodes, el: row } = mountRow('designer-range')
    row.label.textContent = f.label
    const i = Object.assign(row.input, { min: f.min, max: f.max, step: f.step, value: look()[f.key] })
    const show = () => (row.readout.textContent = String(Number(i.value)))
    show()
    i.oninput = () => {
      look()[f.key] = Number(i.value)
      show()
      onChange()
    }
    box.append(...nodes)
  ...
  const selectRow = (box, f) => {
    const { nodes, el: row } = mountRow('designer-select')
    row.label.textContent = f.label
    for (const o of f.options) row.input.append(new Option(o, o))
    row.input.value = look()[f.key]
    row.input.onchange = () => {
      look()[f.key] = row.input.value
      onChange()
    }
    box.append(...nodes)
    return row.input
  }
  const toggleRow = (box, f) => {
    const { nodes, el: row } = mountRow('creator-toggle')
    row.label.textContent = f.label
    row.input.id = `creator-toggle-${f.key}`
    Object.assign(row.track, { textContent: f.label, htmlFor: row.input.id })
    row.input.checked = Boolean(look()[f.key])
    row.input.onchange = () => {
      look()[f.key] = row.input.checked ? 1 : 0
      onChange()
    }
    box.append(...nodes)
    return row.input
  }
  const colorRow = (box, f) => {
    const { nodes, el: row } = mountRow('designer-color')
    row.label.textContent = f.label
    row.readout.textContent = look()[f.key]
    row.label.after(
      colorMenu({
        families: brandFamilies(),
        value: look()[f.key],
  ...
  /* Stops: a colour and a place, up to six. */
  const paintStops = () => {
    stopsBox.replaceChildren()
    look().stops.forEach((stop, idx) => {
      const { root, el } = mountRow('creator-stop')
      el.pos.value = Math.round(stop.p * 100)
      el.readout.textContent = `${Math.round(stop.p * 100)}%`
      // The brand picker, not a text field: every colour in a look should be
      // one of ours, and the ladder is how you find the right step of it.
      el.pick.append(
        colorMenu({
          families: brandFamilies(),
          value: stop.c,
      ...
      el.pos.oninput = () => {
        stop.p = Number(el.pos.value) / 100
        el.readout.textContent = `${el.pos.value}%`
        onChange()
      }
      el.remove.disabled = look().stops.length <= 2
      el.remove.onclick = () => {
        look().stops.splice(idx, 1)
        paintStops()
        onChange()
      }
      stopsBox.append(root)
    })
    if (look().stops.length < rt.LOOK_MAX_STOPS) {
      const add = control('button', { className: 'btn ghost', textContent: 'Add a stop' })
      add.onclick = () => {
        const last = look().stops[look().stops.length - 1]
        look().stops.push({ c: last.c, p: Math.min(1, last.p + 0.1) })
        paintStops()
        onChange()
      }
      stopsBox.append(add)
  ...
  /*
   * What "on" means for each group, and how to get there.
   *
   * Most groups have a yes/no key. The rest are on when their amount is not
   * zero, or their mode is not "none"; switching those on gives them a first
   * setting worth seeing, and switching them off puts the zero back.
   */
  const def = (key) => rt.LOOK_SCHEMA.find((f) => f.key === key).def
  const flag = (key) => ({ key, is: () => Boolean(look()[key]), on: () => (look()[key] = 1), off: () => (look()[key] = 0) })
  const amount = (keys, first) => ({
    is: () => keys.some((k) => Number(look()[k]) > 0),
    on: () => (look()[keys[0]] = first ?? def(keys[0])),
    off: () => keys.forEach((k) => (look()[k] = 0)),
  })
  const mode = (key, first) => ({ is: () => look()[key] !== 'none', on: () => (look()[key] = first), off: () => (look()[key] = 'none') })
  ...
  const GROUP_GATE = {
    ...
    motion: flag('an'),
    light: mode('lt', 'spot'),
    dither: mode('dt', 'bayer4'),
    optics: amount(['sf', 'ab'], 0.3),
    pixelate: amount(['px'], 8),
    grain: amount(['g']),
    /* The picture is on once one is chosen; switching on first shows the upload. */
    picture: { is: () => pictureOpen || Boolean(look().img), on: () => (pictureOpen = true), off: () => ((pictureOpen = false), (look().img = '')) },
  }

  let stopsBox = null
    sec('Stops')
    stopsBox = mountRow('creator-stops').root
    host.append(stopsBox)
    paintStops()
    for (const [group, label] of rt.LOOK_GROUPS) {
      let fields = rt.LOOK_SCHEMA.filter((f) => f.group === group)
      if (!fields.length) continue
      const gate = GROUP_GATE[group]
      let box
      if (gate) {
        /*
         * The heading carries the group's switch, and the group's own on/off
         * dial, where it has one, is that switch: two ways to say "waves" is one
         * too many. The dials below stay hidden until the switch is on.
         */
        const { root: heading, el: head } = mountRow('creator-sec')
        head.title.textContent = label
        head.input.id = `creator-sec-${group}`
        Object.assign(head.track, { textContent: label, htmlFor: head.input.id })
        host.append(heading)
        box = g()
        if (gate.key) fields = fields.filter((f) => f.key !== gate.key)
        const paintGate = () => {
          head.input.checked = gate.is()
          box.hidden = !head.input.checked
        }
        paintGate()
        head.input.onchange = () => {
          if (head.input.checked) gate.on()
          else gate.off()
          paintGate()
          onChange()
        }
      ...
      if (group === 'picture') {
        /*
         * An upload, not a URL field. The file goes where added brand assets go
         * and is served from there, so the same look draws in the Studio, in a
         * saved scene and in a render — the staging copies added pictures in.
         */
        const { nodes, el: row } = mountRow('creator-picture')
        const paintPicture = () => {
          row.name.textContent = look().img ? look().img.split('/').pop() : 'none'
          row.clear.hidden = !look().img
        }
        paintPicture()
        row.pick.onclick = () => row.file.click()
        row.clear.onclick = () => {
        ...
        row.file.onchange = async () => {
          const file = row.file.files?.[0]
          row.file.value = ''
          if (!file) return
          row.pick.disabled = true
          const r = await fetch(`/api/brand/asset?name=${encodeURIComponent(file.name)}`, { method: 'POST', headers: { 'content-type': 'application/octet-stream' }, body: file, duplex: 'half' })
            .then(responseJson)
            .catch((e) => ({ error: e.message }))
          row.pick.disabled = false
          if (r.error) return say(r.error, 'bad')
          look().img = `/added/${r.file}`
          paintPicture()
          onChange()
        }
        box.append(...nodes)
      ...
      for (const f of fields) {
        if (f.type === 'range') rangeRow(box, f)
        else if (f.type === 'select') selectRow(box, f)
        else if (f.type === 'toggle') toggleRow(box, f)
        else if (f.type === 'color') colorRow(box, f)
      }
    }
}

/**
 * Creator: one look, every treatment.
 *
 * The designer for <rm-look> (components/rm-video.js), which is where the
 * shader, the schema, the presets and the look codec live — this page only
 * arranges dials around them. The look string is the design: the textarea
 * shows it, Copy look copies it, a saved row keeps it, and the Framer component
 * takes it as its Look property.
 */
function vCreator(m) {
  const ui = mountPanel('creator', m)
  const { look: stage, scrub, clock, markup, play: playBtn } = ui
  let rt = null
  let L = null
  let saved = []
  let stateLook = null
  ...
  /* The look, as the element and the text both see it. One draw per frame: a
     dragged slider fires far faster than the screen can show. */
  let applyQueued = false
  let applyFromText = false
  const apply = ({ fromText = false } = {}) => {
    applyFromText = applyFromText || fromText
    if (applyQueued) return
    applyQueued = true
    requestAnimationFrame(() => {
      applyQueued = false
      const text = rt.encodeLook(stateLook)
      stage.setAttribute('look', text)
      if (!applyFromText) ui.lookText.value = text
      applyFromText = false
      markup.textContent = `<rm-look at="0" for="8000" look="${esc(text)}"></rm-look>`
      // No seek here: the element redraws itself on the attribute change, and a
      // seek on top of it drew every frame twice, once small and once full.
    })
  }
  const tag = () => `<rm-look at="0" for="8000" look="${esc(rt.encodeLook(stateLook))}"></rm-look>`

  /* ── the dials, from the schema ── */
  const panel = mountRow('designer-panel').root
  const sec = (t) => {
    const heading = mountRow('designer-sec').root
    heading.textContent = t
  ...
  const buildDials = () => {
    panel.replaceChildren()
    lookDials({ rt, look: () => stateLook, onChange: apply, host: panel, say })
    /* Export settings are settings: the PNG's width and shape sit here, and the
       footer keeps only the buttons that act on them. */
    sec('Export')
    const ex = g()
    const sizeRow = mountRow('designer-select')
    sizeRow.el.label.textContent = 'PNG width'
    ...
    const shapeRow = mountRow('designer-select')
    shapeRow.el.label.textContent = 'PNG shape'
    for (const [v, l] of [['16/9', '16:9'], ['1/1', '1:1'], ['4/3', '4:3'], ['3/2', '3:2'], ['21/9', '21:9'], ['9/16', '9:16'], ['4/5', '4:5'], ['3/4', '3:4']]) shapeRow.el.input.append(new Option(l, v))
    shapeRow.el.input.value = exportSettings.shape
    shapeRow.el.input.onchange = () => (exportSettings.shape = shapeRow.el.input.value)
    ex.append(...shapeRow.nodes)
    /* The composition outlets: the same insert every designer shares, but in
       the rail, because two selects and a name field are settings, not actions. */
    sec('Into a composition')
    const outlets = g()
    outlets.append(...sceneOutlets({ tag, startMs: () => Number(scrub.value), name: 'look', durationMs: 8000 }))
    intoRail(panel)
  }

  /* ── presets and saved looks, as live thumbnails ── */
  /*
   * Thumbnails are stills, drawn one at a time off-screen and cached by look.
   * Live elements here meant a dozen WebGL contexts each redrawing on every
   * seek, which is what made the page drag.
   */
  const thumbs = new Map()
  let thumbQueue = Promise.resolve()
  const thumbFor = (look, img) => {
    if (thumbs.has(look)) {
      img.src = thumbs.get(look)
      return
    }
    thumbQueue = thumbQueue.then(async () => {
      try {
        const url = await rt.renderLook({ look, width: 192, height: 108, ms: 1200 })
        thumbs.set(look, url)
        img.src = url
      } catch {
        /* a thumbnail that cannot draw leaves the card's ground showing */
      }
    })
  }
  const presetCard = ({ name, look, id = null, editable = false }) => {
    const { root, el } = mountRow('creator-preset')
    thumbFor(look, el.art)
    el.pick.setAttribute('aria-label', `Use ${name}`)
    el.name.value = name
    el.name.readOnly = !editable
    root.onclick = (e) => {
      if (e.target === el.name && editable) return
      stateLook = rt.decodeLook(look)
      onId = id
      buildDials()
      apply()
      say(`Loaded ${name}.`, 'ok')
    ...
    if (editable) {
      el.name.onclick = (e) => e.stopPropagation()
      el.name.onkeydown = (e) => {
        if (e.key === 'Enter') el.name.blur()
      }
      el.name.onblur = async () => {
        ...
        const r = await post('/api/looks', { id, name: next }, 'PATCH')
        if (r.error) {
          el.name.value = name
          return say(r.error, 'bad')
        }
        ...
        actionMenu([
          {
            ...
            busy: 'Deleting…',
            run: async () => {
              const r = await post('/api/looks', { id }, 'DELETE')
              if (r.error) return r.error
  ...
  const paintPresets = () => ui.presets.replaceChildren(...rt.LOOK_PRESETS.map((p) => presetCard(p)))
  const paintSaved = () => {
    ui.saved.replaceChildren(...saved.map((row) => presetCard({ name: row.name, look: row.look, id: row.id, editable: true })))
    ui.savedCount.textContent = saved.length ? `${saved.length}` : ''
  ...
  /* ── the look as text ── */
  ui.lookText.onchange = () => {
    stateLook = rt.decodeLook(ui.lookText.value)
    buildDials()
    apply({ fromText: true })
  }

  /* ── outlets ── */
  copyButton(ui.copyLook, 'Copy look', () => rt.encodeLook(stateLook))
  /* Straight to the Showcase with this look as the backdrop, saved or not. */
  ui.toShowcase.onclick = () => {
    try {
      sessionStorage.setItem('rm-showcase-look', rt.encodeLook(stateLook))
    } catch {
      /* no storage: the Showcase falls back to its presets */
    }
    go('showcase')
  }
  /*
   * Onto the Framer canvas in one press: the server drives Framer's CLI against
   * the open project, makes the RoleModelLook code file once, and adds an
   * instance with this look in its control. No paste, no second component.
   */
  ui.toFramer.onclick = async () => {
    ui.toFramer.disabled = true
    ui.toFramer.lastChild.textContent = 'Adding to Framer…'
    say('Placing the look on the Framer canvas — this takes a few seconds.')
    const r = await post('/api/looks/framer', { look: rt.encodeLook(stateLook), source: rt.lookFramerSource() })
    ui.toFramer.disabled = false
    ui.toFramer.lastChild.textContent = 'Add to Framer'
    if (r.error) return say(r.error, 'bad')
    say('Added to the Studio looks page in Framer (/studio-looks). Copy it from there to where it goes.', 'ok')
  }
  ui.png.onclick = async () => {
    const width = Number(exportSettings.width)
    const [aw, ah] = exportSettings.shape.split('/').map(Number)
    const height = Math.round((width * ah) / aw)
    ui.png.disabled = true
    try {
      const dataUrl = await rt.renderLook({ look: stateLook, width, height, ms: Number(scrub.value) })
      const name = saved.find((x) => x.id === onId)?.name || 'look'
      const r = await post('/api/looks/png', { dataUrl, name, projectId: currentProject() || null })
      if (r.error) throw new Error(r.error)
      say(`Saved ${width}×${height} as ${r.where}.`, 'ok')
      if (currentProject()) S = await responseJson(await fetch('/api/state'))
    } catch (e) {
      say(e.message, 'bad')
    }
    ui.png.disabled = false
  ...
  ui.save.onclick = async () => {
    const name = await askName('Name this look', saved.find((x) => x.id === onId)?.name || 'New look', { lede: 'Saved for the team; the Showcase offers it as a backdrop.' })
    if (!name) return
    ui.save.disabled = true
    try {
      const preview = await rt.renderLook({ look: stateLook, width: 480, height: 270, ms: Number(scrub.value) })
      const r = await post('/api/looks', { name: name.trim(), look: rt.encodeLook(stateLook), preview, projectId: currentProject() || null })
      if (r.error) throw new Error(r.error)
      saved.unshift(r.look)
      onId = r.look.id
      paintSaved()
      say(`Saved ${r.look.name} for the team.`, 'ok')
    } catch (e) {
      say(e.message, 'bad')
    }
    ui.save.disabled = false
  ...
  /* ── load ── */
  ;(async () => {
    rt = await loadFieldRuntime()
    stateLook = rt.decodeLook(rt.LOOK_PRESETS[0].look)
    buildDials()
    paintPresets()
    apply()
    const r = await fetch('/api/looks').then(responseJson).catch((e) => ({ looks: [], problem: e.message }))
    saved = r.looks ?? []
    paintSaved()
    if (r.problem) say(`Saved looks are off: ${r.problem}`, 'warn')
  })()
}
...
function vWallpapers(m) {
  ...
  const paint = () => {
    ...
    for (const r of recipes) {
      ...
      markCurrent(c, editing?.name === r.name)
      const cv = cardEl.cv
      cv.width = THUMB_W
...
function buildEditor(r, onSaved) {
  ...
  save.onclick = async () => {
    ...
    window.posthog?.capture('wallpaper_saved', { wallpaper_name: r.name || null })
...
async function loadFieldRuntime() {
  fieldRuntime ??= await runtimeModule()
  return fieldRuntime
}
...
function vTimeline(m) {
  ...
  const { lede, empty, stage, video, scene, bed, gap, play, clock, under, stat, save, canvas, pick, add, shelf, shelfProject, shelfList, shelfEmpty, shelfToggle, shelfEditor, editBack, editFull, editInsert, editSave, editName, editWallpaper, editParts, editStatus, slice, removeClip, rippleClip, zoomIn, zoomOut, zoomFit, exportParts, exportVideo, shelfNew, newName, newBrief, draftScene, newStatus } = ui
  const project = currentProject()
  if (!project) {
    stage.hidden = true
    empty.hidden = false
  ...
  /* The button says what it will do next, in the icon and to a screen reader
     both — there is no label left to carry it. */
  const setPlay = () => {
    ui.playIcon?.setAttribute('name', playing ? 'pause' : 'play')
   ...
   */
  const graphics = () => (cut?.tracks ?? []).filter((t) => t.kind === 'graphic').flatMap((t) => t.clips ?? [])

  /* A graphic clip names its element three ways depending on how it was built:
     a mounted sub-composition, a HyperFrames id, or a plain element id. */
  const hostFor = (doc, clip) =>
    (clip.name && doc.querySelector(`[data-composition-id="${CSS.escape(clip.name)}"]`))
    || doc.querySelector(`[data-hf-id="${CSS.escape(clip.id)}"]`)
  ...
  /*
   * Fill the composition's seams in the source, then hand over the whole page.
   *
   ...
   * of this showed a blank overlay while insisting eight scenes were there.
   *
   * Filled as text rather than in the live frame, because order is the point:
   * the composition's own inline scripts run while the page parses, and the
   * transcript's word tweens resolve their targets the moment they are created.
   * Filling a frame that had already loaded meant every cue landed on a selector
   * with nothing behind it, and the words never lit — present, seeked, and
   * permanently dark. The renderer fills the holes before the scripts run; so
   * does this. It also makes the seam scripts ordinary again: markup parsed by
   * the frame executes, so nothing has to be recreated by hand.
   */
  const mountScene = async () => {
    const base = new URL(sceneBase(), location.href).href
    const text = await fetch(base).then((r) => (r.ok ? r.text() : '')).catch(() => '')
    if (!text) return
    const parsed = new DOMParser().parseFromString(text, 'text/html')
    /* srcdoc resolves relative URLs against the Studio page; the composition's
       assets live under the scene route, and this keeps them pointing there. */
    const anchor = parsed.createElement('base')
    anchor.href = base
    parsed.head.prepend(anchor)
    for (const host of parsed.querySelectorAll('[data-composition-src]')) {
      ...
      for (const sheet of sub.querySelectorAll('style, link[rel=stylesheet]')) parsed.head.append(parsed.importNode(sheet, true))
      host.append(...[...sub.body.childNodes].map((node) => parsed.importNode(node, true)))
    }
    /* The composition's media elements point at the original files, and preload
       would fetch them even hidden. The editor never opens the original media:
       an element that becomes the picture is handed the proxy instead. */
    for (const media of parsed.querySelectorAll('[data-assembly-media]')) media.removeAttribute('src')
    scene.srcdoc = parsed.documentElement.outerHTML
  }
  ...
  const scenesAt = (t) => {
    ...
    try {
       ...
       */
      /*
       * Every graphic the cut knows about, shown only while it is under the
       * playhead.
       *
       * Switching just the mounts left the composition's full-frame elements —
       * its background haze above all — painting the whole time. In the piece
       * itself those sit *behind* the footage; here the whole composition is one
       * layer above the proxy, so the background came out on top and covered the
       * picture completely. It looked like the video was not playing.
       *
       * The cut already lists them as clips with their own start and end. Asking
       * it, rather than trying to read depth out of someone else's markup, is
       * both simpler and the same answer a render would give.
       */
      /* Each graphic by its own window, not "the one under the playhead":
         two overlapping graphics are a stack — a title over a closing shader
         is the intended picture — and showing only the later one cut the
         words off every card that shares its frame with a background. */
      for (const other of graphics()) {
        const host = hostFor(doc, other)
        if (host) host.style.visibility = t >= other.at && t < other.at + (other.out - other.in) ? 'visible' : 'hidden'
      }
      /*
       * Footage belongs wherever the composition puts it.
       *
       * A pip piece frames its speakers in small circles over the brand wall,
       * so a full-frame proxy under the iframe is the wrong picture twice: the
       * wrong size, and behind the wall — which is CSS on every composition
       * root, not an element the host switching ever touched. When the
       * composition holds a media element for the clip under the playhead,
       * that element is the picture. It is pointed at the proxy — never the
       * original — muted, because the editor's own element keeps the sound and
       * the clock, and seeked on the same playhead. Only footage with no such
       * home shows full frame, with the wallpaper stripped so it can be seen;
       * a graphics-only cut keeps its wallpaper, because there it IS the
       * picture.
       */
      const footage = under_(t, 'video')
      const home = footage ? hostFor(doc, footage) : null
      const pip = home && typeof home.play === 'function' ? home : null
      doc.documentElement.classList.toggle('tl-footage', Boolean(footage) && !pip)
      /*
       * A mounted graphic's own root carries the brand wall too — the wallpaper
       * rule matches every composition root, nested ones included — and it is a
       * later sibling of the pips, so it painted straight over the speaker: the
       * pip showed for the sliver before the first caption arrived and never
       * again. While footage is in its pips, a nested root keeps its words and
       * loses its wall; the top root is already providing the ground.
       */
      doc.documentElement.classList.toggle('tl-pip-footage', Boolean(pip))
      for (const el of doc.querySelectorAll('.tl-pip')) {
        if (el !== pip) {
          el.classList.remove('tl-pip')
          el.pause?.()
        }
      }
      if (pip) {
        if (pip.dataset.tlProxy !== footage.source) {
          pip.dataset.tlProxy = footage.source
          pip.muted = true
          pip.src = `/api/edit/cache/${encodeURIComponent(project)}/proxy/${footage.source}.mp4`
        }
        pip.classList.add('tl-pip')
        const into = footage.in + (t - footage.at)
        if (Math.abs(pip.currentTime - into) > (playing ? 0.25 : 1 / (cut?.fps || 60))) pip.currentTime = into
        if (playing && pip.paused) pip.play().catch(() => {})
        if (!playing && !pip.paused) pip.pause()
      }
      /* From the clip's own in point: a sliced graphic's second half carries
         the offset in `in`, and seeking from zero would restart its animation
         at the join. */
      if (clip) win.__timelines?.[clip.name]?.seek?.(Math.max(0, (clip.in ?? 0) + (t - clip.at)))
  ...
  const fit = () => {
    ...
    /* Every overlay frame, not just the composition's: an inserted scene is a
       sibling with the same class and the same registration problem. */
    for (const fr of scene.parentElement.querySelectorAll('.tl__scene')) {
      fr.style.inlineSize = `${cut.width}px`
      fr.style.blockSize = `${cut.height}px`
      fr.style.setProperty('--tl-scale', String(box.width / cut.width))
    }
  }

  /*
   * Scenes inserted from the shelf.
   *
   * An inserted scene is not part of this composition, so the mounted iframe
   * knows nothing about it. Each one gets a frame of its own — the same
   * preview the builder and the gallery render — shown while its clip is under
   * the playhead and seeked from the same clock as everything else.
   */
  const insertedFrames = new Map()
  const shelfScenes = new Map()
  const scenesOf = (proj) => {
    if (!shelfScenes.has(proj)) {
      shelfScenes.set(proj, fetch('/api/scenes?project=' + encodeURIComponent(proj)).then(responseJson).then((r) => r.scenes ?? []).catch(() => []))
    }
    return shelfScenes.get(proj)
  }
  const previewUrl = async (proj, sc) => {
    const r = await fetch('/api/scene/preview', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        body: sc.body,
        name: sc.name,
        /* The ground it was designed on, not the default — same rule as the
           gallery, because this is the same picture at a different size. */
        ...(sc.ground?.wallpaper ? { wallpaper: sc.ground.wallpaper } : {}),
        ...(sc.ground?.brand ? { brand: sc.ground.brand } : {}),
        ...(sc.footage?.rel
          ? { footage: { src: `/media/${encodeURIComponent(proj)}/${encodeURI(sc.footage.rel)}`, inSec: sc.footage.inSec, outSec: sc.footage.outSec } }
          : {}),
      }),
    }).then(responseJson).catch(() => null)
    return r?.url ?? null
  }
  const insertedFrame = (clipRef) => {
    if (insertedFrames.has(clipRef.id)) return insertedFrames.get(clipRef.id)
    /* Cloned from the composition's frame rather than built here: the template
       owns the markup, and the clone carries its class, title and tabindex. */
    const fr = scene.cloneNode(false)
    fr.hidden = true
    fr.removeAttribute('srcdoc')
    scene.parentElement.append(fr)
    insertedFrames.set(clipRef.id, fr)
    ;(async () => {
      const list = await scenesOf(clipRef.scene?.project ?? project)
      const sc = list.find((s) => s.name === clipRef.name)
      const url = sc ? await previewUrl(clipRef.scene?.project ?? project, sc) : null
      if (!url) return
      fr.onload = async () => {
        fit()
        try {
          await fr.contentWindow?.RM?.ready?.()
        } catch {
          /* torn down while loading */
        }
        draw()
      }
      fr.src = url
  ...
  const showInserted = (clipRef) => {
    /* While a scene is being edited, its stage owns the picture — a clip
       preview popping over the thing being edited would read as the edit
       going wrong. */
    if (editing) clipRef = null
    for (const [id, fr] of insertedFrames) if (id !== clipRef?.id) fr.hidden = true
    if (!clipRef) return
    const fr = insertedFrame(clipRef)
    fr.hidden = false
    try {
      fr.contentWindow?.RM?.seek?.(Math.max(0, ((clipRef.in ?? 0) + (state.playhead - clipRef.at)) * 1000))
    } catch {
      /* still loading; the next paint catches it */
    }
  }
  ...
  scene.onload = async () => {
    ...
    /*
     * The mounted-root rule is layout, not styling. A filled seam's root is a
     * full-frame block, and blocks stack: say-1 sat in the frame and every
     * later mount flowed on below it — 1080, 2160, 3240 — clipped by the root's
     * overflow, which read as "only the first say works". The renderer pins a
     * mounted composition onto the frame; mounting is the editor's job here, so
     * pinning is too.
     */
    const style = doc.createElement('style')
    style.textContent = 'html,body{background:transparent !important}[data-assembly-media],[data-assembly-clock]{display:none !important}[data-assembly-media].tl-pip{display:block !important}html.tl-footage [data-composition-id]{background:none !important}html.tl-pip-footage [data-composition-id] [data-composition-id]{background:none !important}[data-composition-src] > [data-composition-id]{position:absolute !important;inset-block-start:0;inset-inline-start:0}'
  ...
  const frame = () => {
    ...
    const pip = scenesAt(state.playhead)
    /* The composition can be the picture with no graphic on at all: a pip clip
       lives inside it, so the frame stays up while one is playing. */
    scene.hidden = !graphic && !pip
    /* When the piece frames the footage itself, a full-frame proxy under it
       would be the picture twice. It keeps running — it is the clock and the
       sound — it just is not shown. */
    video.style.visibility = pip ? 'hidden' : ''
    showInserted(graphic?.scene ? graphic : null)
    sound()
  ...
  const draw = () => {
    ...
    requestAnimationFrame(() => {
      ...
      save.disabled = !dirty
      removeClip.hidden = !state.selection
      rippleClip.hidden = !state.selection
      frame()
    })
  }
  ...
  const chooseFrom = (compositions) => {
    ...
    if (!compositions.length) {
      /*
       * Footage is the thing an editor exists to cut.
       *
       * This used to be a dead end: a brand-new project with recordings in it
       * and no composition answered "nothing to edit", and the way forward was
       * in another panel. And the first button here cut EVERYTHING, which put
       * the wrong take first the very first time it was used — so the footage
       * is a list to choose from, not a fate.
       */
      empty.append(control('hint', { textContent: 'No composition here yet — pick the footage to cut. Click takes in the order they should play; rest on one to preview it.' }))
      ;(async () => {
        const media = await fetch(`/api/project/media?project=${encodeURIComponent(project)}`).then(responseJson).catch(() => null)
        const videos = (media?.catalog?.files ?? []).filter((x) => x.kind === 'video')
        if (!videos.length) {
          empty.append(control('hint', { textContent: 'No footage yet either — record something, or drop a video into the project.' }))
          return
        }
        const { root: pickerRoot, el: picker } = mountRow('timeline-takes')
        /* Click order IS the running order, so the choice is a list, not a set. */
        const chosen = []
        const cards = []
        const paint = () => {
          for (const c of cards) {
            const idx = chosen.indexOf(c.rel)
            c.root.classList.toggle('tl__take--on', idx !== -1)
            c.root.setAttribute('aria-pressed', String(idx !== -1))
            c.order.hidden = idx === -1
            if (idx !== -1) c.order.textContent = String(idx + 1)
          }
          picker.start.disabled = !chosen.length
          picker.start.textContent = chosen.length
            ? `Cut ${chosen.length} selected take${chosen.length === 1 ? '' : 's'}`
            : 'Cut the selected footage'
        }
        for (const v of videos) {
          const { root: card, el: take } = mountRow('timeline-take')
          take.shot.style.backgroundImage = `url('/thumb/${encodeURIComponent(project)}/${encodeURI(v.rel)}')`
          take.name.textContent = v.name
          take.name.title = v.name
          take.meta.textContent = [v.media?.durationSec ? dur(v.media.durationSec) : null, human(v.bytes)].filter(Boolean).join(' · ')
          card.onclick = () => {
            const i = chosen.indexOf(v.rel)
            if (i === -1) chosen.push(v.rel)
            else chosen.splice(i, 1)
            paint()
          }
          /* The preview streams only while the pointer rests on the card, and
             the file is only asked for on the first visit. */
          card.onpointerenter = () => {
            if (!take.clip.src) take.clip.src = `/media/${encodeURIComponent(project)}/${encodeURI(v.rel)}`
            take.clip.hidden = false
            take.clip.play().catch(() => {})
          }
          card.onpointerleave = () => {
            take.clip.pause()
            take.clip.hidden = true
          }
          cards.push({ root: card, order: take.order, rel: v.rel })
          picker.grid.append(card)
        }
        /* One take is not a choice to make — it arrives ready to cut. */
        if (videos.length === 1) chosen.push(videos[0].rel)
        paint()
        picker.start.onclick = async () => {
          if (!chosen.length) return
          picker.start.disabled = true
          picker.start.textContent = 'Reading the footage…'
          const made = await fetch('/api/edit/start', {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ project, rels: [...chosen] }),
          }).then(responseJson).catch((e) => ({ error: e.message }))
          if (made.error) {
            paint()
          ...
          timelineFolder = made.folder
          go('timeline')
        }
        empty.append(pickerRoot)
      })()
      return
    }
  ...
  ;(async () => {
    ...
    ui.audio.hidden = !tracks.length

    /* Every project the Studio knows, because a scene saved anywhere is fair
       game here. The one being edited is the default, not the limit. */
    for (const p of S?.projects ?? []) shelfProject.append(new Option(p.name ?? p.id, p.id, false, p.id === project))
    ...
    mod.input.attachTimeline(canvas, {
      ...
      onView: draw,
      /* One undo entry per gesture: the snapshot was taken at pointerdown,
         and it is kept only if the gesture actually changed the cut. */
      commit: keepGesture,
    })
    fill()
    draw()
  ...
  /*
   * The scene shelf: saved scenes, from any project, inserted at the playhead.
   *
   * A drawer rather than a page, because inserting is something you do while
   * looking at the cut — the picture contracts to make room and comes back
   * when the shelf closes. Inserting only edits the cut in hand; Save cut is
   * still the thing that writes, same as a drag is.
   */
  let shelfLoaded = null
  const loadShelf = async () => {
    const proj = shelfProject.value || project
    shelfLoaded = proj
    shelfList.replaceChildren()
    shelfEmpty.hidden = true
    const scenes = await scenesOf(proj)
    if (shelfLoaded !== proj) return
    shelfEmpty.hidden = scenes.length > 0
    for (const sc of scenes) {
      const { root: cardRoot, el: card } = mountRow('timeline-scene')
      card.name.textContent = sc.name
      /* Scaled on every resize, not once on load — the drawer is animating
         open while these cards find their width. */
      const fitCard = () => {
        const w = card.shot.clientWidth
        if (w > 0 && cut?.width) card.frame.style.transform = `scale(${w / cut.width})`
      }
      new ResizeObserver(fitCard).observe(card.shot)
      ;(async () => {
        const url = await previewUrl(proj, sc)
        if (!url) return
        card.frame.title = sc.name
        card.frame.onload = async () => {
          fitCard()
          try {
            const rm = card.frame.contentWindow?.RM
            await rm?.ready?.()
            /* The scene's own length, kept on the card so Insert can state an
               out point — half of it is also where the picture is worth
               showing, for the same reason the gallery seeks there. */
            const ms = rm?.duration?.() || 4000
            cardRoot.dataset.ms = String(ms)
            rm?.seek?.(Math.round(ms / 2))
          } catch {
            /* torn down while loading; the card stays as it is */
          }
        }
        card.frame.hidden = false
        card.frame.src = url
      })()
      card.edit.onclick = () => openSceneEditor(proj, sc)
      card.insert.onclick = () => insertSceneClip(proj, sc.name, (Number(cardRoot.dataset.ms) || 4000) / 1000)
      shelfList.append(cardRoot)
    }
  }
  shelfToggle.onclick = () => {
    const open = shelf.classList.toggle('tl__shelf--open')
    shelfToggle.setAttribute('aria-expanded', String(open))
    if (open && shelfLoaded === null) loadShelf()
    if (!open) closeSceneEditor()
  }
  shelfProject.onchange = () => {
    closeSceneEditor()
    loadShelf()
  }

  /*
   * The scene editor, in place. The stage is the viewer: the same preview the
   * Scenes page renders mounts over the picture, so a scene is edited exactly
   * where the cut will show it. The shelf swaps its cards for the fields.
   *
   * Deliberately the errand-sized editor: the words a part already carries,
   * and when it runs. The palette, footage and revisions live on the Scenes
   * page, one click away — a second full builder here would be a second thing
   * to keep in step.
   */
  let editCat = null
  const catalogue = async () => {
    editCat ??= await fetch('/api/compose/catalogue').then(responseJson).catch(() => null)
    return editCat
  }
  const escAttr = (v) => String(v ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/"/g, '&quot;')
  /* The same reading the Scenes page does, plus the style attribute — that is
     where an accent lives, and dropping it on a round-trip would repaint a
     scene nobody touched. */
  const partsFrom = (html, components) => {
    const parsed = new DOMParser().parseFromString(`<div>${html}</div>`, 'text/html')
    const found = []
    for (const node of parsed.querySelectorAll('*')) {
      const spec = components.find((c) => c.tag === node.tagName.toLowerCase())
      if (!spec) continue
      const attrs = {}
      for (const fld of spec.fields) {
        const v = node.getAttribute(fld)
        if (v != null && fld !== 'at' && fld !== 'for') attrs[fld] = v
      }
      found.push({ tag: spec.tag, at: Number(node.getAttribute('at')) || 0, holds: Number(node.getAttribute('for')) || 2600, attrs, style: node.getAttribute('style') || '' })
    }
    return found
  }
  const partsMarkup = (parts) =>
    parts
      .map((e) => {
        const attrs = [`at="${Number(e.at) || 0}"`, `for="${Number(e.holds) || 2600}"`]
        for (const [k, v] of Object.entries(e.attrs)) if (String(v ?? '').trim()) attrs.push(`${k}="${escAttr(v)}"`)
        const style = e.style ? ` style="${escAttr(e.style)}"` : ''
        return `<${e.tag} ${attrs.join(' ')}${style}></${e.tag}>`
      })
      .join('\n')
  ...
  let editTimer = null
  let editRequest = 0
  const sayEdit = (t, tone_) => {
    editStatus.hidden = !t
    editStatus.textContent = t ?? ''
    tone(editStatus, tone_)
  }
  const editPreview = async () => {
    ...
    const r = await fetch('/api/scene/preview', {
      ...
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        body: partsMarkup(editing.parts),
        wallpaper: editWallpaper.value,
        brand: editing.brand || null,
        name: editName.value,
        footage: editing.footage?.rel
          ? { src: `/media/${encodeURIComponent(editing.proj)}/${encodeURI(editing.footage.rel)}`, inSec: editing.footage.inSec, outSec: editing.footage.outSec }
          : null,
      }),
    }).then(responseJson).catch(() => null)
    if (request !== editRequest || !editing) return
    if (r?.url && editFrame) editFrame.src = r.url
  }
  const queuePreview = () => {
    clearTimeout(editTimer)
    editTimer = setTimeout(editPreview, 350)
  }
  const paintParts = () => {
    editParts.replaceChildren()
    for (const part of editing.parts) {
      const { root: row, el } = mountRow('timeline-part')
      row.open = editing.parts.length === 1
      el.tag.textContent = part.tag.replace(/^rm-/, '')
      const when = () => {
        el.when.textContent = `${(part.at / 1000).toFixed(1)}s → ${((part.at + part.holds) / 1000).toFixed(1)}s`
      }
      when()
      el.at.value = part.at
      el.holds.value = part.holds
      el.at.oninput = () => {
        part.at = Number(el.at.value) || 0
        when()
        queuePreview()
      }
      el.holds.oninput = () => {
        part.holds = Number(el.holds.value) || 0
        when()
        queuePreview()
      }
      for (const [fld, value] of Object.entries(part.attrs)) {
        if (!String(value ?? '').trim()) continue
        const { root: fieldRow, el: f } = mountRow('timeline-field')
        f.label.textContent = fld
        f.input.value = value
        f.input.oninput = () => {
          part.attrs[fld] = f.input.value
          queuePreview()
        }
        el.fields.append(fieldRow)
      }
      editParts.append(row)
    }
  }
  const openSceneEditor = async (proj, sc) => {
    const cat = await catalogue()
    if (!cat?.components) return void toast('The component catalogue did not load.', 'bad')
    if (playing) play.onclick()
    editing = { proj, saved: sc.name, brand: sc.ground?.brand ?? '', footage: sc.footage ?? null, parts: partsFrom(sc.body, cat.components) }
    editName.value = sc.name
    editWallpaper.replaceChildren(new Option('no wallpaper', ''))
    for (const w of cat.wallpapers ?? []) editWallpaper.append(new Option(w.label, w.name))
    editWallpaper.value = sc.ground?.wallpaper ?? ''
    editWallpaper.onchange = queuePreview
    editName.oninput = queuePreview
    /* Deep work — the palette, footage, revisions — is the Scenes page's. The
       door only opens onto the project it can actually edit. */
    editFull.hidden = proj !== project
    editFull.onclick = () => {
      openScene = sc.name
      go('scenes')
    }
    ...
    shelfEmpty.hidden = true
    shelfNew.hidden = true
    shelfEditor.hidden = false
    if (!editFrame) {
      editFrame = scene.cloneNode(false)
      editFrame.removeAttribute('srcdoc')
      editFrame.onload = async () => {
        fit()
        try {
          const rm = editFrame.contentWindow?.RM
          await rm?.ready?.()
          rm?.seek?.(Math.round((rm?.duration?.() || 4000) / 2))
        } catch {
          /* torn down while loading */
      ...
      scene.parentElement.append(editFrame)
    }
    editFrame.hidden = false
    editPreview()
  }
  const closeSceneEditor = () => {
    ...
    shelfList.hidden = false
    shelfNew.hidden = false
    if (editFrame) editFrame.hidden = true
    draw()
  }
  editBack.onclick = closeSceneEditor
  editSave.onclick = async () => {
    if (!editing) return
    if (!editName.value.trim()) {
      sayEdit('Give the scene a name first — it becomes the filename.', 'bad')
      editName.focus()
      return
    }
    ...
    const r = await fetch('/api/scene', {
      ...
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        projectId: editing.proj,
        name: editName.value,
        body: partsMarkup(editing.parts),
        wallpaper: editWallpaper.value,
        brand: editing.brand || '',
        ...(editing.footage ? { footage: editing.footage } : {}),
      }),
    }).then(responseJson).catch((e) => ({ error: e.message }))
    editSave.disabled = false
    if (r.error) return void sayEdit(r.error, 'bad')
    sayEdit(`Saved to ${r.file}`, 'ok')
    /* The shelf and every inserted copy now describe an older scene: refetch
       the cards, and drop the mounted frames so the next paint remounts them
       against what was just written. */
    shelfScenes.delete(editing.proj)
    for (const [id, fr] of [...insertedFrames]) {
      const clipRef = (cut?.tracks ?? []).flatMap((tr) => tr.clips ?? []).find((c) => c.id === id)
      if (clipRef?.name === r.name && (clipRef.scene?.project ?? project) === editing.proj) {
        fr.remove()
        insertedFrames.delete(id)
      }
    }
    editing.saved = r.name
    loadShelf()
  }

  /* One insert, from a card or from the editor: a graphic clip at the
     playhead, carrying its home project so the preview — and one day the
     render — can find the file again. */
  const insertSceneClip = (proj, sceneName, seconds) => {
    if (!cut) return
    let track = cut.tracks.find((t) => t.kind === 'graphic')
    if (!track) {
      track = { id: 'g1', kind: 'graphic', clips: [] }
      cut.tracks.unshift(track)
    }
    let n = track.clips.length + 1
    while (track.clips.some((c) => c.id === `scene-${n}`)) n += 1
    track.clips.push({
      id: `scene-${n}`,
      name: sceneName,
      in: 0,
      out: Number(Math.max(1, seconds).toFixed(3)),
      at: Number(state.playhead.toFixed(3)),
      scene: { project: proj },
    })
    dirty = true
    draw()
    toast(`${sceneName} at the playhead — Save cut keeps it.`)
  }
  editInsert.onclick = () => {
    if (!editing) return
    let ms = 4000
    try {
      ms = editFrame?.contentWindow?.RM?.duration?.() || 4000
    } catch {
      /* still loading; four seconds is the same default a card uses */
    }
    insertSceneClip(editing.proj, editing.saved, ms / 1000)
  }

  /*
   * The cutting tools, and the undo that makes them safe to hold.
   *
   * Every edit here is a cut edit like a drag: it changes the cut in hand,
   * and Save cut is what writes. Undo keeps whole snapshots of the cut — it
   * is kilobytes — pushed BEFORE each edit; drags snapshot at pointerdown and
   * keep it only when the gesture commits, which is what the input layer's
   * one-commit-per-gesture rule exists for. Restores mutate the cut in
   * place, never reassign it: the input layer holds the object.
   */
  const history = []
  const future = []
  ...
  const snapshot = () => remember(JSON.stringify(cut))
  const keepGesture = () => {
    if (pendingSnap && pendingSnap !== JSON.stringify(cut)) remember(pendingSnap)
    pendingSnap = null
  }
  const applyState = (json) => {
    const next = JSON.parse(json)
    cut.tracks = next.tracks
    cut.sources = next.sources
    state.selection = null
    for (const [id, fr] of [...insertedFrames]) {
      if (!cut.tracks.some((tr) => (tr.clips ?? []).some((c) => c.id === id))) {
        fr.remove()
        insertedFrames.delete(id)
      }
    }
  ...
  const undo = () => {
    if (!history.length) return void toast('Nothing to undo.')
    future.push(JSON.stringify(cut))
    applyState(history.pop())
  }
  const redo = () => {
    if (!future.length) return
    history.push(JSON.stringify(cut))
  ...
  const uniqueClipId = (base) => {
    const all = new Set((cut?.tracks ?? []).flatMap((tr) => (tr.clips ?? []).map((c) => c.id)))
    let n = 2
    let id = `${base}.2`
    while (all.has(id)) id = `${base}.${++n}`
    return id
  }
  const frameSlack = () => 1 / (cut?.fps || 60)

  /* The blade. The selected clip when the playhead is inside it; otherwise
     the clip under the playhead, footage first — that is the lane a blade
     means by default. The tail keeps everything the head does not, its `in`
     offset included, which is why a sliced graphic resumes instead of
     restarting. */
  const sliceAt = () => {
    if (!cut) return
    const t = state.playhead
    const inside = (c) => t > c.at + frameSlack() && t < c.at + (c.out - c.in) - frameSlack()
    let found = state.selection ? clipById(state.selection) : null
    if (found && !inside(found.clip)) found = null
    if (!found) {
      for (const kind of ['video', 'graphic', 'audio']) {
        const track = (cut.tracks ?? []).find((tr) => (tr.kind ?? 'video') === kind && (tr.clips ?? []).some(inside))
        if (track) {
          found = { track, clip: track.clips.find(inside) }
          break
        }
      }
    }
    if (!found) return void toast('Put the playhead inside a clip to slice it.', 'bad')
    snapshot()
    const { track, clip } = found
    const offset = t - clip.at
    const tail = { ...clip, id: uniqueClipId(clip.id), in: Number((clip.in + offset).toFixed(3)), at: Number(t.toFixed(3)) }
    clip.out = Number((clip.in + offset).toFixed(3))
    track.clips.splice(track.clips.indexOf(clip) + 1, 0, tail)
    state.selection = tail.id
    dirty = true
    draw()
  }

  /* An app clipboard, not the system one: a clip is a reference into this
     cut's sources, and it means nothing pasted anywhere else. */
  let clipboard = null
  const copySelected = () => {
    const found = state.selection && clipById(state.selection)
    if (!found) return void toast('Select a clip to copy first.', 'bad')
    clipboard = { kind: found.track.kind ?? 'video', clip: JSON.parse(JSON.stringify(found.clip)) }
    toast(`${found.clip.name ?? found.clip.id} copied.`, 'ok')
  }
  const pasteClipboard = () => {
    if (!cut) return
    if (!clipboard) return void toast('Nothing to paste — copy a clip first.', 'bad')
    snapshot()
    let track = cut.tracks.find((tr) => (tr.kind ?? 'video') === clipboard.kind)
    if (!track) {
      track = { id: clipboard.kind === 'audio' ? 'a1' : clipboard.kind === 'graphic' ? 'g1' : 'v1', kind: clipboard.kind, clips: [] }
      cut.tracks.push(track)
    }
    const pasted = { ...JSON.parse(JSON.stringify(clipboard.clip)), id: uniqueClipId(clipboard.clip.id), at: Number(state.playhead.toFixed(3)) }
    track.clips.push(pasted)
    state.selection = pasted.id
    dirty = true
    draw()
  }

  const deleteSelected = ({ ripple = false } = {}) => {
    if (!cut || !state.selection) return
    const found = clipById(state.selection)
    if (!found) return
    snapshot()
    const { track, clip } = found
    const seconds = clip.out - clip.in
    track.clips.splice(track.clips.indexOf(clip), 1)
    if (ripple) {
      /* Close the gap: everything after the cut on this lane moves up by the
         length that left. Other lanes hold still — a caption timed to
         footage on another lane is not this delete's to move. */
      for (const later of track.clips) {
        if (later.at >= clip.at - frameSlack()) later.at = Number(Math.max(0, later.at - seconds).toFixed(3))
      }
    }
    state.selection = null
    const fr = insertedFrames.get(clip.id)
    if (fr) {
      fr.remove()
      insertedFrames.delete(clip.id)
    }
    dirty = true
    draw()
    toast(`${clip.name ?? clip.id} removed${ripple ? ' and the gap closed' : ''} — Save cut keeps it.`)
  }
  const cutSelected = () => {
    const found = state.selection && clipById(state.selection)
    if (!found) return void toast('Select a clip to cut first.', 'bad')
    clipboard = { kind: found.track.kind ?? 'video', clip: JSON.parse(JSON.stringify(found.clip)) }
    deleteSelected()
  }

  slice.onclick = sliceAt
  removeClip.onclick = () => deleteSelected()
  rippleClip.onclick = () => deleteSelected({ ripple: true })
  canvas.addEventListener('pointerdown', () => {
    canvas.focus()
    if (cut) pendingSnap = JSON.stringify(cut)
  })
  /*
   * Zoom from the bar and the keys. The wheel handler zooms too, with ⌘ or ctrl
   * held, but nothing on the page said so, and a zoom you cannot find is a zoom
   * you do not have. Anchored on the playhead when it is in view, so the frame
   * being worked on stays put; on the middle of the view otherwise.
   */
  const ZOOM_MIN = 2
  const ZOOM_MAX = 400
  const zoomBy = (factor) => {
    if (!mod) return
    const width = canvas.getBoundingClientRect().width - mod.canvas.HEAD_W
    const first = view.scrollSeconds
    const last = first + width / view.pxPerSecond
    const anchor = state.playhead >= first && state.playhead <= last ? state.playhead : (first + last) / 2
    const anchorX = (anchor - first) * view.pxPerSecond
    view.pxPerSecond = Math.min(ZOOM_MAX, Math.max(ZOOM_MIN, view.pxPerSecond * factor))
    view.scrollSeconds = Math.max(0, anchor - anchorX / view.pxPerSecond)
    draw()
    fill()
  }
  const zoomToFit = () => {
    if (!mod) return
    const width = canvas.getBoundingClientRect().width - mod.canvas.HEAD_W
    const total = cutSeconds()
    if (!(total > 0) || !(width > 0)) return
    view.pxPerSecond = Math.min(ZOOM_MAX, Math.max(ZOOM_MIN, (width * 0.96) / total))
    view.scrollSeconds = 0
    draw()
    fill()
  }
  ...
  canvas.addEventListener('keydown', (e) => {
    ...
    if (e.key === '=' || e.key === '+') return act(() => zoomBy(1.5))
    if (e.key === '-' || e.key === '_') return act(() => zoomBy(1 / 1.5))
    if (e.key === '0') return act(zoomToFit)
    if (cmd && e.key.toLowerCase() === 'z') return act(e.shiftKey ? redo : undo)
    if (cmd && e.key.toLowerCase() === 'c') return act(copySelected)
    if (cmd && e.key.toLowerCase() === 'x') return act(cutSelected)
    if (cmd && e.key.toLowerCase() === 'v') return act(pasteClipboard)
    if (!cmd && e.key.toLowerCase() === 's') return act(sliceAt)
    if (e.key === 'Delete' || e.key === 'Backspace') return act(() => deleteSelected({ ripple: e.shiftKey }))
  })

  /*
   * A scene from a sentence, without leaving the cut.
   *
   * The same drafting the Scenes page has: Claude writes the parts and the
   * file is saved. The editor then opens on it right here, because the next
   * step is always to read what was written — and Insert is one click away.
   */
  const sayNew = (t, tone_) => {
    newStatus.hidden = !t
    newStatus.textContent = t ?? ''
    tone(newStatus, tone_)
  }
  draftScene.onclick = async () => {
    const proj = shelfProject.value || project
    if (!newName.value.trim()) {
      sayNew('Give the scene a name first — it becomes the filename.', 'bad')
      newName.focus()
      return
    }
    if (!newBrief.value.trim()) {
      sayNew('Say what the scene is — a sentence is enough.', 'bad')
      newBrief.focus()
      return
    }
    draftScene.disabled = true
    sayNew('Drafting…')
    const r = await fetch('/api/scene/draft', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ projectId: proj, name: newName.value, about: newBrief.value }),
    }).then(responseJson).catch((e) => ({ error: e.message }))
    draftScene.disabled = false
    if (r.error) return void sayNew(r.error, 'bad')
    sayNew('')
    newName.value = ''
    newBrief.value = ''
    shelfScenes.delete(proj)
    await loadShelf()
    const sc = (await scenesOf(proj)).find((s) => s.name === r.name)
    if (sc) openSceneEditor(proj, sc)
  }

  /*
   * Export: the cut as files. Parts is every clip on its own; Video is the
   * clips joined. The cut on screen goes with the request, so the export is
   * what you are looking at. The job reports in the header like every other;
   * the button comes back when it ends, with a word on where the files went.
   */
  const exportCut = async (button, mode) => {
    if (!cut) return
    button.disabled = true
    const r = await fetch('/api/edit/export', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ project, folder, mode, cut }),
    }).then(responseJson).catch((e) => ({ error: e.message }))
    if (r.error) {
      toast(r.error, 'bad')
      button.disabled = false
    ...
    save.disabled = true
    if (r.alreadyRunning) {
      toast('That export is already running.', 'warn')
      button.disabled = false
      return
    }
    const job = await start(r.step)
    if (!job) return void (button.disabled = false)
    watchJobInPlace(job, null, (current) => {
      button.disabled = false
      if (current?.code === 0) toast(`Exported to ${r.out}${mode === 'parts' ? '/parts' : ''}.`)
      else toast('The export failed — see the Console.', 'bad')
      refreshJobs()
    })
  }
  exportParts.onclick = () => exportCut(exportParts, 'parts')
  exportVideo.onclick = () => exportCut(exportVideo, 'joined')
...
function vRestyle(m) {
  ...
  /* The instruction is the prompt here, and it is the same handful of
     instructions every time. */
  promptLibrary(ui.promptLib, {
    where: 'restyle',
    get: () => state.prompt,
    set: (v) => {
      state.prompt = v
  ...
  const paintPanel = () => {
    ...
    if (spec?.controls.includes('keepAudio')) {
      ...
      check.setAttribute('aria-label', 'Keep audio')
    ...
    if (spec?.controls.includes('generateAudio')) {
      ...
      check.setAttribute('aria-label', 'Generate audio')
 ...
 *
 * On Storage rather than on Review, because this page is already the answer to
 * "where do things end up" — and because Slack is a destination whether or not
 * anyone shares a page.
 *
 * The token is write-only, like every credential here: it goes in, it is stored 0600,
...
function slackPanel(host) {
   ...
   */
  const change = control('button', { className: 'btn btn--hint', textContent: 'Change', hidden: true })
  change.onclick = () => {
...
function vStorage(m) {
  ...
  /*
   * Where permanent links come from.
   *
   * A presigned link is the best rclone can mint and R2 caps it at a week — the
   * expiry warning on Copy link is honest, but a link that dies on Monday is
   * not a link anyone can put in a doc. A bucket made public in the provider
   * has a stable URL; name the bucket and its URL here, and Copy link answers
   * with a permalink for anything inside it.
   */
  const pubBucket = mk('Public bucket', control('input', { placeholder: 'openscreen' }), 'The bucket the public URL serves. Sent projects live in the openscreen bucket, so leave this blank to mean that one.')
  const pubBase = mk('Public base URL', control('input', { placeholder: 'https://pub-<hash>.r2.dev' }), 'The bucket’s public URL — R2’s public development URL, or a custom domain. Enabling it in the dashboard is half the job: Studio cannot see inside Cloudflare, so the URL has to be pasted here. Links into the public bucket then never expire.')
  ...
  const edit = async (n) => {
    ...
    pubBucket.value = r.publicBucket ?? ''
    pubBase.value = r.publicBase ?? ''
  ...
  go.onclick = async () => {
    ...
    const body = {
      ...
      publicBucket: pubBucket.value.trim(),
      publicBase: pubBase.value.trim(),
    }
  ...
  const paintCrumbs = () => {
    ...
    mk2.onclick = async () => {
      const folder = await askName('Name the folder', '', { confirm: 'Create' })
      if (!folder) return
  ...
  const paintList = () => {
    ...
    for (const e of sorted) {
          ...
          run: async () => {
            ...
            toast(
              r.permanent
                ...
                : r.expiry
                  ? `Link copied — it expires in ${r.expiry}. Set a public bucket on this remote for permanent links.`
                  : 'Link copied.',
              'ok',
            )
          },
      ...
      items.push({
        ...
        text: 'Rename',
        run: async () => {
          const next = await askName('Rename to', e.name, { confirm: 'Rename' })
 ...
 * The notes poll.
 *
 * It was written and never started, which is why nothing ever carried a count:
 * the badge, the bell and the centre all read the same list, and the list was
 * never asked for. Once on load, then every two minutes, and again whenever the
 * window is focused — coming back to it is when somebody wants to know.
 */
;(() => {
  const tick = () => void refreshReviewNotices()
  tick()
