...
function vTimeline(m) {
  ...
  const chooseFrom = (compositions) => {
    ...
    if (!compositions.length) {
       ...
       */
      empty.append(control('hint', { textContent: 'No composition here yet — pick the footage to cut.' }))
      ;(async () => {
        ...
        for (const v of videos) {
          const { root: row, el: take } = mountRow('timeline-take')
          ...
          picks.push({ rel: v.rel, box: take.pick })
          empty.append(row)
        }
        const start = control('button', { className: 'btn', textContent: 'Cut the selected footage' })
        start.onclick = async () => {
          const rels = picks.filter((p) => p.box.checked).map((p) => p.rel)
          if (!rels.length) return void toast('Tick at least one video first.', 'bad')
          start.disabled = true
          start.textContent = 'Reading the footage…'
          const made = await fetch('/api/edit/start', {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({ project, rels }),
          }).then(responseJson).catch((e) => ({ error: e.message }))
          if (made.error) {
            start.disabled = false
            start.textContent = 'Cut the selected footage'
