...
function vStoryboard(m) {
  ...
  function paintRail() {
    ...
    const slackIn = el('button', 'btn ghost btn--small slackin')
    slackIn.type = 'button'
...
function vRestyle(m) {
  ...
  const generating = () => takesOf() === 'text' || takesOf() === 'image+text'
  const wantsClip = () => takesOf() === 'video' || takesOf() === 'video+audio'
  ...
  const paintMode = () => {
    ...
    runBtn.textContent = generating() ? 'Generate the shot' : avatar ? 'Make the video' : 'Restyle the clip'
  ...
  const paintPanel = () => {
    ...
    for (const [field, label, key, options] of [
      ...
      const box = g()
      const pick = el('select')
      pick.className = 'wide form-control'
      for (const option of options) {
        pick.append(Object.assign(el('option', null, option), { value: option, selected: option === state[field] }))
      }
      pick.onchange = () => { state[field] = pick.value }
      box.append(el('label', 'form-label', label), pick)
    }
    ...
    if (spec?.controls.includes('generateAudio')) {
      const box = g()
      const check = Object.assign(el('input'), { type: 'checkbox', className: 'form-control form-control--medium', checked: state.generateAudio })
      check.onchange = () => { state.generateAudio = check.checked }
      // Control first: the label is styled as the checkbox's partner, and a
      // sibling selector cannot reach backwards.
      box.append(check, el('label', 'form-label', 'Generate audio'))
    }
  ...
  runBtn.onclick = async () => {
    ...
    stream.onmessage = async (event) => {
      ...
      if (data.done) {
        ...
        void refreshJobs()
        if (data.code === 0) {
          finish(generating() ? 'Generated. The shot is in Footage.' : avatar ? 'Made. The video is in Footage.' : 'Restyled. The result is in Footage beside the original.', 'ok')
