...
function vHyperframes(m) {
  ...
  const draw = async () => {
    ...
    for (const project of projects) {
      ...
      card.kind.textContent = latest ? 'video' : 'source'
      card.name.textContent = project.title
...
function vInterview(m) {
  ...
  const render = () => {
    const parts = [ui.questionCount, ui.questionTitle, ui.questionHint, ui.shots, ui.problems]
    ...
    if (!state?.turns?.length) {
      ask('Question 1', 'What is this video for, and who watches it?')
    ...
    ask(`Question ${state.turns.length}`, turn.question)
    answerGroup.hidden = false
