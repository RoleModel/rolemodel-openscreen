...
function vMake(m) {
  ...
  const fillClips = () => {
    ...
    const files = p?.catalog?.files ?? []
    for (const [sel, kind, none] of [
      [webcam, 'video', 'No webcam'],
      ...
      for (const f of files) {
        ...
        sel.append(Object.assign(el('option', null, f.name), { value: f.rel }))
      }
...
function slashValues(key, project) {
  ...
  switch (key) {
    ...
    case 'captions':
      return named([{ v: 'on', hint: 'burn subtitles in' }, { v: 'off', hint: 'no subtitles' }])
    case 'voice':
...
function attachSlashMenu(ta, getProject) {
  const menu = el('div')
  menu.style.cssText =
    'position:absolute;z-index:40;max-block-size:16rem;overflow:auto;inline-size:min(38rem,100%);' +
    'background:var(--op-color-neutral-plus-eight);border:1px solid var(--op-color-border);' +
    'border-radius:var(--op-radius-medium);box-shadow:var(--op-shadow-large);display:none'
  const holder = el('div')
  ...
  const paint = () => {
    ...
    items.forEach((it, i) => {
      const row = el('div')
      row.style.cssText =
        'display:flex;gap:var(--op-space-small);align-items:baseline;padding:var(--op-space-2x-small) var(--op-space-small);cursor:pointer;' +
        (i === cursor ? 'background:var(--op-color-primary-minus-six)' : '')
      row.append(
        Object.assign(el('span'), { textContent: it.label, style: 'font-family:var(--font-mono);min-inline-size:9rem' }),
        Object.assign(el('span'), { textContent: it.hint || '', style: 'color:var(--fg-dim);font-size:var(--op-font-small)' }),
      )
      row.onmousedown = (e) => {
        e.preventDefault()
        accept(it)
...
function vScriptEditor(m, doc) {
   ...
   */
  attachHighlight(body)
  attachSlashMenu(body, () => proj.value)
  ...
  back.append(icon('arrow-left-01'), Object.assign(el('span'), { textContent: 'All scripts' }))
  const said = el('div', 'hint')
  row.append(save, back, said)
  f.append(row)
