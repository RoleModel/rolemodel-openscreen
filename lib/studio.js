...
/*
 * The views where you are between projects rather than inside one.
 *
 * The Library is the picker and New project is the form that makes one, so on
 * both of them "which project am I in" is the question being answered rather than
 * a setting to carry — a switcher there is a second, worse copy of the page you
 * are already looking at, and the pipeline group offers eight panels that all
 * belong to a project you have not chosen yet.
 */
const CHOOSING_PROJECT = new Set(['library', 'new'])
...
function projectSwitcher() {
  ...
  if (CHOOSING_PROJECT.has(view)) return wrap
  ...
  trigger.append(
    icon(record ? 'folder-library' : 'folder-add', 'projmenu__mark'),
    Object.assign(el('span', 'projmenu__label'), { textContent: label }),
    icon('arrow-up-01', 'projmenu__caret'),
  )
  ...
  const choose = async (id) => {
    ...
    // Reload before re-rendering: a panel reads footage, scripts and scenes off
    // `S`, so switching without refetching would redraw the new project's name
    // over the old project's contents.
    await load()
  }
...
function paintNavGroups() {
  ...
  const show = Boolean(currentProject()) && !CHOOSING_PROJECT.has(view)
...
function vProject(m, id) {
  ...
  /*
   * Deleting the project goes in the menu too.
   *
   * It was a full-size red button in the toolbar, beside Re-index and the media
   * filters — the single most destructive action in the app given the same weight
   * as "show me the audio files", and the widest thing in the row. Re-index stays
   * out here because it is safe and frequently wanted; this is neither.
   *
   * `kind` is what makes it safe on the server: it refuses a project root without
   * one, so a mistyped path can never take a whole client's work.
   */
  const projectActions = actionMenu([
    {
      icon: 'delete-02',
      text: 'Delete project',
      danger: true,
  ...
  // Static here, not pinned to a corner: this row is a toolbar, and the card rule
  // would take it out of the flow and float it over the media filters.
  projectActions.classList.add('kebab--inline')

  row.append(re, projectActions)
...
function vMake(m) {
   ...
   */
  const proj = { get value() { return currentProject() ?? '' } }
...
function vScripts(m) {
  ...
  const recount = () => {
    ...
    count.textContent = spoken.length
      ? `${spoken.length} spoken line${spoken.length === 1 ? '' : 's'}${secs ? ` · about ${Math.round(secs)}s` : ''}`
  ...
  const load_ = (sc) => {
    ...
    recount()
    tone(status)
    status.textContent = `Editing ${sc.name}. Saving under the same name overwrites it.`
    body.focus()
  }
  ...
  const paintSaved = () => {
    ...
    for (const sc of S.scripts) {
      ...
      b.append(
        el('div', 'nm', sc.name),
        el('div', 'path', owner),
        el('div', 'path', sc.body.slice(0, SNIPPET_CHARS) + (sc.body.length > SNIPPET_CHARS ? '…' : '')),
      )
      /*
  ...
  draft.onclick = async () => {
    ...
    tone(status)
    status.textContent = 'Run it below. When it finishes, reload and it is on the shelf and in Voice.'
  ...
  function paintCards() {
    ...
    elements.forEach((e, i) => {
      ...
      const labelled = (text, control) => {
        ...
        const real = control.matches?.('input, select, textarea')
          ? [control]
