...
function vStickers(m) {
  ...
  const rangeRow = (box, label, key, min, max, step) => {
  ...
  const buildRail = () => {
    ...
    if (settings.printLayout === 'sheet') {
      ...
      selectRow(pr, 'Sheet size', paper, 'printPage')
      rangeRow(pr, 'Sticker (mm)', 'sheetStickerMm', 15, 120, 1)
      /* Zero sits the cut on the artwork's own keyline, which is what these
         stickers are drawn with. */
      rangeRow(pr, 'Cut outside (mm)', 'dieOffsetMm', 0, 5, 0.25)
      rangeRow(pr, 'Cut corner (mm)', 'dieRoundMm', 0, 8, 0.5)
    } else rangeRow(pr, 'Longest side (mm)', 'printSizeMm', 10, 300, 1)
    rangeRow(pr, 'Bleed (mm)', 'printBleedMm', 0, 10, 0.5)
