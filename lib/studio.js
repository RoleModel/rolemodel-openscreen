...
function vCreate(m) {
  const { tabs, host } = mountPanel('create', m)
  const paint = () => {
    tabs.innerHTML = ''
    ...
    /*
     * The tab's own panel renders into `host`, and clears the shared slots first.
     *
     * These three are full panels with their own config and actions, so switching
     * tabs has to empty Optics' sidebar and footer the way render() does — without
     * it the previous tab's fields and buttons stay on screen, wired to a form that
     * is no longer there.
     */
    clearPanelRegions()
