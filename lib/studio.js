 ...
 */
const GLOBAL_VIEWS = new Set(['skills', 'brand', 'components', 'field', 'wallpapers', 'storage', 'usage', 'console', 'library', 'new'])
...
function waveform(host, url, { height = 64 } = {}) {
  ...
  /* Resolved, never written down. A token that resolves to nothing is left out
     so WaveSurfer falls back to its own default — this file states no colours
     of its own, and a hex here would be the first. */
  const paint = {}
  const styles = getComputedStyle(document.documentElement)
  for (const [option, name] of [['waveColor', '--muted'], ['progressColor', '--accent'], ['cursorColor', '--fg']]) {
    const value = styles.getPropertyValue(name).trim()
    if (value) paint[option] = value
  }
...
function vMultiAssembly(m) {
  ...
  const build = async () => {
    ...
    await refresh()
    say(`${made.selections ?? state?.picks?.length ?? 0} selected clips plus an opening title and closing screen are ready. Open the first cut in HyperFrames; rendering stays locked until you have reviewed it.`, 'ok')
  }
...
function vStoryboard(m) {
  ...
  renderCut.onclick = async () => {
    ...
    says(status, `Cut built — ${r.clips} shot${r.clips === 1 ? '' : 's'}. Rendering the MP4; progress is in Console.${skipped}`, skipped ? 'warn' : 'ok')
...
function vField(m) {
  ...
  /* The markup is what you paste into a composition by hand, so it is one click
     rather than a select-and-drag across a <pre>. copyButton() because it goes
     through the host's clipboard and reports a failure instead of claiming
   ...
   *
   * The other route out of here is Save as scene, which needs a Canvas node to
  ...
  const loadCompositions = async () => {
    ...
    for (const c of found) {
      const name = typeof c === 'string' ? c : c.folder ?? c.name
      if (name) target.append(Object.assign(el('option', null, name), { value: name, selected: name === hyperframesWorkspace?.folder }))
    }
  ...
  const nameIn = Object.assign(el('input'), { value: 'animated-field', placeholder: 'scene name' })
  ...
  intoFooter(target, insertBtn, copyBtn, nameIn, saveBtn)
