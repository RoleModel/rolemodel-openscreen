...
/** The same icon, for the call sites that still build their row in JS. *//** The same icon, for the call sites that still build their row in JS. */
...
function vFraming(m, workspace) {
  ...
  ui.save.onclick = async () => {
    ...
    says(`Framing saved — ${result.clips} clips rebuilt`, 'ok')
...
function vHyperframes(m) {
  ...
  if (hyperframesWorkspace?.url) {
    ...
    ws.toEditor.onclick = async () => {
      ws.toEditor.disabled = true
    ...
    ws.fromEditor.onclick = async () => {
      ws.fromEditor.disabled = true
      ...
      const result = await fetch('/api/hyperframes/from-openscreen', {
        ...
        .then(responseJson)
        .catch((error) => ({ error: error.message }))
      ws.fromEditor.disabled = false
      if (result.error) {
        setCutState(result.error, true)
        says(result.error, 'bad')
        return
      }
      ws.fromEditor.hidden = true
...
function vInterview(m) {
  ...
  const render = () => {
     ...
     */
    const INTERVIEW_CEILING = 6
    const ask = (count, title, at = null) => {
      ui.progress.hidden = at == null
      if (at != null) {
        ui.progress.replaceChildren()
        ui.progress.title = `Question ${at} of at most ${INTERVIEW_CEILING}`
        for (let n = 1; n <= INTERVIEW_CEILING; n += 1) {
          const { root: tick, el: bits } = mountRow('stepper-tick')
