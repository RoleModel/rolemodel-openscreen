...
function vShowcase(m) {
  ...
  ui.gizmo.onpointerdown = (e) => {
    ...
    if (!r0) return
    const start = { x: e.clientX, y: e.clientY, lx: valueOf(l, 'x'), ly: valueOf(l, 'y'), w: l.w, zoom: valueOf(l, 'zoom'), rect: r0 }
    drag(e, (ev) => {
      const dx = ev.clientX - start.x
      const dy = ev.clientY - start.y
      if (!corner) {
        /* X and Y are percentages of the card's own size, as translate() reads them. */
        setProp(l, 'x', Math.round((start.lx + (dx / start.rect.width) * 100) * 2) / 2)
        setProp(l, 'y', Math.round((start.ly + (dy / start.rect.height) * 100) * 2) / 2)
      } else {
        /* A corner scales the card about its centre; the far side of the box
           is what the hand is measuring against. */
        const sx = corner.includes('e') ? 1 : -1
        const grow = (start.rect.width + sx * dx * 2) / start.rect.width
