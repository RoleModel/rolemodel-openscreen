...
function vStickers(m) {
  ...
  let sheetSizeList = []
  const settings = { sticker: 'telegram', cutout: '', vectorize: '', endpoint: '', prompt: '', sheetName: 'stickers', columns: 4, size: 300, gap: 40, remote: '', printSizeMm: 76.2, printBleedMm: 3.175, printLayout: 'sheet', printPage: '6x8', sheetStickerMm: 50.8, quoteQty: 250, quoteProduct: 'die-cut', quotePublish: true }
  let affinityWatch = 0
  let makeBtn = null
  ...
  ui.printPdf.onclick = async () => {
    ...
    const r = await post('/api/stickers/print', { projectId: project(), name: settings.sheetName, items: [...ticked], sizeMm: settings.printLayout === 'sheet' ? settings.sheetStickerMm : settings.printSizeMm, bleedMm: settings.printBleedMm, layout: settings.printLayout, page: settings.printPage })
    busy(false)
    if (r.error) return say(r.error, 'bad')
