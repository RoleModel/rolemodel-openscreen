 ...
 *
 * render() empties #main on every navigation, so the builder's rows — which were
 * held in a closure and nowhere else — died the moment you looked at another page.
 * Ten minutes of work, gone, with no way to tell it had been kept anywhere. It is
 * kept here now, per project, on every edit.
 *
 * localStorage rather than the server: this is an unfinished draft, not a document.
 * Pressing "Set up the capture" is what writes the script into the project, and that
 * is still the thing that makes it real.
 */
const DRAFT_KEY = (projectId) => `rm.record.rows.${projectId || 'none'}`

function saveDraft(projectId, rows) {
  try {
    if (rows.length) localStorage.setItem(DRAFT_KEY(projectId), JSON.stringify(rows))
...
function vRecord(m) {
  ...
  proj.addEventListener('change', () => builder.load(loadDraft(proj.value)))
  const draft = loadDraft(proj.value)
