...
function vStickers(m) {
  ...
  let sheetSizeList = []
  const settings = { sticker: 'telegram', cutout: '', vectorize: '', endpoint: '', prompt: '', sheetName: 'stickers', columns: 4, size: 300, gap: 40, remote: '', printSizeMm: 76.2, printBleedMm: 1, printLayout: 'sheet', printPage: '4x6', sheetStickerMm: 50.8, sheetFit: true, dieOffsetMm: 0, dieRoundMm: 0, quoteQty: 250, quoteProduct: 'die-cut', quotePublish: true }
  let affinityWatch = 0
  let makeBtn = null
  ...
  const buildRail = () => {
    ...
    /* How far the ink runs past the cut on every sticker, so a die that drifts
       outward still lands on colour. */
    rangeRow(pr, 'Bleed (mm)', 'printBleedMm', 0, 5, 0.25, previewSoon)
