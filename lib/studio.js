...
function vStickers(m) {
  ...
  const setWork = (next) => {
    ...
    ui.affinity.title = !work ? 'Pick a sticker first' : !work.rel ? 'Run a step on this picture first — Affinity opens files in the project' : work.kind === 'vector' ? 'Open the SVG in Affinity; save there and it updates here' : 'Open the PNG in Affinity; export back over the same file to update it here'
  ...
  const paintSheets = () => {
    ...
    for (const sh of data.sheets) {
      ...
      el.affinity.onclick = async () => {
        ...
        if (r.error) return say(r.error, 'bad')
        say(`Opened the sheet in ${r.app}. Save there; publish sends what is saved.`)
      }
  ...
  ui.affinity.onclick = async () => {
    ...
    if (ticked.size >= 2) {
      ...
      busy(false)
      if (r.error) return say(r.error, 'bad')
      status(`Opened ${ticked.size} stickers in ${r.app} as ${r.rel.split('/').pop()}.`, 'ok')
      return say(`Opened ${ticked.size} stickers in ${r.app}.`)
    }
    ...
    if (r.error) return say(r.error, 'bad')
    say(work.kind === 'vector' ? `Opened in ${r.app}. Save there and the sticker updates here.` : `Opened in ${r.app}. Export over the same file and the sticker updates here.`)
    clearInterval(affinityWatch)
    let last = r.hash
    const rel = work.rel
    affinityWatch = setInterval(async () => {
      const h = await fetch(`/api/stickers/hash?project=${encodeURIComponent(project())}&rel=${encodeURIComponent(rel)}`)
        .then(responseJson)
        .catch(() => null)
      if (!h?.hash || h.hash === last) return
      last = h.hash
      await reload()
      if (work?.rel === rel) setWork({ ...work, url: mediaUrl(rel, h.hash.slice(0, 8)) })
      say('Picked up your Affinity edit.')
    }, 1500)
  }
