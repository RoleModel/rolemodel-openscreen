...
function vRestyle(m) {
  ...
  const state = { model: null, file: null, prompt: '', keepAudio: true, resolution: null, images: [] }
  let models = []
  ...
  m.append(el('div', 'sec', 'The clip'), list, el('div', 'sec', 'The instruction'), promptIn, clipNote)
  ...
  const loadClips = async () => {
    ...
    for (const clip of result.clips ?? []) {
      ...
      card.type = 'button'
      card.disabled = Boolean(clip.problem)
      card.append(
      ...
      card.onclick = () => {
        state.file = clip.rel
        for (const other of list.querySelectorAll('.card')) other.classList.remove('on')
  ...
  runBtn.onclick = async () => {
    ...
    const built = await fetch('/api/fal/edit', {
      ...
      body: JSON.stringify({ projectId: currentProject(), model: state.model, file: state.file, prompt: state.prompt, keepAudio: state.keepAudio, resolution: state.resolution, images: state.images }),
    }).then(responseJson).catch((error) => ({ error: error.message }))
