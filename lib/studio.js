...
function vStickers(m) {
  ...
  ui.quote.onclick = async () => {
    ...
    ui.quotesAsked.textContent = `· ${settings.quoteQty} at a time`
    ui.quotes.textContent = ''
    /* Two lists, never one: a sheet is priced by the page and a single one at
    ...
    for (const [group, label, per] of [[r.singles, 'Die-cut singles', 'sticker'], [r.sheets, 'Sticker sheets', 'sheet']]) {
      if (!group?.quotes?.length) continue
      const head = control('div', { className: 'client', textContent: label })
      ui.quotes.append(head)
      for (const q of group.quotes) {
        const { nodes, el: row } = mountRow('quote-row')
        row.name.textContent = q.name
        if (q.error) {
          row.unit.textContent = '—'
          row.detail.textContent = q.error
          row.source.textContent = ''
        } else {
          row.unit.textContent = `$${q.unit.toFixed(2)}`
          row.detail.textContent = `$${q.total} for ${q.quantity} · ${q.size}`
          const how = q.live ? 'read from their page just now' : q.kind === 'api' ? 'quoted live' : 'from the table'
          row.source.textContent = `${how} · ${q.checkedAt}`
          if (group.best && q.id === group.best.id) nodes[0].classList.add('stickers__quote--best')
        }
        ui.quotes.append(...nodes)
      }
      if (group.best) say(`${label}: ${group.best.name} at $${group.best.unit.toFixed(2)} a ${per}.`)
    }
    const bits = [r.singles?.best && `${r.singles.best.name} $${r.singles.best.unit.toFixed(2)} a sticker`, r.sheets?.best && `${r.sheets.best.name} $${r.sheets.best.unit.toFixed(2)} a sheet`].filter(Boolean)
