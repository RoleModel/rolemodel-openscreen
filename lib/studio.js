...
function vScenes(m) {
  ...
  const refreshFootage = async (selected = '') => {
    ...
    previewFootage = asPreviewFootage(chosen)
    setFootageHint(count ? `${count} video${count === 1 ? '' : 's'} attached to this shot.` : 'Choose an existing recording or add one to this project.')
    // The editor is the place to add the treatment, so it must open on the
    // Canvas pick itself — not an empty graphic stage beside a separate player.
    void preview()
  }
  ...
  if (footageGroup) {
    ...
    const usePassage = async (passage) => {
      ...
      previewFootage = asPreviewFootage({ rel: footage.value, inSec: passage.inSec, outSec: passage.outSec })
    ...
    const renderFootageReview = () => {
      ...
      const useManual = el('button', 'btn btn--small', selected.length ? `Use ${selected.length} selected passage${selected.length === 1 ? '' : 's'}` : 'Select transcript words')
      ...
      manual.append(el('span', 'hint', selected.length ? `${selectedWords} selected word${selectedWords === 1 ? '' : 's'} make ${selected.length} passage${selected.length === 1 ? '' : 's'} for this scene. Shift-click extends a continuous range; ordinary clicks add or remove individual words.` : 'Click the exact words you want. Shift-click a second word to select the range between them.'), useManual, clear)
      const lines = el('div', 'paper-edit__lines scene-footage-review__lines')
      for (const cue of cues) {
        const line = el('div', 'paper-edit__line')
        const text = el('span', 'paper-edit__text paper-edit__words')
        const wordIndexes = wordIndexesForCue(cue)
        line.append(el('time', 'paper-edit__time', footageTime(cue.startSec)), text)
        for (const index of wordIndexes) {
          const word = transcriptWords()[index]
          if (!word) continue
          const token = el('button', 'paper-edit__word', word.text)
          token.type = 'button'
          token.dataset.word = String(index)
          ...
          token.onclick = (event) => {
            ...
            footagePlayer.currentTime = Number(word.startSec) || 0
            renderFootageReview()
          }
    ...
    attachFootage.onclick = async () => {
      ...
      footageBoard = result.board
      const takes = (footageBoard?.takes ?? []).filter((take) => take.slotId === boardScene.id)
      previewFootage = asPreviewFootage(takes.at(-1))
