...
function go(v, { preserveProject = false } = {}) {
  ...
  if (v !== 'storyboard') openBoardScene = null
  saveWorkflowStage(v)
...
async function watchJobInPlace(job, status, done) {
  const check = async () => {
    const data = await fetch('/api/jobs').then(responseJson).catch(() => null)
...
function vProject(m, id) {
  ...
  const loadRecentCaptures = async () => {
    ...
    for (const recording of recordings) {
      ...
      captureSelect.append(Object.assign(el('option'), {
  ...
  addCapture.onclick = async () => {
    ...
    const importedName = String(result.file ?? 'the recording').split(/[\\/]/).pop()
...
function vMake(m) {
  ...
  if (incomingScriptName) {
    ...
    const normal = (value) => String(value).replace(/\.[^.]+$/, '').trim().toLowerCase()
...
function slashValues(key, project) {
  ...
  switch (key) {
    case 'script': {
      ...
      return named(scripts.map((script) => ({
        v: script.name,
  ...
  const refresh = () => {
    ...
    if (withValue) {
      ...
      if (key === 'voice' && !slashVoices) {
        ...
        slashVoices = []
        Promise.all(['elevenlabs', 'kokoro'].map(async (provider) => {
          const result = await fetch('/api/voices?provider=' + provider).then(responseJson).catch(() => ({ voices: [] }))
...
function vScripts(m) {
  ...
  const saveScript = async () => {
    ...
    const fresh = await fetch('/api/state').then(responseJson).catch(() => null)
...
function vWorkflow(m) {
  ...
  const draw = ({ interview, board, workflow }) => {
    ...
    const steps = [
      { id: 'interview', view: 'interview', label: 'Plan', detail: hasPlan ? `${interview.state.plan.shots.length} beats planned` : 'Answer a few questions', done: hasPlan, title: 'Start with the outcome', body: 'Answer a few practical questions. Claude turns what you know into the working plan for this video.', action: hasPlan ? 'Review the plan' : 'Start the interview' },
      {
        id: 'script',
        view: 'scripts',
        label: 'Script',
    ...
    status.append(el('strong', null, active.done ? 'Complete' : 'Up next'), document.createTextNode(` · ${active.detail}`))
...
function vMultiAssembly(m) {
  ...
  const switchMode = el('button', 'btn ghost btn--small', 'Use one recording instead')
  switchMode.onclick = () => { assemblyMode = 'single'; render() }
  m.append(switchMode)
  if (!videos.length) {
  ...
  const openAlignedTimeline = el('button', 'btn ghost btn--small', 'Review aligned timeline')
  const renderAligned = el('button', 'btn btn--primary btn--small', 'Render aligned video')
  ...
  const paint = () => {
    ...
    for (const [index, pick] of state.picks.entries()) {
      ...
      clip.addEventListener('loadedmetadata', () => { try { clip.currentTime = pick.inSec } catch {} }, { once: true })
  ...
  const paintAlignment = () => {
    ...
    const head = el('div', 'multi-assembly__review-head')
    const title = el('div')
    title.append(el('div', 'paper-edit__title', `Visual alignment · ${alignment.segments.length} screen cuts`), el('p', 'hint', 'Review the screen moments Claude chose for each part of the narration. Adjust the timeline in the editor before you render.'))
    const actions = el('div', 'multi-assembly__alignment-actions')
    actions.append(openAlignedTimeline, renderAligned)
    ...
    for (const [index, segment] of alignment.segments.entries()) {
      ...
      cut.onclick = () => {
        ...
        try { player.currentTime = segment.screenInSec; player.play().catch(() => {}) } catch {}
      }
      cuts.append(cut)
    }
  ...
  const refresh = async () => {
    const result = await fetch(`/api/multi-assembly?project=${encodeURIComponent(currentProject() ?? '')}`).then(responseJson).catch((error) => ({ error: error.message }))
    ...
    paint()
    const aligned = await fetch(`/api/multi-assembly/audio-align?project=${encodeURIComponent(currentProject() ?? '')}`).then(responseJson).catch((error) => ({ error: error.message }))
    if (!aligned.error && aligned.state?.document) {
      alignment = { ...aligned.state, prepareRenderStep: aligned.prepareRenderStep, renderStep: aligned.renderStep, rendered: aligned.rendered }
      paintAlignment()
    }
  }
  const build = async () => {
  ...
  const runAssembly = async (revision = false) => {
    ...
    if (drafted.error) { assemble.disabled = false; return say(drafted.error, 'bad') }
    const job = await start(drafted.step, { status })
    if (!job) { assemble.disabled = false; return }
  ...
  transcribe.onclick = async () => {
    ...
    if (result.error) { transcribe.disabled = false; return say(result.error, 'bad') }
    ...
    let failed = false
    if (!left) { transcribe.disabled = false; return }
    for (const job of jobs) watchJobInPlace(job, status, async (finished) => {
  ...
  analyze.onclick = async () => {
    ...
    if (result.error) { analyze.disabled = false; return say(result.error, 'bad') }
    say(`Analyzing ${result.steps.length} screens in the background…`, 'ok')
    const jobs = (await Promise.all(result.steps.map(({ step }) => start(step, { status })))).filter(Boolean)
    let left = jobs.length
    let failed = false
    if (!left) { analyze.disabled = false; return }
    for (const job of jobs) watchJobInPlace(job, status, async (finished) => {
      if (finished?.code !== 0) failed = true
      if (--left === 0) {
  ...
  alignButton.onclick = async () => {
    ...
    if (prepared.error) { alignButton.disabled = false; return say(prepared.error, 'bad') }
    ...
    let left = jobs.length
    let failed = false
    if (!left) { alignButton.disabled = false; return }
    for (const job of jobs) watchJobInPlace(job, status, async (finished) => {
      if (finished?.code !== 0) failed = true
      if (--left !== 0) return
      if (failed) { alignButton.disabled = false; return say('The screen analysis or narration transcription did not finish. Check the job details, then try again.', 'bad') }
      ...
      if (drafted.error) { alignButton.disabled = false; return say(drafted.error, 'bad') }
      const agentJob = await start(drafted.step, { status })
      if (!agentJob) { alignButton.disabled = false; return }
      watchJobInPlace(agentJob, status, async (agentFinished) => {
        ...
        alignment = { ...alignment, document: built.document, prepareRenderStep: built.prepareRenderStep, renderStep: built.renderStep }
        paintAlignment()
  ...
  renderAligned.onclick = async () => {
    ...
    renderAligned.disabled = true
    const audioJob = await start(alignment.prepareRenderStep, { status })
    if (!audioJob) { renderAligned.disabled = false; return }
    say('Building the narration track for the reviewed cuts…', 'ok')
    watchJobInPlace(audioJob, status, async (audioFinished) => {
      if (audioFinished?.code !== 0) { renderAligned.disabled = false; return }
      await runWithStatus(alignment.renderStep, renderAligned, status, async (finished) => {
        if (finished?.code === 0) say('The aligned video is now in this project’s renders.', 'ok')
      })
    })
  }
...
function vPaperEdit(m) {
  ...
  useMulti.onclick = () => { assemblyMode = 'multi'; render() }
  m.append(useMulti)
...
function vStoryboard(m) {
  if (openBoardScene) return vBoardSceneEditor(m, openBoardScene)
  ...
  function nodeCard(node, slot) {
    ...
    edit.type = 'button'
    edit.title = `Describe “${node.name || 'this scene'}” and attach video`
    edit.onclick = (event) => {
      event.stopPropagation()
      openBoardScene = node.id
      render()
    }
