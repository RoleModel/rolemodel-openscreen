...
function fileCard(project, f) {
  ...
  c.append(
    actionMenu([
       ...
       *
       * The file on this machine has no URL anyone else can open; its sent copy
       * does. "Send project to storage" puts the whole project under one fixed
       * prefix, so the path here is arithmetic, and the storage link route
       * checks the object is actually there before minting anything — a project
       * never sent gets a sentence, not a dead URL. Permanence follows the
       * remote's public-bucket setting, same as the storage browser.
       */
      ...(project.remote && project.remote !== 'local' ? [{
        icon: 'link-01',
        text: 'Copy link',
        busy: 'Making a link…',
        run: async () => {
          const path = `openscreen/projects/${project.id}/media/${f.rel}`
          const r = await fetch(`/api/storage/${encodeURIComponent(project.remote)}/link?path=${encodeURIComponent(path)}`)
            .then(responseJson)
...
function vStorage(m) {
   ...
   */
  const pubBucket = mk('Public bucket', control('input', { placeholder: 'renders' }), 'A bucket you made public in your provider. Leave blank for presigned links only.')
  const pubBase = mk('Public base URL', control('input', { placeholder: 'https://pub-<hash>.r2.dev' }), 'The bucket’s public URL — R2’s public development URL, or a custom domain. Links to files in the public bucket never expire.')
