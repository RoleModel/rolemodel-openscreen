...
function vProject(m, id) {
  ...
  /*
   * A finished assembly is project work, not console output.
   *
   * Jobs can finish while this page is closed. Ask the durable project record for
   * its latest assembly whenever the project opens, so the next useful click is
   * present even if the person never saw the background completion message.
   */
  const assemblyHandoff = el('section', 'card card--padded')
  assemblyHandoff.hidden = true
  const assemblyTitle = el('div', 'paper-edit__title')
  const assemblyCopy = el('p', 'hint')
  const reviewAssembly = el('button', 'btn btn--primary', 'Review assembly')
  reviewAssembly.onclick = () => go('paperedit')
  const openAssemblyTimeline = el('button', 'btn ghost', 'Open visual timeline')
  const assemblyActions = el('div', 'row')
  assemblyActions.append(reviewAssembly, openAssemblyTimeline)
  assemblyHandoff.append(assemblyTitle, assemblyCopy, assemblyActions)
  m.append(assemblyHandoff)
  void Promise.all([
    ...
    .then(([multi, aligned]) => {
      ...
      const alignment = aligned.state?.segments?.length ? aligned.state : null
      const result = picks.length || multi.state?.document ? multi.state : alignment
      if (!result) return
      const isSourceStack = !picks.length && Boolean(multi.state?.document)
      const count = picks.length || (isSourceStack ? multi.state.sources?.length ?? 0 : alignment.segments.length)
      const isAlignment = !picks.length && !isSourceStack
      assemblyTitle.textContent = isAlignment
        ? `Visual alignment ready · ${count} screen cut${count === 1 ? '' : 's'}`
        : isSourceStack
          ? `Source timeline ready · ${count} recording${count === 1 ? '' : 's'}`
          : `Claude assembly ready · ${count} selected clip${count === 1 ? '' : 's'}`
      assemblyCopy.textContent = result.document
        ? isAlignment
          ? 'The screen cuts and matching narration are saved with this project. Review the alignment before you render it.'
          : isSourceStack
            ? 'The selected recordings are stacked in one editable timeline. Trim, reorder, or use it as the starting point for another Claude pass.'
            : 'The selects and editable visual timeline are saved with this project. Review the choices before you make further changes.'
        : 'The timestamped selects are saved with this project. Review the choices, then build the visual timeline when they look right.'
      openAssemblyTimeline.hidden = !result.document
      openAssemblyTimeline.onclick = async () => {
        const opened = await openDocument({ projectId: p.id, path: result.document })
        if (opened?.error) assemblyCopy.textContent = opened.error
      }
      assemblyHandoff.hidden = false
    })
    .catch(() => {})
  watchStorage()
...
async function openHyperframesProject(folder, button, status) {
  // Keep the new tab inside the click's user gesture.  Opening it after the
  // asynchronous API call is a popup on Safari/Chromium and gets blocked.
  const tab = window.open('about:blank', '_blank')
  if (tab) {
    tab.document.title = 'Opening HyperFrames Studio…'
    tab.document.body.textContent = 'Opening HyperFrames Studio…'
  }
  ...
  /* The preview server needs a beat to bind its port.  The same tab stays in
     place while it starts rather than producing an app error page that a person
     has to reload manually. */
  status.textContent = 'Opening HyperFrames Studio…'
  setTimeout(() => {
  ...
  setTimeout(() => {
    ...
    tone(status, result.state === 'failed' ? 'bad' : 'ok')
    status.textContent = result.error || 'HyperFrames Studio is open. Changes save back to this project.'
  }, 1100)
}
...
function vScenes(m) {
  ...
  const { frame, scrub, palette, cards, rawText, pick, save, draft, footageGroup, footage, attachFootage, addFootage, footagePicker, footageHint } = ui
  /*
  ...
  if (footageGroup) {
    ...
    footagePicker.onchange = async () => {
      ...
      if (added) setFootageHint(`${added.name} is ready. Attach it to this shot.`, 'ok')
    }
    if (boardScene) void refreshFootage()
  }
...
function vWorkflow(m) {
  ...
  const draw = ({ interview, board, workflow, assembly, alignment }) => {
    ...
    const assemblyReady = assemblyPicks.length > 0 || alignmentSegments.length > 0
    const visualAlignmentReady = !assemblyPicks.length && alignmentSegments.length > 0
    const assemblyCount = assemblyPicks.length || alignmentSegments.length
    const steps = [
      {
        id: 'interview',
        view: 'interview',
      ...
      { id: 'record', view: 'record', label: 'Record', detail: videos.length ? `${videos.length} recording${videos.length === 1 ? '' : 's'} ready` : 'Capture or add footage', done: videos.length > 0, title: 'Record the work', body: 'Use the script as your guide, then record the screen or add the footage you already have.', action: videos.length ? 'View recordings' : 'Record or add footage' },
      {
        ...
        view: 'paperedit',
        label: 'Assembly',
        detail: assemblyReady ? `${assemblyCount} ${visualAlignmentReady ? 'screen cut' : 'selected clip'}${assemblyCount === 1 ? '' : 's'} ready to review` : videos.length ? 'Choose the lines and make a first cut' : 'Waiting for footage',
        done: false,
...
function vMultiAssembly(m) {
  ...
  for (const file of videos) {
    ...
    input.onchange = () => {
      if ([...checks.values()].filter((item) => item.checked).length <= 8) return
      input.checked = false
  ...
  m.append(el('div', 'paper-edit__title', 'Source recordings · choose up to 8'), sourceList)
  ...
  scriptSelect.append(Object.assign(el('option', null, 'No script — choose the strongest moments'), { value: '' }))
  ...
  const assemble = el('button', 'btn btn--primary', 'Make an assembly with Claude')
  const stackSources = el('button', 'btn ghost', 'Stack selected recordings')
  const buildTimeline = el('button', 'btn ghost', 'Build visual timeline')
  const openTimeline = el('button', 'btn ghost', 'Open visual timeline')
  buildTimeline.hidden = true
  openTimeline.hidden = true
  const actions = el('div', 'row')
  actions.append(assemble, stackSources, buildTimeline, openTimeline)
  ...
  const openAlignedTimeline = el('button', 'btn ghost btn--small', 'Edit visual cuts')
  ...
  const paint = () => {
    ...
    if (!state?.picks?.length) {
      ...
      const hasTimeline = Boolean(state?.document)
      if (!hasSources) return
      const recovery = el('section', 'card card--padded')
      const copy = hasTimeline
        ? 'The selected recordings are stacked in an editable timeline. Open it to trim and reorder the source clips, or make a new Claude pass if you want a different first cut.'
        : hasSources
        ? 'The last pass saved these source recordings, but it did not leave any timestamped selects to review. Start a new pass, or stack the recordings into an editable timeline and make the first cut yourself.'
        : 'Choose project recordings above, then either ask Claude for a first cut or stack them into an editable timeline yourself.'
      recovery.append(el('div', 'paper-edit__title', hasTimeline ? 'Source timeline ready' : 'Assembly needs another pass'), el('p', 'hint', copy))
      const recoveryActions = el('div', 'row')
      if (hasTimeline) {
        const open = el('button', 'btn ghost btn--small', 'Open source timeline')
    ...
    head.append(el('div', 'paper-edit__title', `Claude’s assembly · ${state.picks.length} clips`), el('p', 'hint', 'Comment on any clip, then ask Claude to revise the assembly.'))
    ...
    review.append(head)
    const picks = el('div', 'multi-assembly__picks')
    for (const [index, pick] of state.picks.entries()) {
      const card = el('article', 'multi-assembly__pick')
      const clip = Object.assign(el('video', 'multi-assembly__pick-video'), { muted: true, playsInline: true, preload: 'metadata', controls: true, ariaLabel: `Review ${pick.source.split('/').pop()} from ${dur(pick.inSec)} to ${dur(pick.outSec)}` })
      clip.src = `/media/${encodeURIComponent(currentProject() ?? '')}/${encodeURI(pick.source)}`
      clip.addEventListener(
        'loadedmetadata',
        () => {
          try {
            clip.currentTime = pick.inSec
          } catch {}
        },
        { once: true },
      )
      const keepInRange = () => {
        if (clip.currentTime < pick.inSec || clip.currentTime >= pick.outSec) {
          clip.pause()
          clip.currentTime = pick.inSec
        }
      }
      clip.addEventListener('timeupdate', keepInRange)
      clip.addEventListener('seeking', keepInRange)
      const comment = Object.assign(el('textarea', 'multi-assembly__comment'), { rows: 2, placeholder: 'Comment for the next edit…', value: state.comments?.[pick.id] ?? '' })
      comment.oninput = () => {
        window.clearTimeout(commentTimer)
        commentTimer = window.setTimeout(async () => {
          const saved = await request('/comments', { pickId: pick.id, comment: comment.value })
          if (!saved.error) state = saved.state
        }, DRAFT_SAVE_MS)
      }
      card.append(clip, el('div', 'multi-assembly__pick-title', `${index + 1}. ${pick.source.split('/').pop()} · ${dur(pick.inSec)}–${dur(pick.outSec)}`), el('p', 'multi-assembly__pick-reason', pick.reason || 'Selected passage'))
      if (pick.text) card.append(el('p', 'multi-assembly__pick-transcript', `“${pick.text}”`))
      card.append(comment)
      picks.append(card)
    }
    review.append(picks)
  ...
  const paintAlignment = () => {
    ...
    if (!alignment?.document || !alignment?.segments?.length) {
    ...
    const title = el('div')
    title.append(el('div', 'paper-edit__title', `Visual alignment · ${alignment.segments.length} screen cuts`), el('p', 'hint', 'Edit the screen cuts in the visual timeline. Rendered review video includes the matching narration; the cut list itself is visual-only.'))
    ...
    const player = Object.assign(el('video', 'multi-assembly__alignment-video'), { muted: true, playsInline: true, controls: true, preload: 'metadata' })
  ...
  const refresh = async () => {
    ...
    buildTimeline.hidden = !state?.picks?.length || Boolean(state.document)
    openTimeline.hidden = !state?.document
    paint()
    const aligned = await fetch(`/api/multi-assembly/audio-align?project=${encodeURIComponent(currentProject() ?? '')}`)
      .then(responseJson)
      .catch((error) => ({ error: error.message }))
    if (!aligned.error && aligned.state?.document) {
  ...
  const build = async () => {
    ...
    await refresh()
    say(`${made.clips} clips are ready in one visual timeline. Review the cards here or open the editor.`, 'ok')
  }
  const stackSelected = async () => {
    ...
    if (made.error) return say(made.error, 'bad')
    await refresh()
    say(`${made.clips} recordings are stacked in one editable timeline.`, 'ok')
  }
  const runAssembly = async (revision = false) => {
    ...
    const askClaude = async () => {
      say(revision ? 'Claude is revising the selected clips from your comments…' : 'Claude is choosing the strongest moments across these recordings…', 'ok')
    ...
    for (const job of jobs)
      watchJobInPlace(job, status, async (finished) => {
        ...
        watchJobInPlace(agentJob, status, async (agentFinished) => {
          ...
          alignment = { ...alignment, document: built.document, renderStep: built.renderStep, rendered: false }
          paintAlignment()
  ...
  openAlignedTimeline.onclick = async () => {
    const opened = await openDocument({ projectId: currentProject(), path: alignment?.document })
  ...
  openTimeline.onclick = async () => {
    const opened = await openDocument({ projectId: currentProject(), path: state?.document })
    if (opened?.error) say(opened.error, 'bad')
  }
  void refresh()
}
...
function vBoardSceneEditor(m, slotId) {
  ...
  crumbs(scopedCrumbs([{ label: 'Video', go: () => go('workflow') }, { label: 'Canvas', go: back }, { label: 'Scene' }]))

  m.append(el('p', 'lede', 'Describe what this part of the video needs to show, then give it the footage that can fulfil it. You can return to the canvas whenever you are ready to arrange and compare it.'))
  ...
  const addTake = el('button', 'btn', 'Use this video')
  ...
  field(media, 'Choose a project video', footage, 'Choose an existing recording, or add one from your computer.')
  const mediaActions = el('div', 'row')
  mediaActions.append(addTake, upload, picker)
  media.append(mediaActions)
  shell.append(heading, form, media, status, backButton)
  m.append(shell)
