...
function paintDocsLink() {
  ...
  b.onclick = () => window.open(S?.docsUrl || 'https://github.com/RoleModel/openscreen/tree/main/website/docs/rolemodel', '_blank', 'noopener')
}
...
function vProject(m, id) {
   ...
   */
  const scripts = (S.scripts ?? []).filter((sc) => sc.project === p.id)
  if (scripts.length && !kind) {
    m.append(el('div', 'client', 'Scripts'))
    const sg = el('div', 'grid')
    for (const sc of scripts) sg.append(scriptCard(p, sc))
    m.append(sg)
  }
  ...
  m.append(el('div', 'client', 'Footage'))
  const g = el('div', 'grid')
