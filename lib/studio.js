...
function scopedCrumbs(parts = []) {
  ...
  trail.push({ label: project.name, go: () => go('library', { preserveProject: true }) })
  return [...trail, ...parts]
}
...
function vProject(m, id) {
  ...
  const actions = S.remotes.length ? S.remotes.map((remote) => ({ icon: 'database', text: `Send project to ${remote}`, run: () => sendProject(remote) })) : [{ icon: 'database', text: 'Set up shared storage', run: () => go('storage') }]
...
function vMake(m) {
  ...
  const showPlan = (r) => {
    ...
    if (r.output === 'template') {
      ...
      templateActions.append(checkBtn, renderBtn, templateStatus)
...
function vVoice(m) {
  ...
  const preview = el('pre', 'voice__lines')
  ...
  linesGroup.append(el('h2', 'voice__label', 'Narration lines'), preview, est)
  f.append(linesGroup, out)
  go.onclick = async () => {
    const r = await fetch('/api/voice', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ projectId: proj.value, script: pick.value, provider: prov.value, voice: voice.value, gap: Number(gap.value) }) }).then(responseJson)
    out.innerHTML = ''
    if (r.error) {
...
function vMultiAssembly(m) {
  m.append(el('p', 'lede', 'Choose the project recordings that belong in this story. Studio makes timestamped screen samples and transcripts in the background, so Claude can choose moments where the words and screen actually match.'))
  const project = currentProjectRecord()
  const videos = (project?.catalog?.files ?? []).filter((file) => file.kind === 'video')
  ...
  for (const file of videos) {
    ...
    label.append(input, el('span', 'multi-assembly__source-name', file.name), el('span', 'multi-assembly__source-meta', dur(file.durationSec) || 'video'))
    sourceList.append(label)
    checks.set(file.rel, input)
  }
  m.append(el('div', 'paper-edit__title', 'Source recordings'), sourceList)
  ...
  const transcribe = el('button', 'btn', 'Transcribe selected')
  const analyze = el('button', 'btn', 'Analyze selected screens')
  const assemble = el('button', 'btn btn--primary', 'Ask Claude to assemble')
  const openTimeline = el('button', 'btn ghost', 'Open visual timeline')
  openTimeline.hidden = true
  const status = el('p', 'hint')
  status.hidden = true
  const actions = el('div', 'row')
  actions.append(transcribe, analyze, assemble, openTimeline)
  const setup = el('section', 'multi-assembly__setup')
  setup.append(notes, actions, status)
  m.append(setup)
  ...
  const paint = () => {
    ...
    for (const [index, pick] of state.picks.entries()) {
      ...
      const clip = Object.assign(el('video', 'multi-assembly__pick-video'), { muted: true, playsInline: true, preload: 'metadata' })
      ...
      card.append(clip, el('div', 'multi-assembly__pick-title', `${index + 1}. ${pick.source.split('/').pop()} · ${dur(pick.inSec)}–${dur(pick.outSec)}`), el('p', 'multi-assembly__pick-reason', pick.reason || 'Selected passage'), comment)
      picks.append(card)
    }
    review.append(picks)
  }
  ...
  const runAssembly = async (revision = false) => {
    ...
    const drafted = await request('/draft', { rels, notes: notes.value })
    if (drafted.error) {
  ...
  transcribe.onclick = async () => {
    ...
    const result = await request('/transcribe', { rels })
    if (result.error) {
      transcribe.disabled = false
      return say(result.error, 'bad')
    }
    say(`Transcribing ${result.steps.length} recordings in the background…`, 'ok')
    const jobs = (await Promise.all(result.steps.map(({ step }) => start(step, { status })))).filter(Boolean)
    ...
    for (const job of jobs)
      watchJobInPlace(job, status, async (finished) => {
        ...
        if (--left === 0) {
          transcribe.disabled = false
          await refresh()
          say(failed ? 'One or more recordings could not be transcribed. Check the job details, then try again.' : 'Transcripts are ready. Ask Claude to assemble them.', failed ? 'bad' : 'ok')
        }
      })
  }
  analyze.onclick = async () => {
    ...
    const result = await request('/analyze', { rels })
    if (result.error) {
      analyze.disabled = false
      return say(result.error, 'bad')
    }
    say(`Analyzing ${result.steps.length} screens in the background…`, 'ok')
    const jobs = (await Promise.all(result.steps.map(({ step }) => start(step, { status })))).filter(Boolean)
    let left = jobs.length
    let failed = false
    if (!left) {
    ...
    for (const job of jobs)
      watchJobInPlace(job, status, async (finished) => {
        ...
        if (--left === 0) {
          analyze.disabled = false
          await refresh()
          say(failed ? 'One or more screen analyses failed. Check the job details, then try again.' : 'Screen evidence is ready. Claude can now use what was actually on screen.', failed ? 'bad' : 'ok')
        }
      })
  }
...
function vStoryboard(m) {
  ...
  function paint() {
    const currentScript = [...(S.scripts ?? [])].filter((item) => item.project === currentProject()).sort((a, b) => String(b.mtime ?? '').localeCompare(String(a.mtime ?? '')))[0]
    const source = board?.brief?.script ?? currentScript
    ...
    if (!slots.length) {
      ...
      const add = el('button', 'btn btn--primary', 'Write the shot list')
      add.onclick = () => briefBox.focus()
      empty.append(add)
  ...
  function nodeCard(node, slot) {
    ...
    remove.onclick = async (event) => {
      ...
      const warning = takesCount ? `Delete “${node.name || 'this scene'}” and its ${takesCount} attached video${takesCount === 1 ? '' : 's'}?` : `Delete “${node.name || 'this scene'}” from the canvas?`
...
/* ── Skills ──────────────────────────────────────────────── */
function vSkills(m) {
  m.append(el('p', 'lede', 'The shared instructions Claude uses across RoleModel Studio. They live with this toolkit, not inside individual video projects.'))
  ...
  m.append(el('div', 'client', 'Studio skills'), list, el('div', 'client', 'Standard shared skills'), standard, el('div', 'client', 'Supporting skills'), support)
  ...
  const paint = (result) => {
    ...
    if (!skills.length) {
      ...
      for (const skill of skills) {
        ...
        const body = el('div', 'body')
        body.append(el('div', 'nm', skill.name), el('div', 'path', skill.description || 'Shared Studio instructions.'), el('div', 'path', skill.path), el('span', 'kind', 'Used automatically'))
        card.append(body)
        list.append(card)
      }
    }
  ...
  const loadSkills = async () => {
    ...
    status.textContent = 'Checking the shared skill directory…'
    ...
    status.textContent = `${result.studio?.length || 0} shared skill${result.studio?.length === 1 ? '' : 's'} available to every Studio project.`
    paint(result)
  }
  ...
  const upload = async (file, replace = false) => {
    ...
    await loadSkills()
    status.textContent = `Installed ${result.installed.join(' · ')} for every Studio project.`
  }
