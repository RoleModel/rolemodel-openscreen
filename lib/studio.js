...
function vBrand(m) {
  ...
  const paintAdded = () => {
    ...
    for (const item of S.added || []) {
      ...
      const tw = el('div', 'thumbwrap')
      const t2 = el('div', 'thumb')
      const img = Object.assign(el('img'), { src: `/added/${encodeURIComponent(item.file)}`, alt: item.name, loading: 'lazy' })
      img.style.cssText = 'max-inline-size:82%;max-block-size:82%;object-fit:contain'
   ...
   */
  const toWallpapers = (pick) => {
    editing = null
    view = 'wallpapers'
    for (const o of document.querySelectorAll('nav button[data-v]')) o.setAttribute('aria-current', String(o.dataset.v === 'wallpapers'))
    render()
    // The editor is opened after the panel exists: vWallpapers may still be
    // fetching its recipes, and `openEditor` is rebound by that render.
    setTimeout(() => pick(), EDITOR_OPEN_MS)
  }
  ...
  wnew.onclick = () => toWallpapers(() => openEditor({ ...WP.DEFAULT_RECIPE, name: '', label: '' }))
  wg.append(wnew)
