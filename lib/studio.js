 ...
 */
const GLOBAL_VIEWS = new Set(['skills', 'brand', 'components', 'wallpapers', 'storage', 'console', 'library', 'new'])
...
function projectSwitcher() {
  ...
  const reserveMenuSpace = () => {
    if (!header || !document.body.classList.contains('has-editor')) return
    const menuRect = menu.getBoundingClientRect()
    const headerRect = header.getBoundingClientRect()
    const clearance = Math.max(0, Math.ceil(menuRect.bottom - headerRect.bottom))
    header.style.setProperty('--projmenu-editor-clearance', `${clearance}px`)
  }
...
function vProject(m, id) {
  ...
  // Assembly is project work, not a media filter. Keep its door in the project
  // header, beside the occasional project actions, so it is always reachable
  // whether the project already has an AI cut or needs a first one.
  const openAssembly = el('button', 'btn ghost btn--small', 'Assembly')
  openAssembly.type = 'button'
  Object.assign(openAssembly.style, {
    position: 'absolute',
    insetBlockStart: 'var(--op-space-x-small)',
    insetInlineEnd: 'calc(3.2rem + var(--op-space-small))',
  })
  openAssembly.onclick = () => go('paperedit')
  projectHead.style.paddingInlineEnd = '12rem'
  projectHead.append(el('p', 'lede', `${p.client || 'No client'} · ${p.brand} · ${held.length} item${held.length === 1 ? '' : 's'}${f.bytes ? ' · ' + human(f.bytes) : ''} · updated ${ago(f.newest)}`), openAssembly, actionMenu(actions), storageStatus)
  ...
  row.insertBefore(addRecentCapture, re)
...
function vMultiAssembly(m) {
  ...
  const paint = () => {
    ...
    if (!state?.picks?.length) {
      ...
      const head = el('div', 'multi-assembly__review-head')
      ...
      head.append(el('div', 'paper-edit__title', hasTimeline ? 'Source timeline ready' : hasSources ? 'No first cut to review yet' : 'Start an assembly'), el('p', 'hint', copy))
      if (hasSources) {
        const retry = el('button', 'btn btn--primary btn--small', 'Make a new Claude pass')
        retry.onclick = () => void runAssembly()
        const stack = el('button', 'btn ghost btn--small', 'Stack source recordings')
        stack.onclick = () => void stackSelected()
        const emptyActions = el('div', 'row')
        if (hasTimeline) {
          const open = el('button', 'btn ghost btn--small', 'Open source timeline')
          open.onclick = () => openTimeline.click()
          emptyActions.append(open)
        }
        emptyActions.append(retry, stack)
        head.append(emptyActions)
      }
      review.append(head)
      return
    }
  ...
  const paintAlignment = () => {
    ...
    if (!alignment?.document || !alignment?.segments?.length) return
