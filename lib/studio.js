...
function vMake(m) {
  ...
  for (const p of S.presets) brand.append(Object.assign(el('option', null, p.label), { value: p.id }))
  const output = mk('Make', el('select'), 'A video asks Claude to render now. A HyperFrames template gives you a reviewable source project; render it only when you are ready.')
