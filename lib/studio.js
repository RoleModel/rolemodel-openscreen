...
function paintStageNav() {
  ...
  for (const button of nav.querySelectorAll('.workflow-nav__stage')) {
    const state = completion?.[button.dataset.stage]
    button.classList.toggle('workflow-nav__stage--done', state?.done === true)
    button.classList.toggle('workflow-nav__stage--started', state?.done !== true && state?.started === true)
    /*
     * The status is in the accessible name, not only in the colour. A ring of
     * seven buttons that differ by tint tells a screen reader nothing, and tells
     * anyone who cannot separate those two greens nothing either.
     */
    const label = WORKFLOW_STAGES.find((stage) => WORKFLOW_STAGE_BY_VIEW[stage.view] === button.dataset.stage)?.label ?? button.dataset.stage
    const said = state?.done === true ? 'done' : state?.started === true ? 'in progress' : state?.done === false ? 'not started' : null
    button.setAttribute('aria-label', said ? `${label} — ${said}` : label)
  }
}
...
function workflowStageNav() {
  ...
  for (const stage of WORKFLOW_STAGES) {
    ...
    button.type = 'button'
    // The stage key, so paintStageNav can find its evidence without matching labels.
    button.dataset.stage = WORKFLOW_STAGE_BY_VIEW[stage.view]
...
function drawItems() {
  ...
  for (const { p, f } of projects) {
    ...
    card.onclick = async () => {
      ...
      //
      // A card that says "Video in progress · Assembly" opens Assembly. The stage
      // is already on screen in the caption, so landing on the media shelf and
      // making somebody walk Video → Assembly to get there was the app declining
      // to use an answer it had just printed.
      await chooseProject(p.id, { resume })
    }
