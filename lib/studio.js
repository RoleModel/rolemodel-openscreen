...
function vStickers(m) {
  ...
  ui.printPdf.onclick = async () => {
    ...
    ui.printSaveText.textContent = `Save the ${r.layout === 'sheet' ? 'sheet' : 'pages'} · ${r.pages} page${r.pages === 1 ? '' : 's'}`
    ui.printOpen.href = href
    status(`${r.pages} page${r.pages === 1 ? '' : 's'} in ${r.rel} — ${how}, CMYK through ${r.profileName}${skipped}.`, 'ok')
