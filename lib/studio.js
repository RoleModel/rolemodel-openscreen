...
function vWorkflow(m) {
  ...
  const draw = ({ interview, board, workflow }) => {
    ...
    const steps = [
      ...
      { id: 'assembly', view: 'paperedit', label: 'Assembly', detail: videos.length ? 'Choose the lines and make a first cut' : 'Waiting for footage', done: false, title: 'Make the first assembly', body: 'Transcribe a recording, choose the lines to keep, and create an editable first cut without touching a timeline first.', action: 'Make the first assembly' },
  ...
  const loadWorkflow = async () => {
    ...
    const [interview, board, workflow] = await Promise.all([read(`/api/interview?project=${id}`), read(`/api/board?project=${id}`), read(`/api/workflow?project=${id}`)])
    draw({ interview, board, workflow: workflow?.workflow ?? null })
  }
  void loadWorkflow()
}
...
function vMultiAssembly(m) {
  ...
  const actions = el('div', 'row')
  actions.append(assemble, openTimeline)
