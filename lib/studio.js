...
function projectSwitcher() {
  ...
  /*
   * The shared shelf is an option, not the empty one.
   *
   * A script with no project is one that travels between them — a real choice
   * Scripts has always supported. Rendering it as "— none —" would read as a thing
   * you have not done yet.
   */
  sel.append(Object.assign(el('option', null, 'Shared — no project'), { value: '' }))
...
function vScripts(m) {
  ...
  const { project, name, seconds, about, body, count, draftOut, saved, status, draft, save } = ui

  project.append(Object.assign(el('option', null, 'Shared shelf (no project)'), { value: '' }))
  for (const p of S.projects) {
    project.append(Object.assign(el('option', null, (p.client ? p.client + ' · ' : '') + p.name), { value: p.id }))
  }
