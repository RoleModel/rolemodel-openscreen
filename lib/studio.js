...
function vRestyle(m) {
  ...
  m.append(el('p', 'lede', 'Hand one clip to a model with an instruction, and the result comes back as project footage. The original is never replaced.'))
  ...
  const faces = el('div', 'grid refshelf')
  const voiceHead = el('div', 'sec', 'The voice')
  const voices = el('div', 'grid')
  ...
  says(dropHint, 'Paste a picture, or drop one here, to use it as a reference.', '')
  m.append(dropHint)
