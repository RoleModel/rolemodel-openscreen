...
function vStickers(m) {
  ...
  let sheetSizeList = []
  const settings = { sticker: 'telegram', cutout: '', vectorize: '', endpoint: '', prompt: '', sheetName: 'stickers', columns: 4, size: 300, gap: 40, remote: '', printSizeMm: 76.2, printBleedMm: 1.5, printLayout: 'sheet', printPage: '4x6', sheetStickerMm: 50.8, sheetFit: true, dieOffsetMm: 0, dieRoundMm: 0, quoteQty: 250, quoteProduct: 'die-cut', quotePublish: true }
  let affinityWatch = 0
  let makeBtn = null
  ...
  const buildRail = () => {
    ...
    if (settings.printLayout === 'sheet') {
      ...
      /* A sheet is priced by the sheet, so spilling onto a second doubles the
         bill for nothing. On unless somebody wants a size of their own. */
      const fit = mountRow('small-switch')
      fit.el.label.textContent = 'Fit them all on one sheet'
      fit.el.input.id = 'sheet-fit'
      fit.el.label.htmlFor = 'sheet-fit'
      fit.el.input.checked = settings.sheetFit !== false
      fit.el.input.onchange = () => {
        settings.sheetFit = fit.el.input.checked
        buildRail()
        previewSoon()
      }
      pr.append(...fit.nodes)
      if (settings.sheetFit === false) rangeRow(pr, 'Sticker (mm)', 'sheetStickerMm', 15, 120, 1, previewSoon)
      else pr.append(control('hint', { textContent: sheetFitAt ? `Sized to ${sheetFitAt}mm so they fit on one.` : 'Sized so they all fit on one sheet.' }))
    ...
    rangeRow(pr, 'Sticker bleed (mm)', 'printBleedMm', 0, 5, 0.25, previewSoon)
    ...
    try {
      while (mine === previewAt || previewAt !== mine) {
        ...
        const r = await post('/api/stickers/print-preview', {
          ...
          sizeMm: settings.sheetStickerMm,
          bleedMm: settings.printBleedMm,
          fit: settings.sheetFit !== false,
        ...
        ui.printPreviewAt.textContent = `· page ${r.at + 1} of ${r.pages}${r.stickerMm ? ` · ${r.stickerMm}mm each` : ''}${r.skipped ? ` · ${r.skipped} missing` : ''}`
        if (r.stickerMm && r.stickerMm !== sheetFitAt) {
          sheetFitAt = r.stickerMm
          if (settings.sheetFit !== false) buildRail()
        }
  ...
  ui.printPdf.onclick = async () => {
    ...
    const r = await post('/api/stickers/print', { projectId: project(), name: settings.sheetName, items: [...ticked], sizeMm: settings.printLayout === 'sheet' ? settings.sheetStickerMm : settings.printSizeMm, fit: settings.sheetFit !== false, bleedMm: settings.printBleedMm, layout: settings.printLayout, page: 'all', dieOffsetMm: settings.dieOffsetMm, dieRoundMm: settings.dieRoundMm })
    busy(false)
    if (r.error) return say(r.error, 'bad')
