...
function vTimeline(m) {
  ...
  const { lede, empty, stage, video, scene, bed, gap, play, clock, under, stat, save, canvas, pick, add, shelf, shelfProject, shelfList, shelfEmpty, shelfToggle, shelfEditor, editBack, editFull, editSave, editName, editWallpaper, editParts, editStatus } = ui
  ...
  const loadShelf = async () => {
    ...
    for (const sc of scenes) {
      ...
      card.insert.onclick = () => {
        if (!cut) return
        const seconds = Math.max(1, (Number(cardRoot.dataset.ms) || 4000) / 1000)
        ...
        track.clips.push({
          ...
          name: sc.name,
          in: 0,
          out: Number(seconds.toFixed(3)),
          at: Number(state.playhead.toFixed(3)),
          /* Which project the scene came from, so the preview — and one day
             the render — can find the file again. */
          scene: { project: proj },
        })
        dirty = true
        draw()
        toast(`${sc.name} at the playhead — Save cut keeps it.`)
      }
