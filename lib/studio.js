...
function vField(m) {
  ...
  saveBtn.type = 'button'
  const note = el('p', 'form-hint hint')
  note.hidden = true
  saveBtn.onclick = async () => {
    ...
    const result = await fetch('/api/scene', {
      ...
      .then(responseJson)
      .catch((error) => ({ error: error.message }))
    tone(note, result.error ? 'bad' : 'ok')
    note.textContent = result.error ?? `Saved as ${result.name}. Canvas can mount it now.`
  }
  // A scene belongs to a project. Say so before the click rather than after it.
  if (!currentProject()) {
    note.hidden = false
    tone(note, 'bad')
    note.textContent = 'Open a project to save this as a scene.'
    saveBtn.disabled = true
  }
  intoFooter(copyBtn, nameIn, saveBtn, note)
