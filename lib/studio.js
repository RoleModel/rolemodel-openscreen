...
function vStickers(m) {
      ...
      el.remove.onclick = async () => {
        if (!sure) {
          sure = setTimeout(() => {
            ...
            el.remove.textContent = '×'
          }, 3000)
          el.remove.classList.add('stickers__remove--sure')
          el.remove.textContent = 'Sure?'
          return
        }
        clearTimeout(sure)
  ...
  const buildRail = () => {
    ...
    makeBtn = control('button', { className: 'btn ghost btn--small', type: 'button' })
  ...
  const run = async (step, button, label) => {
    ...
    if (step === 'vectorize') Object.assign(body, { model: settings.vectorize, endpoint: settings.endpoint })
...
function lookDials({ rt, look, onChange, host, say = toast }) {
  ...
  const paintStops = () => {
    ...
    if (look().stops.length < rt.LOOK_MAX_STOPS) {
      const add = control('button', { className: 'btn ghost btn--small', textContent: 'Add a stop' })
      add.onclick = () => {
