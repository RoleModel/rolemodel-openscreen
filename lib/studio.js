...
async function load() {
  ...
  const WHY = {
    ...
    hyperframes: [
      'the /hyperframes skills a Make render uses, and Kokoro for Voice. Not installed anywhere — npx fetches it on first use.',
      'not in the npx cache yet. Nothing to fix: the first render or voice line fetches it.',
    ],
...
function vProject(m, id) {
  ...
  doImport.onclick = async () => {
    ...
    importHint.textContent =
      `Added to ${r.into}` + (r.renamed ? ` as ${r.renamed} — something was already called that` : '') + '.'
    importPath.value = ''
    await load()
  }
...
function vEditor(m) {
  ...
  m.append(el('h2', null, 'Editor'), el('p', 'lede', 'Zooms, annotations, trims and captions. The editor opens here, in this window, with the navigation still beside it — nothing is exported until you say so.'))
 ...
 */
function vReview(m) {
  m.append(el('h2', null, 'Review'), el('p', 'lede', 'Send a finished video for review and get a link a client opens without an account. Their notes land on the frame they are about, rather than in a paragraph of email.'))
  ...
  const draw = async () => {
    ...
    if (!d.configured) {
      ...
      save.onclick = async () => {
        ...
        save.disabled = true
        const r = await (
          await fetch('/api/review/settings', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(payload) })
        ).json()
        save.disabled = false
        if (r.error) {
          tone(hint, 'bad')
    ...
    if (shared) {
      ...
      for (const p of d.projects) {
        for (const v of p.videos) {
          const c = el('div', 'card')
          const b = el('div', 'body')
          b.append(Object.assign(el('div', 'nm'), { textContent: v.title }), Object.assign(el('div', 'path'), { textContent: p.workspace + ' · ' + p.name }))
          /*
          ...
          const openIt = el('button', 'btn ghost', 'Open in OpenFrame')
          openIt.onclick = async () => {
            const link = await resolve(openIt)
            if (link) open(link)
          }
          const copyIt = copyButton(el('button', 'btn ghost', 'Copy link'), 'Copy link', () => resolve(copyIt))
          b.append(openIt, copyIt, why)
          c.append(b)
          g.append(c)
        }
      }
...
function demoStepsToScript(rows) {
  ...
  for (const r of rows) {
    ...
    out.push('```do', `${r.verb} ${bare ? args[0] : args.map(demoQuote).join(' ')}`, '```', '')
  }
  return out.join('\n').trim() + '\n'
}
...
function demoBuilder(onChange) {
  ...
  const emit = () => onChange(demoStepsToScript(rows), rows.length)
  ...
  const draw = () => {
    ...
    rows.forEach((r, i) => {
      ...
      const kill = el('button', 'btn ghost', 'Remove')
      up.disabled = i === 0
  ...
  const add = el('button', 'btn ghost', 'Add a step')
  add.onclick = () => {
...
function vRecord(m) {
   ...
   */
  const builder = demoBuilder((text, count) => {
    if (!handEdited) script.value = text
    stepCount.textContent = count ? `${count} step${count === 1 ? '' : 's'}` : ''
    recheck()
  })
  ...
  const syncKnobs = () => {
    ...
    for (const c of driverOnly) {
      c.disabled = !scripted
      c.closest('.form-group')?.style.setProperty('opacity', scripted ? '1' : '0.45')
    }
  ...
  const recheck = () => {
    ...
    checking = setTimeout(async () => {
      ...
      tone(scriptHint, 'ok')
      scriptHint.textContent = `${d.actions} action${d.actions === 1 ? '' : 's'} · ${d.narration} narration line${d.narration === 1 ? '' : 's'}${d.urls.length ? ' · visits ' + d.urls.join(', ') : ''}`
    }, DEMO_CHECK_MS)
  }
  recheck()
  ...
  go.onclick = async () => {
    ...
    steps.append(
      plan([
        r.script
          ? ['Drive the app through the script, and record it.', 'The capture ends when the script does. ' + (secs.value ? secs.value + 's is the backstop.' : 'No backstop set.') + ' Script saved at ' + r.script]
...
function vRecast(m) {
  ...
  const syncOpts = () => {
    ...
    fmtHint.textContent =
      fmt.value === 'webm'
        ? 'webm skips the narration mux: rm-mux writes mp4, and it is the step that reconciles the render’s clock with the narration’s. Subtitles get burned by recast instead, against the wrong clock.'
        : ''
  }
