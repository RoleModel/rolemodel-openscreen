...
function vStickers(m) {
  ...
  const buildRail = () => {
    ...
    if (settings.printLayout === 'sheet') {
      selectRow(pr, 'Paper', [{ id: 'letter', label: '8.5" × 11"' }, { id: '4x6', label: '4" × 6"' }, { id: 'a4', label: 'A4' }], 'printPage')
    ...
    selectRow(pr, 'Quoting', [{ id: 'die-cut', label: 'Die-cut singles' }, { id: 'sheet', label: 'Sticker sheets' }], 'quoteProduct')
  ...
  ui.quote.onclick = async () => {
    ...
    const r = await post('/api/stickers/quotes', { sizeMm: settings.printSizeMm, quantity: settings.quoteQty, product: settings.quoteProduct })
    busy(false)
    if (r.error) return say(r.error, 'bad')
    const per = settings.quoteProduct === 'sheet' ? 'sheet' : 'sticker'
    ui.quotesHead.hidden = false
    ui.quotes.hidden = false
    ui.quotesAsked.textContent = `· ${settings.quoteQty} × ${Math.round(settings.printSizeMm)}mm ${per}s`
    ...
    for (const q of r.quotes) {
      ...
      if (q.error) {
        ...
        /* "table" is the honest word for a number somebody typed in once. */
        const how = q.live ? 'read from their page just now' : q.kind === 'api' ? 'quoted live' : q.kind === 'page' ? 'from the table (their page shows another size)' : 'from the table'
        row.source.textContent = `${how} · ${q.checkedAt}`
        if (r.best && q.id === r.best.id) nodes[0].classList.add('stickers__quote--best')
      }
      ui.quotes.append(...nodes)
    }
    if (r.best) status(`Cheapest: ${r.best.name} at $${r.best.unit.toFixed(2)} a ${per} — $${r.best.total} for ${r.best.quantity}.`, 'ok')
  }
