...
function vComponents(m) {
  ...
  /* The field designer saves a scene into a project, so it needs to be told
     which one. Without the id it still designs and copies a tag; it just cannot
     save, and says so rather than failing at the click. */
  ui.openField.onclick = () => {
    const id = currentProject()
    window.open(`/components/field-designer.html${id ? `?project=${encodeURIComponent(id)}` : ''}`, '_blank')
  }
}
