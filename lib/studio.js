...
function vProject(m, id) {
   ...
   */
  const recentCaptures = el('div', 'project-capture-import')
  ...
  const addCapture = el('button', 'btn secondary btn--small', 'Add')
  ...
  recentCaptures.append(captureSelect, addCapture, refreshCaptures)
  ...
  m.append(recentCaptures, captureHint)
...
function vMultiAssembly(m) {
  ...
  alignRow.append(screen, audio, script, alignButton, openAlignedTimeline, renderAligned)
  ...
  alignButton.onclick = async () => {
    ...
    for (const job of jobs) watchJobInPlace(job, status, async (finished) => {
      ...
      watchJobInPlace(agentJob, status, async (agentFinished) => {
        ...
        say(`${alignment.segments.length} narration moments are mapped to screen cuts. Review the timeline before rendering.`, 'ok')
      })
    })
  }
