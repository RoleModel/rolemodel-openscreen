...
function vProject(m, id) {
  ...
  void fetch(`/api/multi-assembly?project=${encodeURIComponent(p.id)}`)
    .then(responseJson)
    .then((result) => {
      if (view !== 'library' || openProject !== p.id || !result.state?.picks?.length) return
      const picks = result.state.picks.length
      assemblyTitle.textContent = `Claude assembly ready · ${picks} selected clip${picks === 1 ? '' : 's'}`
      assemblyCopy.textContent = result.state.document
      ...
      openAssemblyTimeline.hidden = !result.state.document
      openAssemblyTimeline.onclick = async () => {
        const opened = await openDocument({ projectId: p.id, path: result.state.document })
...
function vWorkflow(m) {
  ...
  const draw = ({ interview, board, workflow, assembly }) => {
    ...
    const assemblyReady = assemblyPicks.length > 0
    const steps = [
      {
        id: 'interview',
        view: 'interview',
      ...
      { id: 'record', view: 'record', label: 'Record', detail: videos.length ? `${videos.length} recording${videos.length === 1 ? '' : 's'} ready` : 'Capture or add footage', done: videos.length > 0, title: 'Record the work', body: 'Use the script as your guide, then record the screen or add the footage you already have.', action: videos.length ? 'View recordings' : 'Record or add footage' },
      {
        ...
        view: 'paperedit',
        label: 'Assembly',
        detail: assemblyReady ? `${assemblyPicks.length} selected clip${assemblyPicks.length === 1 ? '' : 's'} ready to review` : videos.length ? 'Choose the lines and make a first cut' : 'Waiting for footage',
        done: false,
        inProgress: assemblyReady,
        title: assemblyReady ? 'Your assembly is ready to review' : 'Make the first assembly',
        body: assemblyReady ? 'Claude’s timestamped selects are saved with this project. Review the clips and comments before you open the visual timeline.' : 'Transcribe a recording, choose the lines to keep, and create an editable first cut without touching a timeline first.',
  ...
  const loadWorkflow = async () => {
    ...
    const [interview, board, workflow, assembly] = await Promise.all([read(`/api/interview?project=${id}`), read(`/api/board?project=${id}`), read(`/api/workflow?project=${id}`), read(`/api/multi-assembly?project=${id}`)])
    draw({ interview, board, workflow: workflow?.workflow ?? null, assembly })
  }
  void loadWorkflow()
}
...
function vMultiAssembly(m) {
  ...
  const assemble = el('button', 'btn btn--primary', 'Prepare clips & ask Claude')
  ...
  const actions = el('div', 'row')
  actions.append(assemble, buildTimeline, openTimeline)
