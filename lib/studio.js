...
function toHex(painted) {
  ...
  return '#' + n.slice(0, 3).map((v) => Math.round(Number(v)).toString(16).padStart(2, '0')).join('')
}
...
function accentStyle(accent) {
  ...
  return [
    `--brand:${accent}`,
    `--on-brand:var(--op-color-${family}-on-base)`,
    `--brand-text:var(--op-color-${family}-minus-two)`,
  ].join(';')
}
...
function colorMenu({ families, value, onPick, format = 'token', noneLabel = null }) {
  ...
  const tokenFor = (family) => `var(--op-color-${family}-original)`
  const valueFor = (family) => (format === 'hex' ? toHex(paintedColor(tokenFor(family))) : tokenFor(family))
  ...
  const familyOf = (v) => {
    if (!v) return null
    const direct = families.find((f) => tokenFor(f) === v)
    if (direct) return direct
    const painted = paintedColor(v)
    return families.find((f) => paintedColor(tokenFor(f)) === painted) ?? null
  }
  ...
  const drawTrigger = () => {
    ...
    label.textContent = family ? colorLabel(family) : current ? current : noneLabel || 'Choose a colour'
  }
  ...
  const drawPanel = () => {
    ...
    for (const family of families) {
      const token = tokenFor(family)
      const painted = paintedColor(token)
      const b = el('button', 'colormenu__swatch')
      b.type = 'button'
      b.setAttribute('role', 'option')
      b.setAttribute('aria-selected', String(familyOf(current) === family))
      if (painted === dead) {
        b.classList.add('colormenu__swatch--dead')
        b.title = `${colorLabel(family)} — not resolving`
      } else {
        b.style.background = token
        b.title = colorLabel(family)
      }
      b.onclick = () => choose(valueFor(family))
      panel.append(b)
    }
  }
...
function vCut(m) {
  m.append(
    el(
      'p',
      'lede',
      'Pick the footage and put it in order. This hands the editor a first assembly — stitching, trimming to the frame and titles happen there, on its timeline. A rough in and out here is optional, and nothing is re-encoded either way.',
    ),
  )
