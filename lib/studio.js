...
function vScenes(m) {
  ...
  if (footageGroup) {
    ...
    const renderFootageReview = () => {
      ...
      footageReviewStatus.textContent = `${footageState.transcript.words?.length ?? 0} words across ${cues.length} timed lines. ${timing} Claude’s suggestions and your own passage choices both save back to this Canvas scene.`
      ...
      for (const cue of cues) {
        ...
        for (const index of wordIndexes) {
          ...
          token.onclick = (event) => {
            ...
            // Leave the target in place just long enough for the browser to
            // complete the click. Replacing it during that event makes both the
            // live UI and the recording driver treat the gesture as interrupted.
            window.setTimeout(renderFootageReview, 50)
          }
      ...
      for (const wordIndex of wordIndexes) {
        ...
        if (canEdit) {
          ...
          token.onclick = (event) => {
            ...
            player.currentTime = Number(word.startSec) || 0
            // Keep the clicked word around until the browser has finished the
            // gesture, then paint the updated selection.
            window.setTimeout(paintReview, 50)
          }
        }
        text.append(token, document.createTextNode(' '))
      }
...
function vBoardSceneEditor(m, slotId) {
  ...
  const renderReview = () => {
    ...
    head.append(el('div', null, `${paperState.transcript.words?.length ?? 0} words · ${cues.length} timed lines · ${timing}`), use)
    ...
    for (const [index, cue] of cues.entries()) {
      ...
      for (const wordIndex of words) {
        ...
        token.onclick = (event) => {
          ...
          selectionAnchorWord = wordIndex
          player.currentTime = Number(word.startSec) || 0
          // Keep this word button alive through the click; re-rendering during
          // that event turns a multi-word selection into a flaky interaction.
          window.setTimeout(renderReview, 50)
        }
        text.append(token, document.createTextNode(' '))
      }
      lines.append(line)
