...
function vStickers(m) {
  ...
  const settings = { sticker: 'telegram', cutout: '', vectorize: '', endpoint: '', prompt: '', sheetName: 'stickers', columns: 4, size: 300, gap: 40, remote: '', dataApi: '' }
  let affinityWatch = 0
  ...
  const busy = (on, label) => {
    for (const b of [ui.make, ui.cut, ui.trace, ui.affinity, ui.build, ui.publish, ui.reveal]) b.disabled = on
  ...
  /*
   * ── comments on the working sticker ──
   *
   * Keyed by project and file name, so they follow the sticker onto any sheet
   * and match what the published page shows. The team writes here as itself;
   * visitors write on the page through the Data API.
   */
  const paintComments = async () => {
    const show = Boolean(work?.rel)
    ui.comments.hidden = !show
    if (!show) return
    const sticker = work.rel.split('/').pop()
    const r = await fetch(`/api/stickers/comments?project=${encodeURIComponent(project())}&sticker=${encodeURIComponent(sticker)}`)
      .then(responseJson)
      .catch(() => ({ comments: [] }))
    if (work?.rel?.split('/').pop() !== sticker) return
    ui.thread.replaceChildren()
    ui.commentCount.textContent = r.comments?.length ? `· ${r.comments.length}` : ''
    if (r.problem) ui.thread.append(control('p', { className: 'hint', textContent: r.problem }))
    for (const c of r.comments ?? []) {
      const { root, el } = mountRow('stickers-comment')
      el.who.textContent = c.author || 'Someone'
      el.when.textContent = ago(c.created_at)
      el.body.textContent = c.body
      el.remove.onclick = async () => {
        const d = await fetch('/api/stickers/comments', { method: 'DELETE', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ id: c.id }) })
          .then(responseJson)
          .catch((e) => ({ error: e.message }))
  ...
  ui.commentForm.onsubmit = async (e) => {
    ...
    if (!body || !work?.rel) return
    ui.commentSend.disabled = true
    const r = await post('/api/stickers/comments', { projectId: project(), sticker: work.rel.split('/').pop(), body })
    ui.commentSend.disabled = false
    if (r.error) return say(r.error, 'bad')
  ...
  const buildRail = () => {
    ...
    /* The page's comments go through Neon's Data API; its URL is a team setting. */
    const api = textRow(pb, 'Comments API', 'dataApi', 'https://….neon.tech/neondb/rest/v1')
    api.input.oninput = null
    api.input.value = settings.dataApi ?? ''
    api.input.onchange = async () => {
      const r = await post('/api/stickers/settings', { dataApi: api.input.value.trim() })
      if (r.error) return say(r.error, 'bad')
      settings.dataApi = r.dataApi ?? ''
      api.input.value = settings.dataApi
      say(settings.dataApi ? 'Comments API saved. Publish a sheet again to put comments on its page.' : 'Comments API cleared.')
    }
    intoRail(panel)
  }
  ...
  ui.make.onclick = () => run('generate', ui.make, work ? 'Making a sticker from the picture…' : 'Making a sticker from the prompt…')
  ...
  ui.build.onclick = async () => {
    ...
    busy(false)
    if (r.error) return say(r.error, 'bad')
    status(`Sheet saved as ${r.sheet.rel}.`, 'ok')
    await reload()
  }
  /*
  ...
  void (async () => {
    await reload()
    const st = await fetch('/api/stickers/settings')
      .then(responseJson)
      .catch(() => ({}))
    settings.dataApi = st.dataApi ?? ''
    buildRail()
    setWork(null)
    await paintSources()
  })()
...
function vShowcase(m) {
  ...
  const paintLayers = () => {
    ...
    scene.layers.forEach((l, i) => {
      ...
      const paint = () => {
        ...
        el.when.textContent = `${l.at.toFixed(1)}s – ${(l.at + l.dur).toFixed(1)}s · ${l.enter} in, ${l.exit} out`
      }
      paint()
  ...
  ui.gizmo.onpointerdown = (e) => {
    ...
    if (!r0) return
    const start = { x: e.clientX, y: e.clientY, lx: l.x, ly: l.y, w: l.w, zoom: l.zoom, rect: r0 }
    drag(e, (ev) => {
      const dx = ev.clientX - start.x
      const dy = ev.clientY - start.y
      if (!corner) {
        /* X and Y are percentages of the card's own size, as translate() reads them. */
        l.x = Math.round((start.lx + (dx / start.rect.width) * 100) * 2) / 2
        l.y = Math.round((start.ly + (dy / start.rect.height) * 100) * 2) / 2
      } else {
        ...
        else l.zoom = Math.max(0.4, Math.min(2.5, Math.round(start.zoom * f * 100) / 100))
      }
      apply()
    }, () => buildRail())
  }
  ...
  const newLayer = (from) => {
    ...
    return { ...rt.showcaseDefaults(), ...(from ? { ...from } : {}), media: '', at, dur: Math.round((end - at) * 10) / 10 }
  }
  ui.addLayer.onclick = () => {
  ...
  const rangeRow = (box, f, target = L) => {
    ...
    const i = Object.assign(row.input, { min: f.min, max: f.max, step: f.step, value: target()[f.key] })
    const show = () => (row.readout.textContent = String(Number(i.value)))
    show()
    i.oninput = () => {
      target()[f.key] = Number(i.value)
      show()
      apply()
    }
    box.append(...nodes)
  ...
  const readScene = (body) => {
    ...
    const layers = shows.map((el) => {
      const l = { ...rt.showcaseDefaults(), media: el.getAttribute('media') || '', at: num(el, 'at', 0) / 1000, dur: num(el, 'for', 4000) / 1000 }
  ...
  const paintTemplates = () => {
    ...
    for (const t of rt.SHOWCASE_TEMPLATES) {
      ...
      root.onclick = () => {
        templateOn = t.name
        const { name, note, look, ...dials } = t
        Object.assign(L(), dials)
