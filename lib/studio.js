...
function crumbs(parts) {
  ...
  if (view === 'hyperframes' && hyperframesWorkspace?.url) {
    ...
    const all = el('button', 'btn ghost btn--small', 'All compositions')
    all.type = 'button'
    all.onclick = () => {
    ...
    if (hyperframesWorkspace.folder === 'multi-clip-assembly') {
      const revise = el('button', 'btn btn--primary btn--small', 'Redo with Claude')
      revise.type = 'button'
      revise.onclick = () => {
    ...
    reload.type = 'button'
    reload.onclick = () => hyperframesWorkspace?.reload?.()
...
function vHyperframes(m) {
  ...
  if (hyperframesWorkspace?.url) {
    ...
    workspace.reload = () => loadWorkspace({ force: true })
    loadWorkspace()
    shell.append(frame)
    m.append(shell)
    return
    ...
    elements.forEach((e, i) => {
      ...
      for (const fld of spec?.fields ?? []) {
        ...
        if (isImageField(e.tag, fld)) {
          ...
          group.append(strip)
          if (e.tag === 'rm-shader' || e.tag === 'rm-pixel-reveal') {
            /*
