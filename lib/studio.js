...
function vTimeline(m) {
  ...
  const { lede, empty, stage, video, scene, bed, gap, play, clock, under, stat, save, canvas, pick, add, shelf, shelfProject, shelfList, shelfEmpty, shelfToggle, shelfEditor, editBack, editFull, editInsert, editSave, editName, editWallpaper, editParts, editStatus, removeClip, shelfNew, newName, newBrief, draftScene, newStatus } = ui
  ...
  const showInserted = (clipRef) => {
    ...
    fr.hidden = false
    try {
      fr.contentWindow?.RM?.seek?.(Math.max(0, state.playhead - clipRef.at) * 1000)
    } catch {
  ...
  /*
   * Take the selected clip out.
   *
   * Removal is a cut edit like a drag: it changes the cut in hand, and Save
   * cut is what writes it. The button appears when something is selected;
   * Delete and Backspace do the same from the keyboard, which is why the
   * canvas takes focus on the click that selects.
   */
  const deleteSelected = () => {
    if (!cut || !state.selection) return
    for (const track of cut.tracks ?? []) {
      const i = (track.clips ?? []).findIndex((c) => c.id === state.selection)
      if (i === -1) continue
      const [gone] = track.clips.splice(i, 1)
      state.selection = null
      const fr = insertedFrames.get(gone.id)
      if (fr) {
        fr.remove()
        insertedFrames.delete(gone.id)
      }
      dirty = true
      draw()
      toast(`${gone.name ?? gone.id} removed — Save cut keeps it.`)
      return
    }
  }
