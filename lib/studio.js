...
function vReview(m) {
  ...
  const draw = async () => {
    ...
    if (!d.configured) {
      tone(status, 'warn')
      status.textContent = 'Not configured yet — ' + d.missing.join(' and ') + ' are unset.'
      const note = el('div', 'note')
      note.append(
        el('p', null, 'OpenFrame is where a client leaves timestamped notes. It runs itself, so it needs two things set in your shell:'),
        Object.assign(el('pre', 'log'), {
          textContent: 'export OPENFRAME_URL=http://localhost:3100\nexport OPENFRAME_TOKEN=…   # from OPENFRAME_API_TOKENS on that instance',
          style: 'height:auto',
        }),
        Object.assign(el('div', 'path'), {
