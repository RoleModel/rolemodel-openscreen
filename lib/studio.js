...
function vProject(m, id) {
  ...
  projectHead.append(el('p', 'lede', `${p.client || 'No client'} · ${p.brand} · ${held.length} item${held.length === 1 ? '' : 's'}${f.bytes ? ' · ' + human(f.bytes) : ''} · updated ${ago(f.newest)}`), actionMenu(actions), storageStatus)
