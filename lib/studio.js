...
function vCreator(m) {
  ...
  /*
   * A colour picker that is built when it is opened. The brand menu is 369
   * swatches; building seven of them while the page mounts is what froze it.
   * The swatch button shows the colour; the first click makes the menu.
   */
  const lazyColor = ({ value, onPick }) => {
    const wrap = control('span', { className: 'creator__pickwrap' })
    const swatch = control('button', { className: 'creator__pickbtn', type: 'button' })
    ...
    swatch.onclick = () => {
      if (!menu) {
        ...
        menu.classList.add('creator__pickmenu')
        wrap.append(menu)
        return
      }
  ...
  const paintStops = () => {
    ...
    stateLook.stops.forEach((stop, idx) => {
      ...
      el.swatch.style.background = stop.c
      el.hex.textContent = stop.c
  ...
  const buildDials = () => {
    ...
    for (const [group, label] of rt.LOOK_GROUPS) {
      const fields = rt.LOOK_SCHEMA.filter((f) => f.group === group)
