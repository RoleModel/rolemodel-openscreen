    ...
    actionMenu([
      ...
      },
      {
        ...
        run: async () => {
          ...
          const copyErr = await copyText(r.url)
...
function vReview(m) {
  ...
  const share = async (f, title, button, out) => {
    ...
    out.textContent = ''
    /* The link goes straight to the clipboard; the card grows a Copy link for later. */
    const copyErr = await copyText(r.share.url)
