...
function vStickers(m) {
  ...
  let sheetSizeList = []
  const settings = { sticker: 'telegram', cutout: '', vectorize: '', endpoint: '', prompt: '', sheetName: 'stickers', columns: 4, size: 300, gap: 40, remote: '', printSizeMm: 76.2, printBleedMm: 3.175, printLayout: 'sheet', printPage: '6x8', sheetStickerMm: 50.8, dieOffsetMm: 0, dieRoundMm: 2, quoteQty: 250, quoteProduct: 'die-cut', quotePublish: true }
  let affinityWatch = 0
  let makeBtn = null
  ...
  const buildRail = () => {
    ...
    if (settings.printLayout === 'sheet') {
      ...
      const paper = sheetSizeList.length ? sheetSizeList.map((z) => ({ id: z.id, label: `${z.label}${z.vendors.length ? ` — ${z.vendors.join(', ')}` : ' — no quote on file'}` })) : [{ id: '6x8', label: '6" × 8"' }]
      selectRow(pr, 'Sheet size', paper, 'printPage', previewSoon)
      rangeRow(pr, 'Sticker (mm)', 'sheetStickerMm', 15, 120, 1, previewSoon)
      ...
      rangeRow(pr, 'Cut corner (mm)', 'dieRoundMm', 0, 8, 0.5, previewSoon)
  ...
  ui.quote.onclick = async () => {
    ...
    for (const [group, label, per] of groups) {
      if (!group?.vendors?.length) {
        ...
        /* `hint`, not `p`: control() builds from the templates in
           lib/templates/controls.html and there is no `p` among them. */
        ui.quotes.append(control('hint', { textContent: 'No printer here quotes this size.' }))
        continue
      }
      ui.quotes.append(control('div', { className: 'client', textContent: label }))
  ...
  const paintPreview = async () => {
    ...
    try {
      while (mine === previewAt || previewAt !== mine) {
        ...
        ui.printPreviewAt.textContent = `· page ${r.at + 1} of ${r.pages}${r.skipped ? ` · ${r.skipped} missing` : ''}`
