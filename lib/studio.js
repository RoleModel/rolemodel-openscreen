...
function crumbs(parts) {
  ...
  if (view === 'hyperframes' && hyperframesWorkspace?.url) {
    ...
    const projectMedia = el('button', 'btn ghost btn--small', 'Project media')
    projectMedia.type = 'button'
    projectMedia.id = 'hyperframes-project-media'
    projectMedia.onclick = () => openProjectLibrary(currentProject())
    actions.append(projectMedia)
    ...
    reload.onclick = async () => {
      ...
      const result = await hyperframesWorkspace?.reload?.()
      reload.disabled = false
      if (result?.error) {
        reload.textContent = 'Retry motion editor'
        reload.title = result.error
      } else {
...
function stageMenu() {
  ...
  const wrap = el('div', 'projmenu projmenu--stage')
  const trigger = el('button', 'projmenu__trigger')
  trigger.type = 'button'
  trigger.setAttribute('aria-haspopup', 'menu')
  trigger.setAttribute('aria-expanded', 'false')
  trigger.append(icon(here.icon, 'projmenu__mark'), Object.assign(el('span', 'projmenu__label'), { textContent: here.label }), icon('arrow-up-01', 'projmenu__caret'))
  const menu = el('div', 'projmenu__menu')
  menu.setAttribute('role', 'menu')
  menu.hidden = true
  ...
  for (const stage of WORKFLOW_STAGES) {
    ...
    b.append(icon(stage.icon), Object.assign(el('span', 'projmenu__text'), { textContent: stage.label }))
    ...
    if (said) {
      b.append(Object.assign(el('span', 'projmenu__note'), { textContent: said === 'done' ? '✓' : '•' }))
...
function jobDetailLink(status, id) {
  const details = el('button', 'btn btn--hint btn--small', 'View details')
  details.type = 'button'
  details.onclick = () => {
    jobId = id
    go('console')
...
function writeStatusText(status, text) {
  ...
  const label = el('span', 'activity__label', text)
  label.title = text
  status.append(label)
}
...
function vProject(m, id) {
   ...
   * is on disk.
   */
  projectActions.append(addRecentCapture, re)
...
function nameWithKind(name, kind) {
  const row = el('div', 'nm cardname')
  row.append(Object.assign(el('span', 'cardname__text'), { textContent: name }))
  if (kind) row.append(el('span', `kind ${kind}`, kind))
  return row
}
...
function scriptCard(project, sc) {
  ...
  tw.append(t)
  const b = el('div', 'body')
  b.append(nameWithKind(sc.name, 'script'))
  /*
...
function openPlayer(project, f) {
  ...
  bar.append(Object.assign(el('span', 'player__name'), { textContent: f.name }))
  const close = el('button', 'btn ghost btn--small')
  close.type = 'button'
   ...
   */
  let wave = null
  const listen = el('div', 'wave')
  if (listening) {
    const canvas = el('div', 'wave__canvas')
    const row = el('div', 'wave__row')
    const play = el('button', 'btn ghost btn--small wave__play')
    play.type = 'button'
    play.setAttribute('aria-label', `Play ${f.name}`)
    play.append(icon('play'))
    const clock = el('span', 'wave__clock', '0:00')
    row.append(play, clock)
    listen.append(canvas, row)
  ...
  frame.append(bar, listening ? listen : video)
  dialog.append(frame)
  document.body.append(dialog)
  dialog.showModal()
}
...
function fileCard(project, f) {
  ...
  if (f.kind === 'video' || f.kind === 'audio') {
    const play = el('button', 'thumbwrap__play')
    play.type = 'button'
  ...
  const b = el('div', 'body')
  b.append(nameWithKind(f.name, f.kind), el('div', 'path', f.rel))
...
function vHyperframes(m) {
  ...
  if (hyperframesWorkspace?.url) {
     ...
     */
    const watchRow = el('label', 'hyperframes-workspace__watch')
    const watchBox = Object.assign(el('input'), { className: 'form-control form-control--small', type: 'checkbox', checked: hyperframesWatchSource })
    watchRow.append(watchBox, el('span', 'form-label', 'Reload when the composition changes on disk'))
  ...
  const draw = async () => {
    ...
    for (const project of projects) {
      ...
      content.append(
        nameWithKind(project.title, latest ? 'video' : 'source'),
      ...
      const remove = el('button', 'btn ghost btn--small del', 'Delete')
      remove.type = 'button'
      remove.onclick = async () => {
        if (await deleteHyperframesProject(project, remove, itemStatus)) await draw()
      }
      const actions = el('div', 'row')
      actions.append(open, remove)
      content.append(actions, itemStatus)
...
function vReview(m) {
  ...
  const draw = async () => {
    ...
    const d = await responseJson(await fetch('/api/review'))
    ...
    if (shared) {
      ...
      for (const p of d.projects) {
        for (const v of p.videos) {
          ...
          acts.append(checkIt, openIt, copyIt, remove)
...
function vVoice(m) {
   ...
   */
  const fromScript = el('button', 'btn ghost btn--small voice__reload', 'Load the script')
  fromScript.type = 'button'
  ...
  linesGroup.append(el('h2', 'voice__label', 'Narration lines'), preview, est, saved, fromScript)
...
function vSceneGallery(m) {
  ...
  ;(async () => {
    ...
    for (const sc of scenes) {
      ...
      tw.append(shot)
      const b = el('div', 'body')
      b.append(nameWithKind(sc.name, 'scene'), el('div', 'path', `scenes/${sc.name}.html`))
      c.append(tw, b)
...
function vScenes(m) {
      ...
      for (const fld of spec?.fields ?? []) {
        ...
        if (isImageField(e.tag, fld)) {
          ...
          for (const item of cat.imagery ?? []) {
            ...
            // them now.
            const thumb = Object.assign(el('img'), { src: item.source === 'added' ? `/added/${encodeURIComponent(item.file)}` : `/brand/imagery/${item.file}`, alt: '', loading: 'lazy' })
  ...
  save.onclick = async () => {
    ...
    const r = await responseJson(await fetch('/api/scene', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ projectId: proj.value, name: name.value, body: toMarkup(), ...(sceneFootage() ? { footage: sceneFootage() } : {}) }),
      }))
...
function vMultiAssembly(m) {
  ...
  const buildTimeline = el('button', 'btn btn--primary', 'Build the first cut')
  const openTimeline = el('button', 'btn btn--primary', 'Open first cut in HyperFrames')
  ...
  const paint = () => {
    ...
    if (!state?.picks?.length) {
      ...
      if (hasTimeline) {
        recoveryActions.append(openTimeline)
      }
    ...
    head.append(el('div', 'paper-edit__title', state.title || 'Claude’s assembly'), el('p', 'hint', `${state.picks.length} selected clips · ${nextStep}`))
    ...
    if (fillers.length) {
      const offer = el('div', 'multi-assembly__fillers')
      offer.append(el('p', 'hint', `${fillers.length} clip${fillers.length === 1 ? ' opens' : 's open'} on a filler word. Trim it, or keep it.`))
      for (const item of fillers) {
        const number = state.picks.findIndex((pick) => pick.id === item.pickId) + 1
        const row = el('div', 'multi-assembly__filler')
        row.append(el('span', 'multi-assembly__filler-text', `${number}. ${item.speaker ? `${item.speaker} — ` : ''}“${item.text}” before “${item.followedBy}” · ${(item.trimToSec - item.startSec).toFixed(2)}s`))
        const trim = el('button', 'btn btn--small', `Trim “${item.text}”`)
        trim.type = 'button'
        trim.onclick = async () => {
          trim.disabled = true
...
function vPaperEdit(m) {
  ...
  const paintReview = () => {
    ...
    head.append(
      ...
      el('p', 'hint', canEdit ? (allIncluded ? 'Everything is in the first cut. Click words to take them out; Shift-click extends a range.' : 'Choose a beat, then click the exact words to include. Shift-click extends a range.') : 'Add at least one beat in Assembly settings. The transcript becomes editable as soon as it has a name.'),
    )
    review.append(head)
    if (!canEdit) {
    ...
    for (const [cueIndex, cue] of state.transcript.cues.entries()) {
      ...
      line.append(el('time', 'paper-edit__time', timecode(cue.startSec)), text)
      ...
      for (const wordIndex of wordIndexes) {
        const word = state.transcript.words[wordIndex]
        if (!word) continue
        const token = el(canEdit ? 'button' : 'span', 'paper-edit__word', word.text)
...
function vBoardSceneEditor(m, slotId) {
  ...
  const addTake = el('button', 'btn ghost', 'Add as scene take')
  ...
  const renderReview = () => {
    ...
    const use = el('button', 'btn btn--primary', passages.length ? `Use ${passages.length} selected passage${passages.length === 1 ? '' : 's'} in this scene` : 'Choose transcript words')
    ...
    head.append(el('div', null, `${paperState.transcript.words?.length ?? 0} words · ${cues.length} timed lines · ${timing}${selectedRange ? ` · reviewing ${timecode(selectedRange.inSec)}–${timecode(selectedRange.outSec)}` : ''}`), use)
    ...
    review.append(el('h3', null, 'Review the recording'), el('p', 'hint', passages.length ? `${passages.length} exact passage${passages.length === 1 ? '' : 's'} selected. Shift-click extends a continuous word range.` : 'Click the exact words for this scene. Shift-click another word to select everything between them.'), player, head, lines, clear)
  }
...
function vStoryboard(m) {
  ...
  function addTakeCard(slot) {
    ...
    card.append(el('div', 'board__add-head', `Add a take for "${slot.name}"`), pickFile, row, said)
    return card
  }
  ...
  function paintRail() {
    ...
    /* Said beside the form, for the same reason the take card says its own piece:
       the shared `status` is at the top of the rail, and a refusal written there
       reads as a button that does nothing. */
    const sbSaid = el('div', 'hint')
    sbSaid.hidden = true
    ...
    const doSignIn = async () => {
       ...
       * responseJson, not `.json()`.
       *
       * A raw `.json()` throws on anything that is not JSON — a 404 from an older
       * server on this port, a proxy page — and the rejection was unhandled, so
       * the button silently did nothing at the one moment somebody needs to be
       * told why they are not signed in.
       */
      ...
      sbPass.value = ''
      sbTell(`Signed in as ${r.signedInAs}.`, 'ok')
      await refreshSharing()
    }
    ...
    signUp.onclick = async () => {
      ...
      if (r.needsConfirmation) {
        ...
        sbTell(`Account created for ${r.email}. Confirm it from the email Supabase just sent, then sign in.`, 'warn')
      } else {
   ...
   */
  const renderCut = el('button', 'btn btn--primary', 'Build and render MP4')
  ...
  const build = el('button', 'btn ghost', 'Build the cut in HyperFrames')
  build.onclick = async () => {
    build.disabled = true
    ...
    if (r.error) {
      build.disabled = false
      build.textContent = 'Build the cut in HyperFrames'
      says(status, r.error, 'bad')
      return
    }
    /*
...
function vCut(m) {
  ...
  const clipCard = (c, i) => {
    ...
    const handle = (side) => {
       ...
       */
      h.style.cssText = `position:absolute;inset-block:-0.25rem;inline-size:0.6rem;background:var(--op-color-academy-primary-base);border:1px solid var(--op-color-primary-base);border-radius:var(--op-radius-small);cursor:ew-resize;touch-action:none;box-shadow:0 1px 3px rgb(0 0 0 / 0.45)`
      h.dataset.side = side
...
function sceneOutlets({ tag, startMs, name, durationMs = 4000, withSave = true }) {
  ...
  const copyBtn = el('button', 'btn ghost', 'Copy tag')
  copyBtn.type = 'button'
  ...
  /*
   * Where in the composition, which is a real choice.
   *
   * The playhead is right when a background is being placed under a moment you
   * are looking at, and wrong for a closing card — which is the other thing
   * people make here, and which belongs after the last clip whatever the
   * preview happens to be showing. The end is resolved by the server, because
   * only the file knows where its last clip finishes.
   */
  const where = el('select')
  where.className = 'form-control'
  for (const [value, label] of [['end', 'At the end'], ['playhead', 'At the playhead']]) {
    where.append(Object.assign(el('option', null, label), { value }))
  }
  const insertBtn = el('button', 'btn', 'Insert into composition')
  ...
  const loadCompositions = async () => {
    ...
    for (const c of found) {
      ...
      if (folder) target.append(Object.assign(el('option', null, folder), { value: folder, selected: folder === hyperframesWorkspace?.folder }))
    }
  ...
  insertBtn.onclick = async () => {
    const result = await fetch('/api/hyperframes/insert', {
      ...
      .catch((error) => ({ error: error.message }))
    toast(
      result.error ?? `Inserted into ${result.folder} at ${(result.startMs / 1000).toFixed(1)}s.`,
      result.error ? 'bad' : 'ok',
    )
  }
  ...
  const nameIn = Object.assign(el('input'), { value: name ?? '', placeholder: 'scene name' })
  nameIn.className = 'form-control'
  const saveBtn = el('button', 'btn', 'Save as scene')
  saveBtn.type = 'button'
...
function vField(m) {
  ...
  m.append(el('p', 'lede', 'Eight motions over one halftone field. Save it as a scene and Canvas can mount it into a composition.'))

  const f = { ...FIELD_DEFAULTS }
  const stage = el('div', 'wpprev')
  const scene = document.createElement('rm-scene')
  const field = document.createElement('rm-study-field')
  field.setAttribute('at', '0')
  field.setAttribute('for', '60000')
  scene.append(field)
  stage.append(scene)

  const scrub = Object.assign(el('input'), { type: 'range', min: 0, max: 10400, step: 20, value: 0, className: 'form-control' })
  const clock = el('span', 'v', '0 ms')
  const playBtn = el('button', 'btn ghost', 'Play')
  playBtn.type = 'button'
  const transport = el('div', 'ctl')
  transport.append(playBtn, scrub, clock)

  const markup = el('pre', 'cmd-note')
  m.append(stage, transport, markup)
  ...
  const textRow = (box, label, key, ph) => {
    const i = Object.assign(el('input'), { value: f[key] ?? '', placeholder: ph || '' })
    i.className = 'wide form-control'
    i.oninput = () => {
      f[key] = i.value
      repaint()
  ...
  const rangeRow = (box, label, key, range) => {
    const i = Object.assign(el('input'), { type: 'range', ...range, value: f[key], className: 'form-control' })
    const readout = el('span', 'v', String(f[key]))
    i.oninput = () => {
      f[key] = Number(i.value)
      readout.textContent = i.value
      repaint()
    }
    box.append(el('label', 'form-label', label), i, readout)
  }
  const selectRow = (box, label, key, opts) => {
    const s = el('select')
    s.className = 'wide form-control'
    for (const o of opts) s.append(Object.assign(el('option', null, o), { value: o, selected: o === f[key] }))
    s.onchange = () => {
      f[key] = s.value
      repaint()
    }
...
function vHaze(m) {
  ...
  m.append(el('p', 'lede', 'A swirling, dithered gradient field. One seekable WebGL pass, so the same instant is always the same frame — insert it into a composition as a background, or save it as a scene.'))

  const f = { ...HAZE_DEFAULTS }
  let hazeImagery = []
  const stage = el('div', 'wpprev')
  const scene = document.createElement('rm-scene')
  /* Where a bare filename resolves from — the same shelf the scene editor
     serves its pictures out of, so a tag copied from here works there. */
  scene.setAttribute('assets', '/brand/imagery')
  const haze = document.createElement('rm-haze')
  haze.setAttribute('at', '0')
  haze.setAttribute('for', '60000')
  scene.append(haze)
  stage.append(scene)
  ...
  const shelf = el('div', 'grid refshelf')
  panel.append(shelf)
  const paintShelf = () => {
    shelf.innerHTML = ''
    const none = el('button', 'card refshelf__tile')
    none.type = 'button'
    none.title = 'No picture'
    const blank = el('div', 'thumb refshelf__art')
    none.append(blank)
    ...
    for (const item of hazeImagery) {
      const tile = el('button', 'card refshelf__tile')
      tile.type = 'button'
      tile.title = item.label ?? item.file
      const art = el('div', 'thumb refshelf__art')
      art.style.backgroundImage = `url('/brand/imagery/${encodeURI(item.file)}')`
      tile.append(art)
  ...
  for (const [key, label, placeholder] of HAZE_COPY) {
    const box = g()
    const input = Object.assign(el(key === 'body' ? 'textarea' : 'input'), { value: f[key] ?? '', placeholder })
    input.className = 'wide form-control'
    if (key === 'body') input.rows = 3
    input.oninput = () => {
      f[key] = input.value
      repaint()
    }
    box.append(el('label', 'form-label', label), input)
  }

  sec('Placement')
  for (const [key, label, min, max, step] of HAZE_PLACE) {
    const box = g()
    const slider = Object.assign(el('input'), { type: 'range', min, max, step, value: f[key] })
    slider.className = 'form-control'
    const read = el('span', 'v', String(f[key]))
    slider.oninput = () => {
      f[key] = Number(slider.value)
      read.textContent = String(f[key])
      repaint()
    }
    box.append(el('label', 'form-label', label), slider, read)
  }
  const alignBox = g()
  const align = el('select')
  align.className = 'wide form-control'
  for (const value of ['left', 'center', 'right']) {
    align.append(Object.assign(el('option', null, value), { value, selected: value === f.align }))
  }
  align.onchange = () => {
    f.align = align.value
    repaint()
  }
  alignBox.append(el('label', 'form-label', 'Align'), align)
  ...
  motion.className = 'wide form-control'
  for (const [value, label] of [['flow', 'Flow — advances with the scene clock'], ['still', 'Still — one frozen frame']]) {
    motion.append(Object.assign(el('option', null, label), { value, selected: value === f.flow }))
  }
  motion.onchange = () => {
    f.flow = motion.value
    repaint()
  }
  motionBox.append(el('label', 'form-label', 'Motion'), motion)
...
function vRestyle(m) {
  ...
  const lede = el('p', 'lede', 'Hand one clip to a model with an instruction, and the result comes back as project footage. The original is never replaced.')
  m.append(lede)

  const state = { model: null, file: null, prompt: '', keepAudio: true, resolution: null, images: [], inSec: null, outSec: null, image: null, audio: null, syncMode: null }
  let models = []
  ...
  const wantsClip = () => takesOf() !== 'image+audio'
  const wantsFace = () => takesOf() === 'image+audio'
  const wantsVoice = () => takesOf() !== 'video'

  const list = el('div', 'grid')
  const promptIn = Object.assign(el('textarea'), { placeholder: 'What should change? The rest of the scene is kept.' })
  promptIn.className = 'wide form-control'
  promptIn.rows = 3
  promptIn.oninput = () => { state.prompt = promptIn.value }
  const clipNote = el('p', 'form-hint hint')
  clipNote.hidden = true
  /* Declared here rather than beside its append: paintMode sets its wording,
     and this file has twice shipped a reference that ran before its const. */
  const dropHint = el('p', 'form-hint hint')

  const chosen = el('p', 'form-hint hint')
  chosen.hidden = true
  ...
  const paintChoice = () => {
    ...
    if (!state.file) return void (chosen.hidden = true)
    const span = state.inSec !== null && state.outSec !== null
      ? ` · ${fmt(state.inSec)}–${fmt(state.outSec)} (${(state.outSec - state.inSec).toFixed(1)}s)`
      : ' · the whole clip'
    says(chosen, `Sending ${state.file.split('/').pop()}${span}`, 'ok')
  }

  const clipHead = el('div', 'sec', 'The clip')
   ...
   * on the tile.
   */
  const faceHead = el('div', 'sec', 'The face')
  const faces = el('div', 'grid refshelf refshelf--faces')
  const voiceHead = el('div', 'sec', 'The voice')
  const voices = el('div', 'grid voiceshelf')
  ...
  const markAvatar = () => {
    ...
    if (subject && state.audio) {
      const spoken = projectVoices.find((v) => v.rel === state.audio)?.seconds
      says(chosen, `${subject.split('/').pop()} speaking ${state.audio.split('/').pop()}${spoken ? ` · ${spoken.toFixed(1)}s` : ''}`, 'ok')
  ...
  const paintAvatar = () => {
    ...
    if (!projectImages.length) {
      faces.append(Object.assign(el('p', 'form-hint hint'), { textContent: 'No pictures in this project yet — paste or drop one.' }))
    }
    ...
    if (!projectVoices.length) {
      voices.append(Object.assign(el('p', 'form-hint hint'), { textContent: 'No audio in this project yet. Build one under Voice, and it lands in Audio.' }))
    }
    for (const voice of projectVoices) {
      const card = el('div', 'card clipcard')
      card.dataset.rel = voice.rel
      const body = el('div', 'body')
      body.append(
        Object.assign(el('div', 'nm'), { textContent: voice.rel.split('/').pop() }),
        Object.assign(el('div', 'path'), { textContent: voice.seconds ? `${voice.seconds.toFixed(1)}s of speech` : voice.rel }),
      )
      /*
      ...
      const listen = el('div', 'wave wave--row')
      const canvas = el('div', 'wave__canvas')
      const play = el('button', 'btn ghost btn--small wave__play')
      play.type = 'button'
      play.setAttribute('aria-label', `Play ${voice.rel.split('/').pop()}`)
      play.append(icon('play'))
      listen.append(play, canvas)
  ...
  const paintMode = () => {
    ...
    clipHead.textContent = lipsync ? 'The take' : 'The clip'
    ...
    clipNote.hidden = true
    dropHint.hidden = lipsync
    if (!lipsync) says(dropHint, wantsFace() ? 'Paste a picture, or drop one here, to use it as the face.' : 'Paste a picture, or drop one here, to use it as a reference.', '')
    promptIn.placeholder = avatar
      ? 'Optional. How should they deliver it? Left blank, the model decides.'
      : 'What should change? The rest of the scene is kept.'
    runBtn.textContent = avatar ? 'Make the video' : 'Restyle the clip'
    chosen.hidden = true
    if (wantsFace() || wantsVoice()) paintAvatar()
    if (wantsClip()) paintChoice()
  }

  const promptHead = el('div', 'sec', 'The instruction')
  m.append(clipHead, list, faceHead, faces, voiceHead, voices, chosen, promptHead, promptIn, clipNote)

  const panel = el('div', 'panel2')
  const runBtn = el('button', 'btn', 'Restyle the clip')
  runBtn.type = 'button'
  ...
  const loadClips = async () => {
    ...
    if (!usable.length) {
      says(clipNote, `No clip in this project fits ${models.find((x) => x.id === state.model)?.label ?? 'this model'}. ${(result.clips ?? [])[0]?.problem ?? ''}`, 'bad')
    ...
    for (const clip of result.clips ?? []) {
      ...
      const art = el('div', 'thumbwrap')
      const frame = el('div', 'thumb')
      ...
      const body = el('div', 'body')
      body.append(
        Object.assign(el('div', 'nm'), { textContent: clip.rel.split('/').pop() }),
        Object.assign(el('div', 'path'), { textContent: clip.problem ?? `${clip.seconds ?? '?'}s` }),
      )
      ...
      const trim = el('button', 'btn ghost btn--small', 'Trim…')
      trim.type = 'button'
       ...
       */
      const replace = el('button', 'btn ghost btn--small', 'Replace…')
      replace.type = 'button'
      const picker = Object.assign(el('input'), { type: 'file', accept: 'video/*' })
      picker.hidden = true
      picker.onchange = async () => {
      ...
      row.append(trim, replace, picker)
      body.append(row)
  ...
  function openTrimmer(clip, spec) {
    const dialog = el('dialog', 'player player--trim')
    const frame = el('div', 'player__frame')
    const video = el('video', 'player__video')
    ...
    const bar = el('div', 'player__bar')
    bar.append(Object.assign(el('span', 'player__name'), { textContent: clip.rel.split('/').pop() }))
    const close = el('button', 'btn ghost btn--small')
    close.type = 'button'
    close.setAttribute('aria-label', 'Close the trimmer')
    close.append(icon('cancel-01'))
    close.onclick = () => dialog.close()
    bar.append(close)

    const inRange = Object.assign(el('input'), { type: 'range', min: 0, max: 1000, value: 0, className: 'form-control' })
    const outRange = Object.assign(el('input'), { type: 'range', min: 0, max: 1000, value: 1000, className: 'form-control' })
    const readout = el('p', 'form-hint hint')
    const use = el('button', 'btn', 'Use this range')
    use.type = 'button'
    ...
    const controls = el('div', 'form')
    const play = el('button', 'btn ghost btn--small', 'Play the range')
    play.type = 'button'
    play.onclick = () => {
    ...
    controls.append(
      el('label', 'form-label', 'Start'), inRange,
      el('label', 'form-label', 'End'), outRange,
      readout,
      play,
      use,
    )
    ...
    frame.append(video)
    dialog.append(bar, frame, controls)
    document.body.append(dialog)
    dialog.showModal()
  }
  ...
  const paintPanel = () => {
    panel.innerHTML = ''
    const sec = (t) => panel.append(el('div', 'sec', t))
    const g = () => { const d = el('div', 'form-group'); panel.append(d); return d }
    ...
    for (const model of models) {
      select.append(Object.assign(el('option', null, model.label), { value: model.id, selected: model.id === state.model }))
    }
    select.onchange = () => {
    ...
    chooser.append(el('label', 'form-label', 'Model'), select)
    const spec = models.find((x) => x.id === state.model)
    if (spec?.hint) panel.append(Object.assign(el('p', 'form-hint hint'), { textContent: spec.hint }))
    ...
    if (spec?.controls.includes('keepAudio')) {
      const box = g()
      const check = Object.assign(el('input'), { type: 'checkbox', className: 'form-control form-control--medium', checked: state.keepAudio })
      ...
      box.append(check, el('label', 'form-label', 'Keep audio'))
    }
    /*
    ...
    if (spec?.controls.includes('syncMode')) {
      const box = g()
      const how = el('select')
      how.className = 'wide form-control'
      const says = {
      ...
      for (const option of spec.limits.syncModes ?? []) {
        how.append(Object.assign(el('option', null, says[option] ?? option), { value: option, selected: option === (state.syncMode ?? spec.limits.defaultSyncMode) }))
      }
      how.onchange = () => { state.syncMode = how.value }
      box.append(el('label', 'form-label', 'If the lengths differ'), how)
    }
    if (spec?.controls.includes('resolution')) {
      const box = g()
      const res = el('select')
      res.className = 'wide form-control'
      ...
      for (const option of spec.limits.resolutions ?? []) {
        res.append(Object.assign(el('option', null, option), { value: option, selected: option === state.resolution }))
      }
      res.onchange = () => { state.resolution = res.value }
      box.append(el('label', 'form-label', 'Resolution'), res)
    }
    ...
    if (spec?.controls.includes('images')) {
      ...
      panel.append(el('div', 'sec', max === 1 ? 'Reference image' : 'Reference images'))
      if (spec.requiresImages) {
        panel.append(Object.assign(el('p', 'form-hint hint'), { textContent: `${spec.label} needs at least one.` }))
      }
      if (!projectImages.length) {
        panel.append(Object.assign(el('p', 'form-hint hint'), { textContent: 'Paste or drop a picture to use one.' }))
      }
      /*
    ...
    sec('fal.ai')
    const keyBox = g()
    const keyIn = Object.assign(el('input'), { type: 'password', placeholder: configured ? 'replace the key' : 'paste your fal key' })
    keyIn.className = 'wide form-control'
    const saveKey = el('button', 'btn ghost', 'Save key')
    saveKey.type = 'button'
    ...
    keyBox.append(el('label', 'form-label', 'Key'), keyIn)
  ...
  const spinnerFor = (card) => {
    const veil = el('div', 'working')
    const mark = Object.assign(el('img', 'working__mark'), { src: '/assets/mark-animated.svg', alt: '' })
    mark.setAttribute('aria-hidden', 'true')
    const line = el('p', 'working__line', 'Starting…')
    veil.append(mark, line)
    card.append(veil)
    return { veil, line }
  }
  ...
  runBtn.onclick = async () => {
    const avatar = avatarMode()
    const subject = wantsFace() ? state.image : state.file
    if (avatar && !(subject && state.audio)) return toast(`Pick a ${wantsFace() ? 'picture' : 'clip'} and a voice track first.`, 'bad')
    if (!avatar && !state.file) return toast('Pick a clip first.', 'bad')
    const built = await fetch('/api/fal/edit', {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ projectId: currentProject(), model: state.model, file: state.file, prompt: state.prompt, keepAudio: state.keepAudio, resolution: state.resolution, images: state.images, inSec: state.inSec, outSec: state.outSec, image: state.image, audio: state.audio, syncMode: state.syncMode }),
    }).then(responseJson).catch((error) => ({ error: error.message }))
    ...
    const held = subject
    const card = (wantsFace() ? faces : list).querySelector(`.card[data-rel="${CSS.escape(held)}"]`)
    ...
    stream.onmessage = async (event) => {
      ...
      if (data.done) {
        ...
        void refreshJobs()
        if (data.code === 0) {
          finish(avatar ? 'Made. The video is in Footage.' : 'Restyled. The result is in Footage beside the original.', 'ok')
  ...
  m.addEventListener('drop', async (event) => {
    const pictures = [...(event.dataTransfer?.files ?? [])].filter((file) => file.type.startsWith('image/'))
    if (!pictures.length) return
    event.preventDefault()
    await takePictures(pictures)
  })
...
function slackPanel(host) {
  const box = el('section', 'panel-card')
  box.append(el('h2', null, 'Slack'))
  const lede = el('p', 'hint', 'Post a finished video straight into a channel. Uploads the file itself, so nothing depends on a link or another service staying up.')
  const who = el('div', 'hint')
  const f = el('div', 'form')
  const mk = (label, node, hint) => field(f, label, node, hint)
  const tokIn = mk(
    'Bot token',
    Object.assign(el('input'), { type: 'password', placeholder: 'xoxb-…' }),
  ...
  const chanIn = mk(
    'Channel ID',
    Object.assign(el('input'), { placeholder: '#demos' }),
   ...
   */
  const change = el('button', 'btn btn--hint btn--small', 'Change')
  change.type = 'button'
  change.hidden = true
  change.onclick = () => {
