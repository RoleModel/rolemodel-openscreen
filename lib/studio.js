...
function jobDetailLink(status, id) {
  const details = el('button', 'btn ghost btn--small', 'View details')
  details.type = 'button'
  details.onclick = () => {
    jobId = id
    go('console')
...
function vRestyle(m) {
  ...
  const state = { model: null, file: null, prompt: '', keepAudio: true, resolution: null, images: [], inSec: null, outSec: null }
  let models = []
  ...
  m.append(el('div', 'sec', 'The clip'), list, chosen, el('div', 'sec', 'The instruction'), promptIn, clipNote)
  ...
  const paintPanel = () => {
    ...
    const keyBox = g()
    if (configured) {
      keyBox.append(Object.assign(el('p', 'form-hint hint ok'), { textContent: 'A key is configured. Replace it by pasting a new one.' }))
    }
  ...
  /*
   * Fetched once for the session, not per spinner: it is one small file served
   * by this same Studio, and a card that starts working should not wait on a
   * request that has already been made.
   */
  const markSvg = (() => {
    let pending = null
    return () => {
      pending ??= fetch('/brand-mark.svg')
  ...
  /*
   * The mark, turning, over the clip being worked on.
   *
   * A model call is minutes long and the work belongs to one card, so it is
   * shown on that card rather than handed to the Console — the Console is where
   * you go to find a task, not where you should have to go to watch the one you
   * just started. The shape echoes the app's own icon so a spinner reads as this
   * app working rather than as a generic throbber.
   */
  const spinnerFor = (card) => {
    ...
    card.append(veil)
    /*
     * The app's own mark, inlined so the triangle inside it can turn.
     *
     * An <img> would draw the same picture and expose nothing to animate, and a
     * hand-drawn approximation of the squircle is what this was first — a shape
     * that looked like neither the icon nor a spinner. The last path in the file
     * is the play triangle; everything before it is the rounded square it sits
     * in, which stays still.
     */
    void markSvg().then((markup) => {
      if (!veil.isConnected || !markup) return
      const holder = el('div', 'working__mark')
      holder.innerHTML = markup
      const svg = holder.querySelector('svg')
      svg?.setAttribute('aria-hidden', 'true')
      holder.querySelector('svg > path:last-of-type')?.classList.add('working__play')
      veil.prepend(holder)
    })
    return { veil, line }
  }
  ...
  runBtn.onclick = async () => {
    if (!state.file) return toast('Pick a clip first.', 'bad')
    const built = await fetch('/api/fal/edit', {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ projectId: currentProject(), model: state.model, file: state.file, prompt: state.prompt, keepAudio: state.keepAudio, resolution: state.resolution, images: state.images, inSec: state.inSec, outSec: state.outSec }),
    }).then(responseJson).catch((error) => ({ error: error.message }))
    if (built.error) return toast(built.error, 'bad')

    const card = list.querySelector(`.card[data-rel="${CSS.escape(state.file)}"]`)
    ...
    stream.onmessage = async (event) => {
      ...
      if (data.done) {
        ...
        void refreshJobs()
        if (data.code === 0) {
          finish('Restyled. The result is in Footage beside the original.', 'ok')
          await loadClips()
        } else {
          finish(lastError ?? 'That restyle did not finish — the Console has the output.', 'bad')
        }
        return
      }
  ...
  void (async () => {
    ...
    projectImages = await fetch(`/api/fal/images?project=${encodeURIComponent(currentProject())}`)
      .then(responseJson)
      .then((r) => r.images ?? [])
      .catch(() => [])
