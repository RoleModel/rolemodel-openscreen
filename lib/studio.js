  ...
  const draw = async () => {
    ...
    go.onclick = async () => {
      ...
      copy.onclick = async () => {
        ...
        setTimeout(() => (copy.textContent = 'Copy link'), COPIED_MS)
      }
    ...
    if (shared) {
      ...
      for (const p of d.projects) {
        for (const v of p.videos) {
          ...
          const copyIt = el('button', 'btn ghost', 'Copy link')
          copyIt.onclick = async () => {
            const link = await resolve(copyIt)
            if (!link) return
            await navigator.clipboard.writeText(link).catch(() => {})
            copyIt.textContent = 'Copied'
            setTimeout(() => (copyIt.textContent = 'Copy link'), COPIED_MS)
          }
