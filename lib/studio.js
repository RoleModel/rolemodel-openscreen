...
function projectSwitcher() {
  const wrap = el('div', 'projswitch')
  /*
   * Shown on every panel, including the ones a project does not scope.
   *
   * It was hidden on Brand, Components, Storage, Console and the Library, on the
   * grounds that a project name over the Components gallery claims a scope that is
   * not there. That was the wrong read: it is the control for which space you are
   * about to work in, and hiding it exactly where you are between pieces of work
   * means the way to switch is to go somewhere else first.
   *
   * Nothing to switch between on a first run, though, and a picker whose only
   * option means "none" reads as a setting you got wrong.
   */
  if (!S?.projects?.length) return wrap

  const sel = el('select', 'form-control form-control--small')
  sel.setAttribute('aria-label', 'Project')
  /*
   * Real projects only.
   *
   * "Shared — no project" was an option here for one release and it was the wrong
   * shape: a project is the space you are working in, and "shared" is a property
   * of a script, not a place to stand. As a workspace it made a half-state where
   * Scripts worked and the other eight panels did not — and once the pipeline group
   * hides without a project, it would have hidden the only panel that state was
   * good for. The choice lives in Scripts' rail now, next to the script it applies
   * to.
   */
  sel.append(Object.assign(el('option', null, 'Choose a project…'), { value: '' }))
  for (const p of S.projects ?? []) {
    sel.append(Object.assign(el('option', null, (p.client ? p.client + ' · ' : '') + p.name), { value: p.id }))
  }
...
function paintNavGroups() {
  ...
  for (const n of document.querySelectorAll('[data-group="make"]')) n.hidden = !inProject
}
