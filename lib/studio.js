...
function vPaperEdit(m) {
  ...
  const saveBeats = el('button', 'btn', 'Set the beats')
  ...
  const review = el('div', 'full')
  form.append(review, status)
  m.append(form)
  ...
  const selectionFromReview = () => {
    const byShot = new Map((state?.plan?.shots ?? []).map((shot) => [shot.name, []]))
  ...
  const paintReview = () => {
    ...
    if (!activeBeat || !(state.plan?.shots ?? []).some((shot) => shot.name === activeBeat)) activeBeat = state.plan?.shots?.[0]?.name ?? ''
    const head = el('div', 'full')
    const canEdit = Boolean(state.plan?.shots?.length)
    head.append(
      el('div', 'paper-edit__title', canEdit ? '3. Shape the first cut' : 'Transcript'),
      el('p', 'hint', canEdit ? 'Choose a beat once, then click or sweep across lines to add them there. Click a line already in that beat to take it back out.' : 'The timed words Studio will hand to Claude for the first assembly.'),
    )
    review.append(head)
    if (!state.plan?.shots?.length) {
      review.append(el('div', 'hint', `Transcript attached: ${state.transcript.words.length} words across ${state.transcript.cues.length} timed passages. Name the beats above, then ask Claude for the first assembly.`))
    }
    ...
    if (state.plan?.shots?.length) review.append(beatBar)
    ...
    const setAssignment = (cueIndex, shot) => {
      ...
      line?.classList.toggle('paper-edit__line--active', shot === activeBeat)
    ...
    for (const [cueIndex, cue] of state.transcript.cues.entries()) {
      ...
      line.classList.toggle('paper-edit__line--active', assigned === activeBeat)
    ...
    save.onclick = async () => {
      ...
      say(`${cut.clips} clips are ready in an editable OpenScreen document. Open Cut or Editor to continue from there.`, 'ok')
    }
    review.append(save)
  }
  ...
  const loadState = async () => {
    ...
    beats.value = (state.plan?.shots ?? []).map((shot) => [shot.name, shot.intent, shot.seconds ? `${shot.seconds}s` : null].filter(Boolean).join(' — ')).join('\n')
  ...
  saveBeats.onclick = async () => {
    const r = await request('plan', { plan: { shots: planFromLines() } })
    if (r.error) return say(r.error, 'bad')
    state = r.state
    paintReview()
    say('The beats are ready for Claude.', 'ok')
  }
  askClaude.onclick = async () => {
    const r = await request('draft', { notes: notes.value })
    if (r.error) return say(r.error, 'bad')
    say('Claude is making the first selection in the background. Keep working here, then load the selection when it finishes.', 'ok')
    await start(r.step, { openConsole: false })
  }
  loadClaude.onclick = async () => {
    const r = await request('selection', { fromFile: true })
    if (r.error) return say(r.error, 'bad')
    state = r.state
    paintReview()
    const missing = r.coverage?.empty?.length ? ` Still missing: ${r.coverage.empty.join(', ')}.` : ''
    say(`Loaded Claude’s first assembly: ${r.coverage?.ranges ?? 0} passages.${missing}`, r.coverage?.empty?.length ? 'warn' : 'ok')
  }
...
function vStoryboard(m) {
  ...
  function paintRail() {
    ...
    if (node) {
      ...
      const head = el('div', 'board__picked')
      head.append(
        Object.assign(el('div', 'board__picked-name'), { textContent: node.name }),
        Object.assign(el('div', 'hint'), { textContent: node.intent || 'No intent set for this shot.' }),
      )
      rail.append(head)
      if (slot) {
      ...
      rail.append(Object.assign(el('div', 'hint'), { textContent: 'Click a shot to see its takes. Drag a shot to move it; drag from its right-hand dot to the next shot to set the order.' }))
    }
    ...
    const refreshSharing = async () => {
      ...
      for (const c of [sbEmail, sbPass, signIn]) c.closest('.form-group')?.style.setProperty('display', configured ? '' : 'none')
      signIn.closest('.form-group')?.style.setProperty('display', configured ? '' : 'none')
      signOut.hidden = !sh.signedInAs
      // Creating an account is only ever the first thing you do here.
      signUp.hidden = Boolean(sh.signedInAs)
