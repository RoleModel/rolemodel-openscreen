...
function vStorage(m) {
  ...
  const refresh = async () => {
    ...
    if (!r.ok) {
      entries = []
      listing.innerHTML = ''
      // The first line only: rclone's failures run to a paragraph, and the first
      // line is the one that says what went wrong.
      listing.append(Object.assign(el('p', 'empty'), { textContent: (r.err || 'could not read that remote').split('\n')[0] }))
      return
    }
    entries = r.entries
    paintList()
