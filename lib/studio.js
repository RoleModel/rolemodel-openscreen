...
function vCreator(m) {
  ...
  ui.png.onclick = async () => {
    const width = Number(ui.pngSize.value)
    const [aw, ah] = ui.ratio.value.split('/').map(Number)
  ...
  intoFooter(...sceneOutlets({ tag, startMs: () => Number(scrub.value), name: 'look', durationMs: 8000 }))
