...
function vStoryboard(m) {
  ...
  const paintScriptGuide = () => {
    ...
    build.textContent = shots.length ? `Create ${shots.length} slots` : 'No speakable lines'
  ...
  function nodeCard(node, slot) {
    ...
    choose.textContent = selected === node.id ? 'Selected' : 'Select'
  ...
  function paintRail() {
     ...
     */
    const sbEmail = control('input', { type: 'email', placeholder: 'you@rolemodelsoftware.com' })
    const sbPass = control('input', { type: 'password', placeholder: 'password' })
    ...
    const signIn = control('button', { className: 'btn btn--small', textContent: 'Sign in' })
     ...
     */
    const signUp = control('button', { className: 'btn btn--small', textContent: 'Create account' })
