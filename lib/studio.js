...
async function load({ restoreLastView = true } = {}) {
  ...
  if (restoreLastView && S.lastView && known.has(S.lastView) && !NEEDS_A_DOCUMENT.has(S.lastView)) view = S.lastView
...
function crumbs(parts) {
  ...
  host.append(nav)
  const stages = workflowStageNav()
  if (stages) host.append(stages)
 ...
 */
const GLOBAL_VIEWS = new Set(['skills', 'brand', 'components', 'wallpapers', 'storage', 'usage', 'console', 'library', 'new'])
...
const WORKFLOW_STAGES = [
  { view: 'interview', label: 'Plan' },
  { view: 'scripts', label: 'Script' },
  { view: 'storyboard', label: 'Canvas' },
  { view: 'record', label: 'Record' },
  { view: 'paperedit', label: 'Assembly' },
  { view: 'editor', label: 'Edit' },
  { view: 'review', label: 'Review' },
]

function workflowStageNav() {
  if (!currentProject() || GLOBAL_VIEWS.has(view)) return null
  const nav = el('nav', 'workflow-nav')
  nav.setAttribute('aria-label', 'Video stages')
  const current = WORKFLOW_STAGES.find((stage) => stage.view === view)
  for (const stage of WORKFLOW_STAGES) {
    const button = el('button', 'workflow-nav__stage', stage.label)
    button.type = 'button'
    button.classList.toggle('workflow-nav__stage--current', current?.view === stage.view)
    if (current?.view === stage.view) button.setAttribute('aria-current', 'step')
    button.onclick = () => go(stage.view)
    nav.append(button)
  }
  return nav
}
...
function showJobStatus(status, job, state = job.running ? 'running' : 'done') {
  ...
  if (state === 'running') {
    tone(status, 'ok')
    status.textContent = `${label} is working in the background.`
    return
  }
  if (state === 'done') {
    tone(status, 'ok')
    status.textContent = `${label} finished. Its result is ready in this project.`
  } else {
    tone(status, 'bad')
    status.textContent = `${label} stopped before it finished.`
  }
...
function paintActivity() {
  ...
  if (!activity) return
  // A page that starts work with its own status owns that feedback. Do not fall
  // back to any running job here: that duplicates the card's message below the
  // content and makes one action look like two separate tasks.
  const job = activeJobId ? allJobs.find((item) => item.id === activeJobId) : null
  activity.hidden = !job
  if (!job) return
  activity.innerHTML = ''
  const status = el('p', 'activity__status')
...
async function watchJobInPlace(job, status, done) {
  const check = async () => {
    ...
    if (!current) {
      tone(status, 'bad')
      status.textContent = 'Studio lost track of this task before it finished.'
      done?.(null)
      return
    }
    if (current.running) {
...
async function start(step, { status = null } = {}) {
  ...
  jobId = r.job.id
  // A panel that supplied its own status owns the feedback for this run. Showing
  // the same job in the page-wide activity strip produces two error messages and
  // two "View details" buttons for one failure. The strip is reserved for work
  // that has no in-place home (for example a free-form Console command).
  activeJobId = status ? null : r.job.id
  refreshJobs()
  return r.job
}
...
function runRow(step, label) {
  ...
  const status = el('p', 'runrow__status hint')
  status.hidden = true
  row.append(b, Object.assign(el('code'), { textContent: show(step) }))
  b.onclick = () => runWithStatus(step, b, status)
  row.append(status)
...
function field(form, label, control, hint) {
  ...
  const labelNode = el('label', 'form-label', label)
...
function smallSwitch(form, label, checked = false, hint) {
  ...
  const input = Object.assign(el('input'), { id, type: 'checkbox', checked })
  ...
  const trackLabel = Object.assign(el('label', null, label), { htmlFor: id })
  const control = el('div', 'switch switch--small')
  control.append(input, trackLabel)
...
function drawItems() {
  ...
  for (const { p, f } of projects) {
    ...
    if (p.workflow?.stages?.plan?.startedAt) {
      const stage = WORKFLOW_STAGES.find((item) => item.view === { plan: 'interview', script: 'scripts', canvas: 'storyboard', record: 'record', assembly: 'paperedit', edit: 'editor', review: 'review' }[p.workflow.currentStage])
      cap.append(el('div', 'projcard__when', `Video in progress · ${stage?.label ?? 'Plan'}`))
    }
    card.append(cap)
...
function nameMediaUploads(files) {
  ...
  return new Promise((done) => {
    const dialog = el('dialog', 'media-name-dialog')
    const form = el('form', 'media-name-dialog__form')
    form.method = 'dialog'
    form.append(el('h2', null, incoming.length === 1 ? 'Name this file' : `Name ${incoming.length} files`))
    form.append(el('p', 'hint', 'Use the person’s name when it matters. Claude sees these names when it matches footage to /speaker sections in the script.'))
    const fields = incoming.map((file) => {
      const match = /\.[^.]+$/.exec(file.name)
      const extension = match?.[0] ?? ''
      const originalBase = extension ? file.name.slice(0, -extension.length) : file.name
      const input = Object.assign(el('input', 'form-control'), { value: originalBase })
      input.setAttribute('aria-label', `Save ${file.name} as`)
      const group = el('label', 'form-group')
      const label = el('span', 'form-label', 'Save as')
      const original = el('span', 'form-hint', `Original: ${file.name}`)
      const row = el('div', 'media-name-dialog__name')
      row.append(input, el('span', 'media-name-dialog__extension', extension))
      group.append(label, row, original)
      form.append(group)
      return { file, input, extension, originalBase }
    })
    const status = el('p', 'hint')
    status.hidden = true
    const actions = el('div', 'move-asset__actions')
    const cancel = el('button', 'btn ghost', 'Cancel')
    cancel.type = 'button'
    cancel.onclick = () => dialog.close()
    const add = el('button', 'btn btn--primary', incoming.length === 1 ? 'Add file' : 'Add files')
    add.type = 'button'
    add.onclick = () => {
...
function vProject(m, id) {
   ...
   */
  const projectHead = el('div', 'project-head')
  const storageStatus = el('p', 'hint')
  ...
  projectHead.append(el('p', 'lede', `${p.client || 'No client'} · ${p.brand} · ${held.length} item${held.length === 1 ? '' : 's'}${f.bytes ? ' · ' + human(f.bytes) : ''} · updated ${ago(f.newest)}`), storageStatus)
  m.append(projectHead)
  ...
  const toolbar = el('div', 'row margin-y-md')
  toolbar.style.justifyContent = 'space-between'
  toolbar.style.flexWrap = 'wrap'
  toolbar.style.gap = 'var(--op-space-small)'
  const projectActions = el('div', 'btn-group')
  const filters = el('div', 'btn-group')
  filters.style.marginInlineStart = 'auto'
   ...
   */
  const recentCaptures = el('details', 'project-capture-import')
  const captureSummary = el('summary', null, 'Choose a recent OpenScreen recording')
  const captureControls = el('div', 'project-capture-import__controls')
  const captureSelect = el('select')
  captureSelect.setAttribute('aria-label', 'Recent OpenScreen recordings')
  captureSelect.disabled = true
  captureSelect.append(Object.assign(el('option'), { value: '', textContent: 'Add a recent OpenScreen recording…' }))
  const refreshCaptures = el('button', 'btn ghost btn--small', 'Refresh')
  refreshCaptures.type = 'button'
  const addCapture = el('button', 'btn secondary btn--small', 'Add to project')
  addCapture.type = 'button'
  addCapture.disabled = true
  const captureHint = el('p', 'hint project-capture-import__status')
  captureHint.hidden = true
  captureControls.append(captureSelect, addCapture, refreshCaptures)
  recentCaptures.append(captureSummary, captureControls, captureHint)
  ...
  const loadRecentCaptures = async () => {
    ...
    if (result.error) {
      captureSelect.disabled = true
      captureSelect.append(Object.assign(el('option'), { value: '', textContent: 'Recent recordings are unavailable' }))
      addCapture.disabled = true
    ...
    captureSelect.append(Object.assign(el('option'), { value: '', textContent: recordings.length ? 'Choose a screen recording…' : 'No recent OpenScreen recordings found' }))
    for (const recording of recordings) {
      const when = new Date(recording.modifiedAt).toLocaleString()
      captureSelect.append(
        Object.assign(el('option'), {
          value: recording.file,
          textContent: `${recording.name} · ${human(recording.bytes)} · ${when}`,
        }),
      )
    }
  ...
  const addRecentCapture = el('button', 'btn ghost btn--pill', 'Add recording')
  addRecentCapture.type = 'button'
  ...
  const openAssembly = el('button', 'btn ghost btn--pill', 'Assembly')
  openAssembly.type = 'button'
  openAssembly.onclick = () => go('paperedit')
  const assemblyBadge = el('span', 'badge')
  assemblyBadge.hidden = true
  openAssembly.append(assemblyBadge)
  projectActions.append(addRecentCapture, re, openAssembly)
  toolbar.append(projectActions, filters)

  // Assembly completion belongs on the action that opens it, not in another
  // card above the project's work. Count the recordings that were prepared,
  // rather than the smaller set of passages Claude selected from them.
  void Promise.all([
    fetch(`/api/multi-assembly?project=${encodeURIComponent(p.id)}`).then(responseJson),
    fetch(`/api/multi-assembly/audio-align?project=${encodeURIComponent(p.id)}`).then(responseJson),
  ])
    .then(([multi, aligned]) => {
      if (view !== 'library' || openProject !== p.id) return
      const picks = multi.state?.picks ?? []
      const sources = multi.state?.sources ?? []
      const hasAssembly = Boolean(picks.length || multi.state?.hyperframesProject)
      const alignmentCuts = aligned.state?.segments ?? []
      const count = hasAssembly ? sources.length || picks.length : alignmentCuts.length
      if (!count) return
      assemblyBadge.textContent = String(count)
      assemblyBadge.hidden = false
      const label = hasAssembly
        ? `${count} selected recording${count === 1 ? '' : 's'} ready to review`
        : `${count} screen cut${count === 1 ? '' : 's'} ready to review`
      openAssembly.title = label
      openAssembly.setAttribute('aria-label', `Assembly — ${label}`)
    })
    .catch(() => {})
  ...
  drop.append(icon('upload-04'), Object.assign(el('span', 'dropzone__lead'), { textContent: 'Add other media' }), Object.assign(el('span', 'dropzone__sub'), { textContent: 'Drop or choose video, audio or stills' }))
...
function moveAssetToProject(project, f) {
  ...
  return new Promise((done) => {
    const dialog = el('dialog', 'move-asset')
    const form = el('form', 'move-asset__form')
    form.method = 'dialog'
    form.append(el('h2', null, `Move ${f.name}`), el('p', 'hint', 'The file moves out of this project and into the destination project’s media library.'))

    const destination = el('select', 'form-control')
    destination.setAttribute('aria-label', 'Destination project')
    for (const candidate of destinations) destination.append(Object.assign(el('option'), { value: candidate.id, textContent: candidate.name }))
    const group = el('label', 'form-group')
    group.append(el('span', 'form-label', 'Move to project'), destination)
    const status = el('p', 'hint')
    status.hidden = true
    const actions = el('div', 'move-asset__actions')
    const cancel = el('button', 'btn ghost', 'Cancel')
    cancel.type = 'button'
    cancel.onclick = () => dialog.close()
    const move = el('button', 'btn', 'Move media')
    move.type = 'button'
    move.onclick = async () => {
      move.disabled = true
    ...
    form.append(group, status, actions)
    dialog.append(form)
    dialog.addEventListener('close', () => {
      dialog.remove()
      done(null)
...
function renameAsset(project, f) {
  return new Promise((done) => {
    ...
    const form = el('form', 'move-asset__form')
    form.method = 'dialog'
    form.append(el('h2', null, `Rename ${f.name}`), el('p', 'hint', 'This updates the project’s transcripts and saved editing work to use the new name.'))
    const name = Object.assign(el('input', 'form-control'), { type: 'text', value: f.name, required: true })
    name.setAttribute('aria-label', 'Media name')
    const group = el('label', 'form-group')
    group.append(el('span', 'form-label', 'Name'), name)
    const status = el('p', 'hint')
    status.hidden = true
    const actions = el('div', 'move-asset__actions')
    const cancel = el('button', 'btn ghost', 'Cancel')
    cancel.type = 'button'
    cancel.onclick = () => dialog.close()
    const rename = el('button', 'btn', 'Rename media')
    rename.type = 'button'
...
async function openHyperframesProject(folder, button, status) {
  ...
  hyperframesWorkspace = { folder: result.folder, url: result.url }
  go('hyperframes')
  return result
}
...
function vHyperframes(m) {
  ...
  if (hyperframesWorkspace?.url) {
    const shell = el('section', 'hyperframes-workspace')
    const header = el('div', 'hyperframes-workspace__header')
    const back = el('button', 'btn ghost btn--small', 'All compositions')
    back.type = 'button'
    back.onclick = () => {
    ...
    const title = el('div')
    title.append(el('div', 'paper-edit__title', hyperframesWorkspace.folder), el('p', 'hint', 'Changes save directly with this project.'))
    const reload = el('button', 'btn ghost btn--small', 'Reload')
    reload.type = 'button'
    const frame = el('iframe', 'hyperframes-workspace__frame')
    frame.title = `HyperFrames · ${hyperframesWorkspace.folder}`
    // The preview server starts just before this route renders. Give it one beat
    // to bind before the frame asks for the document; otherwise Chromium can
    // keep the initial connection-refused response instead of retrying.
    frame.src = 'about:blank'
    frame.allow = 'autoplay; fullscreen'
    const loadWorkspace = () => { frame.src = hyperframesWorkspace.url }
    window.setTimeout(loadWorkspace, 850)
    reload.onclick = loadWorkspace
    header.append(back, title, reload)
  ...
  const status = el('p', 'hint')
  const body = el('div')
  m.append(el('p', 'lede', 'Editable motion projects from Claude live here. OpenScreen remains the editor for captured demos and screenshots; use HyperFrames for the generated video, timing, titles and composition.'), status, body)
  ...
  const draw = async () => {
    ...
    if (!projects.length) {
      ...
      empty.append(
        el('p', null, 'When you make a video from a script, Claude now creates an editable HyperFrames project first. Its source and every later render stay together in this project.'),
        Object.assign(el('button', 'btn', 'Make a HyperFrames video'), { type: 'button', onclick: () => go('make') }),
      )
      body.append(empty)
      return
    }
    ...
    for (const project of projects) {
      ...
      const thumb = el('div', 'thumbwrap')
      const art = el('div', 'thumb')
      const latest = project.renders?.[0]
      if (latest) art.style.backgroundImage = `url('/thumb/${id}/${encodeURI(latest.rel)}')`
      else art.append(icon('play-circle', 'scriptcard__icon'))
      thumb.append(art, el('span', 'kind', latest ? 'video' : 'source'))

      const content = el('div', 'body')
      content.append(
        el('div', 'nm', project.title),
        el('div', 'path', `media/Renders/${project.folder}/index.html`),
        el('div', 'meta', latest ? `${latest.name} · ${human(latest.bytes)}` : 'Editable source · not rendered yet'),
      )
      const open = el('button', 'btn', 'Open in HyperFrames')
      open.type = 'button'
      const itemStatus = el('p', 'hint')
      itemStatus.hidden = true
      open.onclick = () => void openHyperframesProject(project.folder, open, itemStatus)
      content.append(open, itemStatus)
...
function vMake(m) {
  ...
  mk('Destination', el('div', 'form-hint', 'HyperFrames motion project — editable first, then render when it is ready.'))
  ...
  const paintPicked = () => {
    ...
    for (const rel of chosenAssets) {
      ...
      row.append(icon(file?.kind === 'audio' ? 'comment-01' : file?.kind === 'still' ? 'image-01' : 'video-01', 's3row__icon'), Object.assign(el('span', 's3row__name'), { textContent: file?.name ?? rel }), Object.assign(el('span', 's3row__meta'), { textContent: file ? `${file.kind}${file.media?.durationSec ? ' · ' + dur(file.media.durationSec) : ''}` : 'missing' }))
   ...
   */
  const voiceProvider = mk('Narration source', el('select'), 'Pick the service that should speak this render.')
  for (const [value, label] of [
    ['kokoro', 'Kokoro — local, on this machine'],
    ['elevenlabs', 'ElevenLabs — your saved voices'],
  ])
    voiceProvider.append(Object.assign(el('option', null, label), { value }))
  const vo = mk('Narration voice', el('select'), 'Choose the exact voice for this render. “No voiceover” keeps it silent.')
  ...
  const loadNarrationVoices = async () => {
    ...
    const d = await (await fetch('/api/voices?provider=' + encodeURIComponent(provider))).json().catch(() => ({ from: 'none', voices: [] }))
  ...
  const showPlan = (r) => {
    ...
    const runBtn = el('button', 'btn', 'Ask Claude to build the motion project')
    const runStatus = el('p', 'runrow__status hint')
    ...
    if (r.output === 'template') {
      const next = el('div', 'note full', 'Claude writes the editable project first. Review and edit it in HyperFrames, then render when the direction is right.')
      const templateActions = el('div', 'row')
      const openEditor = el('button', 'btn', 'Open in HyperFrames')
      ...
      const templateStatus = el('p', 'runrow__status hint')
      templateStatus.hidden = true
      openEditor.onclick = () => void openHyperframesProject(r.hyperframesProject, openEditor, templateStatus)
      checkBtn.onclick = () => runWithStatus(r.lintStep, checkBtn, templateStatus)
      renderBtn.onclick = () => runWithStatus(r.renderStep, renderBtn, templateStatus)
      templateActions.append(openEditor, checkBtn, renderBtn, templateStatus)
...
function vRecord(m) {
   ...
   */
  const savedScript = el('select', 'form-control form-control--small')
  const insertSavedScript = el('button', 'btn ghost btn--small', 'Insert saved script')
  insertSavedScript.type = 'button'
  const refreshSavedScripts = el('button', 'btn ghost btn--small', 'Refresh')
  refreshSavedScripts.type = 'button'
  const savedScriptHint = el('div', 'hint')
  const fillSavedScripts = () => {
    ...
    savedScript.append(Object.assign(el('option', null, scripts.length ? 'Choose a saved script…' : 'No saved scripts in this project yet'), { value: '' }))
    for (const item of scripts) savedScript.append(Object.assign(el('option', null, item.name), { value: item.name }))
  ...
  fillSavedScripts()
  const savedScriptRow = el('div', 'row')
  savedScriptRow.append(savedScript, insertSavedScript, refreshSavedScripts)
  const savedScriptGroup = el('div', 'form-group')
  savedScriptGroup.append(el('label', 'form-label', 'Saved project script'), savedScriptRow, savedScriptHint)
  ...
  audio.append(Object.assign(el('option', null, 'Use the recorded sound'), { value: '' }))
  for (const f of projectAudio) audio.append(Object.assign(el('option', null, f.name), { value: f.rel }))
  const audioMode = el('select')
  for (const [v, label] of [
    ['replace', 'Replace the recorded sound'],
    ['mix', 'Mix with the recorded sound'],
  ])
    audioMode.append(Object.assign(el('option', null, label), { value: v }))
  const audioOffset = Object.assign(el('input'), { type: 'number', value: 0, min: 0, step: 0.1 })
  ...
  for (const [v, l] of [
    ...
    session.append(Object.assign(el('option', null, l), { value: v }))
  }
  ...
  go.onclick = async () => {
    ...
    const all = el('button', 'btn', 'Record, brand, and add the video to this project')
...
function vVoice(m) {
  m.append(el('p', 'lede', 'Turn a saved script into narration and timed captions. Each line is measured as it is spoken, so the captions stay in sync.'))
  ...
  if (!S.tools.voice) {
    ...
    b.onclick = async () => {
      ...
      const status = el('p', 'voice__status')
      out.append(Object.assign(el('div', 'path'), { textContent: 'Setting up voice on this machine…' }), status)
  ...
  const pick = mk('Script', el('select'), 'Choose a saved script for this project. In a script field, type /script to insert one.')
  /*
  ...
  const preview = Object.assign(el('textarea', 'voice__lines form-control'), {
    placeholder: 'Narration lines appear here when you choose a script.',
    spellcheck: true,
  })
  const est = el('p', 'voice__estimate')
  const saved = el('p', 'voice__saved hint')
  let saveTimer = null
  let loadRequest = 0
  let draftRequest = 0
  ...
  const show = async () => {
    ...
    const lines = Array.isArray(draft.lines) && draft.lines.length ? draft.lines : original
    preview.value = lines.join('\n')
    preview.disabled = false
    saved.textContent = draft.lines?.length ? 'Editing saved narration copy.' : 'Edit these lines before building the narration.'
    updateEstimate()
  }
  // No change handler: switching project reloads state and re-renders this panel.
  pick.onchange = () => {
  ...
  preview.oninput = () => {
    updateEstimate()
    clearTimeout(saveTimer)
    saveTimer = setTimeout(() => void saveNarrationDraft(), 550)
  }

  const out = el('section', 'voice__result')
  const go = el('button', 'btn', 'Build the narration')
  const linesGroup = el('section', 'voice__script')
  linesGroup.append(el('h2', 'voice__label', 'Narration lines'), preview, est, saved)
  ...
  go.onclick = async () => {
    ...
    if (!draft.script || !draft.lines.length) {
      out.append(Object.assign(el('p', 'voice__status bad'), { textContent: 'Add narration lines before building.' }))
      return
    }
    ...
    if (r.error) {
      out.append(Object.assign(el('p', 'voice__status bad'), { textContent: r.error }))
      return
    }
    const status = el('p', 'voice__status')
    out.append(status)
    runWithStatus(r.step, go, status, (job) => {
      ...
      out.append(Object.assign(el('p', 'voice__output'), { textContent: 'Narration audio: ' + r.out }))
      out.append(Object.assign(el('p', 'voice__output'), { textContent: 'Captions: ' + r.srt }))
    })
  }
  m.append(f)
  intoRail(rail)
...
function vScripts(m) {
  ...
  const paintSaved = () => {
    ...
    if (!visible.length) {
      saved.append(el('div', 'hint', project.value ? 'No scripts in this project yet. Draft one, or write it and press Save.' : 'Nothing on the shared shelf yet.'))
      return
    }
    for (const sc of visible) {
      const card = el('div', 'card')
      const b = el('div', 'body')
      b.append(el('div', 'nm', sc.name), el('div', 'path', sc.body.slice(0, SNIPPET_CHARS) + (sc.body.length > SNIPPET_CHARS ? '…' : '')))
      /*
...
function vScenes(m) {
  ...
  const { frame, scrub, palette, cards, rawText, pick, save, draft, footageGroup, footage, attachFootage, transcribeFootage, suggestFootage, addFootage, footagePicker, footageHint, footageReview, footagePlayer, footageReviewStatus, footageTranscript } = ui
  /*
  ...
  const fillFootage = (selected = '') => {
    footage.replaceChildren(Object.assign(el('option', null, 'Choose project footage…'), { value: '' }))
    for (const file of videoFiles()) footage.append(Object.assign(el('option', null, `${file.name} · ${clock(file.media?.durationSec ?? 0)}`), { value: file.rel }))
  ...
  const refreshFootage = async (selected = '') => {
    ...
    const chosenId = footageBoard?.picks?.[boardScene.id]
    const chosen = takes.find((take) => take.id === chosenId) ?? takes.at(-1)
    ...
    fillFootage(selected || chosen?.rel || '')
    setFootageHint(count ? `${count} video${count === 1 ? '' : 's'} attached to this shot.` : 'Choose an existing recording or add one to this project.')
  }
  ...
  if (footageGroup) {
    ...
    let selectedCueIndexes = new Set()
    ...
    const selectedPassages = () => {
      ...
      const selected = [...selectedCueIndexes].sort((a, b) => a - b)
      const passages = []
      ...
      return passages
        .filter(({ first, last }) => cues[first] && cues[last])
        .map(({ first, last }) => ({ first, last, inSec: Number(cues[first].startSec) || 0, outSec: Number(cues[last].endSec ?? cues[last].startSec) || 0, text: cues.slice(first, last + 1).map((cue) => cue.text).join(' ') }))
    }
    ...
    const renderFootageReview = () => {
      ...
      const source = `/media/${encodeURIComponent(proj.value)}/${encodeURI(footage.value)}`
      ...
      footageReviewStatus.textContent = `${footageState.transcript.words?.length ?? 0} words across ${cues.length} timed lines. Claude’s suggestions and your own passage choices both save back to this Canvas scene.`
      const selected = selectedPassages()
      if (suggestedPassages.length) {
        const suggestions = el('div', 'scene-footage-review__suggestions')
        suggestions.append(el('strong', null, 'Claude’s proposed passages'))
        for (const range of suggestedPassages) {
          ...
          const use = el('button', 'btn ghost btn--small', `Use ${footageTime(proposed.inSec)}–${footageTime(proposed.outSec)}`)
          use.type = 'button'
          use.title = proposed.text
          use.onclick = () => void usePassage(proposed)
          const line = el('div', 'scene-footage-review__suggestion')
          line.append(use, el('span', null, proposed.text))
          suggestions.append(line)
        }
        footageTranscript.append(suggestions)
      }
      const manual = el('div', 'scene-footage-review__manual')
      const useManual = el('button', 'btn btn--small', selected.length ? `Use ${selected.length} selected passage${selected.length === 1 ? '' : 's'}` : 'Select transcript lines')
      ...
      const clear = el('button', 'btn ghost btn--small', 'Clear')
      clear.type = 'button'
      ...
      manual.append(el('span', 'hint', selected.length ? `${selected.length} selected passage${selected.length === 1 ? '' : 's'} will be saved to this scene. Shift-click selects a continuous range; ordinary clicks add or remove individual lines.` : 'Click lines to add or remove them. Shift-click a second line to select the whole range.'), useManual, clear)
      const lines = el('div', 'paper-edit__lines scene-footage-review__lines')
      for (const [index, cue] of cues.entries()) {
        const line = el('button', 'paper-edit__line')
        line.type = 'button'
        line.classList.toggle('paper-edit__line--active', selectedCueIndexes.has(index))
        line.append(el('time', 'paper-edit__time', footageTime(cue.startSec)), el('span', 'paper-edit__text', cue.text))
        line.onclick = (event) => {
          if (event.shiftKey && selectionAnchor != null) {
            const from = Math.min(selectionAnchor, index)
            const to = Math.max(selectionAnchor, index)
            for (let cueIndex = from; cueIndex <= to; cueIndex++) selectedCueIndexes.add(cueIndex)
          } else if (selectedCueIndexes.has(index)) selectedCueIndexes.delete(index)
          else selectedCueIndexes.add(index)
          selectionAnchor = index
          footagePlayer.currentTime = Number(cue.startSec) || 0
          renderFootageReview()
        }
        lines.append(line)
      }
    ...
    attachFootage.onclick = async () => {
      ...
      if (result.error) return setFootageHint(result.error, 'bad')
      footageBoard = result.board
      const count = (footageBoard?.takes ?? []).filter((take) => take.slotId === boardScene.id).length
      setFootageHint(`Attached. This shot now has ${count} video${count === 1 ? '' : 's'}.`, 'ok')
    }
    ...
    if (boardScene) void refreshFootage().then(loadFootageReview)
  }
  ...
  const linkToCanvas = async (scene) => {
    ...
    const response = await fetch('/api/board/node/update', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ projectId: proj.value, nodeId: boardScene.id, scene }),
    }).catch(() => null)
...
function vWorkflow(m) {
  const project = currentProjectRecord()
  m.append(el('p', 'lede', 'The path is always in the header. This is only the next useful move.'))

  const workspace = el('section', 'workflow workflow--next')
  const panel = el('section', 'workflow__panel')
  ...
  const draw = ({ interview, board, workflow, assembly, alignment }) => {
    ...
    const steps = [
      {
        ...
        action: hasPlan ? 'Review the plan' : interviewStarted ? 'Continue the interview' : 'Start the interview',
      },
      {
        id: 'script',
        view: 'scripts',
      ...
      { id: 'edit', view: 'editor', label: 'Edit', detail: 'Refine the visual timeline', done: false, title: 'Shape the visual edit', body: 'The first cut opens in the visual editor, where you can review timing, make trims, add captions and decide when it is ready to render.', action: 'Open the visual editor' },
      { id: 'review', view: 'review', label: 'Review', detail: 'Share the finished render', done: false, title: 'Review and share', body: 'When the render is ready, send it for review and keep the feedback connected to the actual video.', action: 'Open review' },
    ]
    const activeAt = steps.findIndex((step) => !step.done)
    const active = steps[Math.max(0, activeAt)]

    panel.replaceChildren()
    panel.append(el('div', 'workflow__eyebrow', `Step ${activeAt + 1} of ${steps.length}`), el('h2', 'workflow__title', active.title), el('p', 'workflow__body', active.body))
    const status = el('p', 'workflow__status')
    status.append(el('strong', null, active.done ? 'Complete' : active.inProgress ? 'In progress' : 'Up next'), document.createTextNode(` · ${active.detail}`))
    if (workflow?.currentStage) status.append(document.createTextNode(` · progress saved at ${workflow.currentStage}`))
    panel.append(status)
    const actions = el('div', 'row')
    const progress = el('p', 'workflow__action-note')
    const proceed = el('button', 'btn btn--primary', active.action)
    ...
    if (active.id === 'script' && hasPlan && !scripts.length) {
      proceed.textContent = 'Draft a script from this plan'
      proceed.onclick = draftScript
    } else if (active.id === 'canvas' && script && !hasCanvas) {
      proceed.textContent = 'Build the canvas from this script'
    ...
    if (videos.length) {
      const editor = el('button', 'btn ghost', 'Open visual timeline')
      editor.onclick = () => go('editor')
      actions.append(editor)
    }
    const restart = el('button', 'btn ghost', 'Start over')
    restart.type = 'button'
    restart.title = 'Archive this video’s working plan and begin again with the same media'
...
function vInterview(m) {
  m.append(el('p', 'lede', 'Answer a few practical questions about the video. Claude turns the conversation into a plan, then uses that plan to draft the script.'))
  const skillHint = el('p', 'hint')
  m.append(skillHint)

  const form = el('div', 'form interview')
  const question = el('section', 'interview__question full')
  const answer = Object.assign(el('textarea'), { rows: 5, placeholder: 'Write what you know. A rough answer is enough to move the plan forward.' })
  field(form, 'Your answer', answer, 'Keep it in your own words. Claude will ask only what changes the plan.')
  const answerGroup = answer.parentElement
  const actions = el('div', 'row')
  const plan = el('section', 'interview__plan full')
  const status = el('div', 'form-hint full')
  form.prepend(question)
  form.append(actions, plan, status)
  m.append(form)
  ...
  const render = () => {
    ...
    if (!state?.turns?.length) {
      question.append(el('div', 'interview__count', 'Question 1'), el('div', 'paper-edit__title', 'What is this video for, and who watches it?'))
      answerGroup.hidden = false
      answer.disabled = false
      const submit = el('button', 'btn btn--primary', 'Submit answer')
    ...
    if (state.plan) {
      ...
      question.append(el('div', 'paper-edit__title', 'Shot list ready'), el('p', 'hint', state.plan.about || 'Claude has turned the conversation into a first plan.'))
      const list = el('ol', 'interview__shots')
      for (const shot of state.plan.shots ?? []) {
        const line = el('li')
        line.append(el('strong', null, shot.name), document.createTextNode(shot.intent ? ` — ${shot.intent}` : ''), shot.seconds ? el('span', 'interview__seconds', `${shot.seconds}s`) : '')
        list.append(line)
      }
      plan.append(list)
      if (state.problems?.length) plan.append(el('p', 'hint', state.problems.join(' ')))
      const script = el('button', 'btn btn--primary', 'Continue to script')
      // This button names a destination. Sending it through the workflow's
      // "next unfinished" calculation made an older canvas or recording skip
      // Script entirely and show a later numbered step instead.
      script.onclick = () => go('scripts')
      const canvas = el('button', 'btn ghost', 'Open canvas')
      canvas.onclick = () => go('storyboard')
      const restart = el('button', 'btn ghost', 'Start over')
      restart.onclick = startFresh
      actions.append(script, canvas, restart)
      return
    }

    const turn = state.turns.at(-1)
    question.append(el('div', 'interview__count', `Question ${state.turns.length}`), el('div', 'paper-edit__title', turn.question))
    answerGroup.hidden = false
    answer.value = turn.answer ?? ''
    if (state.pendingReply) {
      answer.disabled = true
      actions.append(Object.assign(el('button', 'btn', 'Load Claude’s reply'), { onclick: loadReply }))
      return
    }
    answer.disabled = false
    const send = el('button', 'btn btn--primary', 'Submit answer')
...
function vMultiAssembly(m) {
  m.append(el('p', 'lede', 'Choose up to eight project recordings. Studio transcribes and samples their screens in the background, then Claude returns a reviewable set of timestamped selects — never invented footage.'))
  const project = currentProjectRecord()
  const videos = (project?.catalog?.files ?? []).filter((file) => file.kind === 'video')
  const audios = (project?.catalog?.files ?? []).filter((file) => file.kind === 'audio')
  const switchMode = el('button', 'btn ghost btn--small margin-bottom-md', 'Use one recording instead')
  switchMode.onclick = () => {
    assemblyMode = 'single'
    render()
  }
  m.append(switchMode)
  const review = el('section', 'multi-assembly__review')
  if (!videos.length) {
    m.append(el('p', 'empty', 'Add recordings to this project first.'))
    return
  }

  const status = el('p', 'hint')
  status.hidden = true
  const updateSources = el('button', 'btn ghost btn--small', 'Update selected recordings')
  updateSources.hidden = true
  const sourceList = el('div', 'multi-assembly__sources')
  const checks = new Map()
  const sourceCards = new Map()
  for (const file of videos) {
    const input = Object.assign(el('input'), { type: 'checkbox', value: file.rel })
    const label = el('label', 'multi-assembly__source')
    const thumb = el('span', 'multi-assembly__source-thumb')
    thumb.style.backgroundImage = `url('/thumb/${encodeURIComponent(project.id)}/${encodeURI(file.rel)}')`
    const details = el('span', 'multi-assembly__source-details')
    const meta = el('span', 'multi-assembly__source-meta', dur(file.durationSec) || 'video')
    const readiness = el('span', 'multi-assembly__source-ready', 'Not prepared')
    details.append(el('span', 'multi-assembly__source-name', file.name), meta, readiness)
    input.onchange = () => {
    ...
    label.append(input, thumb, details)
    sourceList.append(label)
    checks.set(file.rel, input)
    sourceCards.set(file.rel, readiness)
  }
  const sourceHeading = el('div', 'paper-edit__title', 'Source recordings · choose up to 8')
  const sourceHeader = el('div', 'multi-assembly__source-header')
  sourceHeader.append(sourceHeading, updateSources)
  m.append(sourceHeader, sourceList)

  const notes = Object.assign(el('textarea'), { rows: 2, placeholder: 'Optional direction for Claude: lead with the customer problem, keep the side camera only for the close…' })
  const projectScripts = (S.scripts ?? []).filter((item) => item.project === currentProject())
  const scriptSelect = el('select', 'form-control')
  scriptSelect.append(Object.assign(el('option', null, 'Best-parts assembly — do not match a script'), { value: '' }))
  for (const item of projectScripts) scriptSelect.append(Object.assign(el('option', null, item.name), { value: item.name }))
  if (projectScripts.length === 1) scriptSelect.value = projectScripts[0].name
  const scriptGroup = el('div', 'form-group')
  scriptGroup.hidden = !projectScripts.length
  scriptGroup.append(el('label', 'form-label', 'Script to match'), scriptSelect, el('div', 'form-hint', 'Choose this when the recordings are takes of a script. Studio applies the Video from script workflow: script order wins, missing beats stay visible, and unused material is parked for later.'))
  const assemble = el('button', 'btn btn--primary', 'Match the script with Claude')
  const stackSources = el('button', 'btn ghost', 'Stack selected recordings')
  const buildTimeline = el('button', 'btn btn--primary', 'Approve selections & build review cut')
  const openTimeline = el('button', 'btn btn--primary', 'Review cut in HyperFrames')
  const renderAndShare = el('button', 'btn btn--primary', 'Render & send for review')
  buildTimeline.hidden = true
  openTimeline.hidden = true
  renderAndShare.hidden = true
  const actions = el('div', 'row')
  actions.append(assemble, stackSources, buildTimeline, openTimeline, renderAndShare)
  const setup = el('section', 'multi-assembly__setup')
  setup.append(scriptGroup, notes, el('p', 'hint', 'Studio saves the selected clips immediately, runs only the missing transcript and screen-analysis jobs, then asks Claude for the strongest supporting moments.'), actions, status)
  m.append(setup)

  const setAssemblyAction = () => {
    const matchingScript = Boolean(scriptSelect.value)
    assemble.textContent = matchingScript ? 'Match the script with Claude' : 'Make a best-parts assembly with Claude'
  }
  scriptSelect.onchange = setAssemblyAction
  setAssemblyAction()

  const align = el('details', 'multi-assembly__align')
  const alignSummary = el('summary', null, 'Align an audio recording to a screen recording')
  const screen = el('select')
  const audio = el('select')
  const script = el('select')
  for (const file of videos) screen.append(Object.assign(el('option'), { value: file.rel, textContent: file.name }))
  audio.append(Object.assign(el('option'), { value: '', textContent: audios.length ? 'Choose project audio…' : 'No project audio yet' }))
  for (const file of audios) audio.append(Object.assign(el('option'), { value: file.rel, textContent: file.name }))
  script.append(Object.assign(el('option'), { value: '', textContent: 'Use the audio transcript' }))
  for (const item of (S.scripts ?? []).filter((entry) => entry.project === currentProject())) script.append(Object.assign(el('option'), { value: item.name, textContent: `Use script: ${item.name}` }))
  const alignButton = el('button', 'btn ghost btn--small', 'Build visual alignment')
  alignButton.disabled = !audios.length
  const openAlignedTimeline = el('button', 'btn ghost btn--small', 'Open in HyperFrames')
  const reviewAlignedVideo = el('button', 'btn ghost btn--small', 'Review rendered video')
  const renderAligned = el('button', 'btn btn--primary btn--small', 'Render audio review video')
  openAlignedTimeline.hidden = true
  reviewAlignedVideo.hidden = true
  renderAligned.hidden = true
  const alignRow = el('div', 'multi-assembly__align-row')
  alignRow.append(screen, audio, script, alignButton)
  align.append(alignSummary, el('p', 'hint', 'Studio first transcribes the narration and samples the screen. Claude maps each spoken range to a matching on-screen moment; it does not lay the entire audio over the original recording.'), alignRow)
  ...
  const alignmentReview = el('section', 'multi-assembly__alignment-review')
  let state = null
  let alignment = null
  let transcriptCues = {}
  ...
  const paint = () => {
    ...
    if (!state?.picks?.length) {
      ...
      recovery.append(el('div', 'paper-edit__title', hasTimeline ? 'HyperFrames assembly ready' : gaps.length ? 'No matching recorded lines yet' : 'Assembly needs another pass'), el('p', 'hint', copy))
      if (gaps.length) {
        const missing = el('ul', 'multi-assembly__script-gaps')
        for (const gap of gaps) missing.append(el('li', null, `${gap.beatId} · ${gap.beat}${gap.reason ? ` — ${gap.reason}` : ''}`))
        recovery.append(missing)
      }
      const recoveryActions = el('div', 'row')
      if (hasTimeline) {
        const open = el('button', 'btn ghost btn--small', 'Open in HyperFrames')
        open.onclick = () => openTimeline.click()
        recoveryActions.append(open)
      }
      const retry = el('button', 'btn btn--primary btn--small', 'Make a new Claude pass')
      retry.onclick = () => void runAssembly()
      recoveryActions.append(retry)
      recovery.append(recoveryActions)
      review.append(recovery)
      return
    }
    const head = el('section', 'card card--padded')
    head.append(el('div', 'paper-edit__title', state.title || 'Claude’s assembly'), el('p', 'hint', `${state.picks.length} selected clips · review each passage here, then approve the selection to build a review cut. Comment on any clip and ask Claude to revise instead if it is not right.`))
    if (state.scriptBeats?.length) {
      const scriptStatus = el('div', 'multi-assembly__script-status')
      ...
      for (const beat of state.scriptBeats) {
        const item = el('span', 'multi-assembly__script-beat', `${beat.id} · ${beat.text}`)
      ...
      head.append(scriptStatus)
    }
    const revise = el('button', 'btn ghost btn--small', 'Revise from comments')
    revise.onclick = () => void runAssembly(true)
    const newPass = el('button', 'btn ghost btn--small', 'Make a new pass')
    newPass.onclick = () => void runAssembly()
    const reviewActions = el('div', 'row')
    reviewActions.append(revise, newPass)
    head.append(reviewActions)
    review.append(head)
    /*
     ...
     */
    const picks = el('section', 'multi-assembly__review-surface')
    const selectedPick = state.picks.find((pick) => pick.id === activePickId) ?? state.picks[0]
    activePickId = selectedPick.id
    const timeline = el('div', 'multi-assembly__timeline')
    timeline.setAttribute('aria-label', 'Assembly timeline')
    const stage = el('div', 'multi-assembly__stage')
    const player = Object.assign(el('video', 'multi-assembly__review-player'), {
      controls: true,
      playsInline: true,
      preload: 'metadata',
      ariaLabel: 'Selected assembly passage',
    })
    ...
    player.volume = 1
    const selectedLabel = el('p', 'multi-assembly__selected-label')
    const selectedText = el('p', 'multi-assembly__selected-text')
    const selectedReason = el('p', 'multi-assembly__selected-reason')
    const playSelection = el('button', 'btn ghost btn--small', 'Play Claude’s selection')
    const removeSelection = el('button', 'btn ghost btn--small', 'Remove from assembly')
    const comment = Object.assign(el('textarea', 'multi-assembly__comment'), { rows: 2, placeholder: 'Comment for the next edit…' })
    comment.oninput = () => {
    ...
    stage.append(player, selectedLabel, selectedText, selectedReason, playSelection, removeSelection, comment)

    const transcript = el('div', 'multi-assembly__transcript')
    const transcriptHead = el('div', 'multi-assembly__transcript-head')
    const transcriptName = el('strong')
    const transcriptHint = el('span', 'multi-assembly__transcript-hint', 'The green range is what Claude selected.')
    transcriptHead.append(transcriptName, transcriptHint)
    const transcriptLines = el('div', 'multi-assembly__transcript-lines')
    transcript.append(transcriptHead, transcriptLines)
    const body = el('div', 'multi-assembly__review-body')
    body.append(stage, transcript)
    ...
    const paintTranscript = () => {
      ...
      if (!cues.length) {
        transcriptLines.append(el('p', 'hint', 'The source transcript is not available yet. Prepare this clip again to attach it.'))
        return
      }
      for (const cue of cues) {
        const line = el('button', 'multi-assembly__transcript-line')
        line.type = 'button'
        ...
        const copy = el('span', 'multi-assembly__transcript-copy', cue.text)
        if (selected && !selectedRow) copy.append(el('span', 'multi-assembly__selection-badge', 'Claude selected'))
        line.append(el('time', null, dur(cue.startSec)), copy)
    ...
    for (const [index, pick] of state.picks.entries()) {
      ...
      const segment = el('button', 'multi-assembly__timeline-pick')
      segment.type = 'button'
      segment.style.setProperty('--assembly-pick-duration', String(duration))
      segment.append(el('span', 'multi-assembly__timeline-number', String(index + 1)), el('span', 'multi-assembly__timeline-label', pick.source.split('/').pop()), el('span', 'multi-assembly__timeline-time', `${dur(pick.inSec)}–${dur(pick.outSec)}`))
      segment.onclick = () => {
  ...
  const paintAlignment = () => {
    ...
    const head = el('div', 'card card--padded multi-assembly__review-head')
    const title = el('div')
    title.append(el('div', 'paper-edit__title', `Visual alignment · ${alignment.segments.length} screen cuts`), el('p', 'hint', 'Edit the screen cuts and narration together in HyperFrames. Rendered review video includes the matching narration.'))
    const actions = el('div', 'multi-assembly__alignment-actions')
    actions.append(openAlignedTimeline, reviewAlignedVideo, renderAligned)
    head.append(title, actions)

    const body = el('div', 'multi-assembly__alignment-body')
    const player = Object.assign(el('video', 'multi-assembly__alignment-video'), { playsInline: true, controls: true, preload: 'metadata' })
    player.muted = false
    player.defaultMuted = false
    player.volume = 1
    player.src = `/media/${encodeURIComponent(currentProject() ?? '')}/${encodeURI(alignment.videoRel)}`
    const cuts = el('div', 'multi-assembly__alignment-cuts')
    for (const [index, segment] of alignment.segments.entries()) {
      const cut = el('button', 'multi-assembly__alignment-cut')
      cut.type = 'button'
      const range = el('span', 'multi-assembly__alignment-range', `${dur(segment.audioInSec)}–${dur(segment.audioOutSec)}`)
      const details = el('span', 'multi-assembly__alignment-detail')
      details.append(el('strong', null, `Screen ${dur(segment.screenInSec)}–${dur(segment.screenOutSec)}`), el('span', null, segment.reason || 'Mapped screen moment'))
      cut.append(el('span', 'multi-assembly__alignment-number', String(index + 1)), range, details)
      cut.onclick = () => {
    ...
    body.append(player, cuts)
    alignmentReview.append(head, body)
  }
  const refresh = async () => {
    ...
    for (const [rel, readiness] of sourceCards) {
      ...
      readiness.textContent = prepared?.transcript && prepared?.visual ? 'Transcript + screen ready' : prepared?.transcript ? 'Transcript ready' : prepared?.visual ? 'Screen ready' : 'Not prepared'
      readiness.classList.toggle('multi-assembly__source-ready--done', Boolean(prepared?.transcript && prepared?.visual))
    }
  ...
  const build = async () => {
    ...
    buildTimeline.textContent = 'Building review cut…'
    say('Building the editable review cut from the approved selections…', 'ok')
    const made = await request('/build')
    if (made.error) {
      buildTimeline.disabled = false
      buildTimeline.textContent = label
      return say(`Could not build the review cut: ${made.error}`, 'bad')
    }
    await refresh()
    say(`${made.clips} approved clips are ready. Open the review cut in HyperFrames; rendering stays locked until you have reviewed it.`, 'ok')
  }
  ...
  const saveSourceSet = async () => {
    ...
    const prepared = await request('/prepare', { rels, notes: notes.value, scriptName: scriptSelect.value })
    if (prepared.error) {
  ...
  const runAssembly = async (revision = false) => {
    ...
    const askClaude = async () => {
      ...
      say(revision ? 'Claude is revising the selected clips from your comments…' : matchingScript ? 'Claude is locating each script line in the recordings…' : 'Claude is choosing the strongest moments across these recordings…', 'ok')
      const drafted = await request('/draft', { rels, notes: notes.value, scriptName: scriptSelect.value })
      if (drafted.error) {
...
function vPaperEdit(m) {
  ...
  m.append(el('p', 'lede', 'Claude finds the usable passages first. Review the visual picks, keep the ones that tell the story, then build the editable assembly.'))
  const useMulti = el('button', 'btn ghost btn--small margin-bottom-md', 'Assemble multiple clips instead')
  useMulti.onclick = () => {
    assemblyMode = 'multi'
    render()
  }
  ...
  const videos = () => (project()?.catalog?.files ?? []).filter((file) => file.kind === 'video')
  const form = el('div', 'form paper-edit__setup')
  const status = el('div', 'form-hint full')
  const clip = el('select')
  clip.append(Object.assign(el('option', null, 'Choose a recording…'), { value: '' }))
  for (const file of videos()) clip.append(Object.assign(el('option', null, file.name), { value: file.rel }))
  ...
  const transcribe = el('button', 'btn btn--primary', 'Transcribe recording')
  const transcriptHint = el('div', 'hint')
  const language = el('select')
  language.append(Object.assign(el('option', null, 'English'), { value: 'en' }))
  ...
  const notes = Object.assign(el('textarea'), { rows: 3, placeholder: 'Optional: keep it direct, avoid the implementation details, lead with the customer problem…' })
  const askClaude = el('button', 'btn btn--primary', 'Ask Claude for picks')
  const loadClaude = el('button', 'btn ghost', 'Load Claude’s picks')
  ...
  const review = el('section', 'paper-edit__review')
  const controls = el('details', 'paper-edit__controls')
  controls.open = true
  controls.append(el('summary', 'paper-edit__controls-summary', 'Assembly settings'), form)
  const transcript = el('section', 'paper-edit__transcript')
  transcript.append(controls, status, review)
  const preview = el('section', 'paper-edit__preview')
  const previewName = el('p', 'paper-edit__preview-name', 'Choose a recording to preview it here.')
  const player = Object.assign(el('video', 'paper-edit__video'), { controls: true, playsInline: true, preload: 'metadata' })
  preview.append(previewName, player)
  const workspace = el('div', 'paper-edit')
  const splitter = el('div', 'paper-edit__splitter')
  splitter.setAttribute('role', 'separator')
  splitter.setAttribute('aria-label', 'Resize transcript panel')
  splitter.setAttribute('aria-orientation', 'vertical')
  splitter.tabIndex = 0
  workspace.append(transcript, splitter, preview)
  m.append(workspace)
  ...
  // Review starts with Claude's selection when it exists, but remains a direct
  // line editor: assigning a cue to a beat is the only edit operation.
  ...
  const cueAssignments = () => {
    ...
    for (const [cueIndex, cue] of (state?.transcript?.cues ?? []).entries()) {
      const start = index.get(cue.from)
      const end = index.get(cue.to)
      const range = (state?.selection?.checked?.ranges ?? []).find((candidate) => start != null && end != null && candidate.fromIndex <= end && candidate.toIndex >= start)
      if (range) assignments.set(cueIndex, range.shot)
    }
    return assignments
  }
  ...
  const selectionFromReview = () => {
    ...
    for (const [cueIndex, shot] of [...reviewAssignments.entries()].sort(([a], [b]) => a - b)) {
      const cue = state.transcript.cues[cueIndex]
      const ranges = byShot.get(shot)
      if (!cue || !ranges) continue
      const previous = ranges.at(-1)
      // A visual pick is a passage, not seventy-seven one-line cuts. Keep adjacent
      // caption cues together so the saved selection stays the same thing the
      // person reviewed as a card.
      if (previous && previous.lastCue === cueIndex - 1) {
        previous.to = cue.to
        previous.lastCue = cueIndex
      } else {
        ranges.push({ from: cue.from, to: cue.to, why: 'Selected in review', lastCue: cueIndex })
      }
    }
    return {
      shots: [...byShot.entries()].map(([shot, ranges]) => ({
        shot,
        ranges: ranges.map(({ lastCue, ...range }) => range),
      })),
    }
  }
  ...
  const paintTranscriptAction = () => {
    ...
    transcribe.textContent = ready ? 'Transcribe again' : 'Transcribe recording'
  ...
  const paintReview = () => {
    ...
    const allIncluded = canEdit && state.transcript.cues.length > 0 && state.transcript.cues.every((_, cueIndex) => reviewAssignments.get(cueIndex) === activeBeat)
    head.append(
      el('div', 'paper-edit__title', state.selection?.checked?.ranges?.length ? 'Review picks' : 'Transcript'),
      el('p', 'hint', canEdit ? (allIncluded ? 'Everything is in the first cut. Click a line or sweep across several to take them out.' : 'Choose a beat, then click a line or sweep across several to take them out.') : 'Add at least one beat in Assembly settings. The transcript becomes editable as soon as it has a name.'),
    )
    review.append(head)
    if (!canEdit) {
      review.append(el('div', 'hint', `Transcript attached: ${state.transcript.words.length} words across ${state.transcript.cues.length} timed passages.`))
    }

    const beatBar = el('div', 'paper-edit__beats')
    for (const shot of shots) {
      const beat = el('button', 'paper-edit__beat-chip')
      beat.type = 'button'
      ...
      beat.onclick = () => {
        ...
        for (const item of beatBar.querySelectorAll('button')) {
          const active = item.textContent === activeBeat
          item.classList.toggle('paper-edit__beat-chip--active', active)
          item.setAttribute('aria-pressed', String(active))
        }
        for (const [cueIndex, assigned] of reviewAssignments) {
          review.querySelector(`[data-cue="${cueIndex}"]`)?.classList.toggle('paper-edit__line--active', assigned === activeBeat)
        }
      }
      beatBar.append(beat)
    }
    ...
    const setAssignment = (cueIndex, shot) => {
      if (!shot) reviewAssignments.delete(cueIndex)
      else reviewAssignments.set(cueIndex, shot)
      const line = review.querySelector(`[data-cue="${cueIndex}"]`)
      line?.classList.toggle('paper-edit__line--assigned', Boolean(shot))
      line?.classList.toggle('paper-edit__line--active', Boolean(activeBeat) && shot === activeBeat)
      line?.setAttribute('aria-label', shot ? `${shot}: ${line.textContent}. Click to remove from this beat.` : `Not in the cut: ${line.textContent}. Click to add to ${activeBeat}.`)
      saveReviewSoon()
    }
    ...
    if (suggested.length) {
      ...
      const cueIndexesFor = (range) =>
        state.transcript.cues
          .map((cue, cueIndex) => ({ cue, cueIndex }))
          .filter(({ cue }) => {
            const from = wordIndex.get(cue.from)
            const to = wordIndex.get(cue.to)
            return from != null && to != null && from <= range.toIndex && range.fromIndex <= to
          })
          .map(({ cueIndex }) => cueIndex)
      const picks = el('section', 'paper-edit__picks')
      picks.append(el('div', 'paper-edit__picks-title', 'Claude’s suggested clips'), el('p', 'hint', 'Choose a card to inspect it. Keep or remove whole picks here; use the transcript below for a finer cut.'))
      const grid = el('div', 'paper-edit__pick-grid')
      for (const [pickIndex, range] of suggested.entries()) {
        const key = `${range.shot}:${range.from.id}:${range.to.id}`
        const cueIndexes = cueIndexesFor(range)
        const included = cueIndexes.length > 0 && cueIndexes.every((cueIndex) => reviewAssignments.get(cueIndex) === range.shot)
        const card = el('article', 'paper-edit__pick')
        card.classList.toggle('paper-edit__pick--active', key === visualPickKey)
        card.classList.toggle('paper-edit__pick--removed', !included)
        const inspect = el('button', 'paper-edit__pick-preview')
        inspect.type = 'button'
        inspect.setAttribute('aria-label', `Preview ${range.shot}: ${range.text}`)
        const thumb = Object.assign(el('video', 'paper-edit__pick-thumb'), { muted: true, playsInline: true, preload: 'metadata' })
        ...
        inspect.append(thumb, el('span', 'paper-edit__pick-time', `${timecode(range.from.startSec)}–${timecode(range.to.endSec)}`))
        inspect.onclick = () => {
          visualPickKey = key
        ...
        const copy = el('div', 'paper-edit__pick-copy')
        copy.append(el('div', 'paper-edit__pick-shot', `${pickIndex + 1}. ${range.shot}`), el('p', 'paper-edit__pick-text', range.text), range.why ? el('p', 'paper-edit__pick-why', range.why) : '')
        const toggle = el('button', included ? 'btn ghost' : 'btn', included ? 'Remove pick' : 'Use this pick')
        toggle.type = 'button'
        toggle.onclick = () => {
          for (const cueIndex of cueIndexes) setAssignment(cueIndex, included ? '' : range.shot)
          visualPickKey = key
          paintReview()
        }
        card.append(inspect, copy, toggle)
        grid.append(card)
      }
      picks.append(grid)
      review.append(picks)
    ...
    let sweeping = false
    let sweepMode = 'assign'
    const sweep = (event) => {
      if (!sweeping) return
      const line = document.elementFromPoint(event.clientX, event.clientY)?.closest('[data-cue]')
      if (line && activeBeat) setAssignment(Number(line.dataset.cue), sweepMode === 'remove' ? '' : activeBeat)
    }
    const stopSweep = () => {
      sweeping = false
      window.removeEventListener('pointermove', sweep)
    ...
    const lines = el('div', 'paper-edit__lines')
    for (const [cueIndex, cue] of state.transcript.cues.entries()) {
      const line = el(canEdit ? 'button' : 'div', 'paper-edit__line')
      if (canEdit) line.type = 'button'
      line.dataset.cue = String(cueIndex)
      line.append(el('time', 'paper-edit__time', timecode(cue.startSec)), el('span', 'paper-edit__text', cue.text))
      const assigned = reviewAssignments.get(cueIndex) ?? ''
      line.classList.toggle('paper-edit__line--assigned', Boolean(assigned))
      line.classList.toggle('paper-edit__line--active', Boolean(activeBeat) && assigned === activeBeat)
      line.classList.toggle('paper-edit__line--playing', cueIndex === playingCue)
      line.setAttribute('aria-label', assigned ? `${assigned}: ${cue.text}. Click to remove from this beat.` : activeBeat ? `Not in the cut: ${cue.text}. Click to add to ${activeBeat}.` : cue.text)
      if (canEdit) {
        line.onpointerdown = (event) => {
          if (event.pointerType === 'mouse' && event.button !== 0) return
          seekToCue(cue)
          const current = reviewAssignments.get(cueIndex)
          sweepMode = current === activeBeat ? 'remove' : 'assign'
          if (sweepMode === 'remove') setAssignment(cueIndex, '')
          else if (activeBeat) setAssignment(cueIndex, activeBeat)
          sweeping = true
          window.addEventListener('pointermove', sweep)
          window.addEventListener('pointerup', stopSweep, { once: true })
        }
        line.onkeydown = (event) => {
          if (event.key !== 'Enter' && event.key !== ' ') return
          event.preventDefault()
          setAssignment(cueIndex, reviewAssignments.get(cueIndex) === activeBeat ? '' : activeBeat)
        }
    ...
    review.append(lines)
    if (!canEdit) return
    const save = el('button', 'btn btn--primary', 'Build editable assembly')
    save.onclick = async () => {
      window.clearTimeout(reviewSaveTimer)
  ...
  const loadState = async () => {
    ...
    if (savedPlan.length) {
      ...
      reviewAssignments = new Map(state.transcript.cues.map((_, cueIndex) => [cueIndex, activeBeat]))
    }
    transcriptHint.textContent = `${state.transcript.words.length} words across ${state.transcript.cues.length} caption lines. Caption-level timing is used until a word-timed transcript is connected.`
    paintReview()
  }
...
function vBoardSceneEditor(m, slotId) {
  ...
  m.append(el('p', 'lede', 'Keep this scene’s source video, transcript, and approved passages together. Use the visual scene editor when you need titles, a shader, or other designed components.'))

  const shell = el('section', 'board-scene')
  const form = el('div', 'form board-scene__form')
  const heading = el('h2', 'board-scene__title', 'Scene')
  const status = el('p', 'form-hint')
  const name = Object.assign(el('input'), { placeholder: 'Name this scene' })
  const intent = Object.assign(el('textarea'), { rows: 6, placeholder: 'Describe exactly what needs to be on screen, what changes, and what the viewer should notice.' })
  const seconds = Object.assign(el('input'), { type: 'number', min: 0, step: 0.5, placeholder: 'seconds' })
  const footage = el('select')
  const addTake = el('button', 'btn ghost', 'Use whole video')
  const transcribe = el('button', 'btn', 'Transcribe and review')
  const upload = el('button', 'btn ghost', 'Add a video')
  upload.type = 'button'
  const picker = Object.assign(el('input'), { type: 'file', accept: 'video/*' })
  picker.hidden = true
  const backButton = el('button', 'btn ghost', 'Back to canvas')
  backButton.type = 'button'
  const visualEditor = el('button', 'btn ghost', 'Open visual scene editor')
  visualEditor.type = 'button'
  ...
  const save = el('button', 'btn btn--primary', 'Save scene')
  fieldRow(form, save)

  const media = el('div', 'board-scene__media')
  media.append(el('h3', null, 'Video for this scene'))
  ...
  const mediaActions = el('div', 'row')
  mediaActions.append(transcribe, addTake, upload, picker)
  const review = el('section', 'board-scene__review')
  review.hidden = true
  media.append(mediaActions)
  media.append(review)
  const footer = el('div', 'row')
  footer.append(visualEditor, backButton)
  shell.append(heading, form, media, status, footer)
  m.append(shell)
  ...
  let slot = null
  let paperState = null
  let selectedCueStart = null
  let selectedCueEnd = null
  ...
  const selectedCueRange = () => {
    if (selectedCueStart == null || selectedCueEnd == null) return null
    const first = Math.min(selectedCueStart, selectedCueEnd)
    const last = Math.max(selectedCueStart, selectedCueEnd)
    const cues = paperState?.transcript?.cues ?? []
    if (!cues[first] || !cues[last]) return null
    return { first, last, inSec: Number(cues[first].startSec) || 0, outSec: Number(cues[last].endSec ?? cues[last].startSec) || 0 }
  }
  ...
  const renderReview = () => {
    ...
    if (!paperState?.transcript) {
      review.append(el('p', 'hint', 'This video has no transcript yet. Transcribe it here before choosing a passage.'))
      return
    }
    const player = Object.assign(el('video', 'board-scene__player'), { controls: true, playsInline: true, preload: 'metadata' })
    player.src = `/media/${encodeURIComponent(currentProject() ?? '')}/${encodeURI(footage.value)}`
    const head = el('div', 'board-scene__review-head')
    const range = selectedCueRange()
    const use = el('button', 'btn btn--primary', range ? `Use ${timecode(range.inSec)}–${timecode(range.outSec)} in this scene` : 'Choose a passage')
    use.disabled = !range
    use.onclick = async () => {
      const picked = selectedCueRange()
      if (!slot || !picked) return
      use.disabled = true
      say('Saving this passage to the scene…')
      const result = await fetch('/api/board/take', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ projectId: currentProject(), slotId: slot.id, rel: footage.value, inSec: picked.inSec, outSec: picked.outSec, durationSec: videoFiles().find((file) => file.rel === footage.value)?.media?.durationSec ?? null }),
      }).catch((err) => ({ error: err.message }))
      ...
      board = saved.board
      const chosen = await fetch('/api/board/pick', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
      ...
      board = chosen.board
      say(`Saved ${timecode(picked.inSec)}–${timecode(picked.outSec)} as this scene’s current take. You can compare or change it on the canvas.`, 'ok')
    }
    head.append(el('div', null, `${paperState.transcript.words?.length ?? 0} words · ${cues.length} timed lines`), use)
    const lines = el('div', 'paper-edit__lines board-scene__lines')
    for (const [index, cue] of cues.entries()) {
      const line = el('button', 'paper-edit__line')
      line.type = 'button'
      line.dataset.cue = String(index)
      const active = range && index >= range.first && index <= range.last
      line.classList.toggle('paper-edit__line--active', Boolean(active))
      line.append(el('time', 'paper-edit__time', timecode(cue.startSec)), el('span', 'paper-edit__text', cue.text))
      line.onclick = () => {
        if (selectedCueStart == null || selectedCueEnd != null) {
          selectedCueStart = index
          selectedCueEnd = null
        } else selectedCueEnd = index
        player.currentTime = Number(cue.startSec) || 0
        renderReview()
      }
      lines.append(line)
    }
    const clear = el('button', 'btn ghost btn--small', 'Clear passage')
    clear.type = 'button'
    clear.disabled = !range
    clear.onclick = () => {
      selectedCueStart = null
      selectedCueEnd = null
      renderReview()
    }
    review.append(el('h3', null, 'Review the recording'), el('p', 'hint', range ? 'This is the passage that will become a take for this scene. Click any line to begin a new passage.' : 'Click the first and last transcript lines for the passage that belongs in this scene.'), player, head, lines, clear)
  }
  ...
  const fillFootage = () => {
    footage.replaceChildren(Object.assign(el('option', null, 'Choose a video…'), { value: '' }))
    for (const file of videoFiles()) footage.append(Object.assign(el('option', null, `${file.name} · ${clock(file.media?.durationSec ?? 0)}`), { value: file.rel }))
  ...
  addTake.onclick = async () => {
    ...
    const result = response.error ? response : await responseJson(response)
    addTake.disabled = false
    if (result.error) return say(result.error, 'bad')
    board = result.board
    const count = (board?.takes ?? []).filter((take) => take.slotId === slot.id).length
    say(`Video attached. This scene now has ${count} take${count === 1 ? '' : 's'} to compare on the canvas.`, 'ok')
  }

  visualEditor.onclick = () => {
    if (!slot) return
    openBoardScene = { id: slot.id, name: slot.name ?? '', intent: slot.intent ?? '', scene: slot.scene ?? '' }
    openScene = openBoardScene.scene || ''
    go('scenes')
  }

  footage.onchange = () => void loadTranscript()
...
function vStoryboard(m) {
  ...
  m.append(el('p', 'lede', 'Turn the approved script into the scenes this video needs. For each scene, be clear about the visual, the screen recording or footage to generate, and the take that best fulfils it.'))
  const scriptGuide = el('section', 'canvas-script')
  scriptGuide.hidden = true
  m.append(scriptGuide)
  ...
  const stage = el('div', 'board')
  const scroller = el('div', 'board__scroller')
  /*
   ...
   */
  const surface = el('div', 'board__surface')
  ...
  const nodeLayer = el('div', 'board__nodes')
  surface.append(wireLayer, nodeLayer)
  // Sized to the SCALED canvas so the scroller knows how far it can scroll — a
  // transform moves pixels without telling layout anything about it.
  const spacer = el('div', 'board__spacer')
  scroller.append(surface, spacer)
  ...
  const paintScriptGuide = () => {
    ...
    const pick = Object.assign(el('select'), { className: 'form-control' })
    for (const script of scripts) pick.append(Object.assign(el('option', null, script.name), { value: script.name }))
    ...
    const status = el('p', 'canvas-script__status')
    const build = el('button', 'btn btn--small', shots.length ? `Create ${shots.length} slots` : 'No speakable lines')
    ...
    build.onclick = async () => {
      ...
      build.disabled = true
      build.textContent = 'Creating slots…'
      const brief = {
        ...(board?.brief ?? {}),
    ...
    const copy = el('div', 'canvas-script__copy')
    copy.append(el('div', 'canvas-script__eyebrow', 'Script to canvas'), el('p', 'canvas-script__body', `${shots.length} scene${shots.length === 1 ? '' : 's'} will be created from its title and spoken lines.`))
    const controls = el('div', 'canvas-script__controls')
    controls.append(pick, build)
    scriptGuide.append(copy, controls, status)
  }

  const load = async () => {
    if (!proj.value) {
      surface.replaceChildren(el('div', 'hint', 'Choose a project first — a storyboard belongs to one.'))
      return
    }
    ...
    if (r.error) {
      ...
      surface.replaceChildren(Object.assign(el('div', 'hint bad'), { textContent: r.error }))
      return
    }
    board = r.board
    graph = r.graph
  ...
  function paint() {
    ...
    if (!slots.length) {
       ...
       */
      const empty = el('div', 'board__empty')
      empty.append(el('div', 'board__empty-head', 'No shots yet'), el('p', 'board__empty-body', 'A shot is a hole in the video — "open on the problem", "the price appears", "the logo". The brief lists them, and each one becomes a column here that you fill with takes.'))
      const add = el('button', 'btn btn--primary', 'Create slots from script')
      add.onclick = () => scriptGuide.querySelector('button')?.click()
      empty.append(add)
  ...
  function nodeCard(node, slot) {
    const card = el('div', 'board__node' + (selected === node.id ? ' board__node--selected' : ''))
    card.style.left = `${node.x}px`
    card.style.top = `${node.y}px`
    ...
    const head = el('div', 'board__node-head')
    head.append(Object.assign(el('div', 'board__node-name'), { textContent: node.name || 'Untitled' }))
    if (node.intent) head.append(Object.assign(el('div', 'board__node-intent'), { textContent: node.intent }))

    const state = el('div', 'board__slot-state')
    if (!takes.length) state.append(Object.assign(el('span', 'board__flag board__flag--empty'), { textContent: 'nothing shot yet' }))
    else if (!chosen) state.append(Object.assign(el('span', 'board__flag board__flag--open'), { textContent: `${takes.length} take${takes.length === 1 ? '' : 's'}, undecided` }))
    else state.append(Object.assign(el('span', 'board__flag board__flag--set'), { textContent: board.picks?.[node.id] ? 'chosen' : 'leading on ratings' }))
    if (node.seconds) state.append(Object.assign(el('span', 'board__slot-target'), { textContent: `${node.seconds}s` }))
    head.append(state)
    card.append(head)

    const actions = el('div', 'board__node-actions')
    const choose = el('button', 'board__node-select', selected === node.id ? 'Selected' : 'Select')
    choose.type = 'button'
    choose.setAttribute('aria-pressed', String(selected === node.id))
    choose.title = 'Select this scene and show its inspector'
    choose.onclick = (event) => {
      event.stopPropagation()
      select(node.id)
    }

    const edit = el('button', 'board__node-edit', 'Edit scene')
    edit.type = 'button'
    edit.title = `Build the visual scene for “${node.name || 'this scene'}”`
    edit.onclick = (event) => {
      event.stopPropagation()
      openBoardScene = { id: node.id, name: slot?.name ?? node.name ?? '', intent: slot?.intent ?? node.intent ?? '', scene: slot?.scene ?? '' }
      openScene = openBoardScene.scene || ''
      go('scenes')
    }
    const reviewFootage = el('button', 'board__node-edit', 'Review footage')
    reviewFootage.type = 'button'
    ...
    actions.append(choose, edit, reviewFootage)
    const remove = el('button', 'board__node-remove', 'Delete')
    remove.type = 'button'
    ...
    remove.onclick = async (event) => {
      ...
      remove.textContent = takesCount ? `Deleting ${takesCount} take${takesCount === 1 ? '' : 's'}…` : 'Deleting…'
    ...
    if (slot?.scene) {
      ...
      const shot = el('div', 'board__scene-preview')
      const sceneFrame = Object.assign(el('iframe', 'board__scene-frame'), {
      ...
      shot.append(sceneFrame)
      shot.append(Object.assign(el('span', 'board__scene-label'), { textContent: chosen ? 'scene + video' : 'scene preview' }))
      card.append(shot)
    } else if (chosen) {
      const shot = el('div', 'board__node-shot')
      shot.style.backgroundImage = `url('/thumb/${proj.value}/${encodeURI(chosen.rel)}')`
      const len = Math.max(0, (chosen.outSec ?? 0) - (chosen.inSec ?? 0))
      shot.append(Object.assign(el('span', 'board__len'), { textContent: clock(len) }))
      card.append(shot)
    } else {
      card.append(Object.assign(el('div', 'board__node-empty'), { textContent: takes.length ? `${takes.length} take${takes.length === 1 ? '' : 's'} — none chosen` : 'no footage yet' }))
    }
    ...
    const inPort = el('div', 'board__port board__port--in')
    const outPort = el('div', 'board__port board__port--out')
    inPort.dataset.in = node.id
    outPort.dataset.out = node.id
    outPort.title = 'Drag to the next shot'
    card.append(inPort, outPort)
  ...
  function takeCard(slot, take, isChosen) {
    const card = el('div', 'board__take' + (isChosen ? ' board__take--chosen' : ''))
    const len = Math.max(0, (take.outSec ?? 0) - (take.inSec ?? 0))

    const shot = el('div', 'board__shot')
    /*
    ...
    shot.append(Object.assign(el('span', 'board__len'), { textContent: clock(len) }))
    if (isChosen) shot.append(Object.assign(el('span', 'board__badge'), { textContent: board.picks?.[slot.id] ? 'chosen' : 'leading' }))
    card.append(shot)

    const meta = el('div', 'board__take-meta')
    meta.append(Object.assign(el('div', 'board__take-name'), { textContent: take.rel.split('/').pop() }), Object.assign(el('div', 'board__take-span'), { textContent: `${clock(take.inSec ?? 0)} → ${clock(take.outSec ?? 0)}` }))
    card.append(meta)

    const { mean, count, votes } = scoreFor(take.id)
    const rate = el('div', 'board__rate')
    const mine = votes.find((v) => v.by === me)?.rating ?? null
    for (const r of ratingMeta) {
      const b = el('button', 'board__vote' + (mine === r.id ? ' board__vote--mine' : ''))
      b.type = 'button'
    ...
    if (count) {
      const tally = el('div', 'board__tally')
      tally.append(
        Object.assign(el('span', 'board__score'), { textContent: mean.toFixed(1) }),
        Object.assign(el('span', 'board__votes'), {
          // "one opinion" rather than "1 vote": a single rating is not a consensus
          // and the wording should not let it look like one.
          textContent: count === 1 ? `one opinion · ${votes[0].by}` : `${count} people`,
        }),
      )
      tally.title = votes.map((v) => `${v.by}: ${v.rating}`).join('\n')
      card.append(tally)
    }

    const acts = el('div', 'board__take-acts')
    const pick = el('button', 'btn btn--small')
    const isPicked = board.picks?.[slot.id] === take.id
    pick.textContent = isPicked ? 'Unpick' : 'Use this one'
  ...
  function addTakeCard(slot) {
    const card = el('div', 'board__take board__take--add')
    const files = footage()
    if (!files.length) {
      card.append(el('div', 'board__add-note', 'No footage in this project yet. Record something, or drop a file on the project page.'))
      return card
    }
    const pickFile = el('select', 'board__add-file')
    pickFile.append(Object.assign(el('option', null, 'Choose a clip…'), { value: '' }))
    for (const f of files) {
      pickFile.append(Object.assign(el('option', null, `${f.name} · ${clock(f.media?.durationSec ?? 0)}`), { value: f.rel }))
    }
    const from = Object.assign(el('input', 'board__add-num'), { type: 'number', min: 0, step: 0.5, placeholder: 'from' })
    const to = Object.assign(el('input', 'board__add-num'), { type: 'number', min: 0, step: 0.5, placeholder: 'to' })
    const add = el('button', 'btn btn--small', 'Add take')
    add.onclick = async () => {
      if (!pickFile.value) {
        says(status, 'Choose a clip first.', 'bad')
        return
      }
      ...
      if (took(r)) says(status, 'Take added.', 'ok')
    }
    const row = el('div', 'board__add-row')
    row.append(from, to, add)
    card.append(el('div', 'board__add-head', `Add a take for "${slot.name}"`), pickFile, row)
    return card
  }

  /* ── the rail: the brief, and who you are ── */

  const briefBox = Object.assign(el('textarea', 'board__brief'), {
    rows: 10,
  ...
  function paintRail() {
    ...
    if (node) {
       ...
       */
      const nf = el('div', 'form')
      const nName = Object.assign(el('input'), { value: node.name ?? '' })
      const nIntent = Object.assign(el('input'), { value: node.intent ?? '', placeholder: 'what this shot has to show' })
      const nSecs = Object.assign(el('input'), { type: 'number', min: 0, step: 0.5, value: node.seconds ?? '', placeholder: 'seconds' })
      ...
      const del = el('button', 'btn btn--small', 'Delete this shot')
      del.onclick = async () => {
      ...
      const editScene = el('button', 'btn ghost btn--small', 'Open scene editor')
      editScene.type = 'button'
      editScene.onclick = () => {
        openBoardScene = { id: node.id, name: slot?.name ?? node.name ?? '', intent: slot?.intent ?? node.intent ?? '', scene: slot?.scene ?? '' }
        openScene = openBoardScene.scene || ''
        go('scenes')
      }
      const reviewFootage = el('button', 'btn ghost btn--small', 'Review footage')
      reviewFootage.type = 'button'
      ...
      rail.append(nf, el('div', 'board__rail-sep'))
      if (slot) {
        const stack = el('div', 'board__stack')
      ...
      rail.append(el('div', 'board__rail-sep'))
    } else {
      rail.append(Object.assign(el('div', 'hint'), { textContent: 'Click a shot to name it, set what it has to show, and see its takes. Drag a shot to move it; drag from its right-hand dot to the next shot to set the order; click a wire to cut it.' }))
    }
    ...
    const applyBrief = el('button', 'btn', 'Set the shot list')
    ...
    const who = Object.assign(el('input'), { value: me, placeholder: 'your name' })
    ...
    const syncPick = el('select')
    for (const a of syncInfo.adapters ?? []) {
      syncPick.append(Object.assign(el('option', null, a.label), { value: a.id }))
    }
    syncPick.value = syncInfo.chosen
    const syncHint = el('div', 'hint')
    /*
     ...
     */
    const setup = el('div', 'board__setup')
    const saySync = () => {
     ...
     */
    const sbEmail = Object.assign(el('input'), { type: 'email', placeholder: 'you@rolemodelsoftware.com' })
    const sbPass = Object.assign(el('input'), { type: 'password', placeholder: 'password' })
    const sbWho = el('div', 'hint')
    const sf = el('div', 'form')
    field(sf, 'Email', sbEmail, null)
    field(sf, 'Password', sbPass, 'Once per machine. Only the session is kept, in the 0600 config file — never the password.')

    const signIn = el('button', 'btn btn--small', 'Sign in')
    const doSignIn = async () => {
      if (!sbEmail.value.trim()) {
        says(status, 'Which email?', 'bad')
        return
      }
      says(status, 'Signing in…')
      const r = await fetch('/api/board/signin', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ email: sbEmail.value, password: sbPass.value }),
      }).then((x) => x.json())
      if (r.error) {
        says(status, r.error, 'bad')
        return
      ...
      sbPass.value = ''
      says(status, `Signed in as ${r.signedInAs}.`, 'ok')
      await refreshSharing()
    }
    signIn.onclick = doSignIn
    // Enter in either field, because a two-field form that needs a mouse to
    // submit is a form people fill in twice.
    for (const f2 of [sbEmail, sbPass]) {
      f2.onkeydown = (e) => {
        if (e.key === 'Enter') {
          e.preventDefault()
          void doSignIn()
    ...
    /*
     * Create account, beside Sign in.
     *
     * The first person to try sharing is always the person who set the project
     * up, and they have no account yet — so the one state everybody passes
     * through was the one with no way out of it.
     */
    const signUp = el('button', 'btn btn--small', 'Create account')
    signUp.onclick = async () => {
      if (!sbEmail.value.trim() || !sbPass.value) {
        says(status, 'An email and a password to create the account with.', 'bad')
        return
      }
      says(status, 'Creating the account…')
      const r = await fetch('/api/board/signup', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ email: sbEmail.value, password: sbPass.value }),
      }).then((x) => x.json())
      ...
      if (r.needsConfirmation) {
        // Not an error. The account exists and is waiting on a link.
        says(status, `Account created for ${r.email}. Confirm it from the email Supabase just sent, then sign in.`, 'warn')
      } else {
        says(status, `Signed in as ${r.signedInAs}.`, 'ok')
      }
      await refreshSharing()
    ...
    const btnRow = el('div', 'board__setup-acts')
    btnRow.append(signIn, signUp)
    fieldRow(sf, btnRow)
    sf.append(sbWho)
    setup.append(sf)
    ...
    const refreshSharing = async () => {
      ...
      for (const c of [sbEmail, sbPass]) c.closest('.form-group')?.style.setProperty('display', askingForCredentials ? '' : 'none')
      signIn.hidden = !askingForCredentials
      signUp.hidden = !askingForCredentials
     ...
     */
    const syncNow = el('button', 'btn btn--small', 'Sync now')
    ...
    const zoomRow = el('div', 'board__zoom')
    const out = el('button', 'btn btn--small', '−')
    const inn = el('button', 'btn btn--small', '+')
    const fit = el('button', 'btn btn--small', 'Fit')
    out.onclick = () => setZoom(zoom * 0.9)
    inn.onclick = () => setZoom(zoom * 1.1)
    fit.onclick = () => setZoom(1)
    zoomRow.append(out, zoomLabel, inn, fit)
  ...
  const zoomLabel = Object.assign(el('span', 'board__zoom-label'), { textContent: '100%' })
   ...
   */
  const addNode = el('button', 'btn', 'Add a shot')
  ...
  const cutName = Object.assign(el('input'), { placeholder: 'rough-cut', value: 'rough-cut' })
  const build = el('button', 'btn btn--primary', 'Build the cut from the picks')
  build.onclick = async () => {
    const r = await api('cut', { name: cutName.value })
    if (r.error) {
      says(status, r.error, 'bad')
      return
    }
    says(status, `${r.clips} shot${r.clips === 1 ? '' : 's'}, ${clock(r.seconds)} — written to ${r.document}`, 'ok')
  }
  intoFooter(addNode, cutName, build)
...
/* ── Skills ──────────────────────────────────────────────── */
function vSkills(m) {
  m.append(el('p', 'lede', 'Shared instructions Claude uses across RoleModel Studio. Install a zip once, edit its instructions here, and every signed-in teammate gets the same skill.'))

  const head = el('div', 'row')
  const install = el('button', 'btn btn--primary', 'Install skill .zip')
  install.type = 'button'
  const picker = Object.assign(el('input'), { type: 'file', accept: '.zip,application/zip' })
  picker.hidden = true
  const refresh = el('button', 'btn ghost', 'Refresh')
  refresh.type = 'button'
  const status = el('span', 'hint', 'Checking the shared skill directory…')
  head.append(install, picker, refresh, status)
  m.append(head)

  const list = el('div', 'grid')
  const standard = el('div', 'hint')
  const support = el('div', 'hint')
  const editor = el('section', 'client')
  const editorHead = el('div', 'row')
  const editorName = el('strong', '', 'Skill editor')
  const editorNote = el('span', 'hint')
  editorHead.append(editorName, editorNote)
  const editorField = Object.assign(el('textarea', 'form-control'), { rows: 22, spellcheck: false })
  const editorActions = el('div', 'row')
  const save = el('button', 'btn btn--primary', 'Save shared skill')
  save.type = 'button'
  const close = el('button', 'btn ghost', 'Close')
  close.type = 'button'
  editorActions.append(save, close)
  editor.append(editorHead, editorField, editorActions)
  editor.hidden = true
  m.append(el('div', 'client', 'Studio skills'), list, editor, el('div', 'client', 'Standard shared skills'), standard, el('div', 'client', 'Supporting skills'), support)
  ...
  const paint = (result) => {
    ...
    if (!skills.length) {
      list.append(el('p', 'hint', 'No SKILL.md files were found in the Studio skill directory.'))
    } else {
      for (const skill of skills) {
        const card = el('article', 'card')
        const body = el('div', 'body')
        const actions = el('div', 'row')
        const edit = el('button', 'btn ghost', skill.source === 'shared' ? 'Edit' : 'View')
        edit.type = 'button'
        edit.onclick = () => void openEditor(skill)
        actions.append(edit)
        if (skill.source === 'local') {
          const publishButton = el('button', 'btn ghost', 'Publish to team')
          publishButton.type = 'button'
          publishButton.onclick = () => void publish(skill)
          actions.append(publishButton)
        }
        body.append(
          el('div', 'nm', skill.name),
          el('div', 'path', skill.description || 'Shared Studio instructions.'),
          el('div', 'path', skill.source === 'shared' ? 'Shared through Supabase' : 'Bundled with this Studio install'),
          el('span', 'kind', skill.source === 'shared' ? 'Used automatically' : 'Fallback'),
          actions,
        )
        card.append(body)
        list.append(card)
...
function vBrand(m) {
  ...
  if (!marks.children.length) {
    const none = el('p', 'hint', 'No marks vendored yet. Run `pnpm run logos` with the rolemodel-brand checkout beside this one.')
    m.append(none)
  } else {
    m.append(marks)
  ...
  addDrop.append(icon('upload-04'), Object.assign(el('span', 'dropzone__lead'), { textContent: 'Drop a logo, product shot or texture' }), Object.assign(el('span', 'dropzone__sub'), { textContent: 'or click to choose — png, jpg, webp, svg, gif or avif' }))
...
function vUsage(m) {
  const intro = el('p', 'lede', 'A running total from Studio’s retained job history. Claude reports token and cost data when a run finishes; local work has time but no model spend.')
  const status = el('p', 'hint')
  const refresh = el('button', 'btn ghost', 'Refresh')
  const head = el('div', 'usage-head')
  head.append(status, refresh)
  const overview = el('div', 'usage-overview')
  const detail = el('div', 'usage-detail')
  m.append(intro, head, overview, detail)

  const stat = (label, value, note) => {
    const card = el('section', 'usage-stat')
    card.append(el('p', 'usage-stat__label', label), el('strong', 'usage-stat__value', value))
    if (note) card.append(el('p', 'usage-stat__note', note))
    return card
  }
  ...
  const loadUsage = async () => {
    ...
    if (!s.jobs) {
      detail.append(el('p', 'empty', 'When Studio runs work, its time and any Claude-reported tokens and spend will appear here.'))
      return
    }

    const accounting = el('section', 'usage-accounting')
    accounting.append(el('h2', null, 'What is included'))
    const accountingCopy = el('p', 'hint')
    const cacheTokens = s.cacheCreationTokens + s.cacheReadTokens
    accountingCopy.textContent = s.reportedJobs
    ...
    accounting.append(accountingCopy)
    if (s.localJobs) accounting.append(el('p', 'hint', `${s.localJobs} local job${s.localJobs === 1 ? '' : 's'} ran without a model bill and are excluded from spend and tokens.`))
    if (s.unknownAgentJobs) accounting.append(el('p', 'hint warn', `${s.unknownAgentJobs} agent run${s.unknownAgentJobs === 1 ? '' : 's'} has no final usage result. Its cost is unknown and is not included.`))
    detail.append(accounting)

    if (report.models?.length) {
      const models = el('section', 'usage-models')
      models.append(el('h2', null, 'By model'))
      for (const model of report.models) {
        const row = el('div', 'usage-model')
        row.append(el('strong', null, model.model), el('span', null, `${model.jobs} run${model.jobs === 1 ? '' : 's'} · ${usageNumber(model.totalTokens)} tokens · ${usageMoney(model.costUsd)}`))
        models.append(row)
      }
      detail.append(models)
    }

    const runs = el('section', 'usage-runs')
    runs.append(el('h2', null, 'Past jobs'))
    const list = el('div', 'joblist usage-runs__list')
    for (const run of report.runs) {
      const row = el('button', 'usage-run')
      row.type = 'button'
      const runStatus = run.interrupted ? 'interrupted' : run.code === 0 ? 'done' : run.code == null ? 'running' : `exit ${run.code}`
      const meta = run.usage
        ? `${usageNumber(run.usage.totalTokens)} tokens · ${usageMoney(run.usage.costUsd)}`
        : run.agent
          ? 'usage not reported'
          : 'local job · no model spend'
      row.append(
        Object.assign(el('strong', 'usage-run__label'), { textContent: run.label || 'Unnamed job' }),
        el('span', 'usage-run__when', `${runStatus} · ${run.startedAt?.slice(...ISO_TIME) || 'unknown time'} · ${usageElapsed(run.elapsedMs)}`),
        el('span', 'usage-run__meta', meta),
      )
      row.onclick = () => {
        jobId = run.id
        go('console')
...
/* ── Storage ─────────────────────────────────────────────── */
function vStorage(m) {
  const storageHead = el('div', 'storage__head')
  storageHead.append(el('p', 'lede', 'Browse the shared media shelf. Connection details stay out of the way until you need them.'))
  m.append(storageHead)
  ...
  const f = el('div', 'form')
  f.classList.add('storage__settings')
  ...
  dropZone.append(icon('upload-04'), Object.assign(el('span', 'dropzone__lead'), { textContent: 'Drop files to upload here' }), Object.assign(el('span', 'dropzone__sub'), { textContent: 'or drop them straight onto a folder above' }))
  ...
  for (const r of S.remotes) {
    ...
    editIt.onclick = () => openSettings({ remote: r })
