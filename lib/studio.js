...
function vBoardSceneEditor(m, slotId) {
  ...
  const renderReview = () => {
    ...
    const use = Object.assign(r.el.use, { textContent: passages.length ? `Use ${passages.length} selected passage${passages.length === 1 ? '' : 's'} in this scene` : 'Choose transcript words' })
