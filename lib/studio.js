...
function vCompose(m) {
  ...
  const paint = () => {
    ...
    if (!segments.length) {
      list.append(control('hint', { textContent: 'Nothing yet. A title scene, then the recording it introduces, is the usual shape.' }))
    }
  ...
  const go = control('button', { textContent: 'Render and open' })
  go.onclick = async () => {
