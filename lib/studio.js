 ...
 *
 * Keyed on the tag because `src` means two different things. On `rm-image` it is a
 * brand picture. On `rm-browser` it is a LIVE URL — the component's own comment
 * says so: `image` is the screenshot, `src` embeds an iframe. A picker keyed on
 * the field name alone offered the clay renders as something to put in a browser's
 * address bar, and offered no way to type the URL that belongs there.
 */
const IMAGE_FIELDS = {
  'rm-image': new Set(['src']),
  'rm-browser': new Set(['image']),
}
...
function vScenes(m) {
  ...
  for (const p of S.projects) {
    proj.append(Object.assign(el('option', null, (p.client ? p.client + ' · ' : '') + p.name), { value: p.id }))
  }
  ...
  function paintCards() {
    ...
    elements.forEach((e, i) => {
      ...
      const kill = el('button', 'btn btn--icon ghost del', '×')
      ...
      for (const fld of spec?.fields ?? []) {
         ...
         */
        if (isImageField(e.tag, fld) && cat.imagery?.length) {
          const group = el('div', 'form-group')
...
function vConsole(m) {
   ...
   * a long list.
   */
  const tools = el('footer', 'shell__footer')
...
function vComponents(m) {
  ...
  const bar = el('footer', 'shell__footer')
