...
/**
 * Which edges the border draws on.
 *
 * `bottom` is the default and the common case: a single rule that settles the
 * bottom edge, which is what the old radial vignette was badly trying to do.
 * `all` is the full frame — `rm-framed` is the one board that wants it, and it
 * is the only reason `radius` exists.
 */
export const BORDER_SIDES = ['bottom', 'all']
...
export const DEFAULT_RECIPE = {
  ...
  // `sides` is 'bottom' by default: a rule, not a box. 'all' draws the full
  // frame, which is the only case `radius` applies to.
  border: { width: 0, color: '#00c278', inset: 0, radius: 0, sides: 'bottom' },
}
...
export function normalize(r = {}) {
  ...
  return {
    ...
    border: {
      ...
      sides: BORDER_SIDES.includes(r.border?.sides) ? r.border.sides : d.border.sides,
    },
  }
}
...
function texture(ctx, r, w, h, unit) {
  ...
  if (type === 'ascii') {
    /*
     * A tiling field of 1s and 0s, the way the brand's own compositions do it.
     *
     * academy-hyperframes builds this as a 224x64 SVG tile — digits every 16px,
     * alternating 1/0, repeated seamlessly across the surface — and the wallpaper
     * has to match, because both end up on screen in the same video.
     *
     * The first version here drew random digits row by row with a half-cell
     * stagger. That was noise: the rows did not line up into columns, the
     * randomness read as texture rather than as data, and no two areas of the
     * frame tiled with each other. A checkerboard of 1 and 0 on an exact grid is
     * what the reference is, and it is also simpler — no seed, no stagger, and
     * deterministic by construction rather than by remembering to seed an LCG.
     */
    const cell = Math.max(4, size * unit)
    // At weight 2 this is 14px digits on a 16px cell, which is the reference
    // tile exactly. Lower thins the field out; higher crowds it.
    const glyph = cell * Math.min(1.2, Math.max(0.3, (weight / 2) * (14 / 16)))

    ctx.fillStyle = hexA(color, opacity)
    ctx.font = `${glyph}px ui-monospace, SFMono-Regular, Menlo, monospace`
    // Centred in the cell both ways, so the grid is the grid rather than the
    // baselines of one — which is what let the old version drift.
    ctx.textAlign = 'center'
    ctx.textBaseline = 'middle'

    // One cell beyond each edge: a digit centred on the last cell would
    // otherwise be clipped in half and read as a broken row.
    const cols = Math.ceil(w / cell) + 1
    const rows = Math.ceil(h / cell) + 1
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ctx.fillText((row + col) % 2 ? '0' : '1', col * cell + cell / 2, row * cell + cell / 2)
      }
    }

    // Left as the canvas found it: textAlign and textBaseline are context-wide,
    // and a later draw inheriting "centre" is a bug a long way from here.
    ctx.textAlign = 'start'
...
/**
 * A solid edge, stroked inside the frame. `width: 0` turns it off entirely, so
 * inset and radius do nothing on their own — there is no line to move.
 *
 ...
 *
 * `sides: 'bottom'` is a rule: one line along the bottom, held off the left and
 * right edges by the same inset so it reads as deliberate rather than clipped.
 * Radius is ignored there — a rule has no corners to round.
 */
function border(ctx, r, w, h, unit) {
  const { width, color, inset, radius, sides } = r.border
  if (!width) return
  const lw = width * unit
  const pad = inset * unit
  const off = pad + lw / 2
...
export function css(recipe) {
  ...
  if (r.tint.alpha > 0) {
    parts.unshift(`linear-gradient(${r.tint.angle}deg, ${hexA(r.tint.color, r.tint.alpha)} 0%, ${hexA(r.tint.color, 0)} 72%)`)
  }
