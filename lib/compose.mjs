...
/**
 * How long a scene written as free markup runs, in milliseconds.
 *
 * The structured form reads `at` and `for` off the element objects. Authored
 * markup was said to have "nothing to read" and given a flat four seconds --
 * but `at` and `for` are right there in the tags, which is how the runtime
 * itself times them. A seventeen-beat walkthrough composed as a four-second
 * card is not a guess that cuts mid-sentence; it is the first beat and nothing
 * else.
 *
 * Returns null when the markup carries no timing at all, so the caller can keep
 * its old default rather than invent one from an empty scan.
 */
export function markupDurationMs(html, { min = 2000 } = {}) {
	let end = 0;
	let found = false;
	/* Tag by tag, because a `for` inside a data- attribute or a comment is not
	   a duration and a regex over the whole file would take it for one. */
	for (const tag of String(html).matchAll(/<rm-[a-z-]+\b[^>]*>/gi)) {
		const at = Number(/\bat\s*=\s*"([^"]*)"/i.exec(tag[0])?.[1]);
		const dur = Number(/\bfor\s*=\s*"([^"]*)"/i.exec(tag[0])?.[1]);
		if (!Number.isFinite(at) && !Number.isFinite(dur)) continue;
		found = true;
		end = Math.max(end, (Number.isFinite(at) ? at : 0) + (Number.isFinite(dur) ? dur : min));
	}
	return found ? Math.max(end, min) : null;
}

/**
 * The sound a scene asks for, named the way the project names it.
 *
 * A scene used to be silent markup with its narration supplied separately, on
 * the command line, by whoever happened to be composing it. So the file could
 * not say what it had been cut to: reopening it lost the voice, and rendering
 * it was a second chance to name the wrong file.
 *
 * An rm-audio layer puts the answer in the scene. Reading it here is what lets
 * a render be silent by accident never again -- there is nothing to remember.
 *
 * Returns null when the scene names no sound, so a caller that was told one
 * explicitly keeps it.
 */
export function markupAudio(html) {
	for (const tag of String(html).matchAll(/<rm-audio\b[^>]*>/gi)) {
...
/**
 * Every sound layer in a scene, with where it starts and how loud it is.
 *
 * `markupAudio` answers "what is this cut to", which is what a composition
 * needed. A render needs the rest of the layer: `at` is why second 0 of the
 * file is not second 0 of the scene, and `gain` is a decision somebody made in
 * the editor and could hear. Dropping either would render a track that is
 * correct and in the wrong place, which is worse than silence because it looks
 * finished.
 */
export function markupAudioLayers(html) {
	const layers = [];
	for (const tag of String(html).matchAll(/<rm-audio\b[^>]*>/gi)) {
		const attr = (name) => new RegExp(`\\b${name}\\s*=\\s*"([^"]*)"`, "i").exec(tag[0])?.[1];
		const src = attr("src");
		if (!src) continue;
		layers.push({
			src,
			atMs: Math.max(0, Number(attr("at")) || 0),
			/* The editor's own default is 1; a gain of 0 is a deliberate mute. */
			gain: Number.isFinite(Number(attr("gain"))) ? Number(attr("gain")) : 1,
		});
	}
	return layers;
}
 ...
 */
export function sceneHtml({ wallpaper, elements, body: authored, title = "Composed scene", base = "..", brand, previewAt = 0, footage } = {}) {
	/*
	...
	/*
	 * A Canvas scene can carry the selected passage underneath its components.
	 *
	 * The Canvas owns the source and the in/out range; this wrapper owns how that
	 * source is presented while somebody adds a lower third or title. Keeping the
	 * video as a direct child makes it a real stage layer, below every slotted
	 * component, instead of a second player that has no relationship to the scene.
	 */
	const footageSource = typeof footage?.src === "string" ? footage.src.trim() : "";
	/*
	 * A still to paint while the video is not there.
	 *
	 * Chrome defers media loading in a tiny, offscreen iframe, and a gallery of
	 * scenes is a dozen of those — so the takes stayed at readyState 0 and every
	 * footage scene showed its lower third on an empty ground. A poster is one
	 * small image the browser fetches like any other, so the card shows the shot
	 * whether or not the video ever loads.
	 */
	const footagePoster = typeof footage?.poster === "string" ? footage.poster.trim() : "";
	const footageStart = Math.max(0, Number(footage?.inSec) || 0);
	const footageEnd = Math.max(footageStart, Number(footage?.outSec) || 0);
	const footageMarkup = footageSource
		/*
		 * `metadata`, not `auto`.
		 *
		 * `auto` asks the browser to buffer the whole file, and these are 70-100MB
		 * takes. A gallery of scenes is a dozen of these at once against a
		 * six-connection limit, so every one of them stalled and the cards showed
		 * their lower thirds on an empty ground — the video was in the markup and
		 * never arrived. Metadata plus a seek fetches the range that is actually
		 * wanted, which is all a still frame ever needed.
		 */
		? `      <video data-rm-footage data-rm-footage-start="${footageStart}" data-rm-footage-end="${footageEnd}" src="${esc(footageSource)}"${footagePoster ? ` poster="${esc(footagePoster)}"` : ""} muted playsinline preload="metadata" aria-label="Selected scene footage"></video>\n`
		: "";
...
<script type="module">
  ...
  const footage = document.querySelector('[data-rm-footage]');
  const footageStart = Number(footage?.dataset.rmFootageStart || 0);
  const footageEnd = Number(footage?.dataset.rmFootageEnd || 0);
  const seekFootage = (ms) => {
    if (!footage || footage.readyState < HTMLMediaElement.HAVE_METADATA) return;
    const requested = footageStart + Math.max(0, Number(ms) || 0) / 1000;
    const duration = Number.isFinite(footage.duration) ? footage.duration : Infinity;
    const limit = footageEnd > footageStart ? Math.min(footageEnd, duration) : duration;
    const time = Math.max(footageStart, Math.min(requested, limit));
    if (Number.isFinite(time) && Math.abs(footage.currentTime - time) > 0.025) footage.currentTime = time;
  };
  if (footage) {
    document.documentElement.addEventListener('rmseek', (event) => seekFootage(event.detail));
    footage.addEventListener('loadedmetadata', () => seekFootage(RM.t), { once: true });
    seekFootage(RM.t);
  }
  await RM.ready();
