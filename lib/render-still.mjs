 ...
 */
export async function renderStill({ body, out, atMs = 0, width = 1920, height = 1080, brand, resolveFile = null }) {
	...
	const srv = createServer((req, res) => {
		...
		/*
		 * The repo is what a still is made of, but a showcase shows a project's
		 * own footage, which lives in the library. A caller can say where a path
		 * outside the repo really is; anything it does not claim stays in ROOT.
		 */
		const outside = resolveFile?.(pathname) ?? null;
		const file = outside ?? resolve(ROOT, p);
		...
		/*
		 * Byte ranges, because a <video> seeks with them. Answered with the whole
		 * file, Chromium keeps asking, the network never goes idle, and the still
		 * never comes.
		 */
