 ...
 *
 *   npm run forks          clone or update them
 *   npm run forks:check    say where they are and how far from upstream
 *
...
console.log("");
if (check && missing) {
	console.log(`  ${missing} not cloned. Run \`npm run forks\` to get them.\n`);
	process.exit(1);
}
if (!check) {
