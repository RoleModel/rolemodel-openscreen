...
export async function stageRenderAssets(dir, { brand = "rolemodel", wallpaper = null, quiet = false } = {}) {
  ...
  const titleOut = preferred
    ...
    : title;
  /*
   * Into templates/, not the root.
   *
   * HyperFrames discovers every root-level HTML file carrying a
   * data-composition-id and treats it as a composition, so a starter card at the
   * root is a second entry point in every project — which is why something had
   * to keep moving it afterwards, and why it came back on every resync. Staged
   * where it is a template and nothing has to move it.
   */
  const starters = join(out, "templates");
  await mkdir(starters, { recursive: true });
  await writeFile(join(starters, "rm-title-template.html"), titleOut);
  /* And clear the old root copy, so a folder staged before this stops being
     discovered as two compositions. */
  await rm(join(out, "title.html"), { force: true }).catch(() => {});
