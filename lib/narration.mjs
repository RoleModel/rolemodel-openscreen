 ...
 * lossy round trip that mis-hears product names ("Ridgeline" becomes "Phoenix",
...
/*
 * Every capture inherits jobs.childEnv(), not the bare process.env.
 *
 * Studio calls this for ffprobe, ffmpeg and rclone. Launched from Finder its
 * PATH is /usr/bin:/bin, so a Homebrew binary the person can plainly see
 * installed came back ENOENT — the same bug fixed one spawn at a time, six
 * times, before it was fixed here for all of them.
 */
export function capture(cmd, args, opts = {}) {
	return new Promise((res) => {
		const child = spawn(cmd, args, { env: childEnv(), ...opts });
		let out = "";
		let err = "";
