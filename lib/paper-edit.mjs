...
/** Bumped when a stored paper edit can no l nger be read by this file. */
 ...
 * trusted.
 */
export const orderedWords = (transcript) => [...(transcript?.words ?? [])].sort((a, b) => a.startSec - b.startSec || String(a.id).localeCompare(String(b.id)))
...
export function selectionToCutlist(validated, { transcript, plan, rel, durationSec, lead = LEAD_SEC, tail = TAIL_SEC } = {}) {
  ...
  const ranges = [...(validated?.ranges ?? [])].sort((a, b) => (order.get(a.shot) ?? 0) - (order.get(b.shot) ?? 0) || a.fromIndex - b.fromIndex)
