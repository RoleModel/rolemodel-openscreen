...
const WALLPAPERS = [
	...
	// Academy curriculum units — wayfinding within one sub-brand.
	...["ruby", "design", "rails"].map((unit) => ({
		name: `academy-${unit}`,
		label: `Academy · ${unit[0].toUpperCase()}${unit.slice(1)} unit`,
		css: dotGrid({ tint: tokens.unitSignatures[unit] }),
	})),

	// One board per sub-brand, tinted with its real Optics signature. Driven by
	// tokens.json so a new sub-brand in the Optics export gets a wallpaper for free.
	...Object.entries(tokens.subBrands ?? {})
		.filter(([id]) => id !== "academy") // Academy is covered by its unit boards
		.flatMap(([id, brand]) => [
			{
				name: `${id}-board`,
				label: `${brand.label} · tinted board`,
				css: dotGrid({ tint: brand.hex, tintAlpha: 0.22 }),
			},
			{
				name: `${id}-wash`,
				label: `${brand.label} · deep wash`,
				css: wash("#16202c", "#101a26", brand.hex),
			},
		]),
];
...
for (const wp of WALLPAPERS) {
	...
	// JPEG, not PNG. These are photographic-ish gradients and dot fields; PNG
	// stores them losslessly at ~2MB each for no visible gain, and OpenScreen's
	// own bundled wallpapers are .jpg anyway. q92 is indistinguishable here and
	// cuts the set from ~38MB to a couple of MB.
	await page.screenshot({ path: resolve(OUT, `${wp.name}.jpg`), type: "jpeg", quality: 92 });
	console.log(`  ${wp.name}.jpg  —  ${wp.label}`);
}

// A contact sheet so a human can pick one without opening 22 files.
// Built from the rendered JPEGs, not by re-running the CSS: re-rendering shows
// absolute background-size tiles at the wrong scale in a small cell, which made
// the ascii panel's texture look 8x too large and the sheet actively misleading.
const cells = WALLPAPERS.map(
	(wp) => `
  <figure style="margin:0">
    <div style="width:100%;aspect-ratio:16/9;border-radius:10px;background:#0e141b url('${wp.name}.jpg') center/cover no-repeat"></div>
    <figcaption style="font:500 15px/1.4 ui-sans-serif,system-ui;color:${palette.light};opacity:.72;padding-top:10px">
      ${wp.label}<br><code style="opacity:.6;font-size:13px">${wp.name}.jpg</code>
    </figcaption>
  </figure>`,
).join("");

await page.setViewportSize({ width: 1680, height: 1400 });
await page.goto(pathToFileURL(resolve(OUT, "_sheet.html")).href).catch(() => {});
await writeFile(
	resolve(OUT, "_sheet.html"),
   ...
   <p style="font:400 16px/1.5 ui-sans-serif,system-ui;color:${palette.light};opacity:.6;margin:0 0 40px">Backdrops behind the screen recording. Rendered at ${W}\u00d7${H}, JPEG q92.</p>
...
await page.screenshot({ path: resolve(OUT, "_contact-sheet.jpg"), type: "jpeg", quality: 90, fullPage: true });
await rm(resolve(OUT, "_sheet.html"), { force: true });
await browser.close();

await writeFile(
	resolve(OUT, "index.json"),
	`${JSON.stringify(
		WALLPAPERS.map(({ name, label }) => ({ name, label, file: `${name}.jpg` })),
		null,
		2,
	)}\n`,
);
