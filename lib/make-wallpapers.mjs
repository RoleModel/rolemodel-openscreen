...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
	...
	if (!has("no-install") && !only) {
		...
		if (!existsSync(join(fork, "public", "wallpapers"))) {
			console.log(`\n  no OpenScreen checkout at ${fork} — skipping the editor install\n`);
		} else {
			await mkdir(join(pub, "thumbs"), { recursive: true });
			// Anything of ours that no longer has a recipe goes, same rule as above.
			const want = new Set(recipes.map((r) => `${r.name}.jpg`));
			...
			for (const r of recipes) {
				...
				// sips rather than a second render: the swatch has to be the same
				// picture as the wallpaper, and re-running the recipe at 240px shows
				// texture at the wrong scale.
				execFileSync("sips", ["-z", "240", "240", src, "--out", join(pub, "thumbs", `${r.name}.jpg`)], {
					stdio: "ignore",
				});
			}
			const manifest = [
			...
			console.log(`\n  installed into ${fork}`);
			console.log(`    public/wallpapers/brand/  (${recipes.length} + thumbs)`);
