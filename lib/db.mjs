...
import { storyboards, studioSettings, studioSkills, styleImages, stylePeople } from "./schema.mjs";
 ...
 */
const imageRow = (r) => r && { id: r.id, subject: r.subject, prompt: r.prompt, model: r.model, aspect: r.aspect, url: r.url, file: r.file, created_by: r.createdBy, created_at: r.createdAt };
...
export async function insertStyleImage({ databaseUrl, image, by = null }) {
	return attempt("save the image", async () => {
		const rows = await db(databaseUrl)
			.insert(styleImages)
			.values({ subject: image.subject, prompt: image.prompt ?? null, model: image.model ?? null, aspect: image.aspect ?? null, url: image.url, file: image.file ?? null, createdBy: by, ...(image.created_at ? { createdAt: image.created_at } : {}) })
			.returning();
