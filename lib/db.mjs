...
import { looks, storyboards, studioSettings, studioSkills, styleImages, stylePeople, videoComments } from "./schema.mjs";
...
/*
 * Notes left on shared video pages. The page writes them through the Data
 * API; the Studio reads them here, as studio_app, and can take one down.
 * Rows come back in the page's own shape (at_ms, created_at) so both readers
 * agree.
 */
/* Postgres writes "2026-09-06 04:12:35.22+00"; the page and the browser want ISO. */
const iso = (t) => (t ? new Date(String(t).replace(" ", "T").replace(/\+00$/, "Z")).toISOString() : t);
const noteRow = (r) => r && { id: r.id, project: r.project, video: r.video, at_ms: r.atMs, author: r.author, body: r.body, created_at: iso(r.createdAt) };

export async function listVideoComments({ databaseUrl, project, video = null }) {
	return attempt("read the notes", async () => {
		const where = video ? and(eq(videoComments.project, project), eq(videoComments.video, video)) : eq(videoComments.project, project);
		return (await db(databaseUrl).select().from(videoComments).where(where).orderBy(asc(videoComments.createdAt))).map(noteRow);
	});
}

export async function deleteVideoComment({ databaseUrl, id }) {
	return attempt("remove the note", async () => {
		const rows = await db(databaseUrl).delete(videoComments).where(eq(videoComments.id, id)).returning();
		return noteRow(rows[0]) ?? null;
	});
}
