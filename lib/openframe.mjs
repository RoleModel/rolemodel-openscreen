/*
 * Hand a finished video to OpenFrame and get a review link back.
 *
 * The pipeline could make a video and never deliver it. Sending an mp4 by email
 * gets you feedback as prose — "around the middle, the bit with the railing" —
 * which is the most expensive possible way to receive a note. OpenFrame puts
 * comments on the timeline, so a note arrives attached to the frame it is about.
 *
 * The whole flow is six calls, discovered by reading the routes and then proved
 * against a live instance:
 *
 *   GET/POST /api/workspaces                                a project needs one
 *   GET/POST /api/projects                                   find or make it
 *   POST     /api/projects/:p/videos/r2-init                 presigned PUT
 *   PUT      <presignedPutUrl>                               the bytes
 *   POST     /api/projects/:p/videos                         register it
 *   POST     /api/projects/:p/videos/:v/share                the link
 *
 * Authentication is a token, which upstream does not have: everything there is
 * next-auth session cookies, right for a browser and unusable from a script. Our
 * fork adds `OPENFRAME_API_TOKENS`, mapping a token to a user so it acts as them
 * and gets no more access than they have. See RoleModel/OpenFrame.
 */
import { createReadStream } from "node:fs";
import { readFile, stat, writeFile } from "node:fs/promises";
import { basename } from "node:path";

/** Videos are large; a slow upload is normal and should not look like a hang. */
const UPLOAD_TIMEOUT_MS = 15 * 60 * 1000;
const CALL_TIMEOUT_MS = 30_000;

export class OpenFrameError extends Error {
	constructor(message, { status, url } = {}) {
		super(message);
		this.name = "OpenFrameError";
		this.status = status;
		this.url = url;
	}
}

/**
 * A client bound to one instance and one token.
 *
 * `base` and `token` are required rather than defaulted: a share link is
 * outward-facing, and guessing which instance to publish to is not a mistake
 * worth making silently.
 */
export function openFrame({ base, token, fetchImpl = fetch } = {}) {
	if (!base) throw new OpenFrameError("no OpenFrame url — set OPENFRAME_URL");
	if (!token) throw new OpenFrameError("no OpenFrame token — set OPENFRAME_TOKEN");
	const root = String(base).replace(/\/+$/, "");

	async function call(path, { method = "GET", body, timeout = CALL_TIMEOUT_MS } = {}) {
		const url = `${root}${path}`;
		const res = await fetchImpl(url, {
			method,
			headers: {
				authorization: `Bearer ${token}`,
				...(body ? { "content-type": "application/json" } : {}),
			},
			body: body ? JSON.stringify(body) : undefined,
			signal: AbortSignal.timeout(timeout),
		}).catch((err) => {
			throw new OpenFrameError(`${method} ${path} failed: ${err.message}`, { url });
		});

		const text = await res.text();
		let json = null;
		try {
			json = text ? JSON.parse(text) : null;
		} catch {
			// An HTML body from a 401 or a proxy is the usual cause, and quoting the
			// first line of it is more use than "unexpected token <".
			const first = text.split("\n").find((l) => l.trim()) ?? "";
			throw new OpenFrameError(`${method} ${path} returned ${res.status}, not JSON: ${first.slice(0, 120)}`, {
				status: res.status,
				url,
			});
		}
		if (!res.ok) {
			throw new OpenFrameError(`${method} ${path} -> ${res.status}: ${json?.error ?? "no reason given"}`, {
				status: res.status,
				url,
			});
		}
		// Every route wraps its payload in `data`; unwrap once here so callers do
		// not each have to remember.
		return json?.data ?? json;
	}

	/** The named workspace, or the first one, or a new one. */
	async function workspace(name) {
		const existing = await call("/api/workspaces");
		const list = Array.isArray(existing) ? existing : (existing?.workspaces ?? []);
		if (name) {
			const match = list.find((w) => w.name === name);
			if (match) return match;
		} else if (list.length) {
			return list[0];
		}
		return call("/api/workspaces", { method: "POST", body: { name: name ?? "RoleModel" } });
	}

	/** The named project in that workspace, or a new one. */
	async function project(name, workspaceId) {
		const page = await call(`/api/projects?workspaceId=${encodeURIComponent(workspaceId)}`);
		const match = (page?.projects ?? []).find((p) => p.name === name);
		if (match) return match;
		return call("/api/projects", { method: "POST", body: { name, workspaceId } });
	}

	/**
	 * Upload a file and register it as a video.
	 *
	 * The PUT goes to a presigned URL, so the bytes never pass through the app —
	 * which is why this can stream from disk rather than buffering a render into
	 * memory. `duplex: "half"` is required for a stream body in undici and is the
	 * kind of omission that fails only on large files.
	 */
	async function upload(projectId, file, title, finish = (body) => call(`/api/projects/${projectId}/videos`, { method: "POST", body })) {
		const info = await stat(file);
		const name = basename(file);
		const init = await call(`/api/projects/${projectId}/videos/r2-init`, {
			method: "POST",
			body: { fileName: name, contentType: "video/mp4", sizeBytes: info.size },
		});

		/* The R2 route returns an empty single-PUT URL when the file needs chunking.
		 * That is a valid response, not an incomplete one. Keep the upload streamed
		 * from disk either way: buffering a large render here defeats multipart before
		 * it begins. */
		const cancelPendingUpload = () => call(`/api/projects/${projectId}/videos/r2-init`, {
			method: "DELETE",
			body: { objectKey: init?.objectKey, uploadToken: init?.uploadToken },
		}).catch(() => {});
		try {
			if (init?.multipart) {
				const multipart = init.multipart;
				const partSize = Number(multipart.partSizeBytes);
				const parts = Array.isArray(multipart.parts) ? multipart.parts : [];
				if (!init.objectKey || !init.uploadToken || !Number.isSafeInteger(partSize) || partSize <= 0 || !parts.length) {
					throw new OpenFrameError("r2-init returned an incomplete multipart upload plan");
				}

				const completed = [];
				for (const part of parts) {
					const partNumber = Number(part?.partNumber);
					const url = typeof part?.url === "string" ? part.url : "";
					const start = (partNumber - 1) * partSize;
					const end = Math.min(info.size, start + partSize);
					if (!Number.isSafeInteger(partNumber) || partNumber < 1 || !url || start < 0 || start >= info.size || end <= start) {
						throw new OpenFrameError("r2-init returned an invalid multipart upload part");
					}

					const put = await fetchImpl(url, {
						method: "PUT",
						headers: { "content-length": String(end - start) },
						body: createReadStream(file, { start, end: end - 1 }),
						duplex: "half",
						signal: AbortSignal.timeout(UPLOAD_TIMEOUT_MS),
					});
					if (!put.ok) throw new OpenFrameError(`upload part ${partNumber} -> ${put.status}`, { status: put.status });
					const etag = put.headers?.get("etag");
					if (!etag) throw new OpenFrameError(`upload part ${partNumber} returned no ETag`);
					completed.push({ partNumber, etag });
				}

				await call(`/api/projects/${projectId}/videos/r2-complete`, {
					method: "POST",
					body: { objectKey: init.objectKey, uploadToken: init.uploadToken, parts: completed },
				});
			} else {
				if (!init?.presignedPutUrl) throw new OpenFrameError("r2-init returned no presignedPutUrl");
				const put = await fetchImpl(init.presignedPutUrl, {
					method: "PUT",
					headers: { "content-type": init.contentType ?? "video/mp4", "content-length": String(info.size) },
					body: createReadStream(file),
					duplex: "half",
					signal: AbortSignal.timeout(UPLOAD_TIMEOUT_MS),
				});
				if (!put.ok) throw new OpenFrameError(`upload -> ${put.status}`, { status: put.status });
			}
		} catch (error) {
			await cancelPendingUpload();
			throw error;
		}

		return finish({
			title: title ?? name.replace(/\.[^.]+$/, ""),
			videoUrl: init.proxyUrl,
			objectKey: init.objectKey,
			uploadToken: init.uploadToken,
			providerId: "r2",
		});
	}

	/**
	 * The same upload, finished as a new version of a video that already exists.
	 *
	 * Everything above — the R2 ticket, the PUT, the multipart walk — is identical;
	 * only the call that claims the bytes differs. So `upload` takes the finisher
	 * rather than naming the endpoint, and this passes the other one.
	 *
	 * Worth the indirection because the difference matters to a reviewer: a new
	 * VIDEO gets a new share link and starts with no comments, so re-sending a cut
	 * that way silently abandons the notes somebody left on the last one. A new
	 * VERSION keeps the id, and therefore the link and the thread.
	 */
	async function addVersion(projectId, videoId, file, { label = null, setActive = true } = {}) {
		return upload(projectId, file, null, (body) =>
			call(`/api/projects/${projectId}/videos/${videoId}/versions`, {
				method: "POST",
				body: {
					videoUrl: body.videoUrl,
					objectKey: body.objectKey,
					uploadToken: body.uploadToken,
					providerId: body.providerId,
					versionLabel: label ?? new Date().toISOString().slice(0, 16).replace("T", " "),
					/* Without this the version is added and nothing shows it — the
					   route defaults isActive to false. */
					setActive,
				},
			}),
		);
	}

	/** A link a client can open without an account. */
	async function share(projectId, videoId, { guests = true, comments = true } = {}) {
		const made = await call(`/api/projects/${projectId}/videos/${videoId}/share`, {
			method: "POST",
			body: { allowGuestComments: guests, permission: comments ? "COMMENT" : "VIEW" },
		});
		if (!made?.shareUrl) throw new OpenFrameError("share returned no shareUrl");
		return made;
	}

	/**
	 * The share link a video already has, or null when it has none.
	 *
	 * A GET, and that is the entire reason this exists next to `share`. OpenFrame's
	 * POST rotates the token on an existing link, so asking "what is the link for
	 * this video" with a POST silently invalidates whatever was already sent to a
	 * client. Nothing that only wants to read a link should use POST.
	 */
	async function shareLink(projectId, videoId) {
		const got = await call(`/api/projects/${projectId}/videos/${videoId}/share`).catch(() => null);
		return got?.shareUrl ?? null;
	}

	/**
	 * Remove the review copy of a video.
	 *
	 * Studio keeps the source file in the local project. This only removes the
	 * video, versions, comments, share links, and stored media from OpenFrame so
	 * a superseded review is no longer visible to reviewers.
	 */
	async function removeVideo(projectId, videoId) {
		return call(`/api/projects/${projectId}/videos/${videoId}`, { method: "DELETE" });
	}

	return { call, workspace, project, upload, addVersion, share, shareLink, removeVideo, base: root };
}

/**
 * Everything, in order: file in, review link out.
 *
 * `onStep` is called before each call so a caller can narrate it. Uploads take
 * minutes and silence reads as a hang.
 */
export async function shareVideo({ base, token, file, project: projectName, title, workspace: workspaceName, onStep = () => {} }) {
	const api = openFrame({ base, token });

	onStep("workspace");
	const ws = await api.workspace(workspaceName);

	onStep(`project ${projectName}`);
	const proj = await api.project(projectName, ws.id);

	/*
	 * Which video this file IS, remembered beside it.
	 *
	 * Matching on title was a guess, and a bad one here: two videos in the same
	 * project can share a name — this project already has two called "CCC Days
	 * Presentation" — and a composition folder is called something else entirely
	 * from the video it produces. Guessing wrong attaches a cut to the wrong
	 * review, which is worse than making a second one.
	 *
	 * So the first send writes the id next to the file, exactly as the poster time
	 * is, and every later send reads it. A sidecar rather than a manifest entry
	 * because it belongs to this rendered file: copy the file and the link comes
	 * with it, delete it and nothing is left pointing at a video that no longer
	 * has a local source.
	 */
	const marker = `${file}.openframe`;
	const remembered = await readFile(marker, "utf8")
		.then((raw) => JSON.parse(raw))
		.catch(() => null);

	/*
	 * A re-send is a new version, not a new video.
	 *
	 * Uploading again made a second video with its own share link, so the address
	 * already in somebody's inbox kept playing the old cut and the notes they had
	 * left stayed on a video nobody would open again. Matched on title within the
	 * project, because that is the only handle both sides already agree on —
	 * nothing here persists an OpenFrame video id, and inventing a store for one
	 * would be a second answer to "which video is this".
	 *
	 * Exact title only. A near-match would eventually attach a cut to the wrong
	 * review, and a stray extra video is a far cheaper mistake than that.
	 */
	const wanted = title ?? basename(file).replace(/\.[^.]+$/, "");
	const inProject = await api.call(`/api/projects/${proj.id}/videos`).then((r) => r?.videos ?? r ?? []).catch(() => []);
	/*
	 * The id first, and the title only as a fallback for a file sent before this
	 * was written. A remembered id that no longer resolves — the video was deleted
	 * in OpenFrame — falls through to making a new one rather than failing.
	 */
	const existing =
		(remembered?.videoId && remembered?.projectId === proj.id
			? inProject.find((v) => v.id === remembered.videoId)
			: null)
		?? inProject.find((v) => String(v.title ?? "") === wanted)
		?? null;

	let video;
	if (existing) {
		onStep(`new version of ${wanted}`);
		await api.addVersion(proj.id, existing.id, file, { label: `Studio ${new Date().toISOString().slice(0, 16).replace("T", " ")}` });
		video = { id: existing.id, title: existing.title };
	} else {
		onStep(`uploading ${basename(file)}`);
		video = await api.upload(proj.id, file, title);
	}

	/* Written after the upload rather than before: a send that failed halfway
	   should not leave a file claiming to be a video it never became. */
	await writeFile(marker, `${JSON.stringify({ projectId: proj.id, videoId: video.id, title: video.title }, null, "\t")}\n`, "utf8").catch(() => {});

	onStep("share link");
	const link = await api.share(proj.id, video.id);

	/*
	 * `shareUrl` and nothing else.
	 *
	 * There used to be a `watchUrl` here, composed as `${base}/watch/${id}`, and it
	 * did not work: /watch/<id> carries no token, so OpenFrame's watch API finds no
	 * share-session cookie and answers 403, which the page renders as "Video not
	 * found or access denied". It looked fine in testing because a signed-in project
	 * member passes checkProjectAccess and never needs the token — so the link works
	 * for whoever made it and for nobody else.
	 *
	 * The token only reaches the browser through ?shareToken=, which /watch strips
	 * into an httpOnly cookie on arrival. That makes the address bar actively
	 * misleading: the URL shown after the redirect is not the URL to send anyone.
	 * `shareUrl` is the one with the token, so it is the only one handed out.
	 */
	return {
		workspace: ws.name,
		project: proj.name,
		video: { id: video.id, title: video.title },
		/* So the caller can say "sent" or "updated" rather than guessing. */
		newVersion: Boolean(existing),
		shareUrl: link.shareUrl,
	};
}
