...
/*
 * Where it lands: a page of its own, /studio-looks, made the first time. A new
 * instance used to drop wherever Framer found free canvas on whichever page
 * the session had open — the homepage, six thousand pixels to the left of it.
 * Here they stack down one page, each below the last, and a look is copied
 * from there to wherever it is going.
 */
const pages = await framer.getNodesWithType("WebPageNode");
let page = pages.find((p) => p.path === "/studio-looks");
if (!page) page = await framer.createWebPage("/studio-looks");
const node = await framer.addComponentInstance({ url: exp.insertURL, parentId: page.id, attributes: { width: 960, height: 540, controls: { look: ${JSON.stringify(look)}, animate: true } } });
try {
  const kids = (await page.getChildren()).filter((k) => k.id !== node.id);
  const rects = (await Promise.all(kids.map((k) => framer.getRect(k.id).catch(() => null)))).filter(Boolean);
  const below = rects.length ? Math.max(...rects.map((r) => r.y + r.height)) + 80 : 0;
  await framer.setAttributes(node.id, { position: "absolute", left: "0px", top: Math.round(below) + "px" });
} catch {}
const placed = await framer.getRect(node.id).catch(() => null);
console.log(JSON.stringify({ ok: true, id: node.id, insertURL: exp.insertURL, at: placed }));
`;
}
