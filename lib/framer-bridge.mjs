...
/*
 * Where it lands. Framer drops a new instance in free canvas, which on a site
 * with wide pages was six thousand pixels left of anything — placed, reported,
 * and never seen. It goes just above the top-left of the page's content instead,
 * where the first scroll up finds it.
 */
try {
  const root = await framer.getCanvasRoot();
  /* The page's frames, not whatever else is loose on the canvas: an earlier
     look left far out would otherwise be the thing the next one lines up on. */
  const kids = (await root.getChildren()).filter((k) => k.id !== node.id && k.__class !== "ComponentInstanceNode");
  ...
  if (widest) await framer.setAttributes(node.id, { position: "absolute", left: Math.round(widest.x) + "px", top: Math.round(widest.y - 600) + "px" });
} catch {}
