...
/**
 * Just the peaks for one file, built once and kept.
 *
 * cacheSource is the whole kit -- proxy, filmstrip, peaks -- and that is right
 * for a clip somebody is going to cut. A narration track in a scene is not
 * being cut: the editor draws its shape so the words can be lined up with the
 * pictures, and nothing else about it is ever asked for. Transcoding a proxy
 * of a voice recording to get there is minutes of ffmpeg for a file nobody
 * will play through it.
 */
export async function peaksFor(file, cacheDir) {
	const key = await keyFor(file);
	await mkdir(join(cacheDir, "peaks"), { recursive: true });
	const out = join(cacheDir, "peaks", `${key}.json`);
	return { key, ...(await buildPeaks(file, out)) };
}
