 ...
 *
 * WHY A 403 IS RETRIED RATHER THAN REPORTED
 *
 * fal answers a bad key with 401. A 403, in practice, is almost never the key:
 * it is fal's gateway turning a request away under load, and the very same
 * request with the very same key succeeds a second later. Calling that "the key
 * was rejected" sent people off to check their key and their balance when
 * neither was wrong, so a 403 — like a 429 and a 5xx — is waited out and tried
 * again, and only said out loud once it has refused three times.
 */
...
export async function callFal(endpoint, body, key, { fetchImpl = fetch } = {}) {
	...
	for (let attempt = 1; attempt <= TRIES; attempt++) {
		...
		if (!TRANSIENT.has(res.status) || attempt === TRIES) break;
		/* A gateway that is busy needs a moment, not another request at once. */
	...
	const why =
		status === 401
			...
			: status === 403
				? ` — fal turned ${TRIES} tries away; it is usually busy rather than broken, so try again in a minute${detail ? ` (${detail.slice(0, 200)})` : ""}`
				: detail
					? ` — ${detail.slice(0, 300)}`
					: "";
	throw new Error(`fal answered ${status}${why}`);
}
