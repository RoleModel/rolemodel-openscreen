...
// Local development keeps the app as a sibling checkout. CI keeps its pinned
// checkout in /tmp so it cannot be mistaken for this toolkit's own source.
const OS_ROOT = resolve(osArg === -1 ? process.env.OPENSCREEN_ROOT || join(ROOT, "..", "openscreen") : argv[osArg + 1]);
