...
/*
 * A word the model learns to attach the look to.
 *
 * IT MUST NOT BE A WORD
 *
 * The trigger goes into every prompt, and FLUX reads it. A board called
 * "sticker-style" once trained as `boardstyle`, and the pictures it made came
 * back with BOARD lettered across them: the model had no way to know the token
 * was a handle rather than a thing to draw. So the name is turned into
 * something no language contains — a short tag with a digit in the middle of
 * it — which the weights can own outright.
 */
export const triggerFor = (name) => {
	/* Stable: the same board always trains and generates under the same token. */
	let n = 0;
	for (const ch of String(name)) n = (n * 31 + ch.codePointAt(0)) >>> 0;
	/* "zq" starts no English word, and what follows is base-36 noise. */
	return `zq${n.toString(36).padStart(6, "0").slice(-6)}`;
};
