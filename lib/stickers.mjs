...
(() => {
  ...
  const token = async (fresh) => {
    ...
    const r = await fetch(AUTH + "/token/anonymous");
    jwt = r.ok ? (await r.json()).token : null;
    return jwt;
  };
  ...
  const load = async (sticker) => {
    const r = await api("/sticker_comments?project=eq." + encodeURIComponent(PROJECT) + "&sticker=eq." + encodeURIComponent(sticker) + "&order=created_at.asc");
    return r.ok ? r.json() : [];
  };
  const paint = (list) => {
  ...
  q("#add").onsubmit = async (e) => {
    ...
    const r = await api("/sticker_comments", { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ project: PROJECT, sticker: current, author: who || null, body }) });
  ...
  api("/sticker_comments?project=eq." + encodeURIComponent(PROJECT) + "&select=sticker").then((r) => (r.ok ? r.json() : [])).then((rows) => { const by = {}; for (const r of rows) by[r.sticker] = (by[r.sticker] || 0) + 1; for (const [k, v] of Object.entries(by)) { const n = q('[data-count="' + k + '"]'); if (n) n.textContent = String(v); } }).catch(() => {});
})();
</script>`
		: ""
}
