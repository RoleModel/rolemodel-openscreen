...
export async function makeSticker({ key, model = "telegram", prompt = "", imageUrl = null, trained = null, fetchImpl = fetch }) {
	 ...
	 * A sticker is square.
	 *
	 * The models that carry a look — flux-lora among them — default to a
	 * landscape frame, so a sticker came back 1024×768 with the drawing adrift
	 * in it. Nothing downstream reshapes it, so it is asked for square here.
	 */
	const body = { prompt: `${withTrigger}${note}`, num_images: 1, output_format: "png", image_size: "square_hd", ...(m.extra ?? {}) };
