...
<style>
   ...
   */
  .s.open { border-color: var(--green); }
  .hot.open { border-color: var(--green); }
...
(() => {
  ...
  /* Under the clicked thing, aligned to its left edge, kept on the page; above it when there is no room below. */
  const place = (el) => {
    const r = el.getBoundingClientRect();
    pop.classList.add("on");
    const w = pop.offsetWidth, h = pop.offsetHeight;
    let left = Math.max(16, Math.min(window.scrollX + r.left, window.scrollX + window.innerWidth - w - 16));
    const below = r.bottom + 12 + h <= window.innerHeight || r.top - 12 - h < 0;
    const top = below ? window.scrollY + r.bottom + 12 : window.scrollY + r.top - 12 - h;
    pop.classList.toggle("up", !below);
    pop.style.left = left + "px"; pop.style.top = top + "px";
    pop.style.setProperty("--ax", Math.max(12, Math.min(w - 24, window.scrollX + r.left + r.width / 2 - left - 6)) + "px");
  };
  const open = async (el, sticker, name) => {
    if (anchor === el) return close();
    close();
  ...
  for (const fig of document.querySelectorAll(".s")) {
    ...
    fig.addEventListener("click", (e) => { if (e.target.closest("a")) return; open(fig, fig.dataset.sticker, fig.querySelector("span").textContent); });
  }
  for (const h of document.querySelectorAll(".hot")) h.addEventListener("click", () => open(h, h.dataset.for, h.title.replace(/ — comment$/, "")));
  document.addEventListener("keydown", (e) => { if (e.key === "Escape") close(); });
  document.addEventListener("pointerdown", (e) => { if (anchor && !pop.contains(e.target) && !e.target.closest(".s, .hot")) close(); });
