...
export const STICKER_MODELS = [
	...
	{
		id: "gpt-image",
		label: "From reference · GPT Image 1.5 (best at keeping the subject)",
		endpoint: null,
		imageEndpoint: "fal-ai/gpt-image-1.5/edit",
		lora: null,
		trigger: "",
		input: "image_urls",
		ref: true,
		/* High input fidelity is the whole point: the subject comes through. */
		extra: { input_fidelity: "high", quality: "high" },
	},
	{
		id: "flux2-edit",
		label: "From reference · FLUX.2 Pro edit",
		endpoint: null,
	...
	{
		id: "kontext-max",
		label: "From reference · Flux Kontext Max",
		endpoint: null,
		imageEndpoint: "fal-ai/flux-pro/kontext/max",
		lora: null,
		trigger: "",
		input: "image_url",
		ref: true,
	...
	{
		id: "seedream-edit",
		label: "From reference · Seedream 4.5 edit",
		endpoint: null,
 ...
 * the only vectorizer fal hosts: it is Recraft's V3 engine under fal's one
 * path. A Custom entry takes any fal endpoint that answers { image: { url } }
 * with an SVG. (A "recraft-v3/vectorize" row was here once — fal has no such
 * path, nor /v3/, /v2/ or recraft-20b; every one answered 404.)
 */
export const VECTORIZE_MODELS = [
	{ id: "fal-ai/recraft/vectorize", label: "Recraft V3 — clean shapes", input: "image_url" },
	/* VecGlypher draws a fresh SVG from a prompt with the picture as reference,
	   rather than tracing it: looser, more like an illustrator's redraw. */
	{ id: "fal-ai/vecglypher/image-to-svg", label: "VecGlypher — redraw as vector (uses the prompt)", input: "reference_image_urls", prompt: true },
	{ id: "custom", label: "Custom endpoint…", input: "image_url" },
];
...
export async function makeSticker({ key, model = "telegram", prompt = "", imageUrl = null, trained = null, fetchImpl = fetch }) {
	/*
	 * A look trained on a mood board is a sticker style like any other.
	 *
	 * It arrives as weights and a word rather than as a row in the list above,
	 * so it is turned into the same shape here — the base model is the one the
	 * built-in LoRA styles ride on, and everything downstream stops caring where
	 * the weights came from.
	 */
	const m = trained
		? {
				id: model,
				label: trained.label ?? "a trained look",
				endpoint: "fal-ai/flux-lora",
				imageEndpoint: "fal-ai/flux-lora/image-to-image",
				lora: { path: trained.url, scale: trained.scale ?? 1 },
				trigger: trained.trigger ?? "",
				input: "image_url",
				strength: true,
				square: { image_size: "square_hd" },
			}
		: (STICKER_MODELS.find((x) => x.id === model) ?? STICKER_MODELS[0]);
	...
	/*
	 * Every model is told what a sticker is, trained looks included.
	 *
	 * A LoRA carries a look, not a shape, and a look trained on a wall of
	 * screenshots is a thin signal on its own — asked for nothing but the trigger
	 * word, FLUX drew a mustard blob on cream. The note costs nothing and the
	 * weights still decide the palette and the line, so both are sent.
	 */
	const note = `. Make it a sticker: bold outline, flat colour, ${imageUrl ? "transparent background, the subject kept as it is" : "on a plain white background"}.`;
	/*
	 * A sticker is square — but only when it is drawn from nothing.
	 *
	 * The models that carry a look, flux-lora among them, default to a landscape
	 * frame, so a sticker came back 1024×768 with the drawing adrift in it. An
	 * edit model is the opposite case: it takes its shape from the picture it is
	 * given, and being told a size as well is at best ignored and at worst a
	 * 422 — GPT Image counts sizes in pixels and rejected "square_hd" outright.
	 *
	 * And "square" is not one word across the field: flux says image_size, the
	 * Gemini models say aspect_ratio. So each model names its own, and a model
	 * that says nothing is asked for nothing.
	 */
	const square = imageUrl ? {} : (m.square ?? {});
	const body = { prompt: `${withTrigger}${note}`, num_images: 1, output_format: "png", ...square, ...(m.extra ?? {}) };
...
export async function vectorize({ key, model = VECTORIZE_MODELS[0].id, endpoint = null, imageUrl, prompt = "", fetchImpl = fetch }) {
	if (!IMAGE_URL.test(String(imageUrl))) throw new Error("the picture needs a URL fal can reach");
	const m = VECTORIZE_MODELS.find((x) => x.id === model) ?? VECTORIZE_MODELS[0];
	...
	/* Each model names its picture field; a prompt model gets the sticker prompt, or a tracing one. */
	const body = m.input === "reference_image_urls" ? { reference_image_urls: [imageUrl] } : { image_url: imageUrl };
	if (m.prompt) body.prompt = String(prompt ?? "").trim() || "the same picture redrawn as a clean flat vector sticker: same shapes, same colours, bold outline";
	const data = await callFal(target, body, key, { fetchImpl });
	const url = data.image?.url ?? data.images?.[0]?.url;
...
/*
 * One reference picture, made fit to train on.
 *
 * WHY A BOARD NEEDS THIS
 *
 * A mood board is pasted screenshots: 300px crops off a sheet, some on black,
 * some on cream, all different shapes. A trainer reads that as the look —
 * a wall trained this way produced a coloured tile with the neighbour's colours
 * bleeding in, because that is honestly what the pictures showed.
 *
 * So each one is enlarged, has its background taken off, and is set in the
 * middle of the same white square as every other. What is left that they share
 * is the drawing, which is the only thing worth learning.
 */
export const UPSCALER = "fal-ai/aura-sr";
export const TIDIER = "fal-ai/nano-banana/edit";

/*
 * WHY AN EDIT MODEL AND NOT A BACKGROUND REMOVER
 *
 * The obvious tool was the cutout model this app already had. It failed badly,
 * and for a reason worth writing down: a sticker screenshotted on a black field
 * is black ink on black ground, and a segmenter asked for the foreground takes
 * the black away — outline included. What came back was a pale ghost with no
 * line at all.
 *
 * An edit model is told the difference in words instead, and keeps the ink.
 */
const TIDY_PROMPT =
	"Isolate the single complete die-cut sticker in the centre and place it alone on a plain pure white background. " +
	"Keep its artwork, its colours and its black outline exactly as they are. " +
	"Delete every partial sticker touching the edges. Do not redraw or restyle anything.";

export async function tidyReference({ key, bytes, name = "reference.png", size = 1024, fetchImpl = fetch }) {
	const url = await falUpload({ key, bytes, contentType: "image/png", name, fetchImpl });
	/* Enlarged first, while the picture is still whole: the edit model reads a
	   300px crop as a 300px crop and gives back mush. */
	const big = await callFal(UPSCALER, { image_url: url, scale: 3 }, key, { fetchImpl });
	const bigUrl = big.image?.url ?? big.images?.[0]?.url ?? url;
	const tidy = await callFal(TIDIER, { prompt: TIDY_PROMPT, image_urls: [bigUrl], num_images: 1, output_format: "png" }, key, { fetchImpl });
	const tidyUrl = tidy.images?.[0]?.url ?? tidy.image?.url ?? bigUrl;
	const cleaned = Buffer.from(await (await fetchImpl(tidyUrl)).arrayBuffer());
	return { bytes: await onWhite(cleaned, { size }), upscaled: bigUrl, tidied: tidyUrl };
}

/* The same white square for every picture, drawn by the same engine as stills. */
export async function onWhite(bytes, { size = 1024, pad = 0.06 } = {}) {
	...
	try {
		...
		await page.setContent(
			`<!doctype html><html><body style="margin:0"><div id="b" style="width:${size}px;height:${size}px;background:#fff;display:flex;align-items:center;justify-content:center;padding:${inset}px;box-sizing:border-box"><img id="i" src="${data}" style="max-width:100%;max-height:100%;display:block"></div></body></html>`,
			{ waitUntil: "load" },
		);
		await page.evaluate(() => document.getElementById("i").decode().catch(() => {}));
		return await page.locator("#b").screenshot({ type: "png" });
	} finally {
		await browser.close();
	}
}
...
export const XML_HEAD = /<\?xml[^>]*>|<!DOCTYPE[^>]*>/gi;
export const SVG_ROOT = /<svg\b([^>]*)>([\s\S]*)<\/svg>\s*$/i;
const VIEWBOX = /\bviewBox\s*=\s*["']([^"']+)["']/i;
const WIDTH = /\bwidth\s*=\s*["']([\d.]+)/i;
const HEIGHT = /\bheight\s*=\s*["']([\d.]+)/i;

/*
 * Make every id in one sticker its own.
 *
 * Ids are global to the document they land in, so two stickers that both name
 * a clip path `_clip1` — Affinity and Illustrator both number from one — end up
 * sharing the first one on a sheet, and the rest are cropped to a shape drawn
 * for someone else. Sixteen of one set shared `Artboard1`. Renaming on the way
 * in costs nothing and cannot collide: the prefix is the cell's own number.
 */
export function isolateIds(svg, prefix) {
	const ids = [...new Set([...String(svg).matchAll(/\bid\s*=\s*["']([^"']+)["']/g)].map((m) => m[1]))];
	if (!ids.length) return svg;
	let out = String(svg);
	for (const id of ids) {
		const safe = id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
		const to = `${prefix}${id}`;
		/* The definition, every url(#id) in a style or attribute, and every
		   href="#id" — the three ways one shape names another. */
		out = out
			.replace(new RegExp(`(\\bid\\s*=\\s*["'])${safe}(["'])`, "g"), `$1${to}$2`)
			.replace(new RegExp(`url\\(\\s*#${safe}\\s*\\)`, "g"), `url(#${to})`)
			.replace(new RegExp(`(\\b(?:xlink:)?href\\s*=\\s*["'])#${safe}(["'])`, "g"), `$1#${to}$2`);
	}
	return out;
}
...
export function sheetSvg({ items, columns = 4, size = 300, gap = 40, margin = 60, title = "Stickers", ground = "dots" }) {
	...
	const cells = items.map((item, i) => {
		...
		if (item.svg) {
			...
			const inner = isolateIds(String(item.svg).replace(XML_HEAD, ""), `s${i}-`).match(SVG_ROOT);
			const body = inner?.[2] ?? "";
 ...
 */
/*
 * What it would cost to print, on the page anyone can read.
 *
 * The point of publishing this beside the stickers is that the person deciding
 * whether to order is usually not the person who ran the comparison. So every
 * row keeps what the Studio showed: the shop, the money, and — plainly, not in
 * a footnote — where the number came from and when it was true.
 */
function priceBlock(quotes) {
	/* Both shapes, when both were asked for — kept apart, because a sheet is
	   priced by the page and a single one at a time. */
	if (quotes?.singles || quotes?.sheets) {
		/* One table per sheet size: a sheet's price is mostly its size, so one
		   table for "sticker sheets" invites reading down a column that does not
		   mean one thing. A size nobody quotes is left out — a heading over an
		   apology is a row of nothing to read. */
		const sheets = (quotes.sheets ?? [])
			.filter((z) => z.group?.vendors?.length)
			.map((z) => priceTable(z.group, `Sticker sheets · ${z.label}`))
			.join("");
		return `${priceTable(quotes.singles, "Die-cut singles")}${sheets}`;
	}
	return priceTable(quotes, "What a run costs");
}

function priceTable(group, heading) {
	if (!group?.vendors?.length) return "";
	const per = group.product === "sheet" ? "sheet" : "sticker";
	const head = group.runs.map((n) => `<th class="num">${n}</th>`).join("");
	const rows = group.vendors
		.map((v) => {
			const cells = v.cells
				.map((c, i) => {
					if (!c) return `<td class="num sub">—</td>`;
					/* When the shop's smallest run is bigger than the column, the
					   money is for that run and the column is not what you would
					   be buying. Saying "$190" under a 50 invites the arithmetic
					   somebody did and it did not come out. */
					const over = c.quantity > group.runs[i];
					const under = over ? `<span class="sub">$${c.total} for ${c.quantity}</span>` : `<span class="sub">$${c.total}</span>`;
					return `<td class="num${group.best[i] === v.id ? " best" : ""}${over ? " over" : ""}"><b>$${c.unit.toFixed(2)}</b>${under}</td>`;
				})
				.join("");
			const first = v.cells.find(Boolean);
			const how = first?.live ? "read from their page" : v.kind === "api" ? "quoted live" : "from a price list kept by hand";
			return `<tr><th scope="row">${esc(v.name)}${v.error ? `<span class="sub"> ${esc(v.error)}</span>` : ""}</th>${cells}<td class="how">${first ? `${how} · ${esc(first.checkedAt)}` : ""}</td></tr>`;
		})
		.join("");
	return `<section class="prices"><h2>${esc(heading)}</h2>
<table><thead><tr><th>Printer</th>${head}<th>Where this came from</th></tr></thead><tbody>${rows}</tbody></table>
<p class="note">Price for one ${esc(per)}, with the run's total under it, at about ${Math.round(Number(group.vendors.find((v) => v.cells.find(Boolean))?.cells.find(Boolean)?.size ? 0 : 0))       || ""}each run size. Cheapest in each column is marked — the cheapest at 50 is regularly not the cheapest at 500. Shipping and tax are not included. Check the shop before ordering; a price list moves.</p></section>`;
}

export function sheetPage({ title, sheetFile, sheetSrc = null, zipFile = null, printFiles = [], items, made = new Date(), comments = null, grid = null, version = null, quotes = null }) {
	/*
	 * A stamp on every address the page fetches.
	 *
	 * The custom domain in front of the bucket serves a cached copy for hours,
	 * so a republished sheet kept showing the old drawing however many times it
	 * was uploaded. A new publish means a new stamp means an address nothing has
	 * cached. The sticker's own file name stays the key a comment is filed under,
	 * so the stamp rides only on what is fetched.
	 */
	const v = String(version ?? Math.floor(made.getTime() / 1000));
	/* A caller that stamped the file name uses it; otherwise a query is the best
	   this can do, and a cache that ignores queries wins. */
	...
	if (grid) {
		...
		hots = items
			.map((it, i) => {
				const cx = margin + (i % cols) * (size + gap);
				const cy = margin + Math.floor(i / cols) * (size + gap);
				return `<button class="hot" type="button" data-for="${esc(it.file)}" data-src="${esc(at(it.file, it.src))}" data-name="${esc(it.name)}" title="${esc(it.name)}" style="left:${pct(cx, W)};top:${pct(cy, H)};width:${pct(size, W)};height:${pct(size, H)}"><b class="n" data-count="${esc(it.file)}"></b></button>`;
			})
			.join("");
	}
	return `<!doctype html>
...
<style>
/* What a run costs, published beside the stickers. */
.prices{margin:2rem 0}
.prices table{width:100%;border-collapse:collapse;font-size:.9rem}
.prices th,.prices td{text-align:left;padding:.45rem .6rem;border-bottom:1px solid rgba(255,255,255,.08)}
.prices th{font-weight:600;opacity:.7;font-size:.8rem}
.prices .num{text-align:right;font-variant-numeric:tabular-nums}
/* The run's total under the unit price, not beside it: "$0.84$84" is two
   numbers that read as one. */
.prices .sub{display:block;font-size:.75rem;opacity:.55;font-variant-numeric:tabular-nums}
/* The cheapest in each column, in the page's own green — and bold with it, so
   it still reads on a printout or to anyone who cannot pick the colour out. */
.prices td.best b{color:var(--green);font-weight:800}
.prices td.best{background:color-mix(in srgb, var(--green) 12%, transparent)}
.prices tbody th{font-weight:600}
/* A run bigger than the column asked for: the number is real, the column is
   not what you would be buying. */
.prices td.over b{opacity:.65}
.prices td.over .sub{opacity:.8}
.prices .how{opacity:.6;font-size:.8rem}
.prices .note{opacity:.6;font-size:.8rem;margin-top:.6rem;max-width:60ch}
  ...
  /* The lightbox: the one sticker, large, on a dimmed page. */
  .box { position: fixed; inset: 0; z-index: 30; display: none; place-items: center; background: rgba(0,0,0,.82); padding: 32px; }
  .box.on { display: grid; }
  .box img { max-width: min(90vw, 1200px); max-height: 78vh; background: repeating-conic-gradient(#2a2a2a 0 25%, #222 0 50%) 0 0 / 24px 24px; border-radius: 12px; box-shadow: 0 30px 80px rgba(0,0,0,.6); }
  .box .bar { position: absolute; left: 0; right: 0; bottom: 24px; display: flex; justify-content: center; gap: 10px; }
  .box .name { position: absolute; top: 20px; left: 24px; color: #fff; font-weight: 600; }
  .box .x { position: absolute; top: 14px; right: 18px; background: none; border: 0; color: #ddd; font-size: 30px; line-height: 1; cursor: pointer; }
...
<section class="sheet">
  <div class="sheetwrap"><img src="${esc(at(sheetFile, sheetSrc))}" alt="${esc(title)} sheet" />${hots}</div>
  <p class="tip">Click a sticker to download it${comments ? " or leave a comment" : ""}.</p>
  ${priceBlock(quotes)}
  <div class="row"><a class="btn" href="${esc(at(sheetFile, sheetSrc))}" download="${esc(sheetFile)}">Download the sheet (SVG)</a>${zipFile ? `<a class="btn" href="${esc(at(zipFile))}" download="${esc(zipFile)}">Download everything (ZIP)</a>` : ""}${printFiles.map((f) => `<a class="btn ghost" href="${esc(at(f.file, f.src))}" download="${esc(f.file)}" title="Printed ${esc(f.madeAt ?? "")}">${esc(f.label)}${f.madeAt ? ` · ${esc(f.madeAt)}` : ""}</a>`).join("")}<a class="btn ghost" href="${esc(at(sheetFile, sheetSrc))}">Open the sheet</a><button class="btn ghost" id="copyLink" type="button">Copy link</button></div>
</section>
<div class="menu" id="menu"><button id="mPreview" type="button">Preview</button><a id="mDownload" href="#" download>Download</a><button id="mComment" type="button"${comments ? "" : " hidden"}>Comment…</button></div>
<div class="box" id="box" role="dialog" aria-modal="true"><span class="name" id="boxName"></span><button class="x" id="boxClose" type="button" aria-label="Close">×</button><img id="boxImg" alt="" /><div class="bar"><a class="btn" id="boxDownload" href="#" download>Download</a><button class="btn ghost" id="boxComment" type="button"${comments ? "" : " hidden"}>Comment…</button></div></div>
...
(() => {
  ...
  const menu = document.getElementById("menu"), dl = document.getElementById("mDownload"), cm = document.getElementById("mComment"), pv = document.getElementById("mPreview");
  const box = document.getElementById("box"), boxImg = document.getElementById("boxImg"), boxName = document.getElementById("boxName"), boxDl = document.getElementById("boxDownload"), boxCm = document.getElementById("boxComment");
  let boxFor = null;
  const openBox = (h) => { boxFor = h; boxImg.src = h.dataset.src; boxImg.alt = h.dataset.name; boxName.textContent = h.dataset.name; boxDl.href = h.dataset.src; boxDl.download = h.dataset.for; box.classList.add("on"); };
  const closeBox = () => { box.classList.remove("on"); boxFor = null; };
  document.getElementById("boxClose").onclick = closeBox;
  box.addEventListener("click", (e) => { if (e.target === box) closeBox(); });
  boxDl.addEventListener("click", () => closeBox());
  boxCm.addEventListener("click", () => { const h = boxFor; closeBox(); window.dispatchEvent(new CustomEvent("sticker-comment", { detail: { el: h, file: h.dataset.for, name: h.dataset.name } })); h.scrollIntoView({ behavior: "smooth", block: "center" }); });
  let on = null;
  ...
  document.addEventListener("keydown", (e) => { if (e.key === "Escape") { closeMenu(); closeBox(); } });
