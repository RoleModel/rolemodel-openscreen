...
function priceBlock(quotes) {
	...
	const rows = quotes.quotes
		...
		.join("");
	if (!rows) return "";
	return `<section class="prices"><h2>What a run costs</h2>
...
export function sheetPage({ title, sheetFile, sheetSrc = null, zipFile = null, items, made = new Date(), comments = null, grid = null, version = null, quotes = null }) {
	/*
...
<section class="sheet">
  ...
  ${priceBlock(quotes)}
  <div class="row"><a class="btn" href="${esc(at(sheetFile, sheetSrc))}" download="${esc(sheetFile)}">Download the sheet (SVG)</a>${zipFile ? `<a class="btn" href="${esc(at(zipFile))}" download="${esc(zipFile)}">Download everything (ZIP)</a>` : ""}<a class="btn ghost" href="${esc(at(sheetFile, sheetSrc))}">Open the sheet</a><button class="btn ghost" id="copyLink" type="button">Copy link</button></div>
</section>
