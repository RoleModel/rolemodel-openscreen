...
function priceTable(quotes, heading) {
	if (!quotes?.quotes?.length) return "";
	const per = quotes.asked?.product === "sheet" ? "sheet" : "sticker";
	const rows = quotes.quotes
		.filter((q) => Number.isFinite(q.unit))
		.map((q) => {
			const how = q.live ? "read from their page" : q.kind === "api" ? "quoted live" : "from a price list kept by hand";
			return `<tr${quotes.best && q.id === quotes.best.id ? ' class="best"' : ""}><td>${esc(q.name)}</td><td class="num">$${q.unit.toFixed(2)}</td><td class="num">$${esc(q.total)}</td><td>${esc(q.quantity)} · ${esc(q.size)}</td><td class="how">${esc(how)} · ${esc(q.checkedAt)}</td></tr>`;
		})
		.join("");
	if (!rows) return "";
	return `<section class="prices"><h2>${esc(heading)}</h2>
<table><thead><tr><th>Printer</th><th class="num">Each</th><th class="num">Total</th><th>Run</th><th>Where this came from</th></tr></thead><tbody>${rows}</tbody></table>
<p class="note">Prices for ${esc(quotes.asked?.quantity ?? "")} ${esc(per)}s at about ${Math.round(Number(quotes.asked?.sizeMm ?? 0))}mm, gathered ${esc(new Date().toISOString().slice(0, 10))}. Shipping and tax are not included unless a row says so. Check the shop before ordering — a price list moves.</p></section>`;
}
...
<section class="sheet">
  ...
  ${priceBlock(quotes)}
  <div class="row"><a class="btn" href="${esc(at(sheetFile, sheetSrc))}" download="${esc(sheetFile)}">Download the sheet (SVG)</a>${zipFile ? `<a class="btn" href="${esc(at(zipFile))}" download="${esc(zipFile)}">Download everything (ZIP)</a>` : ""}${printFiles.map((f) => `<a class="btn ghost" href="${esc(at(f.file))}" download="${esc(f.file)}">${esc(f.label)}</a>`).join("")}<a class="btn ghost" href="${esc(at(sheetFile, sheetSrc))}">Open the sheet</a><button class="btn ghost" id="copyLink" type="button">Copy link</button></div>
</section>
