...
function priceBlock(quotes) {
	...
	if (quotes?.singles || quotes?.sheets) {
		/* One table per sheet size: a sheet's price is mostly its size, so one
		   ...
		   mean one thing. A size nobody prices says so rather than being hidden,
		   because the gap is worth knowing about. */
		const sheets = (quotes.sheets ?? [])
			.map((z) =>
				z.group?.vendors?.length
					? priceTable(z.group, `Sticker sheets · ${z.label}`)
					: `<section class="prices"><h2>Sticker sheets · ${esc(z.label)}</h2><p class="note">No printer here quotes this size.</p></section>`,
			)
			.join("");
...
function priceTable(group, heading) {
	...
	const rows = group.vendors
		.map((v) => {
			const cells = v.cells
				.map((c, i) => (c ? `<td class="num${group.best[i] === v.id ? " best" : ""}"><b>$${c.unit.toFixed(2)}</b><span class="sub">$${c.total}</span></td>` : `<td class="num sub">—</td>`))
				.join("");
