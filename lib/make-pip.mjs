...
export async function sayTrack({ projectDir, clips }) {
	...
	for (const [index, clip] of clips.entries()) {
		...
		const blocks = groups.map((group, groupIndex) => {
			...
			phrases.push([groupId, +at.toFixed(3), +Math.max(at, nextAt - PHRASE_FADE).toFixed(3), +nextAt.toFixed(3)]);
			/*
			...
			const said = group.map((word) => {
				...
				wordCues.push([id, +wordAt.toFixed(3)]);
		...
		outs.push([`say-${index + 1}-in`, +(out - 0.4).toFixed(3), +out.toFixed(3)]);
		lines.push(
	...
	return { lines, phrases, words: wordIndex, wordCues, outs };
}
...
export async function buildPip(projectId, folder = "canvas-pip-transcript", { clips: given, title, closing, opening, pip, wallpaper = "rm-brand.jpg", scrim = true, keep = [] } = {}) {
	...
	const { lines, phrases, words: wordIndex, wordCues, outs } = await sayTrack({ projectDir, clips });
...
${closing === false || closing === ""
  ...
  <script>
    ...
    (function () {
       ...
       *
       * PHRASE  [id, in, out, gone]   WORD [id, at]   OUT [id, from, gone]
       */
      ...
      PHRASE.forEach(function (p) {
        ...
        tl.set(at, { opacity: 0 }, 0)
          .to(at, { opacity: 1, duration: FADE, ease: 'power1.out' }, p[1])
          .to(at, { opacity: 0, duration: FADE, ease: 'power1.in' }, p[2])
          ...
          .set(at, { opacity: 0 }, p[3]);
      });
      WORD.forEach(function (w) {
        var at = '#' + w[0];
        tl.set(at, { opacity: DIM }, 0).to(at, { opacity: 1, duration: FILL, ease: 'power1.out' }, w[1]);
      });
      OUT.forEach(function (o) {
        var at = '#' + o[0];
        tl.to(at, { opacity: 0, duration: 0.4, ease: 'none' }, o[1]).set(at, { opacity: 0 }, o[2]);
      });
