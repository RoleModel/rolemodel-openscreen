...
/**
 * The words, as blocks and tweens, for a given set of clip windows.
 *
...
export async function sayTrack({ projectDir, clips }) {
	...
	return { lines, tweens, words: wordIndex };
}
...
export async function buildPip(projectId, folder = "canvas-pip-transcript", { clips: given, title, closing, opening, pip, wallpaper = "rm-brand.jpg" } = {}) {
	...
	const { lines, tweens, words: wordIndex } = await sayTrack({ projectDir, clips });
	...
	const media = clips.map((clip, index) => {
		...
		return `    <video id="pip-${index + 1}" class="clip pip" data-assembly-media src="source/${clip.src}" data-start="${seconds(clip.start * 1000)}" data-duration="${seconds(clip.dur * 1000)}" data-media-start="${seconds(clip.ms * 1000)}" data-track-index="${index}" data-has-audio="true" playsinline preload="auto" style="--pip-focus:${focus}%"></video>`;
	});
...
<html lang="en">
  <head>
    ...
    <style>
       ...
       * or on one <video> to move only that one. --pip-focus is the crop
       * itself: object-position across the frame, where 0% shows the left edge
       * and 100% the right, and the vertical stays centred because a square cut
       * from 16:9 has no vertical slack to move.
       */
