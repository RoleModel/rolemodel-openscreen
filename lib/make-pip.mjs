...
async function wordsFor(projectDir, rel) {
	...
	const words = doc?.transcript?.words ?? [];
...
export async function buildPip(projectId, folder = "canvas-pip-transcript", { clips: given, title, closing, opening, wallpaper = "rm-brand.jpg" } = {}) {
	let clips = given;
...
<html lang="en">
  <head>
    ...
    <style>
       ...
       */
      .pip { position: absolute; right: -8cqw; bottom: -4cqw; width: 46cqw; height: 46cqw;
...
`;
	...
	await writeFile(join(outDir, "clips.json"), `${JSON.stringify({ title, closing, wallpaper, opening, clips }, null, 2)}\n`, "utf8");
