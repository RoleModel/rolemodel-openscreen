...
export async function buildPip(projectId, folder = "canvas-pip-transcript", { clips, title, closing } = {}) {
	...
	for (const [index, clip] of clips.entries()) {
		...
		const spans = inRange.map((word) => {
			const at = clip.start + (word.startSec - clip.ms);
			const id = `w${(wordIndex += 1)}`;
			tweens.push(
				`      tl.fromTo('#${id}', { opacity: 0, y: 6 }, { opacity: 1, y: 0, duration: 0.18, ease: 'none' }, ${at.toFixed(3)});`,
			);
  ...
  <head>
    ...
    <style>
      ...
      [data-composition-id] { position: relative; width: 1920px; height: 1080px; overflow: hidden;
        background: var(--color-dark); font-family: var(--font-display); container-type: inline-size; }
      ...
      .say__words { color: var(--color-light); font-size: 3.4cqw; font-weight: 700;
                    line-height: 1.28; letter-spacing: -.02em; }
      /* Each word is its own element so it can arrive on its own beat. */
      .word { opacity: 0; display: inline-block; margin-right: .28em; }
