...
/*
 * How far out of focus a line starts, and how far it goes as it leaves.
 *
 * 14px at 1920 wide is about a quarter of the cap height — enough to read as a
 * focus pull rather than a smudge, and it resolves inside PHRASE_FADE so the
 * line is sharp by the time the first word fills. The exit is softer than the
 * entrance on purpose: a line leaving should not compete with the one arriving.
 */
export const PHRASE_BLUR_IN = 14;
export const PHRASE_BLUR_OUT = 8;

/** The word's own focus tick — a fraction of the line's, for the reason above. */
export const WORD_BLUR = 3.5;
/*
...
export const TIMELINE_LOOPS = `      /*
      ...
      /*
       * A line arrives out of focus and settles.
       *
       * Opacity alone gives a line no direction — it is simply less there, then
       * there. Coming out of blur reads as the words resolving, which is what a
       * caption is doing: catching up with something already being said. It also
       * covers the one frame where a long line paints, because a blurred edge
       * has nothing to alias against.
       *
       * Only the phrase blurs, never the words inside it. The per-word fill runs
       * every 300ms or so, and a blur that short reads as a flicker rather than
       * a focus pull.
       *
       * Every state is stated, including the reset after the kill, because the
       * renderer seeks this timeline once per frame rather than playing it — a
       * property left undeclared at a boundary is whatever the last seek left
       * behind, which is how a caption ends up permanently soft.
       */
      PHRASE.forEach(function (p) {
        var base = offset(p[1]);
        if (base === null) return;
        var at = '#' + p[0];
        tl.set(at, { opacity: 0, filter: 'blur(' + BLUR_IN + 'px)' }, 0)
          .to(at, { opacity: 1, filter: 'blur(0px)', duration: FADE, ease: 'power2.out' }, base + p[2])
          .to(at, { opacity: 0, filter: 'blur(' + BLUR_OUT + 'px)', duration: FADE, ease: 'power1.in' }, base + p[3])
          ...
          .set(at, { opacity: 0, filter: 'blur(' + BLUR_IN + 'px)' }, base + p[4]);
      });
      /*
       * The word being said comes forward, and sharpens as it does.
       *
       * A word is only on screen for a fifth of a second before the next one
       * takes over, so its blur has to be a fraction of the line's — enough to
       * feel like focus catching up with the voice, not enough to read as a
       * separate move. At WORD_BLUR the effect is deniable frame by frame and
       * obvious across a sentence, which is the right way round.
       *
       * The resting state carries the blur too. Half-lit and soft is one idea
       * — text you can read ahead but are not being read yet — where half-lit
       * and sharp is just dim text.
       */
      WORD.forEach(function (w) {
        var base = offset(w[1]);
        if (base === null) return;
        var at = '#' + w[0];
        tl.set(at, { opacity: DIM, filter: 'blur(' + WORD_BLUR + 'px)' }, 0)
          .to(at, { opacity: 1, filter: 'blur(0px)', duration: FILL, ease: 'power1.out' }, base + w[2]);
      });
...
export async function buildPip(projectId, folder = "canvas-pip-transcript", { clips: given, title, closing, opening, pip, wallpaper = "rm-brand.jpg", scrim = true, keep = [] } = {}) {
	 ...
	 *
	 * Both the host and the file's root say `data-no-timeline`, and it is not
	 * cosmetic. Anything carrying a composition id is a composition to the
	 * producer, which polls `window.__timelines[id]` for FORTY-FIVE SECONDS
	 * before giving up — and a transcript has no clock of its own, its cues are
	 * on the parent's timeline. Six of these is four and a half minutes added to
	 * every render, waiting for six timelines that were never going to appear.
	 *
	 * They carry the parent's duration for the same reason: these fragments are
	 * measured on the parent's clock, so that is the honest length, and it is the
	 * one number here that does not change when a clip is retimed.
	 */
	const mounts = parts.map((part, index) =>
		`    <div id="say-${index + 1}-mount" class="say-mount" data-composition-id="say-${index + 1}" data-no-timeline`
...
${closing === false || closing === ""
  ...
  <script>
    ...
    (function () {
      ...
      var BLUR_IN = ${PHRASE_BLUR_IN}, BLUR_OUT = ${PHRASE_BLUR_OUT}, WORD_BLUR = ${WORD_BLUR};
...
<body>
<div data-composition-id="say-${index + 1}" data-width="1920" data-height="1080" data-duration="${seconds(total * 1000)}" data-no-timeline>
${part.body}
</div>
</body>
</html>
