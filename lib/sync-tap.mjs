...
function carryGenerated(want, have) {
	return want.replace(/^(\s*)(url|version|sha256)(\s+)"[^"]*"/gm, (whole, indent, key, gap) => {
		// Same key, same nesting: a cask has two sha256 lines under different
		// arch blocks, and matching on the key alone would give both the first.
		const at = new RegExp(`^${indent}${key}${gap}"([^"]*)"`, "m");
		const found = at.exec(have);
		return found ? `${indent}${key}${gap}"${found[1]}"` : whole;
	});
}
...
for (const { from, to } of LAYOUT) {
	 ...
	 * printed, `npm run sync-tap`, would then overwrite the released formula
